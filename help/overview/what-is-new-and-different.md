...
* 建立和管理新方案。 請參閱 [了解方案和方案類型](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/program-types.md) 以取得更多詳細資訊。
...
* 部署需求：
   ...
   * 代碼更新；任何針對舊版AEM開發的客戶應用程式都需經過審核，且可能會更新。

   * 內容移轉

>[!TIP]
>
>如需入門程式的完整概觀，請參閱 [入門歷程。](/help/journey-onboarding/overview.md)
...
AEMas a Cloud Service中不再提供Web主控台，可管理OSGI套件組合及其相關設定，且先前是AEM QuickStart的一部分。 新的開發人員控制台提供用於執行階段資訊的唯讀介面。 透過此主控台，開發人員可以選取並直接登入製作或發佈服務的任何特定節點，並檢視相關資訊。
...
這需要使用 [AdobeAdmin Console](https://helpx.adobe.com/enterprise/using/admin-console.html) 用於管理使用者和使用者群組。 使用者帳戶可讓您的使用者存取Adobe產品和服務，因為使用者設定檔資訊會集中在AdobeIdentity Management系統(IMS)中，以便在所有雲端服務間共用。 指派存取權至AEM後，即可在AEMas a Cloud Service中參考使用者帳戶（如前所述）;例如，用於從AEM Security使用者介面定義角色和權限。
