...
Web控制台(其中管理OSGI捆綁包及其關聯配置，並且以前是AEMQuickStart的一部分)不再可供as a Cloud Service環境的用戶直AEM接訪問。 此介面仍可以通過使用新的開發人員控制台以只讀模式訪問。 使用此控制台，開發人員可以直接選擇並登錄作者或發佈服務的任何特定節點，然後訪問預設阻止的區域。
