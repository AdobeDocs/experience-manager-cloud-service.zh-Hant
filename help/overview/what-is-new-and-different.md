...
>[!NOTE]
>
>如需內部部署和受管理服務版本的詳細資訊，請參閱[AEM 6.5](https://helpx.adobe.com/tw/support/experience-manager/6-5.html)的檔案集。
...
>[!NOTE]
>如需詳細資訊，請參閱[AEM Version Updates](/help/implementing/deploying/aem-version-updates.md)。
...
* 在這些程式中建立和管理AEM環境。 有關詳細資訊，請參閱[管理環境](/help/implementing/cloud-manager/manage-environments.md)。
...
* AEM a Cloud Service QuickStart:a `.jar`為最新AEM程式碼庫的獨立安裝程式，具有相同的功能和API表面。
...
這需要使用[Adobe管理控制台](https://helpx.adobe.com/tw/enterprise/using/admin-console.html)來管理使用者和使用者群組。 使用者帳戶可讓您的使用者存取Adobe產品和服務，因為使用者個人檔案資訊會集中在Adobe Identity Management System(IMS)中，以便在所有雲端服務間共用。 一旦指派AEM的存取權後，使用者帳戶就可在AEM中以雲端服務的身分被參照（如先前）;例如，用於從AEM Security使用者介面定義角色和權限。
