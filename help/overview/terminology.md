...
以下術語適用於與 Adobe Experience Manager (AEM) as a Cloud Service 相關之處：
...
## 執行個體和管道 {#instances-and-pipelines}
...
| 資產微服務 | 雲端數位資產處理服務，適用於多種資產處理使用案例，例如產生轉譯成品、排列 PDF、處理子資產、提取文字等。如需詳細資訊，請參閱[資產微服務概觀](/help/assets/asset-microservices-overview.md)。 |
| Cloud Manager Git 儲存庫 | 客戶儲存程式碼和組態設定的位置。 |
| 雲端服務供應商 | AEM as a Cloud Service 是從多個幕後廠商 (例如 Microsoft Azure 或 Amazon Web Services) 在公共雲端基礎結構上執行，以提供已簽訂 SLA 的服務。 |
| 內容傳遞網路 (CDN) | AEM 雲端服務隨附預設的 CDN。其主要用途，就是透過從瀏覽器附近的邊緣 CDN 節點傳遞可快取的內容，以便減少延遲的情形。它已完全受管理，並且已設定為提供最佳的 AEM 應用程式效能。 |
| 內容儲存庫 | 保留內容的位置。 |
| 企業隔離 | AEM as a Cloud Service 的每個執行個體均與其他執行個體隔離。 |
