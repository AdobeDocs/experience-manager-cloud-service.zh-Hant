...
以下章節概述與2023.4.0功能啟用相關的Experience Manageras a Cloud Service維護版本的技術發行說明。
...
- SITES-13636 - 報告的問題是一些客戶缺少圖像和輪播網站 clientlib 的 clientlib 相依性。 如果客戶嵌入了另一個clientlib，其中包括影像或輪播clienlib，則相依性無法解析。
...
- SKYOPS-51970 — 在buildImage步驟中使用的FACT版本的已識別回歸，導致不相符的使用者對應。
...
- SKYOPS-56928 - Apache HTTPD 回迴歸可能會導致 404 錯誤。如果您遇到這些問題，基於安全理由，建議復原到先前的版本，並避免在該時段內執行任何管道。
