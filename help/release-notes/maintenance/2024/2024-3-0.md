...
下面是 15787 維護版本持續改善內容的摘要；該版本於 2024 年 4 月 4 日公開發佈。之前的維護版本是版本 15575。
...
* SITES-19059 - 內容片段 - OpenAPI - 允許列舉欄位的 defaultValue
* SITES-20013 - 內容片段 - OpenAPI - 當未出現 ServletResolver 時，WCMScriptHelper 應中止轉譯
* SITES-19926 - 內容片段 - OpenAPI - OnOffTriggerProcessor 改善
...
* SITES-15557 - 內容片段 - OpenAPI - 允許列舉欄位的 defaultValue
...
* SITES-18702 - 內容片段 - OpenAPI - [後端] 能力 (可在資料夾層級發佈)
...
* SKYOPS-63033 - HTTPD - 修剪 Dispatcher 環境變數的空白
...
* 在規則編輯器中，使用者可以：
   ...
   * FORMS-11714：在自訂函數中將參數定義為選項。依預設，自訂函數中宣告的參數為必要參數。
...
* SITES-17519 - 網站頁面屬性編輯器 - 長的頁面名稱會使按鈕成為不可點選
...
* SITES-11571 - 內容片段 - GraphQL API - PersistedQueryServlet 應在格式錯誤的 URL 上傳送 400 秒
* SITES-19946 - 內容片段 - GraphQL API - 啟動時不再掃描模式
* SITES-1605 - 核心後端 - 如果來源路徑為空，DeletePageCommand 會拋出 NPE
...
* SITES-17164 - 頁面編輯器 - [雲端，6.5 表單] AF 主題編輯器預覽已損壞
...
* SITES-16834 - 傳統 UI (舊版) - 當文字中包含逗號時，使用大量編輯器編輯文字後文字消失
* SITES-17767 - 內容片段 - 管理員 - 對於沒有 jcr:content 節點的資料夾也支援允許的 cf-models
* SITES-17683 - 內容片段 - 核心後端 - 內容片段無法使用 Jackson 滙出器進行序列化
* SITES-18797 - 內容片段 - 核心後端 - 索引自訂後 GraphQL 結果重複
* SITES-18076 - 內容片段 - 核心後端 - 多重值文字擁有不正確的 @TypeHint
* SITES-17856 - 內容片段 - 模式與模式編輯器 - 如果設定不是資料夾，則不會顯示 CF 模式
* SITES-17071 - 核心後端 - 特定頁面未在時間表中顯示版本
* SITES-17285 - 核心元件 - 內嵌影像裁切在 AEMaCS 中的製作和發佈上有所不同
* SITES-19187 - 核心元件 - 將資產放置在影像元件中的兩種方法在對話框中提供不同的結果
* SITES-20077 - 核心元件 - 內嵌影像裁切 - 裁切錯誤且影像模糊
* SITES-17211 - 體驗片段 - 體驗片段元件路徑選擇器對話框，顯示已刪除的體驗片段
* SITES-17894 - 啟動 - 巢狀啟動的推廣會將啟動內容還原為其上階的版本
* SITES-16042 - MSM - Live Copies - 推出後，Livecopy 中的註解顯示不正確
* SITES-16691 - MSM - Live Copies - 當客戶從元件上的「轉出」圖示中選擇「轉出」或「轉出至」時，「繼續」按鈕將呈現灰色。
* SITES-16733 - MSM - Live Copies - 當 rep:policy 應用於節點時，無法轉出 Blueprint 索引頁面
* SITES-17155 - MSM - Live Copies - LiveCopy 重新命名後，標題和頁腳會翻譯回英文
* SITES-17492 - MSM - Live Copies - AEM Page Live Copy 版面不一致
* SITES-19316 - MSM - Live Copies - 體驗片段的參考連結不會在語言副本中更新
* SITES-19347 - MSM - Live Copies - 產品作者緩慢度和服務中斷訊息 - Pod 頻繁重新啟動 - 運行狀況警報
* SITES-19790 - MSM - Live Copies - 建立 LiveCopy 後預覽資訊不正確
* SITES-20086 - MSM - Live Copies - 資產中 CF 的 MSM 轉出將推出所有即時版本，即使選擇推出一個即時版本也一樣
* SITES-20088 - MSM - Live Copies - 屬性中 XF 轉出後的空白頁問題 - AEM as a Cloud Service
* SITES-16854 - 頁面編輯器 - 放置目標覆蓋層以覆蓋具有「選取面板」功能元件中的 parsys
* CQ-4355563 - 專案 - 路徑參數錯誤。為專案收件匣搜尋指令碼填入 &quot;?appId=aemshell&quot;
* SITES-16876 - 快速網站 - 主題部署 - 預覽頁 2 上缺少 CSS 和 JS
* SITES-18418 - 網站管理 UI - 當欄位為必填時，路徑欄位 Widget 的顯示/隱藏功能無法正常運作
* SITES-19534 - 網站管理 UI - AEM 6.5.19 升級後無法開啟有效權限對話框
...
* SITES-17393 - 工作流程擴充 - 使用 cq:Tag 的工作流程啟動器時發佈內容樹失敗
* SITES-17759 - 工作流程擴充功能 - 標籤的樹啟動工作流程在 AEMaaCS 中無法運作
...
* FORMS-13267：當使用者新增最適化表單下拉式選單元件時，下拉式選單的 ID 無法生成
* FORMS-13544：當使用者新增有精靈版面配置的最適化表單容器元件時，在表單製作期間無法正常運作
* FORMS-13091、FORMS-13414：如果使用者嘗試在 Azure Blob 儲存空間中設定自訂網域 URL，則會失敗
* FORMS-13595：當使用者嘗試將表單儲存在其他位置時，如果瀏覽器解析度設定為 100%，則使用者將無法看到「選取資料夾」和「取消」按鈕。但是，當解析度設定為 75% 時，會看見該選項
...
* FORMS-11786：當使用者在行事曆 Widget 中的月份之間切換時，日期選擇器元件會顯示額外的一行。
* FORMS-12093、FORMS-12093：當使用者勾選「最適化表單」勾選方格並提交表單時，文字方塊中會儲存附有額外  `\` 的錯誤值
* FORMS-11993：當使用者在 iOS 裝置上使用附件元件中的「拍照」來點選影像時，所有影像都會新增至有相同名稱的資料夾中
* FORMS-12555：當使用者嘗試將 AEM Forms 整合至有 AEM 發佈 URL 的郵件平台時，AEM Forms 在呈現頁面時不會新增 method=post。即使在提交動作中使用 URL 設定了 POST，也會出現此問題。這會導致郵件平台無法識別這是表單
* FORMS-12938：HTML5 表單無法在有 IE 相容模式的 Microsoft Edge 瀏覽器中正常運作或載入
* FORMS-12032：當使用者提交表單時，提交動作路徑的路徑未正確指向
* FORMS-12445：變更單選按鈕選項的順序並發佈表單後，表單資料模式中會擷取錯誤值。
...
* FORMS-11546：當表單作者在最適化表單中使用重複面板時，HTML 標記中會少了 ARIA 屬性。這會提高最適化表單重複面板的存取性
* FORMS-11826：由於匹配標籤 Arial® labelledby 和 Arial® label，螢幕閱讀器無法區分這兩者。為了解決該問題 - 表單欄位的標籤 aria-labelledby 會換為 aria-descriptedby。(F). 這會改善最適化表單的協助工具功能
* FORMS-12626、FORMS-13094：使用者無法使用鍵盤存取工具列來儲存或編輯表單編輯器頁面上的內容。此協助工具問題已修正。
* FORMS-13102：在表單製作過程中，「表單和文件」頁面上的可用圖示缺乏針對不同能力個人的描述性功能。此協助工具問題已修正。
