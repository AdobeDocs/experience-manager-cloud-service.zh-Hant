...
通用編輯器的 [Extension Manager](https://developer.adobe.com/uix/docs/extension-manager/) 發行許多新的擴充功能，增強製作體驗。

* **MSM 擴充功能**：您現在可以使用此擴充功能來中斷並重新實例化元件/區塊的繼承。
* **頁面屬性擴充功能**：透過此擴充功能，可以直接從通用編輯器存取頁面的頁面屬性視窗。
* **工作流程擴充功能**：透過此擴充功能使用頁面上以及在頁面上嵌入的內容片段之工作流程。
* **頁面鎖定擴充功能**：透過此擴充功能，可以直接從通用編輯器將頁面鎖定和解除鎖定。
