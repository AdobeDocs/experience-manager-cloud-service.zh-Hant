...
* 容器內的元件現在可以使用「[複製並貼上](/help/sites-cloud/authoring/universal-editor/authoring.md#copy-paste)」。
   * 複製和貼上必須在同一個瀏覽器索引標籤上進行。
   * 目標容器不能[設定任何篩選器](/help/implementing/universal-editor/filtering.md)。
* 現在支援 6.5 LTS SP1。
