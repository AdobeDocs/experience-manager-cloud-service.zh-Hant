...
* [!UICONTROL Experience Manager Assets Brand Portal]現在每12小時執行一次自動工作，以刪除發佈至AEM的所有Brand Portal資產。 因此，您不需要手動刪除「貢獻」資料夾中的資產，以使資料夾大小低於臨界值限制。 請參閱[Experience Manager Assets Brand Portal的新增功能](https://experienceleague.adobe.com/docs/experience-manager-brand-portal/using/introduction/whats-new.html?lang=zh-Hant)。
...
* 復寫代理程式管理畫面的&#x200B;**Distribute索引標籤**&#x200B;下的「新增樹狀結構」選項（先前宣佈已棄用），已於2022年6月20日或隨後不久移除。 應改用[管理發布](/help/operations/replication.md#manage-publication)或[Publish內容樹狀工作流程](/help/operations/replication.md#publish-content-tree-workflow)，來復寫具有樹狀階層內容的套件。

* 使用復寫代理程式管理畫面或復寫API來分發大於10 MB的內容套件（具有屬性的節點，不包括二進位檔案）已過時，並自2022年9月12日或隨後不久強制棄用。 必須改用[管理出版物](/help/operations/replication.md#manage-publication)或[Publish內容樹狀工作流程](/help/operations/replication.md#publish-content-tree-workflow)來復寫這些大型內容封裝。 在7月，如果嘗試復寫這些大型內容套件，復寫代理程式管理畫面的&#x200B;**散發標籤**&#x200B;中會出現警告訊息，而且每當使用復寫API復寫這些大型內容套件時，AEM錯誤記錄中也會出現警告訊息。 在9月，警告取代為錯誤。 相應地調整您的流程。
...
從2022年6月30日開始，as a Cloud ServiceExperience Manager將需要與使用者系統進行更安全的網路通訊和資料交換。 AEM將僅使用傳輸層安全性(TLS) 1.2通訊協定。 舊版TLS 1.0和1.1現已棄用。
