...
description: '[!DNL Adobe Experience Manager] as a 2021.2.0的Cloud Service發行說明。'
...
* 資產的來源可為[!DNL Experience Manager Assets Brand Portal]。 這有助於為新的行銷活動、像片和專案從代理商使用者處取得資產。

* [!DNL Experience Manager Assets] as a [!DNL Cloud Service] 有權擁有預先設定的執 [!DNL Brand Portal] 行個體。[!DNL Cloud Manager]使用者可在[!DNL Experience Manager Assets]上以[!DNL Cloud Service]的形式啟動[!DNL Brand Portal]。 請參閱[啟用Brand Portal](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/assets/brand-portal/configure-aem-assets-with-brand-portal.html?lang=en)。

* 企業現在可以使用[!DNL Brand Portal]來源資產。 資產來源補充功能運用[!DNL Brand Portal]，協助客戶與代理使用者互動，為新的行銷活動、像片和專案來源提供資產。 請參閱 [!DNL Brand Portal]](https://experienceleague.adobe.com/docs/experience-manager-brand-portal/using/asset-sourcing-in-brand-portal/brand-portal-asset-sourcing.html)中的[資產來源補充。

* [!DNL Brand Portal]使用狀況報表現在只顯示使用中使用者。 現在不會顯示非使用中使用者。 作用中使用者是指其帳戶已指派給[!DNL Admin Console]中之產品設定檔的使用者。 請參閱[[!DNL Brand Portal] reports](https://experienceleague.adobe.com/docs/experience-manager-brand-portal/using/admin-tools/brand-portal-reports.html)。

* 在[!DNL Brand Portal]中引入新的下載設定，可讓您在下載資料夾、集合等時，為每個資產建立個別的資料夾。 請參閱[下載設定](https://experienceleague.adobe.com/docs/experience-manager-brand-portal/using/download/brand-portal-download-assets.html)。

## [!DNL Assets] {#bug-fixes-assets}中的錯誤修正

* 選取多個資產以更新屬性時，有時會發生錯誤，或取消選取資產的屬性會更新。 (CQ-4316532)
* 嘗試開啟[!UICONTROL Assets管理搜尋邊欄]時，頁面會保持空白，按一下[!UICONTROL Edit] > [!UICONTROL Settings]會產生錯誤。 (CQ-4315079)
...
* CIF Venia Reference Site - 2021.02.24版已發行，其中包含最新CIF核心元件1.8.0版。如需詳細資訊，請參閱[CIF Venia Reference Site](https://github.com/adobe/aem-cif-guides-venia/releases/tag/venia-2021.02.24)。

* CIF核心元件v1.8.0已發行。如需詳細資訊，請參閱[CIF核心元件](https://github.com/adobe/aem-core-cif-components/releases/tag/core-cif-components-reactor-1.8.0) 。
...
* 能夠偵測AEM Forms和AEM Forms實作的使用情形，並指出與移轉至AEM Forms作為Cloud Service相關的區域。
...
請參閱[Unified Experience](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/moving/refactoring-tools/unified-experience.html?lang=en#benefits)以深入了解此外掛程式。
