...
* 適用於Forms作為Cloud Service的AEM原型專案現在包含[4個新主題，以及Microsoft Dynamics和Salesforce.com](https://experienceleague.adobe.com/docs/experience-manager-forms-cloud-service/forms/setup-environment/setup-local-development-environment.html?#forms-cloud-service-local-development-environment)的表單資料模型。
...
* **統一儲存連接器：** 統一儲存連接器可讓您將資料來源連結至AEM工作流程或最適化表單，而不會將任何資料保留在AEM FormsCloud Service儲存上。它可讓您安全地處理PII（個人識別資訊），並直接將資訊儲存至Azure資料存放區。
...
本節概述AEM as a 2021.8.0和2021.7.0Cloud Service中Cloud Manager的發行說明。
