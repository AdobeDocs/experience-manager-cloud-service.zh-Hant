...
* [最適化Forms （基礎元件）中註腳預留位置的增強功能](/help/forms/footnotes-richtextsupport.md)：
   * 註腳呈現已進一步改良，可透過分行符號支援多行格式，讓註腳內容呈現得更清晰、更有表現力。
   * 現在，無論相關面板的可見度為何，註腳都會持續顯示在註腳預留位置內，確保關鍵資訊的存取方式維持一致。


### 全新 AEM Forms 搶先體驗功能 {#forms-new-early-access-features}

* [從JSON陣列擷取值](/help/forms/invoke-service-enhancements-rule-editor.md#retrieve-property-values-from-a-json-array)：擴充的資料整合功能現在可讓您透過自訂函式叫用API，以有效地從JSON陣列擷取值，並將它們直接繫結到調適型表單欄位。 此增強功能可簡化資料消耗、將手動對應降至最低，並支援更多動態的資料導向表單體驗。

* **在發佈執行個體上叫用關聯UI**：擴充支援現在可用於直接在發佈執行個體上叫用關聯UI。 此功能會定義所需的設定、裝載結構和呼叫流程，藉此簡化整合併加速跨環境的部署。
