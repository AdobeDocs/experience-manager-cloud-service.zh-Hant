...
聊天應用程式（例如ChatGPT）和現代IDE （例如Cursor）可以使用模型內容通訊協定(MCP)，讓MCP伺服器公開大型語言模型(LLM)呼叫工具。 與其根據低階API規格來撰寫程式碼，您可以用自然語言描述您想要的，並讓LLM將該用途轉譯為適當的API呼叫。 在某些情況下，提示可能比導覽使用者介面更方便。

例如，您可以提示AEM中的ChatGPT： *Edit /en/coffee.html：將內容中的Frescopa Coffee文字變更為20歲的人想要的內容。*

AEM提供兩部內容MCP伺服器 — **唯讀**&#x200B;與&#x200B;**讀取/寫入** — 搭配使用&#x200B;**頁面**、**內容片段**&#x200B;與&#x200B;**Assets**&#x200B;的工具。 這些伺服器可從下列MCP使用者端使用： **ChatGPT**、**Claude**、**Cursor**&#x200B;和&#x200B;**Microsoft Copilot Studio**。
...
AI搜尋引進了智慧型內容感知搜尋體驗，其透過瞭解使用者查詢背後的含義和意圖，超越了傳統關鍵字比對。 由AI和機器學習提供技術支援，即使查詢的措辭不同、包含拼字錯誤、使用同義字或以不同語言提交，也能提供更準確的結果，讓使用者更快找到相關內容，事半功倍。

如需詳細資訊，請參閱[Assets檢視](/help/assets/search-assets-view.md#ai-search)和[管理員檢視](/help/assets/search-assets.md#ai-search)中的AI搜尋。
