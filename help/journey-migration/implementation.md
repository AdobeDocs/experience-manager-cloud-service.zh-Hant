...
* 向您介紹Cloud Manager，AEM用來將程式碼部署到AEM as a Cloud Service的持續整合和傳遞架構
* 透過內容轉移工具，讓您快速上手
...
* [入門歷程](/help/journey-onboarding/overview.md)以瞭解有關Experience Manageras a Cloud Service入門的自助資源。
...
如需有關工具如何運作以及Adobe建議您如何使用的完整詳細資訊，請參閱[內容轉移工具檔案](/help/journey-migration/content-transfer-tool/using-content-transfer-tool/overview-content-transfer-tool.md)。
...
* 瞭解如何透過下載[AEM as a Cloud Service SDK](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developing/aem-as-a-cloud-service-sdk.html?lang=zh-Hant)來設定本機開發環境
...
AEM Maven專案必須進行某些變更，才能與雲端相容。 AEM as a Cloud Service需要將&#x200B;*內容*&#x200B;和&#x200B;*程式碼*&#x200B;分離為不同的套件，才能部署至AEM：

* `/apps`和`/libs`視為AEM的不可變區域，因為在AEM啟動後（也就是在執行階段）無法變更它們。 這包括建立、更新或刪除作業。 在運行時更改不可變區域的任何嘗試都將失敗。
...
* [Repository Modernizer](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/moving/refactoring-tools/repo-modernizer.html?lang=zh-Hant)，此工具會將AEM多重模式專案當作輸入，並將其轉換為AEM as a Cloud Service專案
...
建議在所有AEM版本中進行[資料一致性檢查](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/consistency-check.html?lang=zh-Hant)，以確保內容存放庫處於良好的狀態，以啟動移轉活動。
...
* 特定環境中的作者和Publish擷取會視為兩個平行擷取，因為它們彼此獨立。
* 根據特定時段中存放庫增長的追加擷取次數。
...
* 追加擷取次數。
* 將內容從Source Author移轉至Cloud Service Author例項，以及從Source Publish移轉至Cloud ServicePublish是避免將所有作者內容內嵌至Cloud ServicePublish的最佳作法。
...
* 解決任何潛在問題，例如：
   * AEM來源上的磁碟空間
   * AEM來源和AEM as a Cloud Service之間的連線
...
在您完全瞭解如何評估您的AEM安裝是否準備好要移至雲端後，當我們瞭解如何使用所需的工具讓安裝準備就緒時，您就可以移至[上線階段](/help/journey-migration/go-live.md)。
