...
* `Mongo took too long to start` — 在極少數的情況下，內嵌工作流程中使用的本機MongoDB可能會無法啟動。 後續重試將很可能更正問題。

#### AZCopy問題 {#azcopy-issues}

* `AZCopy critical failure` — 在極少數情況下，用來執行內嵌的預先複製步驟的AZCopy工具可能會意外失敗。 在這種情況下，應該嘗試重試擷取。
