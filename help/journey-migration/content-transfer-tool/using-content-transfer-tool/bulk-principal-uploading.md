...
* 必須先授與對Admin Console的管理員存取權，才能依照這些指示操作
* 請注意，在IMS中建立使用者和群組有幾種不同的方式。  請參閱[Adobe Experience Manager as a Cloud Service的IMS支援](/help/security/ims-support.md)，瞭解所有可用的選項。  此處僅說明Admin Console大量上傳方法
* IMS中可能有三種身分型別： **Adobe ID**、**Enterprise ID**&#x200B;和&#x200B;**Federated ID**。 此頁面上的指示僅適用於&#x200B;**Adobe ID**。  如果您需要使用Enterprise ID或Federated ID，請參閱完整的[Admin Console檔案](https://helpx.adobe.com/tw/enterprise/using/admin-console.html)以及[Admin Console大量群組上傳](https://helpx.adobe.com/tw/enterprise/using/user-groups.html)和[Admin Console大量使用者上傳](https://helpx.adobe.com/enterprise/using/bulk-upload-users.html)的特定檔案。  對於這兩種身分型別，上傳檔案的規格有些不同
