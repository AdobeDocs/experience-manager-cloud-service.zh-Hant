...
* 將資料從Amazon S3和Azure Blob儲存傳輸出去需要支付相關費用。 傳輸成本會與您現有儲存容器中的資料總量相關（無論是否在AEM中參照）。 如需詳細資訊，請參閱[Amazon S3](https://aws.amazon.com/s3/pricing/)和[Azure Blob儲存空間](https://azure.microsoft.com/en-us/pricing/details/bandwidth/)。
...
#### Azure Blob儲存資料存放區 {#azure-blob-storage}

從Azure入口網站的現有容器屬性頁面中，使用&#x200B;**計算大小**&#x200B;按鈕來決定容器中所有內容的大小。 例如：
...
最新CTT版本包含Amazon S3、Azure Blob Storage和檔案資料存放區的AzCopy支援。
...
>[!NOTE]
>此設定檔案的內容會有所不同，端視您的來源AEM執行個體使用Azure、Amazon S3資料存放區或檔案資料存放區而定。

#### Azure Blob儲存資料存放區 {#azure-blob-storage-data}
