...
1. 導覽至 [Adobe開發人員控制台](https://console.adobe.io) 使用您的Adobe ID。
1. 建立新專案或開啟現有專案。
...
1. 按一下 **建立用戶映射配置**.
   ...
   * **組織ID**:輸入AdobeIdentity Management系統(IMS)組織ID，了解使用者要移轉的組織。
      ...
      >[!NOTE]
      >若要取得組織ID，請登入 [Admin Console](https://adminconsole.adobe.com/) 如果您屬於多個組織，請選擇您的組織（位於右上角）。 組織ID會以下列格式顯示於該頁面的URL中 `xx@AdobeOrg`，其中xx是IMS組織ID。  或者，您也可以在 [Adobe開發人員控制台](https://console.adobe.io) 產生存取權杖的頁面。
   ...
   * **存取權杖**:輸入您從「設定」步驟儲存的「存取Token」。
      ...
      >[!NOTE]
      >存取權杖每24小時過期一次，而需要建立新的權杖。 若要建立新代號，請返回 [Adobe開發人員控制台](https://console.adobe.io)，選擇您的專案，按一下 **使用者管理API** 並將相同的私密金鑰貼到方塊中。
