...
1. **提取封存並移除最終首碼**
   ...
   將封存提取至資料夾，並確認直接子資料夾是否以「conf」、「conf.d」、「conf.dispatcher.d」和「conf.modules.d」開頭。如果沒有，請在階層中向上移動它們。
...
1. **移除任何虛擬主機檔案**
   ...
   conf.d/enabled_vhosts中名稱含有author、unhealthy、health、lc或flush。 您也可以移除所有在「conf.d/available_vhosts」中未連結的虛擬主機檔案。
...
1. 移除或註解未引用連接埠 80 的虛擬主機區段
   ...
   `...`
   `</VirtualHost>`
請移除或加上註解。系統不會處理這些區段中的陳述式，但如果您保留這些陳述式，您最終可能仍會編輯它們，但將徒勞無功而倍感困惑。
...
1. **檢查重新寫入**
   ...
   * 然而，如果資料夾包含多個虛擬主機專屬檔案，則應將其內容複製到虛擬主機檔案中引用這些檔案的Include陳述式中。
...
1. **檢查變數**
   ...
   1. 然而，如果資料夾包含多個虛擬主機專屬檔案，則應將其內容複製到虛擬主機檔案中引用這些檔案的Include陳述式中。
