...
* CRA只能由用戶或組 `admin` 中的用戶運 `Administrators` 行。
...
* CRA可以在任何環境下運行，但最好在 *Stage* 。
   ...
   >[!NOTE]
   >為避免對業務關鍵型實例造成影響，建議在 *Author* staging環境上運行CRA，該環境盡可能接近定制、配置、內容和用戶應用程式方面的生產環境。 或者，它可在生產作者環境的克隆上 *運行* 。
...
### AEM 6.3和更新版本 {#aem-older-version}

對於AEM 6.3和更高版本，執行Cloud Readiness Analyzer的主要方式是：

1. 使用Adobe Experience Manager使用者介面導覽至「工具」->「作業」 **->** 「雲端就緒分析器」 ****。

   >[!NOTE]
   >CRA將開始一個背景程式，在工具開啟時立即產生報告。 它會顯示報表產生在準備就緒之前的指示。 您可以關閉瀏覽器標籤，稍後再返回，在報表完成時檢視報表。

產生並顯示CRA報表後，您可以選擇按一下工具頁面右上角的 **CSV** 按鈕，以逗號分隔值(CSV)格式下載報表。

您可以按一下左上角的「重新整理報表」按鈕，強制CRA清除其快取並重新產生報表。

### AEM 6.2和6.1 {#aem-specific-versions}

CRA使用者介面在AEM 6.2中僅限於產生和下載CSV報表的連結。 對於AEM 6.1，使用者介面無法運作，只能使用HTTP介面。

在所有版本中，隨附的圖樣偵測器可獨立執行。

## CRA摘要報告 {#cra-summary-report}

當CRA在AEM使用者介面中執行時，報表會在工具視窗中顯示為結果。 報表格式為：

* 報表概述： 報表本身的相關資訊，包括報表產生的時間。
* 系統概述： CRA所在AEM系統的相關資訊。
* 尋找類別： 每個區段都處理同一類別的一或多個發現。 每個區段都包含下列項目： 類別名稱、子類型、尋找計數和重要性、摘要、類別檔案的連結，以及個別尋找資訊。

系統會為每個尋找指派重要度，以指出動作的粗略優先順序。 使用的重要級別如下：
...
## CRA CSV報表 {#crs-csv-report}

當按下「CSV」按鈕時，CRA報表的CSV格式會從結果快取建立，並傳回至您的瀏覽器。 根據您的瀏覽器設定，此報告會自動下載為預設名稱的檔案 `results.csv`。 如果快取已過期，則報表將在建立和下載CSV檔案之前重新產生。

請依照下列步驟，從您的AEM例項產生摘要報表的CSV格式：
...
CRA提供HTTP介面，可做為AEM使用者介面的替代選項。 該介面同時支援HEAD和GET命令。 它可用於生成CRA報告，並以三種格式之一返回： JSON、CSV和Tab分隔值(TSV)。
...
您也可以使用命令列工具，例如或 `curl` 以 `wget` 及任何HTTP用戶端應用程式。 當未使用瀏覽器標籤和已驗證的作業時，您必須提供管理使用者名稱和密碼作為注釋的一部分。 以下是如何做到的範例：
...
在提出請求後，用戶端就不需要維持作用中狀態，即可產生報表。 報表產生可以由一個用戶端使用HTTP GET請求啟動，報表產生後，可從另一個用戶端的快取或AEM使用者介面的CSV工具檢視。
...
預設的CRA快取存留期為24小時。 在使用者介面和HTTP介面中，使用重新整理報表和重新產生快取的選項，此預設值可能適用於CRA的大部分使用。 如果您的AEM例項的報表產生時間特別長，您可能想要調整快取存留期，以將報表的重新產生降至最低。

快取存留期值儲存為以下儲存庫 `maxCacheAge` 節點上的屬性：
`/apps/readiness-analyzer/content/CloudReadinessReport/jcr:content`

此屬性的值是快取存留期（以秒為單位）。 管理員可使用CRX/DE Lite介面調整快取存留期至AEM。
