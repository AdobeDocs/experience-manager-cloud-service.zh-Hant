...
* **沒有資料來源**：如果您尚未設定表單資料模型(FDM)的資料來源，仍可建立沒有資料來源的表單。 您可以使用表單資料模型(FDM)來撰寫最適化Forms <!--and interactive communication-->，並使用範例資料加以測試。 當資料來源可用時，您可以將表單資料模型(FDM)與資料來源繫結，這會自動反映在關聯的調適型Forms中。
