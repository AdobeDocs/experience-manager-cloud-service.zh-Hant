...
![HTML5 Forms預覽](/help/forms/assets/mobile_form_on_an_ipad_date_14.png)
...
* [從JSON陣列擷取屬性值](/help/forms/invoke-service-enhancements-rule-editor.md#retrieve-property-values-from-a-json-array)：使用自訂函式叫用API，以從JSON陣列擷取值，並將值直接繫結至表單欄位。

### Forms Manager中的版本設定支援

Forms Manager現在支援最適化Forms （核心元件和基本元件）、表單片段、主題、XDP範本和二進位資產的版本設定。 您可以從「Forms和檔案」主控台建立版本、檢視歷程記錄和還原先前的狀態。 請參閱[在Forms管理員中管理表單版本](/help/forms/manage-form-versions-forms-manager.md)。

### 改善表單元件的協助工具

最適化Forms核心元件針對核取方塊群組、選項按鈕群組和面板，引入符合WCAG的語意標籤。 這些元件現在會利用`<fieldset>`和`<legend>`元素，在群組標籤與其輔助技術選項之間建立有意義的關係。 請參閱最適化Forms[中的](/help/forms/creating-accessible-adaptive-forms.md#fieldset-legend-accessibility)欄位集和圖例支援。
