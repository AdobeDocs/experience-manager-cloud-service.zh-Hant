...
description: 您無法在適用性Forms中將這些保留字詞當作識別碼。
...
# 適用性Forms關鍵字 {#adaptive-forms-keywords}

適用性Forms關鍵字是預先定義、保留的識別碼，對適用性Forms有特殊意義。 您無法在適用性Forms中將這些關鍵字當作識別碼。 下表列出所有為適用性Forms保留識別碼的關鍵字。
...
除了上述關鍵字，請避免使用類似 [適用性Forms JavaScript API](https://adobe.com/go/learn_aemforms_javascript_api_63).
