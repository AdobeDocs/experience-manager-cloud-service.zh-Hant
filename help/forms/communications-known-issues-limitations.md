...
有兩種型別的®字型。 一種型別是PCL支援的TrueType ®字型。 另一個是CFF ®。 PDF和PostScript輸出支援內嵌Type-1、TrueType和OpenType®字型。 PCL輸出支援內嵌的TrueType字型。

Type-1和®字型未內嵌在PCL輸出中。 使用Type-1和®字型格式化的內容會點陣化並產生為點陣圖影像，而且產生起來可能會比較大而且比較慢。
