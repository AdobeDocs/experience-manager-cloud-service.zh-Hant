...
若要完整存取Communications API的轉譯功能，建議您使用XDP檔案作為輸入。 有時候，可以使用PDF檔案。 不過，使用PDF檔案作為輸入有限制：

不包含XFA資料流的PDF檔案無法轉譯為PostScript、PCL或ZPL。 通訊API可以將具有XFA串流(即在Designer中建立的表單)的PDF檔案轉譯為雷射影像和標籤格式。 如果PDF檔案經過簽署、認證或包含使用許可權(使用AEM Forms Reader擴充功能服務套用)，將無法轉譯為上述列印格式。
...
有兩種型別的OpenType®字型。 一種型別是PCL支援的TrueType OpenType®字型。 另一個是CFF OpenType®。 PDF和PostScript輸出支援內嵌Type-1、TrueType和OpenType®字型。 PCL輸出支援內嵌的TrueType字型。

Type-1和OpenType®字型未內嵌在PCL輸出中。 使用Type-1和OpenType®字型格式化的內容會點陣化並產生為點陣圖影像，而且產生起來可能會比較大而且比較慢。
...
* 您只能在列印選項清單中使用一次特定轉譯型別(PDF、PRINT)。 例如，不能有兩個PRINT選項，每個選項都指定PCL演算型別。
...
* Adobe建議您在AEM雲端服務使用的雲端區域中託管資料檔案blob容器存放區。

## 常見問題 {#faq}

**我可以使用watched資料夾或其他儲存機制來儲存輸入和輸出嗎？**

目前，您可以使用Microsoft Azure Storage來儲存輸入資料和產生的檔案。 Microsoft Azure儲存提供各種選項，可[自動化資料移動作業](https://docs.microsoft.com/en-us/azure/storage/common/storage-use-azcopy-v10)。

**Microsoft Azure儲存體帳戶是否包含在Experience Manager Forms Cloud Service授權中？**

Microsoft Azure儲存帳戶獨立於Experience Manager Forms Cloud Service授權。

**通訊API會在Experience Manager Forms Cloud Service伺服器上儲存資料嗎？**

輸入和輸出資料僅儲存在Microsoft Azure儲存空間中。

**通訊API是否僅適用於Experience Manager Forms Cloud Service？ 我可以在內部部署環境中取得類似的功能嗎？**

您可以使用AEM Forms Output服務將範本(XFA或PDF)與客戶資料結合，以產生PDF、PS、PCL和ZPL格式的檔案。

相較於內部部署環境，Cloud Service提供自動調整規模與成本效益的額外優點。
