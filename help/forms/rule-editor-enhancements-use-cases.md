...
下表列出調適型Forms中規則編輯器的最近增強功能，以及每項功能的簡短說明和主要優點。：
...
**案例**：電子商務公司提供訂購單，使用者可在此選擇產品及偏好的送貨方法。 雖然產品價格是透過表單欄位擷取，但出貨成本會根據選取的方法與選取的國家/地區動態決定。

為了保持表單結構整潔並避免新增不必要的隱藏欄位，企業想要將運費處理為臨時值，以支援即時計算總金額。

在規則編輯器中使用設定變數值和取得變數值函式的&#x200B;**實作**

規則已設定為使用&#x200B;**設定變數值**&#x200B;函式設定名為&#x200B;**extracharge**&#x200B;的暫存變數。 此變數的值取決於選取的國家/地區。 例如，如果使用者選取「美國」，則值設為50。 若是其他國家/地區，則設為100。

![設定變數值](/help/forms/assets/setvalue.png)

稍後，在計算總出貨成本時，**取得變數值**&#x200B;函式會根據選取的國家/地區擷取&#x200B;**extracharge**&#x200B;值。

![取得變數值](/help/forms/assets/getvalue.png)

然後將此值新增至產品的送貨成本，結果會顯示在&#x200B;**總送貨成本**&#x200B;欄位中。

![輸出](/help/forms/assets/getsetvalue-output.png)

此方法可讓您動態計算並顯示額外費用，而不需將其儲存在可見欄位中，實現乾淨、回應式且無程式碼的使用者體驗。
...
**案例**：工作申請表單已與執行背景驗證的外部HR系統整合。 檢查完成後，系統會使用&#x200B;**背景驗證完成來更新表單！**&#x200B;訊息。 表單必須根據此結果動態調整申請人看到的內容。

此表單使用自訂事件型方法來改善模組化程度和可維護性，而不是直接將邏輯繫結到接收狀態的欄位。

**使用分派事件和觸發事件時的實作**

當背景檢查狀態更新時，規則會使用&#x200B;**分派事件**&#x200B;發出自訂事件作為&#x200B;**bgvmsg**&#x200B;以及狀態結果。 另一個規則會使用&#x200B;**觸發事件**&#x200B;接聽此事件。

以下熒幕擷取畫面會顯示套用至「背景驗證是否完成？」的規則。 選項按鈕和「bgvmsg」文字欄位。

![分派事件](/help/forms/assets/dispatch-event-rule.png)

![於觸發事件](/help/forms/assets/trigger-event-rule.png)

偵測到事件時，會檢查狀態並據此更新表單。 例如：

* 如果背景檢查通過，表單會顯示確認訊息。
* 如果需要其他檔案，表單會顯示一個區段，要求申請人上傳所需資訊以及警示訊息。

![分派事件輸出](/help/forms/assets/dispatch-trigger-output.png)
