...
* [設定變數的值](variable-in-aem-workflows.md#set-a-variable) 使用設定變數工作流程步驟。
* [使用變數](variable-in-aem-workflows.md#use-a-variable) 在全部 [!DNL AEM Forms] 擷取儲存值的工作流程步驟，以及在OR Split和Goto步驟中定義路由表示式。
...
您可以使用變數來定義使用運算式編輯器的路由運算式。 如需有關對「OR分割」步驟使用路由運算式的詳細資訊，請參閱 [OR拆分步驟](https://experienceleague.adobe.com/docs/experience-manager-65/developing/extending-aem/extending-workflows/workflows-step-ref.html#extending-aem#or-split).

在此範例中，定義路由運算式之前，請使用 [範例2](variable-in-aem-workflows.md#example2) 設定 **totalvalue** 變數中。 如果下列專案的值，則分支1為作用中： **totalvalue** 變數大於50000。 同樣地，您可以定義規則以使「分支2」在下列情況下啟動： **totalvalue** 變數小於50000。
