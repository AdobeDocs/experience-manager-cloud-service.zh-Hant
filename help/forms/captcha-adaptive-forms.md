...
`captchaPropertyNodePath` 請參考Sling存放庫中驗證碼元件的資源路徑。 此屬性用於包含特定於驗證碼元件的詳細資訊。 例如， `captchaPropertyNodePath` 包含在CAPTCHA元件上設定的reCAPTCHA雲端組態資訊。 雲端設定資訊提供 **[!UICONTROL 網站金鑰]** 和 **[!UICONTROL 秘密金鑰]** 實作reCAPTCHA服務的設定。
