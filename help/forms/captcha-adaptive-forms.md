...
1. 為reCAPTCHA設定雲端服務。
   ...
   1. 在您的Experience Manager作者例項上，前往 ![tools-1](assets/tools-1.png) > **[!UICONTROL Cloud Services]**.
...
1. 從 **[!UICONTROL 驗證碼服務]** 下拉式清單，選取 **[!UICONTROL reCaptcha]** 啟用reCAPTCHA服務(如果您已按照 [ReCAPTCHA服務，由Google提供](#google-recaptcha). 從「設定」下拉式清單中選取設定。
1. 選取類型為 **[!UICONTROL 正常]** 或 **[!UICONTROL 緊湊]** 用於reCAPTCHA介面工具集。 您也可以選取 **[!UICONTROL 不可見]** 僅在可疑活動時顯示驗證碼驗證碼挑戰的選項。 受reCAPTCHA保護的徽章（如下所示）會顯示在受保護的表單上。

   ![Google受reCAPTCHA徽章保護](assets/google-recaptcha-v2.png)

   >[!NOTE]
   >
   >不選擇 **[!UICONTROL 預設]** 從驗證碼服務下拉式清單中取代，因為預設Experience Manager驗證碼服務已遭取代。
