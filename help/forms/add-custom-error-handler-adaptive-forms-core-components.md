...
* `errors` 提及未通過驗證條件的欄位運算式以及驗證錯誤訊息。
...
## 使用規則編輯器的Invoke服務新增錯誤處理常式的需求 {#before-you-start-to-add-error-handler}

使用規則編輯器的Invoke服務新增錯誤處理常式之前：

* [為您的AEM Cloud Service環境啟用最適化Forms核心元件](enable-adaptive-forms-core-components.md).

* 瞭解如何 [建立自訂函式](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/adaptive-forms-authoring/authoring-adaptive-forms-foundation-components/add-rules-and-use-expressions-in-an-adaptive-form/rule-editor.html?lang=en#write-rules).


## 使用規則編輯器新增錯誤處理常式 {#add-error-handler-using-rule-editor}

使用 [規則編輯器的叫用服務](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/adaptive-forms-authoring/authoring-adaptive-forms-foundation-components/add-rules-and-use-expressions-in-an-adaptive-form/rule-editor.html#invoke) 動作，您會根據您用於最適化表單的資料來源來定義驗證條件。 如果您使用RESTful Web服務做為資料來源，則可以在Swagger定義檔案中定義驗證條件。 使用調適型Forms中的錯誤處理常式函式和規則編輯器，您便可有效地管理和自訂錯誤處理。 您可以使用規則編輯器定義條件，並設定要在規則觸發時執行的所需動作。 「最適化表單」會根據預先設定的驗證條件，驗證您在欄位中輸入的輸入。 如果輸入值不符合驗證標準，錯誤訊息會在最適化表單的欄位層級顯示。
...
若要瞭解如何使用預設錯誤處理常式，請 [規則編輯器的叫用服務](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/adaptive-forms-authoring/authoring-adaptive-forms-foundation-components/add-rules-and-use-expressions-in-an-adaptive-form/rule-editor.html?lang=en#invoke) 動作，以具有兩個欄位的簡單調適型表單為例， **寵物ID** 和 **寵物名稱** 並在以下位置使用預設錯誤處理常式： **寵物ID** 用於檢查由配置為呼叫外部服務的REST端點傳回的各種錯誤的欄位，例如 `200 - OK`，`404 - Not Found`， `400 - Bad Request`. 若要使用規則編輯器的叫用服務動作新增預設錯誤處理常式，請執行下列步驟：
...
若要瞭解如何使用建立和使用自訂錯誤處理常式， [規則編輯器的Invoke服務](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/adaptive-forms-authoring/authoring-adaptive-forms-foundation-components/add-rules-and-use-expressions-in-an-adaptive-form/rule-editor.html?lang=en#invoke) 動作，以含有兩個欄位的最適化表單為例， **寵物ID** 和 **寵物名稱** 並在以下位置使用自訂錯誤處理常式： **寵物ID** 用於檢查由配置為呼叫外部服務的REST端點傳回的各種錯誤的欄位，例如 `200 - OK`，`404 - Not Found`， `400 - Bad Request`.
...
讓我們將下列程式碼新增至JavaScript檔案，以在瀏覽器主控台中顯示從REST服務端點收到的回應和標題。
   ...
   >[!NOTE]
   >
   > 在 `.content.xml` 檔案，新增 `categories = [custom-errorhandler-name]`. 例如，在此案例中， [custom-errorhandler-name] 提供為 `customfunctionsdemoV2`.
...
![在調適型表單容器設定中新增使用者端資料庫的名稱](/help/forms/assets/client-library-category-name-core-component.png)
...
![在表單中新增自訂錯誤處理常式，以處理錯誤回應](/help/forms/assets/custom-error-handler.png)s


根據此規則，您為下列專案輸入的值 **寵物ID** 檢查驗證 **寵物名稱** 使用REST端點叫用的外部服務。 如果以資料來源為基礎的驗證條件失敗，則錯誤訊息會顯示在欄位層級。

![在表單中新增自訂錯誤處理常式，以處理錯誤回應](/help/forms/assets/custom-error-handler-message-core-component.png)
