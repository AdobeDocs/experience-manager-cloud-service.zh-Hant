---
title: 如何將使用者資料中的資訊新增至表單提交中繼資料？
description: 瞭解如何使用使用者提供的資料，將資訊新增至已提交表單的中繼資料。 深入瞭解如何在CRX存放庫中檢視更新的表單提交中繼資料。
...
1. 使用「中繼資料」標籤的「值」欄位來新增指令碼。 您新增的指令碼會從表單上的元素收集資料，並計算饋送至中繼資料的值。
   ...
   例如， **[!DNL true]** 如果輸入的年齡大於21歲，則會記錄在中繼資料中，並且 **[!DNL false]** 小於21時記錄。 在「中繼資料」標籤中輸入下列指令碼：
...
如果您在中繼資料中新增核取方塊元素，選取的值會儲存為逗號分隔的字串。 例如，您在表單中新增核取方塊元件，並將其名稱指定為 `checkbox1`. 在核取方塊元件屬性中，您可新增值0、1和2的「駕駛執照」、「社會安全號碼」和「護照」專案。
