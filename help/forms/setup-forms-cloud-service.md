...
在您完成服務上線後，請登入您的[!DNL AEM Forms] as a Cloud Service環境、開啟作者和發佈執行個體，並根據使用者的角色將其新增到Forms特定的[AEM群組](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/accessing/aem-users-groups-and-permissions.html?lang=zh-Hant#accessing)。 下表列出Forms專屬的AEM群組（可立即使用）以及對應的使用者型別。 此表格也提供每種使用者型別的AEM執行個體型別：
