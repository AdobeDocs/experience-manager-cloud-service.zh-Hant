...
雖然每個表單都是為特定目的而設計的，但大多數表單中有一些常見的部分，例如提供個人詳細資訊按讚姓名和位址、家庭詳細資訊、收入詳細資訊等。 每次建立新表單時，表單開發人員都必須建立這些公用區段。 自我調整Forms提供了一種方便的機制，只需區段 按讚面板或欄位群組創建表單一次，並在自我調整Forms中重複使用它們。 這些可重複使用的獨立區段稱為最適化表單片段。


## 建立片段 {#create-a-fragment}

您可以從頭開始創建最適化表單片段，或將現有最適化表單中的面板另存為片段。
...
* 當您建立最適化表單片段時，會建立片段節點，類似于CRXDe Lite中最適化表單的 guideContainer 節點。
* 不支援最適化表單中使用不同表單資料模型的片段。 例如，基於 XDP 的自我調整表單不支援基於 XDP 的片段，反之亦然。
* 您可透過 Finder 中的「最適AEM 內容表單片段」標籤使用最適化表單片段。
* 獨立自我調整表單片段中的任何運算式、腳本或樣式在通過引用插入或嵌入自我調整表單時都會保留。
* 您無法從最適化表單中編輯通過引用插入的最適化表單片段。 要進行編輯，您可以編輯獨立的最適化表單片段或將片段嵌入最適化表單中。
