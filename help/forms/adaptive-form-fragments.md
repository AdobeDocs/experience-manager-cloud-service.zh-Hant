...
1. 指定片段的標題、名稱、說明和標記。
   ...
   >[!NOTE]
   >
   >請確定您為片段指定了唯一名稱。 如果已存在另一個名稱相同的片段，則片段無法建立。

1. 按一下以開啟 **表單模型** 標籤，然後從 **「選擇自** 」下拉式功能表中為片段選取下列模型之一：
   ...
   * **表單範本**：指定使用上傳至的XDP範本建立片段 [!DNL AEM Forms]. 選取適當的XDP範本作為片段的表單模型。
   ...
   也會顯示在選取的表單中標記為片段的子表單範本。 您可以從下拉式清單中選擇最適化表單片段的子表單。

   ![從指定的表單中選擇子範本](assets/fragment-subform.png)
...
1. 在編輯模式中開啟要插入最適化表單片段的最適化表單。
1. 按一下 **側欄中 Assets** ![ 資產瀏覽器 ](assets/assets-browser.png) 。 在資產瀏覽器中，從下拉式清單中選擇 **最適化表單片段** 。

   您也可以選擇顯示所有最適化表單片段或根據其表單模型、XML 結構或基本的篩選。

1. 將最適化表單片段拖曳到最適化表單。

   >[!NOTE]
   >
   >最適化表單片段未啟用自我調整表單中的創作。 此外，您無法在JSON型最適化表單中使用XSD型片段，反之亦然。
...
您可以選擇在最適化表單中嵌入最適化表單片段，方法是按一下 **已新增片段面板工具列上的「內嵌資產」： *fragmentName* >** 按鈕，如下列範例影像所示。

![以最適化表單內嵌表單片段](assets/embed-fragment.png)
...
* 當您重新發佈更新的最適化表單片段時，所做的變更會反映在使用的最適化表單的已發佈例項中。
* 包含「驗證」元件的自我調整表單不支援匿名使用者。 此外，也不會在最適化表單片段中使用「驗證」元件。
* （ **僅** Mac）要確保表單片段功能在所有案例中均能正常運作，請將下列專案新增至/private/etc/hosts 檔案：
  `127.0.0.1 <Host machine>`**主機電腦** ：部署的 Apple Mac 電腦 [!DNL AEM Forms] 。
