...
# 與 [!DNL Adobe Analytics] 整合  {#integrate-aem-forms-with-adobe-analytics}
...
除了這些現成可用的事件之外，您還可以使用規則編輯器在調適型表單中定義自訂事件，並將這些事件對應至中的事件。 [!DNL Adobe Analytics]
...
1. 點選 ![選取產品](assets/select-analytics.png) 並選取 **[!UICONTROL 分析]**.
...
1. 點選 ![視覺效果](assets/visualization-icon.svg) 並將圖表型別拖放至「自由格式」區段。 同樣地，您可以將多個圖表型別新增至自由格式區段。
