...
# 將AEM Forms與整合 [!DNL Adobe Analytics] {#integrate-aem-forms-with-adobe-analytics}
...
<span class="preview"> 本檔案概述在最適化表單上啟用Adobe Analytics的手動程式。 不過，Adobe建議使用 [使用Experience Cloud設定自動化啟用最適化表單的Adobe Analytics](/help/forms/enable-adobe-analytics-adaptive-form-using-experience-cloud-setup-automation.md). </span>
...
除了這些現成可用的事件之外，您還可使用規則編輯器在適用性表單中定義自訂事件，並將這些事件對應至中的事件。 [!DNL Adobe Analytics]
...
1. 在 **[!UICONTROL Prop]** 區段，將下拉式清單中可用的報表套裝選項，與使用定義的資料元素相對應 [設定資料元素](#configure-data-elements).
   ...
   1. 在 **[!UICONTROL Prop]** 區段，選取報表套裝中可用表單標題的prop，然後點選 ![資料庫圖示](assets/database-icon.svg) 以將其對應至在中建立的表單標題 [設定資料元素](#configure-data-elements).
...
1. 選取 **[!UICONTROL 清除變數]** 作為「動作型別」。 點選 **[!UICONTROL 保留變更]**. 執行這些步驟後， **[!UICONTROL 動作]** 區段顯示為：
   ...
   自訂 **[!UICONTROL 動作]** 區段。 例如，您可以定義兩個 **傳送信標** 傳送資料的目標動作流程步驟 [!DNL Adobe Analytics] 並在單一步驟中將其視為頁面檢視，並將資料傳送至 [!DNL Adobe Analytics] 且請勿將其視為第二個步驟中的頁面檢視。
...
1. 在 **[!UICONTROL 開發]** 區段，點選 ![更多選項](assets/more-options-icon.svg) 然後點選 **[!UICONTROL 核准並發佈到生產環境]**.

1. 確認變更和發佈流程即將顯示在中 **[!UICONTROL 已發佈]** 區段。
...
若要使用 [!DNL Adobe Launch] 以現有的最適化表單進行設定：
...
1. 點選欄位並選取 ![規則編輯器](assets/rule-editor-icon.svg) 以開啟規則編輯器頁面。
...
1. 點選 ![選取產品](assets/select-analytics.png) 並選取 **[!UICONTROL Analytics]**.
...
1. 點選 ![視覺效果](assets/visualization-icon.svg) 並將圖表型別拖放至自由格式區段。 同樣地，您可以將多個圖表型別新增至自由格式區段。
...
>[!MORELIKETHIS]
>
>*[在最適化表單中啟用Adobe Analytics](/help/forms/enable-adobe-analytics-adaptive-form-using-experience-cloud-setup-automation.md)