---
title: 如何將AEM最適化表單連結至Microsoft&amp； reg； SharePoint清單？
description: 將最適化表單連線至Microsoft&amp； reg； SharePoint清單。 瞭解如何設定Microsoft&amp； reg； SharePoint清單，並使用設定建立表單資料模型(FDM)。 此外，您也會瞭解如何將FDM與最適化表單整合。
role: User, Developer
...
**Microsoft® SharePoint**： Microsoft® SharePoint提供動態且有效率的團隊網站，供所有團隊、部門及部門共同作業。 它可用來儲存、組織、共用及存取使用任何網頁瀏覽器(例如Microsoft、Edge®Internet Explorer、Chrome或Firefox)之任何裝置的資訊。 **Microsoft® SharePoint**&#x200B;的兩個主要元件為：

* **Microsoft® SharePoint檔案庫**： Microsoft® SharePoint檔案庫會顯示檔案和資料夾清單及其關鍵資訊，例如上次修改日期和檔案所有者。 此功能可讓您輕鬆組織和導覽檔案。
如需如何整合**Microsoft® SharePoint檔案庫**&#x200B;與最適化表單的說明，請參閱[最適化表單提交動作](/help/forms/configuring-submit-actions.md#submit-to-sharepoint)文章。
...
若要將Microsoft® SharePoint清單整合至您的最適化表單[設定最適化表單以使用表單資料模型(FDM)](/help/forms/creating-adaptive-form-core-components.md#configure-a-schema-or-form-data-model-for-an-adaptive-formconfigure-schema-or-data-model-for-form)
...
瞭解如何[使用AEM Workflow](https://experienceleague.adobe.com/docs/experience-manager-learn/foundation/workflow/use-workflow.html)，在最適化表單中共同作業、管理及處理內容。
