...
# 處理表單資料模型 (FDM) {#work-with-form-data-model}
...
當您在表單資料模型(FDM)中設定OData服務時，實體容器中的所有導覽屬性都可透過表單資料模型(FDM)中的服務使用。 在此TripPin OData服務範例中，可以使用表單資料模型(FDM)中的一個`Person`服務來讀取`GET LINK`實體容器中的三個導覽屬性。
...
>[!NOTE]
>
>**[!UICONTROL NavigationPropertyName]**&#x200B;引數的&#x200B;**預設值**&#x200B;欄位中可用的值取決於&#x200B;**[!UICONTROL 傳回陣列的狀態？]**&#x200B;切換按鈕。 當啟用時，它會顯示集合型別的導覽屬性。
