...
# 設計最適化表單的 XML 結構 {#creating-adaptive-forms-using-xml-schema}
...
`bindRef`是最適化表單元件與結構描述元素或屬性之間的連線。 它指定從這個元件或欄位擷取的值可在輸出XML中使用的`XPath`。 從預填（預填） XML預先填入欄位值時，也會使用`bindRef`。
...
>[!MORELIKETHIS]
>
>* [為最適化表單設計JSON結構描述](/help/forms/adaptive-form-json-schema-form-model.md)