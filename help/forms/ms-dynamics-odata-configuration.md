...
description: 瞭解如何根據中定義的實體、屬性和服務來建立表單資料模型(FDM) [!DNL Microsoft Dynamics] 服務。
...
[!DNL Microsoft Dynamics] 是一款客戶關係管理(CRM)與企業資源規劃(ERP)軟體，提供企業解決方案，用於建立及管理客戶帳戶、聯絡人、銷售機會、商機與案例。 [[!DNL Experience Manager Forms] 資料整合](data-integration.md) 提供OData雲端服務設定，將Forms與線上和內部部署整合 [!DNL Microsoft Dynamics] 伺服器。 它可讓您根據中定義的實體、屬性和服務來建立表單資料模型(FDM) [!DNL Microsoft Dynamics] 服務。 表單資料模型(FDM)可用來建立與互動的最適化Forms [!DNL Microsoft Dynamics] 伺服器以啟用業務工作流程。 例如：

* 查詢 [!DNL Microsoft Dynamics] 資料伺服器並預先填入Adaptive Forms
* 將資料寫入 [!DNL Microsoft Dynamics] 在最適化表單提交上
* 將資料寫入 [!DNL Microsoft Dynamics] 透過表單資料模型(FDM)中定義的自訂實體，反之亦然
...
開始設定之前 [!DNL Microsoft Dynamics]，確定您擁有：
...
* [已登入應用程式 [!DNL Microsoft Dynamics] 線上服務，使用 [!DNL Microsoft Azure] 主動式目錄](https://docs.microsoft.com/en-us/dynamics365/customer-engagement/developer/walkthrough-register-dynamics-365-app-azure-active-directory). 記下註冊服務的使用者端ID （也稱為應用程式ID）和使用者端密碼的值。 這些值用於 [為您的設定雲端服務 [!DNL Microsoft Dynamics] 服務](#configure-cloud-service-for-your-microsoft-dynamics-service).

## 設定已登入的回覆URL [!DNL Microsoft Dynamics] 應用計畫 {#set-reply-url-for-registered-microsoft-dynamics-application}

執行下列動作，設定已登入的回覆URL [!DNL Microsoft Dynamics] 應用程式

>[!NOTE]
>
>請僅在整合時使用此程式 [!DNL Experience Manager Forms] 使用線上 [!DNL Microsoft Dynamics] 伺服器。

1. 前往 [!DNL Microsoft Azure] Active Directory帳戶並在中新增下列雲端服務設定URL **[!UICONTROL 回覆URL]** 註冊應用程式的設定：
...
[!DNL Microsoft Dynamics] 使用宣告型驗證來提供資料存取權，位置： [!DNL Microsoft Dynamics] CRM伺服器給外部使用者。 若要啟用此功能，請執行以下動作以設定 [!DNL Microsoft Dynamics] 適用於網際網路對向部署(IFD)，並設定宣告設定。
...
1. 執行以下命令：
   ...
   * `redirect-uri` 為的URL [!DNL Microsoft Dynamics] 上的OData雲端服務 [!DNL Experience Manager Forms]. 預設的雲端服務已安裝 [!DNL Experience Manager Forms] 部署於下列URL：
     `https://'[server]:[port]'/libs/fd/fdm/gui/components/admin/fdmcloudservice/createcloudconfigwizard/cloudservices.html`
...
1. [!DNL Microsoft Dynamics] 使用HTTPS通訊協定。 從啟動AD FS端點 [!DNL Forms] 伺服器，安裝 [!DNL Microsoft Dynamics] 使用Java憑證存放區的網站憑證 `keytool` 執行的電腦上的命令 [!DNL Experience Manager Forms].
...
在 **[!UICONTROL 驗證設定]** 標籤：
   ...
   1. 取代 **[!UICONTROL 使用者端ID]** (也稱為 **應用程式ID**)， **[!UICONTROL 使用者端密碼]**， **[!UICONTROL OAuth URL]**， **[!UICONTROL 重新整理記號URL]**， **[!UICONTROL 存取記號URL]**、和 **[!UICONTROL 資源]** 包含下列專案之值的欄位： [!DNL Microsoft Dynamics] 服務組態。 您必須在「 」中指定動態執行個體URL **[!UICONTROL 資源]** 要設定的欄位 [!DNL Microsoft Dynamics] 使用表單資料模型(FDM)。 使用服務根URL衍生動態執行個體URL。 例如， [https://org.crm.dynamics.com](https://org.crm.dynamics.com/).
...
表單資料模型(FDM)
1. 按一下 **[!UICONTROL 連線至OAuth]**. 您被重新導向至 [!DNL Microsoft Dynamics] 登入頁面。
1. 使用您的帳戶登入 [!DNL Microsoft Dynamics] 認證並接受，以允許雲端服務設定連線至 [!DNL Microsoft Dynamics] 服務。 建立表單資料模型(FDM) 、雲端服務和服務是一次性工作。
...
* 叫用 [!DNL Microsoft Dynamics] 使用最適化表單規則的表單資料模型(FDM)中定義的伺服器作業
