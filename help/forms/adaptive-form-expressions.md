...
™是最適化Forms的運算式語言。 所有運算式都是有效的™運算式，並使用最適化Forms指令碼模型API。 這些運算式會傳回某些型別的值。 如需最適化Forms類別、事件、物件和公用API的完整清單，請參閱最適化Forms的[JavaScript™資料庫API參考](https://helpx.adobe.com/tw/experience-manager/6-5/forms/javascript-api/index.html)。
