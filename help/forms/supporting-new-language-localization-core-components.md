...
1. **URL地區設定選擇器([地區設定])**：
   ...
   系統會使用，為URL中指定的地區設定優先順序 [地區設定] 選擇器。 此格式允許快取以獲得更好的效能。

   格式： URL遵循以下格式： http:/[AEM Forms伺服器URL]/content/forms/af/[afName].[地區設定].html？wcmmode=disabled.

   範例： https://[伺服器]/content/forms/af/contact-us.hi.html會以印地語轉譯表單。


1. **afAcceptLang請求引數**：

   若要覆寫使用者的瀏覽器地區設定，您可以使用 `afAcceptLang` URL中的引數。

   範例： https://[伺服器]/forms/af/survey.ca-fr.html？afAcceptLang=ca-fr強制表單以加拿大法文轉譯。

1. **使用者的瀏覽器地區設定（Accept-Language標頭）**：

   如果未指定其他語言環境，AEM Forms會考慮使用者透過以下方式傳送的瀏覽器語言環境： `Accept-Language` 標頭。
...
* 如果無法取得所要求地區設定的使用者端資源庫（新增新地區設定的程式，稍後將在本文中說明），AEM Forms會根據地區設定中的語言代碼檢查資源庫。
  ...
  如果找不到合適的使用者端資料庫，預設字典(大部分 `en`)作為表單的編寫語言。
...
* 純文字編輯器(IDE)：雖然任何純文字編輯器都可以運作，但整合式開發環境(IDE)喜歡 [Microsoft Visual Studio Code](https://code.visualstudio.com/download) 提供進階功能，讓編輯更輕鬆。

* Git：管理程式碼變更需要此版本控制系統。 如果您尚未安裝，請從下載 [https://git-scm.com](https://git-scm.com).
...
1. 執行以下命令以複製存放庫：
   ...
   此命令會下載存放庫並建立名為的資料夾 `aem-core-forms-components` 在您的電腦上。 在本指南中，我們將此資料夾稱為 `[Adaptive Forms Core Components repository]`
...
1. 在編輯器中開啟存放庫資料夾。
   ...
   ![編輯器中的存放庫資料夾](/help/forms/assets/repository-folder-in-an-editor.png)

1. 找到 `Guide Localization Service.cfg.json` 檔案。 此檔案可控制您的AEM Forms應用程式支援的地區設定。 您可以編輯此檔案以新增語言環境。

   * **現有檔案**：如果檔案已存在，請在AEM Forms專案目錄中找出該檔案。 典型位置為：

     ```Shell
     [AEMaaCS project directory]/ui.config/src/main/content/jcr_root/apps/<appid>/osgiconfig/config`. 
     ```

     取代 `<appid>` 與您的專案特定應用程式ID搭配使用。 您可以找到 `<appid>` 中的AEM專案 `archetype.properties` 檔案。

     ![原型屬性](/help/forms/assets/archetype-properties.png)

   * **新增檔案**：如果檔案不存在，則必須在上述相同位置中建立檔案。 請勿從此檔案複製貼上檔案名稱，而是手動輸入名稱。 檔案名稱 `Guide Localization Service.cfg.json` 包含空格。 這是刻意的操作，不是檔案中的拼寫錯誤。
...
1. 將您所需語言的地區設定代碼新增到檔案中。
   1. 使用 [ISO 639-1代碼清單](https://en.wikipedia.org/wiki/List_of_ISO_639_language_codes) 尋找代表您所需語言的兩字母代碼。

   1. 將地區設定代碼包含到 `Guide Localization Service.cfg.json` 檔案。 以下是一些範例：
...
1. 進行變更後，請確定 `Guide Localization Service.cfg.json` 檔案的格式正確顯示為有效的JSON檔案。 JSON格式設定錯誤可能會使其無法正常運作。 儲存檔案。



### 利用範例使用者端資料庫輕鬆進行地區設定

AEM Forms提供實用的使用者端資料庫範例， `clientlib-it-custom-locale`，以簡化新增地區設定的作業。 此程式庫屬於 [最適化Forms核心元件存放庫](https://github.com/adobe/aem-core-forms-components)，可在GitHub上取得。


開始之前，請確定您有 [最適化Forms核心元件存放庫]. 如果不適用，您就可以使用Git透過以下命令輕鬆複製：
...
![本機電腦上的Adaptive Forms核心元件存放庫目錄](/help/forms/assets/core-forms-components-repo-on-local-machine.png)
...
1. 找到範例使用者端程式庫：
   ...
   在您的本機復本內 [最適化Forms核心元件存放庫]，導覽至下列路徑：
...
1. 複製並貼上程式庫：
   ...
   1. 複製 `clientlib-it-custom-locale` 資料夾。
      ...
      ![複製clientlib-it-custom-locale](/help/forms/assets/clientlib-it-custom-locale-copy.png)
   ...
   1. 導覽至以下目錄： [AemCS專案目錄]：
      ...
      **重要**：取代 `<app-id>` 以及應用程式的實際ID。

   1. 貼上複製的 `clientlib-it-custom-locale` 資料夾放入此目錄中。
...
1. 瀏覽到地區設定目錄：
   ...
   在您的 `[AEMaaCS project directory]`，導覽至下列路徑：

   ```
       /ui.apps/src/main/content/jcr_root/apps/<program-id>/clientlibs/clientlib-it-custom-locale/resources/i18n/
   ```

   **重要**：取代 `<program-id>` 以及您的實際應用程式ID。

1. 找到範例英文檔案：

   AEM Forms提供 [GitHub上的範例英文地區設定檔案(.json)](https://github.com/adobe/aem-core-forms-components/blob/master/ui.af.apps/src/main/content/jcr_root/apps/core/fd/af-clientlibs/core-forms-components-runtime-all/resources/i18n/en.json).
...
1. 建立必要的資料夾（如果遺失）：
   ...
   如果 `etc` 資料夾不存在於 `jcr_root` 資料夾，建立它。 內部 `etc`，建立另一個名為的資料夾 `languages` 如果遺失的話。

1. 建立地區設定檔：

   在 `languages` 資料夾，建立名為的新檔案 `.content.xml`. 請勿從此檔案複製貼上檔案名稱，而是手動輸入名稱。

   ![建立名為的新檔案 `.content.xml`](etc-content-xml.png)

   開啟此檔案並貼上下列內容，取代 [LOCALE_CODE] 實際地區設定代碼（例如，阿拉伯文的ar-ae）。
...
1. 在filter.xml中包含新資料夾：
   ...
   導覽至 `/ui.content/src/main/content/meta-inf/vault/filter.xml` 中的檔案 [AemCS專案目錄].
...
* 部署AEMas a Cloud Service、 [AemCS專案目錄]，前往您的Cloud Service環境。 若要部署至您的Cloud Service環境：
   ...
   1. 提交您的變更：
      ...
      新增地區設定後，請以描述地區設定新增的清晰Git訊息(例如「新增支援 [地區名稱]「)。
...
* 當數值方塊和日期選擇器元件轉換為特定地區設定時，可能會出現格式問題。 若要減少此問題， **語言** 選項已納入的「設定」對話方塊 [日期選擇器元件](https://experienceleague.adobe.com/en/docs/experience-manager-core-components/using/adaptive-forms/adaptive-forms-components/number-input#formats-configure-tab) 和 [Numerc Box元件](https://experienceleague.adobe.com/en/docs/experience-manager-core-components/using/adaptive-forms/adaptive-forms-components/numeric-box#formats-configure-tab).


* 處理新欄位：

   * **機器翻譯**：如果使用機器翻譯，您需要重新建立字典並重新[執行翻譯專案](/help/forms/using-aem-translation-workflow-to-localize-adaptive-forms-core-components.md) 將新欄位新增到現有的最適化表單之後。 初始翻譯專案後新增的新欄位仍維持未翻譯狀態。
