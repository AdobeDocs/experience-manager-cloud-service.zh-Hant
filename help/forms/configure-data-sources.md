...
[!DNL Experience Manager Forms] 資料整合可讓您設定並連線至不同的資料來源。 下列類型可立即使用。 不過，只要很少自訂，您也可以整合其他資料來源。
...
RESTful Web服務可使用 [Swagger規格](https://swagger.io/specification/) JSON或YAML格式 [!DNL Swagger] 定義檔案。 要配置RESTful Web服務，請在 [!DNL Experience Manager] as a Cloud Service，確保您擁有 [!DNL Swagger] 檔案，或托管檔案的URL。
