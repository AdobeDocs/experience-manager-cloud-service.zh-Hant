...
>[!NOTE]
>
>**佈景主題名稱：**&#x200B;當您將Forms佈景主題內嵌到網站佈景主題中，並透過前端管道進行部署時，**不需要變更任何佈景主題名稱**。 表單樣式會成為現有網站主題的一部分，該主題會以其目前名稱建置和部署。 只有在您從專用主題存放庫部署`package.json`獨立&#x200B;**Forms主題時，才需要變更主題名稱（例如**）；此情境在[使用主題來設定核心元件型最適化Forms](/help/forms/using-themes-in-core-components.md)中的說明。
