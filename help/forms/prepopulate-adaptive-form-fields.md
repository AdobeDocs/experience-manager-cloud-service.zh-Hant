...
- 針對適用性Forms，使用 [XFA表單範本](#xfa-based-af)，請使用與XFA範本資料結構相容的預填XML。
...
## 配置預填服務 {#configuring-prefill-service-using-configuration-manager}

使用 `alloweddataFileLocations` 屬性 **預設預填服務配置** 設定資料檔案的位置，或設定資料檔案位置的規則運算式（規則運算式）。
