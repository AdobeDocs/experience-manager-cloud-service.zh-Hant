...
建議的設定是使用AEM網站與Adobe Commerce存放區檢視之間的1:1關係。

若要將AEM網站和AEM CIF核心元件連線到專用的商店檢視，請執行以下操作：

## 設定 {#configuration}

1. 根據[Adobe Commerce網站、商店和檢視中所述的模式，設定多個商店和商店檢視。](https://experienceleague.adobe.com/docs/commerce-admin/start/setup/websites-stores-views.html?lang=zh-Hant)

1. 請確定AEM與Adobe Commerce之間的連線正常運作。

1. 依照下列步驟建立CIF Cloud Service設定的子設定：
...
1. 將子設定指派至AEM網站。
   ...
   * 前往AEM Sites主控台。
   * 導覽至網站的區域或語言根目錄.structure。 例如，Venia範例頁面為`/content/venia/us _or_ /content/venia/us/en`。
   * 選取頁面並開啟頁面屬性。
   * 選取「進階」標籤。
