...
* `{{variant_sku}}` 將替換為當前選定的變數，例如， `VP09-KH-S`
...
### 多個目錄頁 {#multiple-product-pages}

當編輯希望完全控制站點的頂級導航時，可能不希望使用單個目錄頁面來呈現目錄的頂級類別。 相反，編輯器可以建立多個目錄頁，其中一個頁面用於他們希望包括在頂級導航中的目錄的每個類別。

對於該使用情形，每個目錄頁都可能具有對特定於為目錄頁配置的類別的產品和類別頁的引用。 的 `UrlProvider` 將使用這些連結為配置類別中的頁面和類別建立連結。 但是，出於效能原因，只考慮站點導航根/登錄頁的直接目錄頁子級。

建議將目錄頁的產品和類別頁後代到該目錄頁，否則，導航或Breadcrumb等元件可能無法正常工作。

>[!NOTE]
>
> 需要對多個目錄頁提供完全支援 [CIF核心元件2.10.0](https://github.com/adobe/aem-core-cif-components/releases/tag/core-cif-components-reactor-2.10.0) 或更新。
...
許多預設URL格式以某種方式彼此相容，這意味著由一個格式化的URL可能由另一個格式解析。 這有助於在URL格式之間遷移。
