...
在用戶端傳送GraphQL請求之前，會檢查是否 **精準** 已快取相同的GraphQL請求，並且可能會傳回快取回應。 為了比對，GraphQL請求必須完全符合，即查詢、操作名稱（如果有）、變數（如果有）必須全部等於快取請求，而且所有可設定的自訂HTTP標題也必須相同。 例如，Adobe Commerce `Store` 標題必須符合。
...
除了純AEM管理內容CIF外，頁面通常還可顯示透過GraphQL從Adobe Commerce動態擷取的商務資料。 雖然頁面結構本身可能永遠不會變更，但商務內容可能會變更，例如，如果某些產品資料（名稱、價格等） 變更Adobe Commerce。
