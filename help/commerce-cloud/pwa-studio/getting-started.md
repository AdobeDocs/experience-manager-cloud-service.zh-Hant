...
使用Adobe Experience Manager中的內容片段模式編輯器，可以輕鬆設計所需的結構。 整合Adobe Experience Manager內容片段（或任何其他資料）與您的PWA Studio應用程式的主要挑戰是從多個GraphQL端點擷取資料。 這是因為PWA Studio可立即使用單一Adobe Commerce GraphQL端點。
...
1. Apollo使用者端預期的AEM GraphQL端點為 `<https://pwa-studio/endpoint.js>`. 若要將端點對應至此位置，您需要自訂PWA Studio應用程式的UPPER設定： a。將AEM_CFM_GRAPHQL變數新增至pwa-root/.env，並將其調整為指向您的AEM內容片段GraphQL端點。
...
為了展示GraphQL端點，我們透過內容套件準備了一些範例內容片段模型和資料。 這些可與PWA Studio擴充功能隨附的React Components搭配使用。
...
* 哪一種方法最適合您，在很大程度上取決於您如何將PWA Studio應用程式提供給一般使用者。

此擴充功能提供兩個範例。

### 部落格 {#blog}

根據某些內容片段模型顯示部落格。 此外，它還包含如何設定Apollo使用者端以使用AEM GraphQL端點的範例，以及如何在PWA Studio中擴充導覽元件。 另請參閱 [GitHub](https://github.com/adobe/aem-pwa-studio-extensions/tree/master/aem-cfm-blog-extension) 以取得更多詳細資料。
