...
# Adobe Experience manager雲端服務架構簡介 {#an-introduction-to-the-architecture-adobe-experience-manager-as-a-cloud-service}
...
* 針對AEM Sites的雲端服務：
   ...
   * 作者層由單個作者群集中的兩個或多個節點組成。 它會根據撰寫活動自動縮放。
      ...
      * 資產整合與處理使用專屬的資產計算服務。
   * 發佈層由單個發佈群中的兩個或多個節點組成：它們可以彼此獨立運作。 每個節點都由AEM發行者和配備AEM Dispatcher模組的網頁伺服器所組成。 它會根據網站流量需求自動調整規模。
