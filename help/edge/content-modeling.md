...
>[!WARNING]
>
>可能的話，不需要或建議實作自訂AEM元件。 AEM提供的Edge Delivery Services元件已足夠，並提供特定護欄，以便利開發。
>
...
例如，Teaser元件可能允許作者僅建立子標題、標題和單一段落說明，以及最多兩個召喚行動按鈕的組合。 將這些元素分組在一起會產生語義標示，無需進一步操作即可設定樣式。
...
在試算表的頁面屬性中，定義您需要的中繼資料欄位以及URL。 然後為每個頁面路徑或頁面路徑模式新增中繼資料。
