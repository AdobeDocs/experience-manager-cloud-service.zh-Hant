---
title: 使用Edge Delivery Services專案進行WYSIWYG製作的內容模型
description: 瞭解內容模型如何搭配Edge Delivery Services專案用於WYSIWYG創作，以及如何模型化您自己的內容。
...
# 使用Edge Delivery Services專案進行WYSIWYG製作的內容模型 {#content-modeling}

瞭解內容模型如何搭配Edge Delivery Services專案用於WYSIWYG創作，以及如何模型化您自己的內容。

## 先決條件 {#prerequisites}

搭配Edge Delivery Services使用WYSIWYG編寫的專案會繼承任何其他Edge Delivery Services專案的大部分機制，不受內容來源或[編寫方法的限制。](/help/edge/wysiwyg-authoring/authoring.md)
...
為了提出一個引人注目的內容模型 (採用與內容來源無關的方式)，了解這些概念至關重要。本檔案提供專為WYSIWYG編寫實作的機制詳細資訊。
...
區塊是用來建立具有特定樣式和功能的更豐富內容。和預設內容不同，區塊確實需要其他語意。

區塊本質上是由 JavaScript 修飾並使用樣式表設定樣式的內容片段。

### 區塊模式定義 {#model-definition}

搭配Edge Delivery Services使用WYSIWYG製作時，必須明確將區塊內容模型化，才能為作者提供建立內容所需的介面。 基本上，您需要建立一個模式，以便製作 UI 知道根據區塊向作者呈現哪些選項。
...
針對使用Edge Delivery Services進行WYSIWYG編寫，這會產生一個問題，即當資訊是使用由多個欄位組成的表單進行編寫時，如何提供引人入勝的語意內容模型，而不是像RTF文字一樣在內容中編輯語意標籤。
...
請參閱檔案[建立檢測成可與Universal Editor搭配使用的區塊](/help/edge/wysiwyg-authoring/create-block.md)，瞭解如何建立檢測成可與Universal Editor搭配使用的區塊，以便在WYSIWYG製作中與Edge Delivery Services專案搭配使用。

如果您已熟悉建立區塊，請參閱檔案[使用Edge Delivery Services進行WYSIWYG編寫的開發人員快速入門手冊](/help/edge/wysiwyg-authoring/edge-dev-getting-started.md)，以使用Edge Delivery Services和通用編輯器來讓您啟動並執行新的Adobe Experience Manager網站，以進行內容編寫。

>[!TIP]
>
>如需使用AEM as a Cloud Service作為內容來源來建立啟用WYSIWYG撰寫的新Edge Delivery Services專案的端對端逐步解說，請檢視[此AEM GEM網路研討會。](https://experienceleague.adobe.com/en/docs/events/experience-manager-gems-recordings/gems2024/wysiwyg-authoring-and-edge-delivery)
