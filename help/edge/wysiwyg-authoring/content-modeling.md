...
為了提出一個引人注目的內容模型 (採用與內容來源無關的方式)，了解這些概念至關重要。本檔案提供專為WYSIWYG編寫所實作的機制詳細資訊。
...
搭配Edge Delivery Services使用WYSIWYG製作時，必須明確將區塊內容模型化，才能為作者提供建立內容的介面。 基本上，您需要建立一個模式，以便製作 UI 知道根據區塊向作者呈現哪些選項。
...
使用Edge Delivery Services進行WYSIWYG製作時，這會產生一個問題：當資訊的製作表單由多個欄位組成，而非像RTF文字那樣編輯內容中的語意標籤時，如何提供引人入勝的語意內容模型。
