...
# 在通用編輯器中建立和使用 Edge Delivery Services 表單片段
...
* **利用表單片段維持一致性**
您可以將片段與不同的表單整合，幫助您維持一致的版面與標準化內容。透過「一次更改，全面反映」的方法，對片段所做的任何更新都會自動套用至所有表單。
...
* 您無法在表單內編輯透過參考插入的表單片段。若要編輯，請修改獨立的表單片段。

## 建立 Edge Delivery Services 表單片段的先決條件
...
1. (選用) 按一下以開啟「**表單模型**」標籤，接著從「**選取表單**」下拉式選單中選取下列其中一種片段模型：
   ...
   * **無模型**：指定此選項，即可不使用任何表單模型，從頭開始建立表單。
   ...
   >[!NOTE]
   >
   > 若要了解如何在通用編輯器中將表單或片段與表單資料模型 (FDM) 整合，以使用不同的後端資料來源，[請按一下此處](/help/edge/docs/forms/universal-editor/integrate-forms-with-data-source.md)。

1. (選用) 在「**進階**」標籤中指定片段的「**發佈日期**」或「**取消發佈日期**」。
...
1. 按一下「**編輯**」，便會在通用編輯器中開啟使用預設範本建立的片段，以供進行製作。

   ![通用編輯器中供製作使用的片段](/help/edge/docs/forms/universal-editor/assets/fragment-in-ue.png)
   ...
   ![聯絡人資訊片段](/help/edge/docs/forms/universal-editor/assets/contact-fragment.png)

   建立片段後，您可以[在 Edge Delivery Services 表單中新增所建立的片段](#adding-form-fragments-in-forms)。

### 在表單中新增表單片段

建立一個簡單的 `Employee Details` 表單，其中包含員工和主管的資訊。您可以在員工和主管面板中使用相同的 `Contact Details` 片段。若要在表單中使用表單片段，請執行下列步驟：
...
1. 按一下「**[!UICONTROL 選取]**」。
   ...
   表單片段即會以參考方式新增至表單，並與獨立的表單片段保持同步。這表示對片段的任何修改，都會同步反映在所有整合該片段的表單實例中。

   ![表單內的片段](/help/edge/docs/forms/universal-editor/assets/fragment-in-form.png)
