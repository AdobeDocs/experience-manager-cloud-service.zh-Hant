...
使用&#x200B;**通用編輯器**&#x200B;建立自適應表單元件的自訂版面時，您必須使用 **CSS 媒體查詢中斷點**&#x200B;來定義回應式行為。此動作可確保表單在不同裝置和螢幕尺寸均正確顯示。
...
- 使用這些中斷點來控制元件在不同裝置上調整大小、堆疊或隱藏的方式。
- 請依照您組織的回應式設計準則，以維持一致的 UX。
- 在多個裝置和方向測試版面配置，來確保可用性和可存取性。
...
>[!NOTE]
>
> 通用編輯器不提供用於定義回應式行為的使用者介面。所有版面自訂均須透過 CSS 處理。
