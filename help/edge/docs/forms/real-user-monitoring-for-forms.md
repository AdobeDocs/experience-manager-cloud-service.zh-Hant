---
title: AEM Formsas a Cloud ServiceEdge Delivery Services的真實使用者監控
description: AEM Formsas a Cloud ServiceEdge Delivery Services的真實使用者監控涉及持續追蹤和分析使用者與表單的互動。
...
# AEM Formsas a Cloud ServiceEdge Delivery Services的真實使用者監控

Real User Monitoring (RUM)可讓您在真實世界中深入瞭解訪客如何與您的Adobe Experience Manager (AEM)網站互動。 此內建工具提供寶貴資料，有助於瞭解使用者行為、診斷效能問題，以及評估網站實驗的成效。 RUM不僅透過擷取真實使用者互動來進行合成測試，更精確地呈現您網站的效能。
