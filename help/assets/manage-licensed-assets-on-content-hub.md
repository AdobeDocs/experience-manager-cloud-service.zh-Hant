...
在AEM Assets Content Hub上管理授權資產可讓管理員確保僅具有有效授權的已核准資產可供使用，同時在AEM作者環境中維持合規性和正確的中繼資料追蹤。

### 如何在Experience Manager as a Cloud Service中將授權欄位新增至資產屬性？

在AEM Assets檢視中，您可以編輯中繼資料表單以包含對應至`dc:license`屬性的新文字欄位，藉此將授權欄位新增至資產屬性。 然後，此欄位會出現在AEM Assets製作環境的資產屬性中。

### 如何將中繼資料表單套用至資產資料夾，以在AEM Assets的資產屬性中包含授權欄位？

在AEM Assets檢視中，編輯中繼資料表單以包含授權欄位。 將此中繼資料表單套用至所需的資產資料夾，以確保新設定已納入該資料夾中的所有資產。

### 如何在AEM Assets檢視中指定資產的授權詳細資訊？

若要指定授權詳細資訊，請選取資產，按一下&#x200B;**詳細資料**&#x200B;以檢視其屬性，並在新增至中繼資料表單的授權欄位中輸入已核准資產授權的絕對路徑。

### 資產授權的AEM Assets Content Hub絕對路徑需要什麼格式？
...
### 誰負責管理AEM Assets Content Hub中的授權資產？

管理員負責編輯中繼資料表單、將表單指派至資產資料夾，以及在AEM Assets Content Hub中核准資產及其授權。
