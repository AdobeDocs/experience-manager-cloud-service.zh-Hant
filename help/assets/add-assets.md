...
使用此功能需要Azure或AWS的外部儲存帳戶或貯體。

>[!NOTE]
>
>將儲存帳戶容器或貯體建立為私人，並僅接受授權請求的連線。 但是，不支援對入口網路連接的其他限制。

>[!NOTE]
>
>外部儲存帳戶的檔案/資料夾名稱規則可能與批量導入工具不同。 請參閱 [大量匯入期間處理檔案名稱](#filename-handling-bulkimport) 有關禁止/逸出名稱的詳細資訊。
...
### 大量匯入期間處理檔案名稱 {#filename-handling-bulkimport}
...
在導入過程中會跳過名稱與這些條件匹配的檔案或資料夾，並標籤為失敗。
...
若是資料夾檔案名稱，系統會使用API處理JCR名稱和路徑： `DamUtil.getSanitizedFolderName`.

* 大寫字元會轉換為小寫
* 未更改Unicode字元
* 以破折號(&#39;-&#39;)取代特殊字元，例如， `new folder` 更新為 `new-folder`:
