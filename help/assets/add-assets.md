...
[!DNL Adobe Experience Manager Assets]接受許多來源的數位資產型別。 它會儲存二進位檔和已建立的轉譯，可以使用各種工作流程和[!DNL Adobe AI]服務進行資產處理，允許透過許多管道在多個表面進行分發。
...
需要來自Azure或AWS的外部儲存帳戶或貯體，才能使用此功能。
...
若要對已上傳的資產執行其他處理，您可以對上傳資料夾套用處理設定檔。 設定檔可在&#x200B;**[!UICONTROL 中資料夾的]**&#x200B;屬性[!DNL Assets]頁面中使用。 沒有副檔名或副檔名不正確的數位資產不會視需要處理。 例如，上傳這類資產時，可能沒有任何反應，或資產適用的處理設定檔不正確。 使用者仍然可以在DAM中儲存二進位檔案。
...
* [!DNL Experience Manager Assets]現在支援包含大量子資產的檔案夾。 一旦資料夾包含1000多個直接子項（資產或子資料夾），Admin UI就會使用非同步更新的索引來列出資料夾內容。 因此，新建立資料夾和資產的可見度可能會有短暫的延遲（通常只需幾秒），當在Admin View中開啟這類資料夾時，會顯示橫幅以通知使用者此行為，並宣告如下：「此目錄包含1000多個專案。 上傳和新資料夾建立可能延遲。」
