...
雖然 [AEM Core Components](https://docs.adobe.com/content/help/en/experience-manager-core-components/using/introduction.html) （核心元件）提供非常完整、有彈性且可自訂的API，可針對此用途提供必要的讀取作業，而且可自訂其JSON輸出，但它們確實需要AEM WCM（網頁內容管理）的相關知識才能實作，因為它們必須裝載在以專用AEM範本為基礎的頁面中。 並非每個SPA開發組織都能直接獲得此類知識。
