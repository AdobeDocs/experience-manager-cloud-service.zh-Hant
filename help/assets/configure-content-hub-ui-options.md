...
>[!VIDEO](https://video.tv.adobe.com/v/3472957/?captions=chi_hant&learn=on&enablevpops){transcript=true}
...
* [篩選器](#configure-filters-content-hub)
...
* [集合與共用](#configure-collections-content-hub)
...
1. 指定屬性的標籤，使用&#x200B;**[!UICONTROL 中繼資料]**&#x200B;欄位將其對應到屬性，然後為新資產中繼資料選取適當的輸入型別。 更新輸入型別標籤，以與Adobe Experience Manager (AEM)一致。 可用的輸入型別包括「文字」、「標籤」、「日期」、「數字」、「下拉式清單」、「多值文字」、「智慧標籤」和「智慧顏色標籤」。 對於某些中繼資料欄位(xcm:colorDistribution， xcm:machineKeywords)，輸入型別會自動指派且無法修改。
...
在Content Hub上![設定UI上傳詳細資料](assets/filter-new.png)
...
1. 指定篩選的標籤，使用&#x200B;**[!UICONTROL 中繼資料]**&#x200B;欄位將其對應到屬性，然後為新篩選選取適當的輸入型別。 更新輸入型別標籤，以與Adobe Experience Manager (AEM)一致。 可用的輸入型別包括「文字」、「標籤」、「日期」、「數字」、「下拉式清單」、「多值文字」、「智慧標籤」和「智慧顏色標籤」。 對於某些中繼資料欄位(xcm:colorDistribution， xcm:machineKeywords)，輸入型別會自動指派且無法修改。
...
在Content Hub![上設定](assets/add-filters.png)使用者介面篩選器
...
1. 啟用&#x200B;**[!UICONTROL 大量搜尋]**&#x200B;切換。 預設分隔符號`[ , | \t | \r\n | \r | \n ]`會自動顯示。 此外，您也可以新增其他分隔字元。 若要這麼做，請在以`pipe symbol (|)`分隔的輸入方塊中指定分隔符號。
   ...
   ![大量搜尋設定](assets/edit-filter-new.png)
...
在Content Hub上![設定UI資產詳細資料](assets/asset-detailsn.png)
...
### 資產可見度{#asset-visibility-content-hub}
...
轉譯是數位資產（例如影像、檔案等）的自訂版本，專為不同裝置和平台而設計，可確保最佳效能。 檢視更多有關Adobe Experience Manager Assets[中](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-cloud-service/content/assets/assets-view/renditions)轉譯的資訊。
...
### 誰可以存取及設定AEM Assets Content Hub使用者介面設定？

只有具有Content Hub管理員許可權的使用者才能存取及設定Content Hub使用者介面設定。 存取設定介面之前，必須先透過Adobe Admin Console授予管理許可權。

### 管理員可以在AEM Assets Content Hub UI中設定什麼？

管理員可以在AEM Assets Content Hub組態使用者介面中設定下列設定：

* 匯入中繼資料欄位（包括必填欄位和自動核准設定）

* 搜尋頁面上顯示的搜尋篩選器

* 用於編制索引的可搜尋中繼資料欄位

* 資產卡片中繼資料（卡片上最多可顯示6個欄位）

* 資產詳細資料檢視屬性

* 品牌元素（標誌、橫幅文字、顏色、橫幅影像）
...
這些設定可控制一般使用者的Content Hub視覺體驗和功能行為。


### 管理員如何在AEM Assets Content Hub中為資產上傳新增中繼資料欄位？

管理員可以在&#x200B;**匯入**&#x200B;索引標籤中按一下&#x200B;**新增中繼資料**，指定標籤、從中繼資料結構描述將其對應至適當的屬性、選取輸入型別，以及決定欄位是必要欄位還是選用欄位，來新增用於資產上傳的中繼資料欄位。 確認並儲存之後，使用者可在資產上傳期間使用新欄位。

### 什麼是AEM Assets Content Hub中資產的自動核准設定，其運作方式？

「自動核准」設定啟用時，新資產可在上傳後立即於Content Hub上使用。 此設定預設為關閉，要求AEM作者或管理員在新資產顯示於Content Hub入口網站之前手動核准。

### 管理員如何在AEM Assets Content Hub中設定用於資產搜尋的篩選器？

管理員可以設定資產搜尋的篩選器，方法是按一下&#x200B;**新增篩選器**、指定篩選器標籤、將其對應至適當的中繼資料屬性、選取資料型別，以及確認新增。 至少有一個資產符合篩選條件時，介面便可使用篩選器。 按一下&#x200B;**儲存**&#x200B;即可套用變更。

### 資產詳細資料檢視中會顯示哪些資產屬性，以及這些屬性在AEM Assets Content Hub中的設定方式？

檔案名稱、標題、格式、大小等資產屬性以及類別等自訂欄位，可顯示在資產詳細資料檢視中。 管理員可按一下&#x200B;**新增中繼資料**、指定標籤、將其對應至中繼資料結構描述、選取資料型別、確認及儲存變更，以新增這些屬性。

### 管理員如何在AEM Assets Content Hub中自訂資產卡上顯示的中繼資料詳細資訊？

管理員可以自訂顯示在資產卡片上的中繼資料詳細資訊，方法是按一下資產卡片設定中的&#x200B;**新增中繼資料**、指定標籤（例如檔案格式）、將其對應到中繼資料結構描述中的適當屬性，以及確認新增。 資產卡上最多可顯示6個中繼資料欄位。

### 管理員可以自訂您的AEM Assets Content Hub入口網站以符合貴組織的品牌需求嗎？

可以。 管理員可以編輯主要橫幅的標題和正文文字、透過檢色器或十六進位編碼選取文字顏色，以及設定背景、文字和圖示的次要和主要顏色，以個人化AEM Assets Content Hub的外觀和風格。
