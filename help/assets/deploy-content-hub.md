...
* [Content Hub管理員](#onboard-content-hub-administrator)：除了存取品牌核准的資產、將資產上傳到Content Hub、Adobe Express整合以編輯影像(如果您有Adobe Express許可權)之外，還能存取Content Hub上的[設定使用者介面](/help/assets/configure-content-hub-ui-options.md)。
...
* [除了存取Content Hub入口網站上的Adobe Express核准資產之外，還有權將資產重新混合到新變體](#onboard-content-hub-users-remix-assets)： [品牌整合](/help/assets/edit-images-content-hub.md) (如果您有Adobe ExpressContent Hub許可權)。
...
1. 導覽至Experience Manager Assetsas a Cloud Service程式，按一下「更多選項」圖示(...)並選取&#x200B;**[!UICONTROL 編輯程式]**。
...
使用Cloud Manager](#enable-content-hub)為Assetsas a Cloud Service[啟用Content Hub後，在Admin Console的AEM Assets as a Cloud Service中建立新的執行個體，尾碼為`contenthub`：
...
1. [將使用者新增至Content Hub產品設定檔](#onboard-content-hub-users)後，請按一下Admin Console上產品清單中的AEM as a Cloud Service產品名稱來存取Experience Manager Assets產品設定檔。
...
有權將資產重新混合到新變數的Content Hub使用者可以[使用Adobe Express修改現有資產，並將資產儲存到存放庫](/help/assets/edit-images-content-hub.md)。 只有使用者擁有Adobe Express權益，才能使用Adobe Express編輯資產。
