...
>[!VIDEO](https://video.tv.adobe.com/v/3472946/?captions=chi_hant&learn=on){transcript=true}
...
### 使用者如何取得AEM Assets Content Hub的存取權，以及可以指派哪些許可權？

透過Adobe Admin Console將使用者指派至AEM Assets的相關產品設定檔，即可透過Content Hub將使用者新增至Content Hub。

使用者可以使用下列許可權：

* Content Hub使用者可以在Content Hub入口網站上存取品牌核准的資產。

* 除了存取品牌核准資產、將資產上傳到Content Hub以及整合Adobe Express以編輯影像(如果您有Content Hub許可權)之外，Content Hub管理員還可以存取Adobe Express上的設定使用者介面。

* 有權新增資產的Content Hub使用者除了可在Content Hub入口網站上存取品牌核准的資產外，還能將資產上傳至Content Hub。

* 有權重新混合資產的Content Hub使用者除了可以在Content Hub入口網站上存取品牌核准的資產外，還可以存取Adobe Express (如果您有Adobe Express許可權)。

### AEM Assets Content Hub中不同型別的使用者有哪些可用的不同產品設定檔？

AEM Assets Content Hub上的不同使用者型別可使用產品設定檔：

* Content Hub使用者： AEM Assets受限使用者

* Content Hub管理員： AEM Assets受限使用者+ AEM管理員

* 有權新增資產的Content Hub使用者： AEM Assets受限使用者+ AEM使用者

* 有權重新混合資產的Content Hub使用者： AEM Assets有限使用者+ AEM使用者

### 管理員如何為其組織啟用AEM Assets Content Hub？

若要為組織啟用AEM Assets Content Hub，管理員必須登入Cloud Manager、選取（或建立）其程式、在解決方案和附加元件索引標籤底下啟用Assets和Content Hub，以及更新程式。 這會在Adobe Admin Console中建立Content Hub例項，以便管理使用者的存取許可權。
...
### 我的Content Hub授權中有多少個AEM Assets積分？

可用的Content Hub積分數量取決於您的AEM Assets授權：

* Assets Ultimate包含三個Content Hub積分。

* Assets Prime包含一個Content Hub評分。

* 現有的Assets as a Cloud Service客戶會獲得一個Content Hub評分。

### 如何使用AEM Assets Content Hub積分？

每個啟用Content Hub的環境都會消耗一個Content Hub評分。 例如，在生產、開發和中繼環境中啟用Content Hub需要三個積分。

### 我可以在較低層級環境中啟用Content Hub嗎？

可以。如果您有AEM Assets Content Hub的可用積分，即可在較低層環境（例如開發或中繼）上啟用Content Hub 。 啟用的每個較低環境會消耗一個評分。

### 我如何擁有在AEM Assets Content Hub上存取已核准資產的許可權？

AEM Assets Content Hub使用者可以在Content Hub入口網站上存取品牌核准的資產。 您必須新增至AEM有限使用者產品設定檔，才能成為Content Hub使用者。

### 我如何擁有在AEM Assets Content Hub上傳資產的許可權？

有權新增資產的AEM Assets Content Hub使用者除了可在Content Hub入口網站上存取品牌核准的資產外，還能將資產上傳至Content Hub。 您必須新增至AEM有限使用者和AEM使用者產品設定檔，才能成為具有新增資產許可權的Content Hub使用者。

### 我如何擁有在AEM Assets Content Hub上存取「設定」使用者介面的許可權？

AEM Assets Content Hub管理員除了可存取品牌核准資產、將資產上傳到Content Hub和Adobe Express整合以編輯影像(如果您有Adobe Express許可權)之外，還能夠存取Content Hub上的設定使用者介面。 您必須新增至AEM有限使用者和AEM管理員產品設定檔，才能成為Content Hub管理員。

### 我如何擁有在AEM Assets Content Hub上使用Adobe Express編輯影像的許可權？

有權重新混合資產的AEM Assets Content Hub使用者除了可以在Content Hub入口網站上存取品牌核准的資產外，還可以存取Adobe Express (如果您有Adobe Express許可權)。 您必須新增至AEM有限使用者和AEM使用者產品設定檔，才能成為具有資產混合許可權的Content Hub使用者。
