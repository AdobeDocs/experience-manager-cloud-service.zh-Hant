...
### 使用者如何取得Content Hub的存取權以及可以指派哪些許可權？

透過Adobe Admin Console將使用者指派至Content Hub的相關產品設定檔，即可將使用者新增至Content Hub。
...
### Content Hub上不同型別的使用者有哪些可用的不同產品設定檔？

Content Hub上的不同使用者型別可使用產品設定檔：
...
### 管理員如何為其組織啟用Content Hub？

管理員必須登入Cloud Manager、選擇（或建立）其計畫、在解決方案和附加元件索引標籤下啟用Assets和Content Hub，以及更新計畫。 這會在Adobe Admin Console中建立Content Hub例項，以便管理使用者的存取許可權。
...
可以。如果您有Content Hub的可用積分，即可在較低環境（例如開發或中繼）上啟用Content Hub。 啟用的每個較低環境會消耗一個評分。

### 我如何擁有在Content Hub上存取已核准資產的許可權？

Content Hub使用者可以在Content Hub入口網站上存取品牌核准的資產。 您必須新增至AEM有限使用者產品設定檔，才能成為Content Hub使用者。

### 我如何擁有在Content Hub上傳資產的許可權？

有權新增資產的Content Hub使用者除了可在Content Hub入口網站上存取品牌核准的資產外，還能將資產上傳至Content Hub。 您必須新增至AEM有限使用者和AEM使用者產品設定檔，才能成為具有新增資產許可權的Content Hub使用者。

### 我如何擁有在Content Hub上存取「設定」使用者介面的許可權？

除了存取品牌核准資產、將資產上傳到Content Hub以及整合Adobe Express以編輯影像(如果您有Content Hub許可權)之外，Content Hub管理員還可以存取Adobe Express上的設定使用者介面。 您必須新增至AEM有限使用者和AEM管理員產品設定檔，才能成為Content Hub管理員。

### 我如何擁有在Content Hub上使用Adobe Express編輯影像的許可權？

有權重新混合資產的Content Hub使用者除了可以在Content Hub入口網站上存取品牌核准的資產外，還可以存取Adobe Express (如果您有Adobe Express許可權)。 您必須新增至AEM有限使用者和AEM使用者產品設定檔，才能成為具有資產混合許可權的Content Hub使用者。
