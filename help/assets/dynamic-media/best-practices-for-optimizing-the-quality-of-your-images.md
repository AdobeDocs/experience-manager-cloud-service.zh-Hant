...
* 最佳實務白皮書 [Adobe Dynamic Media Classic影像品質和銳利化最佳作法](/help/assets/dynamic-media/assets/sharpening_images.pdf) 也適用於Experience Manager。

* Watch [使用影像銳利化搭配Experience Manager- Dynamic Media](https://experienceleague.adobe.com/docs/experience-manager-learn/assets/dynamic-media/dynamic-media-image-sharpening-feature-video-use.html#dynamic-media).
...
* 遮色片銳利化( `&op_USM`) — 遮色片銳利化是業界標準的銳利化濾鏡。 最佳作法是依照下列准則，使用非銳利化遮色片來銳利化影像。 遮色片銳利化可讓您控制下列三個參數：
   ...
   * `&op_sharpen=`數量，半徑，臨界值
      ...
      * **[!UICONTROL 臨界值]** （0-255，效應的敏感性。）
      此參數可決定銳化像素與周圍區域的差異程度，之後才會被視為邊緣像素，濾鏡會銳化這些像素。此 **[!UICONTROL 臨界值]** 參數有助於避免色彩類似的區域過度銳利化，例如膚色。 例如，閾值為12會忽略膚色亮度的微小變化，以避免加上「雜訊」，同時仍會加上邊緣對比度至高對比區域，例如睫毛與皮膚相遇的區域。
...
參數 `jpegSize` 如果您想要保證影像不會超過特定大小，以便傳送至記憶體有限的裝置，則此功能會很實用。
...
* Experience Manager也提供更進階的影像品質調整方法，例如在擷取時套用銳利化影像。 為優化渲染結果， [Adobe諮詢服務](https://business.adobe.com/customers/consulting-services/main.html) 可協助您運用自訂的深入分析和最佳實務。
