...
然而，這項失效不適用於不支援智慧型影像的一般網域，例如 `s7d1.scene7.com`. 這類網域仍需要完整URL才能成功讓工作失效。
...
1. 在 **[!UICONTROL CDN失效]** - **[!UICONTROL 新增詳細資料]** 頁面上，選取要讓CDN失效的資產。
...
1. 在 **[!UICONTROL CDN失效]** - **[!UICONTROL 確認]** 頁面，在 **[!UICONTROL URL]** 清單方塊中，會顯示從您先前建立的CDN失效範本以及您剛才新增的資產所產生的一或多個URL清單。

   例如，使用先前步驟中顯示的CDN失效範本範例，假設您新增了名為的單一資產 `spinset`. 當您前往 **[!UICONTROL 工具]** > **[!UICONTROL 資產]** > **[!UICONTROL CDN失效]**，而會在中產生下列5個URL **[!UICONTROL CDN失效 — 確認]** 使用者介面：
...
| 錯誤 | 解釋 |
| --- | --- |
| *無法擷取所選資產的URL。* | 在符合下列任一情況時發生：<br> — 找不到Dynamic Media設定。<br> — 擷取讀取Dynamic Media設定的服務使用者時會發生例外狀況。<br>- Dynamic Media設定中缺少用來組成URL的發佈伺服器或公司根目錄。 |
