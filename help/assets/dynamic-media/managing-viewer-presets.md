...
請參閱[動態媒體影像提供與轉譯API說明](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/responsive-static-image-library/c-about-responsive-static-image-library.html?lang=zh-Hant#about-responsive-image-library)中的&#x200B;*回應式靜態影像庫*，以取得有關如何將回應式檢視器內嵌至網頁的詳細資訊。
...
1. 執行下列任一項作業：
   ...
   * 如果您已在個別的CSS檔案中定義樣式設定，則可將CSS檔案上傳至Experience Manager Assets。 若要尋找已上傳的CSS檔案並將其與檢視器預設集建立關聯，請選取&#x200B;**[!UICONTROL 選取型別]**&#x200B;下拉式選單下方的&#x200B;**[!UICONTROL 匯入CSS]** （如有必要，請向上捲動視覺編輯器以檢視它）。
     ...
     只要為指定檢視器正確定義CSS標籤，您就可以匯入任意的手工製作CSS。 (CSS標籤在&#x200B;*檢視器參考指南*&#x200B;的任何「自訂[&lt;檢視器名稱>](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources.html?lang=zh-Hant)檢視器」說明主題中都有說明。 例如，如果您想要閱讀有關縮放檢視器的CSS標籤，請參閱[自訂縮放檢視器](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/viewers-aem-assets-dmc/zoom/customizing-zoom/c-html5-20-zoom-viewer-customizingviewer.html?lang=zh-Hant)。)不過，視覺化編輯器可能不瞭解某些CSS值。 在這種情況下，視覺化編輯器會嘗試覆寫錯誤，讓CSS仍可運作。
...
當您建立或編輯互動式視訊檢視器預設集時，可以選擇要使用的顯示模式設定。 當您從`InteractiveSwatches`行為&#x200B;**[!UICONTROL 標籤下的]**&#x200B;選取的元件&#x200B;**[!UICONTROL 下拉式功能表選取]**&#x200B;時，就會發生這個選擇。 您選擇的「顯示」模式會影響縮圖在視訊播放時的顯示方式和時間。 您可以選擇「顯 `segment`示」模式 (預設) 或「顯 `continuous` 示」模式。
...
<table>
 <tbody>
  ...
  <tr>
   <td>連續</td>
   <td><p>在<code>continuous </code>顯示模式中，假設區段中的縮圖數目小於面板中顯示的數目。 在這種情況下，檢視器會自動包含顯示下一個區段或上一個區段（最後顯示縮圖）的縮圖。</p> <p>此主題<a href="/help/assets/dynamic-media/interactive-videos.md">中的</a>影片是<code>continuous </code>顯示模式的範例。</p> </td>
  </tr>
 </tbody>
</table>
