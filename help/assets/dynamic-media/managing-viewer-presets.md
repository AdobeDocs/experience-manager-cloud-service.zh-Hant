...
例如， *視頻播放器* 類型，在 **[!UICONTROL 修改量]** > **[!UICONTROL 播放]**，可以從三個自適應比特率流選項中選擇一個：
   ...
   * **[!UICONTROL 划線]**  — 視頻流僅作為DASH。 但是，在Safari/iOS設備上，必須選擇 **[!UICONTROL 合肥]** 的下界。
   * **[!UICONTROL 合肥]**  — 視頻流僅作為HLS。
   * **[!UICONTROL 自動]**  — 最佳做法。 DASH和HLS流的建立是儲存優化的。 因此，Adobe建議您始終選擇 **[!UICONTROL 自動]** 鍵。 視頻流為破折號、合肥光源或累進光源，如下所示：
   ...
   >[!NOTE]
   >
   >查看和使用 **[!UICONTROL 划線]** 選項，必須首先通過帳戶上的Adobe技術支援來啟用它。 請參閱 [啟用帳戶上的DASH](/help/assets/dynamic-media/video.md#enable-dash)。
...
1. 發佈新查看器預設。 必須發佈預設，以便您可以在網站上使用其生成的URL。

   請參閱 [發佈查看器預設](#publishing-viewer-presets)。

   >[!IMPORTANT]
   >
   >對於使用自適應比特率流配置檔案的舊視頻，URL將繼續照常播放，直到您 [重新處理視頻資產](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets)。 重新處理後，同一URL將繼續工作，但現在 *兩者* 已啟用DASH和HLS流。

### 建立互動式查看器預設的特殊注意事項 {#special-considerations-for-creating-an-interactive-viewer-preset}

**關於面板中影像縮略圖的顯示模式：**

建立或編輯互動式視頻查看器預設時，可以選擇使用哪種「顯示模式」設定。 當您選擇 `InteractiveSwatches` 從 **[!UICONTROL 所選元件]** 下面的下拉菜單 **[!UICONTROL 行為]** 頁籤。 您選擇的「顯示」模式會影響縮覽圖在視頻播放時的顯示方式和顯示時間。 您可以選擇「顯 `segment`示」模式 (預設) 或「顯 `continuous` 示」模式。
