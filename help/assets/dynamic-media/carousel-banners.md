...
1. [識別熱點和影像對應變數](#identifying-hotspot-and-image-map-variables) （僅適用於使用Adobe Experience Manager Assets + Dynamic Media的客戶）
   ...
   從識別現有快速檢視實作所使用的動態變數開始。 這麼做可協助您在Experience Manager Assets的轉盤橫幅建立程式中，正確輸入熱點和影像地圖資料。
...
1. 執行下列任一項作業：
   ...
   * [在Experience ](/help/assets/dynamic-media/adding-dynamic-media-assets-to-pages.md) Manager中將轉盤橫幅新增至您的網站如果您是Experience Manager Sites客戶，可使用互動式媒體元件將轉盤集直接新增至Experience Manager的頁面。
...
從識別現有快速檢視實作所使用的動態變數開始。 這有助於您在Experience Manager Assets的轉盤集建立程式中，正確輸入熱點或影像地圖資料。

將熱點或影像地圖新增至橫幅影像時，您會指派SKU和可選的額外變數至每個熱點或影像地圖。 這些變數稍後會用來比對熱點或影像地圖與快速檢視內容。
...
但是，在複雜情況下，快速檢視URL除了SKU以外，還有不同的元素，例如類別ID、色彩碼、大小碼等。 在這種情況下，每個元素都是熱點中的個別變數，或轉盤橫幅功能中的影像地圖資料定義。
...
如果您已上傳您要使用的影像，請進入下一個步驟[建立轉盤集](#creating-carousel-sets)。 在啟用動態媒體後，必須上傳轉盤中使用的影像。
...
1. 您可以點選或按一下「行為與外觀」標籤，並調整轉盤橫幅的外觀或特定元件的行為，以變更轉盤集的視覺設計與行為。 如需如何使用檢視器編輯器的詳細資訊，請參閱[管理檢視器預設集](/help/assets/dynamic-media/viewer-presets.md)。
   ...
   >    * 編輯檢視器預設集之前，請選擇您要建立預設集的樣式。 如果您不選擇樣式，當您開始編輯檢視器預設集時，如果您決定變更為其他預設集，您會遺失所有變更。
...
>[!NOTE]
>
>當您在[互動式影像](/help/assets/dynamic-media/interactive-images.md)或轉盤橫幅中新增熱點至影像時，熱點資訊會儲存在相對於影像位置的相同中繼資料位置。 無論是「互動式影像」或「轉盤橫幅」，此點都成立。 這項功能表示，您可以在任一檢視器中，輕鬆地重複使用相同的影像及其定義的熱點資料。
...
1. 在&#x200B;**[!UICONTROL Actions]**&#x200B;標籤中執行下列操作之一：
   ...
   * 點選&#x200B;**[!UICONTROL 快速檢視]**。
      ...
      * 如果您是Experience Manager Sites <!-- and Ecommerce-->客戶，請點選「產品選擇器」圖示（放大鏡）以開啟「選擇產品」頁面。 若要返回轉盤橫幅編輯器，請點選您要使用的產品，然後點選頁面右上角的核取標籤。
      * 如果您不是Experience Manager Sites <!-- or Ecommerce -->客戶：

         * 定義變數。 請參閱[識別熱點變數](#identifying-hotspot-and-image-map-variables)。
         * 然後，手動輸入SKU值。 在「SKU值」文字欄位中，輸入產品的SKU（庫存保留單位），此為您提供之每個不同產品或服務的唯一識別碼。 輸入的SKU值會自動填入「快速檢視」範本的變數部分，讓系統知道將點選的熱點與特定SKU的「快速檢視」產生關聯。
   ...
   * 點選&#x200B;**[!UICONTROL 體驗片段]**。
      ...
      * 如果您是AEM Sites客戶，請點選「搜尋」圖示（放大鏡）以開啟「體驗片段」頁面。 若要返回熱點管理頁面，請點選或按一下您要使用的體驗片段，然後點選頁面右上角的「選取」。
請參閱[體驗片段](/help/sites-cloud/authoring/fundamentals/experience-fragments.md)。
...
1. 點選&#x200B;**[!UICONTROL Save]**。
1. 發佈轉盤集。 發佈會建立您可在網站頁面上使用的內嵌代碼或URL。 如果您是Experience Manager Sites客戶，請將轉盤集直接新增至網頁。
...
您可以使用「預覽」來查看轉盤橫幅對客戶的外觀，並測試轉盤橫幅熱點和影像地圖，以確保其運作如預期。

當您對轉盤橫幅感到滿意時，可以發佈它。
請參閱[將視訊或影像檢視器內嵌至網頁](/help/assets/dynamic-media/embed-code.md)。
請參閱[將URL連結至您的Web應用程式](/help/assets/dynamic-media/linking-urls-to-yourwebapplication.md)。 如果您的互動式內容具有相對URL的連結，尤其是AEM Sites頁面的連結，則無法使用以URL為基礎的連結方法。
...
發佈轉盤以使用它。 發佈轉盤集會啟動URL和內嵌代碼。 此外，它還會將轉盤發佈至與CDN整合的Dynamic Media雲端，以進行可擴充且具效能的傳送。
...
不過，如果您是獨立的Experience Manager Assets客戶，您可以依本主題所述，手動將轉盤橫幅新增至網站登陸頁面。

1. 複製已發佈的轉盤集的內嵌代碼。
請參閱[將視訊或影像檢視器內嵌至網頁](/help/assets/dynamic-media/embed-code.md)。

1. 將您從Experience Manager Assets複製的內嵌代碼新增至網頁。
