...
您也可以立即發佈上傳的資產，不需任何使用者干預。 或者，您可以選擇性地發佈這些資產。 請參閱[配置Dynamic Media。](config-dm.md) 或者，您可以使用資料夾層級的「選擇性發佈」，將資產選擇性地發佈至Dynamic Media或AEM彼此互斥的「選擇性發佈」( **[!UICONTROL Selective]** Publishat)。請參閱[在 Dynamic Media 中使用選擇性發佈](/help/assets/dynamic-media/selective-publishing.md)。
