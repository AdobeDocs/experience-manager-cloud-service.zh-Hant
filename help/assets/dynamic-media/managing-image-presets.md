...
| 寬度上限 | 2048 | 產生的預覽轉譯的最大寬度（畫素）。 |
| 高度上限 | 2048 | 所產生預覽轉譯的最大高度（畫素）。 |
| 解析度 | 72 | 點陣化第一頁的解析度，以ppi （每英吋畫素）為單位。 |
...
1. 在 **[!UICONTROL 編輯影像預設集]** 視窗中，輸入值至 **[!UICONTROL 基本]** 和 **[!UICONTROL 進階]** 適當標籤，包括名稱。 這些選項在「影像預設 [集選項」中概述](#image-preset-options)。預設集會出現在左窗格中，並可與其他資產一起即時使用。
...
| 欄位 | 說明 |
| --- | --- |
| **名稱** | 輸入描述性名稱，不含任何空格。 為協助使用者識別此影像預設集，請在名稱中包含影像大小規格。 |
...
| **銳利化** | 選取 **啟用簡單銳利化** 以在執行所有縮放後套用基本銳利化濾鏡。 銳利化有助於彌補以不同大小顯示影像時可能產生的模糊。 |
