...
1. 執行下列任一項作業：
   ...
   * 若要在「詳細資料檢視」頁面中開啟它，請選取影像。 在工具列中，選取 **[!UICONTROL 編輯]**.
...
1. 執行下列任一項作業：
   ...
   * 選取 **[!UICONTROL 快速檢視]**.
      ...
      * 如果您是Experience Manager Sites或電子商務客戶，請選取「產品選擇器」圖示（放大鏡）以開啟「選取產品」頁面。 選取您要使用的產品，然後選取 **選取** 在頁面的右上角。 您會返回「熱點」管理頁面。
      * 如果您是 *非* Experience Manager Sites或電子商務客戶
         ...
         * （選用）如果快速檢視中有其他變數可用來進一步識別產品，請選取「 」 **[!UICONTROL 新增一般變數]**. 在文字欄位中，指定額外的變數。 例如， `category=Mens` 是新增的變數。
