...
智慧型裁切座標視長寬比而定。 對於影像描述檔中的智慧型裁切設定，如果在影像描述檔中新增的尺寸的長寬比相同，則會傳送相同的長寬比給Dynamic Media。 Adobe建議您使用相同的裁切區域。 這樣做可確保對「影像描述檔」中使用的不同尺寸沒有影響。

您建立的每個智慧型裁切產生都需要額外處理。 例如，新增超過5種智慧型裁切外觀比例會導致資產擷取速度變慢。 它還可能增加系統的負載。 由於您可以在資料夾級別應用智慧裁切，因此Adobe建議您僅在需要智慧裁切的&#x200B;*資料夾*&#x200B;上使用智慧裁切。
...
除了將描述檔套用至資料夾外，您也可以全域套用一個描述檔。 任何上傳至任何資料夾中「Experience Manager資產」的內容都已套用選取的描述檔。
