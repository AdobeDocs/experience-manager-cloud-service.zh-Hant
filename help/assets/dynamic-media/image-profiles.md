...
description: 建立動態媒體影像描述檔，其中包含非銳利遮色片和智慧型裁切或智慧型色票的設定，或兩者皆有。 然後，將描述檔套用至影像資產的檔案夾。
...
智慧型裁切座標視長寬比而定。 對於影像描述檔中的智慧型裁切設定，如果影像描述檔中新增尺寸的外觀比例相同，則會將相同的外觀比例傳送至動態媒體。 Adobe建議您使用相同的裁切區域。 這樣做可確保對「影像描述檔」中使用的不同尺寸沒有影響。

您建立的每個智慧型裁切產生都需要額外處理。 例如，新增超過5種智慧型裁切外觀比例會導致資產擷取速度變慢。 它還可能增加系統的負載。 由於您可以在資料夾層級套用智慧型裁切，Adobe建議您只在需要智慧型裁切的資料夾&#x200B;**&#x200B;上使用。
...
<table>
 <tbody>
  ...
  <tr>
   <td>顏色及影像樣本</td>
   <td>批量為每個影像生成一個影像色板。</td>
   <td><p><strong>注意</strong>:Dynamic Media Classic不支援智慧型色票。</p> <p>從顯示顏色或紋理的產品影像自動尋找並產生高品質色票。</p> <p>若要使用色票和影像色票，請從「裁切選項」下拉式清單中選取「智慧裁切」<strong>。 </strong>然後，在「顏色和影像色票」的右側，啟用（開啟）功能。 在「寬度」和「高度」文字方塊中輸入像素值。</p> <p>雖然所有影像裁切都可從「轉譯」邊欄使用，但色票僅能透過「複製URL」功能使用。 使用您自己的檢視元件來轉換您網站上的色票。 此規則的例外是轉盤橫幅。 動態媒體提供轉盤橫幅中所用色票的檢視元件。</p> <p><strong>使用影像色票</strong></p> <p>影像色票的URL很簡單：</p> <p><code>/is/image/company/&lt;asset_name&gt;:Swatch</code></p> <p>其中<code>:Swatch</code>會附加至資產請求。</p> <p><strong>使用色票</strong></p> <p>若要使用色票，請提出<code>req=userdata</code>要求，並包含以下內容：</p> <p><code>/is/image/&lt;company_name&gt;/&lt;swatch_asset_name&gt;:Swatch?req=userdata</code></p> <p>例如，以下是Dynamic Media Classic中的色票資產：</p> <p><code>https://my.company.com:8080/is/image/DemoCo/Sleek:Swatch</code></p> <p>以下是色票資產對應的<code>req=userdata</code> URL:</p> <p><code>https://my.company.com:8080/is/image/DemoCo/Sleek:Swatch?req=userdata</code></p> <p><code>req=userdata</code>回應如下：</p> <p><code class="code">SmartCropDef=Swatch
...
要定義其他資產類型的高級處理參數，請參閱[配置資產處理](config-dm.md#configuring-asset-processing)。
...
1. 輸入遮色片、裁切或色票或兩者的描述檔名稱和值。
   ...
   提示：使用與其預期用途特定的描述檔名稱。 例如，假設您想要建立僅產生色票的描述檔。 也就是說，智慧型裁切會停用（關閉），並啟用（開啟）顏色和影像色票。 在這種情況下，您可以使用描述檔名稱「智慧色票」。

   另請參 [閱智慧型裁切和智慧型色票選項](#crop-options)[和遮色片銳利化](#unsharp-mask)。

   ![作物](assets/crop.png)

1. 點選&#x200B;**[!UICONTROL Save]**。 新建立的配置檔案將出現在可用配置檔案清單中。
...
將映像配置檔案分配給資料夾時，任何子資料夾都會自動從其父資料夾繼承配置檔案。 因此，您只能將一個「影像描述檔」指派給資料夾。 因此，請仔細考慮您上傳、儲存、使用和封存資產的檔案夾結構。

如果您將不同的影像描述檔指派給資料夾，新的描述檔會覆寫先前的描述檔。 舊有的資料夾資產仍維持不變。 新的描述檔會套用至稍後新增至資料夾的資產。

在用戶介面中會指明為其分配了配置檔案的資料夾，其名稱將顯示在卡中。

<!-- When you add smart crop to an existing Image Profile, you need to re-trigger the [DAM Update Asset workflow](assets-workflow.md) if you want to generate crops for existing assets in your asset repository. -->

您可以將影像描述檔套用至特定資料夾，或全域套用至所有資產。

您可以重新處理已有現有影像描述檔的資料夾中的資產，而您稍後會變更該資料夾。 請參閱編輯資料夾的處理設定檔](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets)後，重新處理資料夾中的資產。[

### 將動態媒體映像配置檔案應用於特定資料夾{#applying-image-profiles-to-specific-folders}

您可以從&#x200B;**[!UICONTROL 工具]**&#x200B;菜單或者如果您在資料夾中，從&#x200B;**[!UICONTROL 屬性]**&#x200B;將影像配置檔案應用到資料夾。
...
1. 點選「**[!UICONTROL 將處理設定檔套用至檔案夾]**」，並選取您要用來接收新上傳資產的檔案夾或多個檔案夾，然後點選／按一下「套用」。 ****&#x200B;已為其分配配置檔案的資料夾將通過資料夾名稱正下方的配置檔案名稱顯示來指示。
...
除了將描述檔套用至資料夾外，您也可以全域套用一個描述檔。 任何資料夾中上傳至Experience Manager Assets的任何內容都已套用選取的描述檔。
...
1. 執行下列任一項作業：
   ...
   * 導覽至CRXDE Lite至下列節點：`/content/dam/jcr:content`。
      ...
      新增屬性`imageProfile:/conf/global/settings/dam/adminui-extension/imageprofile/<name of image profile>`並點選&#x200B;**[!UICONTROL 全部儲存]**。
...
將包含智慧裁切的映像配置檔案應用到資料夾後，該資料夾中的所有映像都將對其應用裁切。 如果需要，您可以手動&#x200B;*重新對齊或調整多張影像中智慧型裁切視窗的大小，以進一步調整其焦點。*
...
您可以從&#x200B;**[!UICONTROL Tools]**&#x200B;菜單中的資料夾或從&#x200B;**[!UICONTROL Properties]**&#x200B;中刪除影像配置檔案。

### 通過配置式用戶介面{#removing-image-profiles-from-folders-via-profiles-user-interface}從資料夾中刪除動態媒體映像配置式
