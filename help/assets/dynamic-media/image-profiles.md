...
建立影像配置檔案時，可以使用 **[!UICONTROL 非銳化蒙版]** 選項來微調最終下採樣影像上的銳化濾鏡效果。 可以控制效果強度、效果半徑（以像素計量）以及忽略的對比度閾值。 此效果使用的選項與Adobe Photoshop的「非銳化掩碼」濾鏡相同。
...
在映像上實施智慧裁剪時，Adobe建議採用以下最佳做法並強制實施以下限制：
...
您建立的每個智慧裁剪生成都需要額外的處理。 例如，添加5個以上智慧裁剪長寬比可能導致資產攝取速率降低。 它還可能增加系統負載。 因為您可以在資料夾級別應用Smart Crop，所以Adobe建議您在資料夾上使用它 *僅* 需要它的地方。

**在影像配置檔案中定義智慧裁剪的指南**
為了控制Smart Crop的使用，並優化農作物的加工時間和儲存，Adobe建議使用以下指導和提示：

* 要應用智慧裁剪的影像資產必須至少為50 x 50像素或更大。
...
* 不允許包含重複智慧裁剪維的影像配置檔案。
* 不允許設定智慧裁剪選項的重複命名映像配置檔案。

您有兩個影像裁剪選項可供選擇：像素裁剪和智慧裁剪。 您還可以選擇自動建立顏色和影像色板，或跨目標解析度保留裁剪內容。
...
>[!NOTE]
>
>16K解析度是水準大約16,000像素的顯示解析度。 最常討論的16K解析度為15360×8640，每維8K UHD像素數翻倍，總像素數是原來的4倍。 此解析度為13270萬像素，是4K解析度的16倍，是1080p解析度的64倍。
