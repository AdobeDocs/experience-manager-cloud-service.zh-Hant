...
* 對視頻跟蹤的支援僅限於核心播放跟蹤，如所述 [跟蹤概述](https://experienceleague.adobe.com/docs/media-analytics/using/tracking/track-av-playback/track-core-overview.html?lang=en#player-events)。 特別是，不支援QoS、廣告、章節/段或錯誤跟蹤。
...
目前，對視頻跟蹤的支援僅限於「核心回放」跟蹤，如所述 [跟蹤概述](https://experienceleague.adobe.com/docs/media-analytics/using/tracking/track-av-playback/track-core-overview.html?lang=en#player-events)。 特別是，不支援QoS、廣告、章節/段或錯誤跟蹤。
...
要瞭解有關如何使用Experience ManagerDynamic Media的嵌入代碼功能的更多資訊，請參見 [在網頁上嵌入視頻或影像查看器](/help/assets/dynamic-media/embed-code.md)。
...
* [Adobe Developer控制台](https://developer.adobe.com/console/home)  — 為Experience Platform標籤建立整合。
...
1. 指定一個或多個要用於跟蹤網頁上的Adobe Analytics查看器行為的Dynamic Media變數。
   ...
   要啟用新的自定義流量變數，請在工具欄上的Adobe Analytics中轉到 **[!UICONTROL 管理]** > **[!UICONTROL 報表套件]**。
...
另請參閱 [配置分路屬性](https://experienceleague.adobe.com/docs/platform-learn/implement-mobile-sdk/initial-configuration/configure-tags.html)。
...
請參閱 [Experience Cloud身份服務擴展](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/client/id-service/overview.html)。
...
請參閱 [Adobe Analytics分機](https://experienceleague.adobe.com/docs/experience-platform/tags/extensions/client/analytics/overview.html)。

* (選用. 僅當需要視頻跟蹤時才需要) *Adobe Medium音頻和視頻分析* 擴展
...
1. 啟用 **[!UICONTROL 建立新證書]**，然後在文本欄位中，為證書輸入任何有意義的值。 比如說， *AdobeLaunchIMSCert*。 選擇 **[!UICONTROL 建立證書]**。
   ...
   *要檢索有效的訪問令牌，必須將新證書的公鑰添加到Adobe Developer的技術帳戶中！*
...
1. 在新瀏覽器頁籤中，導航至 [Adobe Developer控制台](https://developer.adobe.com/console/integrations)。
...
1. 在 **[!UICONTROL 暫存]** 第（2/3頁） **[!UICONTROL 建立Experience Platform標籤配置]** 的子菜單。
   ...
   僅出於解釋目的，此步驟使用部署到Experience PlatformCDN的Adobe標籤庫。
