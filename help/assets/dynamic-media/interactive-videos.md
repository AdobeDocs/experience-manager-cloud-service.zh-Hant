...
[在AEM Assets](https://adobecustomersuccess.adobeconnect.com/p1yxzdo4aec/)中使用互動式視訊、連結分享和YouTube分享網路研討會教您如何使用互動式視訊和其他功能，將轉換驅動的事件連結至您的視訊行銷內容。
...
當您完成最後一個範例章節中的教學課程時，[這是您使用完全整合的互動式影片的最終示範網頁，看起來會像](https://marketing.adobe.com/resources/help/en_US/dm/shoppable-video/john-lewis/landing-3.html)。
...
1. **（可選）識別Quickview變數** -首先，確定現有Quickview實施所使用的動態變數。當您建立互動式視訊時，可使用變數將產品縮圖對應至其對應的產品Quickview。 請參閱[（可選）識別Quickview變數](#optional-identifying-quickview-variables)。
   **只有在以下所有步驟都正確時，才需要此步驟**:·您想要透過觸發快速檢視，為視訊新增互動功能。·您的AEM實作&#x200B;*not*&#x200B;會使用電子商務整合架構，將產品資料從任何電子商務解決方案（例如IBM Websphere Commerce、Elastic Path、hybris或Intershop）拉入AEM。
...
請參閱[建立新檢視器預設集](/help/assets/dynamic-media/managing-viewer-presets.md#creating-a-new-viewer-preset)（這是選用）和[建立互動檢視器預設集的特殊考量事項](/help/assets/dynamic-media/managing-viewer-presets.md#special-considerations-for-creating-an-interactive-viewer-preset)。
...
1. **將互動式視訊新增至您的網站或**
AEMI中的網站如果您使用AEM Sites或AEM eCommerce，或兩者皆使用，您可以將互動式視訊直接拖曳至AEM的網頁上。請參閱[新增動態媒體資產至頁面。](/help/assets/dynamic-media/adding-dynamic-media-assets-to-pages.md)
...
* 您的AEM實作&#x200B;*not*&#x200B;會使用電子商務整合架構，將產品資料從任何電子商務解決方案（例如IBM Websphere Commerce、Elastic Path、hybris或Intershop）拉入AEM。<!-- See [eCommerce concepts in AEM Assets](/help/sites-administering/concepts.md).-->
...
1. 在「建立互動式視訊」頁面上，執行下列任一項作業：
   ...
   * 點選&#x200B;**[!UICONTROL 播放]**&#x200B;按鈕，開始播放影片。 當您要反白顯示的特定產品、服務或詳細資訊出現在檢視中時，點選&#x200B;**[!UICONTROL Pause]**。 點選&#x200B;**[!UICONTROL 新增區段]**。
   ...
   <table> 
    <tbody> 
      ...
      <td><p>在「Action Type（操作類型）」下，按一下「Quickview（快速視圖）」<strong>。</strong></p> <p>如果您是AEM Sites和Ecommerce客戶：</p> 
       ...
       <ul> 
       <li>請參閱<a href="/help/assets/dynamic-media/carousel-banners.md#identifying-hotspot-and-image-map-variables">識別熱點變數</a>。 您需要定義這些變數。 </li> 
      ...
      <td><p>在「動作類型」下，點選「體驗片段」，然後執行下列動作：<strong></strong><p> 
       <ul> 
       <li>如果您是AEM Sites客戶，請點選或按一下「搜尋」圖示（放大鏡）以開啟「體驗片段」頁面。 點選或按一下您要使用的體驗片段，然後點選頁面右上角的<strong>選擇</strong>以返回上一頁的「動作」面板。<br /> 請參閱 <a href="/help/sites-cloud/authoring/fundamentals/experience-fragments.md">體驗片段</a>。</li> 
      </ul> 
       <ul> 
