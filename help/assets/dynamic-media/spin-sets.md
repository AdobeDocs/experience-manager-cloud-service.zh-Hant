...
>[!NOTE]
>
>如需Assets使用者介面的詳細資訊，請參閱 [使用觸控式UI管理資產](/help/assets/manage-digital-assets.md) 並將其套用至影像集資產上傳所在的新資料夾。
...
1. [上傳影像供多個檢視使用](#uploading-assets-for-spin-sets).
   ...
   另請參閱 [Dynamic Media — 支援的點陣影像格式](/help/assets/file-format-support.md#image-support-dynamic-media) 以取得迴轉集所支援的格式清單。
...
1. 設定 [迴轉集檢視器預設集](/help/assets/dynamic-media/managing-viewer-presets.md)，視需要。
...
如有需要，您可以 [編輯迴轉集](#editing-spin-sets). 此外，您還可以檢視和修改 [迴轉集屬性](/help/assets/manage-digital-assets.md#editing-properties).
...
您可以在使用者介面中或自動使用 [批次集預設集](/help/assets/dynamic-media/config-dm.md). 不過，使用批次集預設集建立的集可以 *非* 會出現在使用者介面中。 您可以透過三種不同的方式，存取透過批次集預設集建立的集合。 （即使您已在使用者介面中建立迴轉集，仍可使用這些方法）。
...
1. 開啟個別資產的屬性時。 屬性會指出所選資產所屬集(位於 **[!UICONTROL 整合員]**)。 若要檢視整個集合，請選取集合的名稱。
