...
* [已在您的Dynamic Media帳戶中啟用多註解和多音訊曲目](#enable-dash).
...
1. （可選）在發佈之前預覽視訊，以確保字幕和音訊如預期般運作。 另請參閱 [預覽含有多個字幕和音訊曲目的視訊](#preview-video-audio-subtitle)
...
>[!CAUTION]
>
>當您上傳註解檔案或音訊檔案至已發佈或取消發佈的視訊時，如果您選擇上傳註解檔案或音訊檔案，則會刪除檔案 [*重新處理*](/help/assets/dynamic-media/about-image-video-profiles.md#reprocessing-assets) 影片。 只有視訊的原始音訊會維持不變。 在這種情況下，您必須再次將註解檔案和音訊曲目檔案重新上傳到視訊。
...
1. 在右下角附近，在音量圖示的左側，選取語音泡泡圖示，然後選取您要聽到的音訊或註解，或視見或兩者皆選。 如有需要，您可以在「註解」下選取 **[!UICONTROL 關閉]** 不顯示任何註解或標題。

   ![視訊檢視器中的「音訊」和「註解」彈出式清單。](/help/assets/dynamic-media/assets/msma-selectaudiosubtitle.png)*模擬使用者為視訊播放選取音訊和註解的情況。*
