...
**案例2：在`classic car`資料夾中使用`automobile`關鍵字搜尋所有檔案。**
...
當您要擷取排除某些關鍵字的結果時，可以使用NOT運運算元。 NOT運運算元使用連字型大小(-)符號來指示AEM搜尋要從搜尋結果中排除的專案。 例如，指定包含`car - classic`但排除`car`的中繼資料的`classic`搜尋查詢。
