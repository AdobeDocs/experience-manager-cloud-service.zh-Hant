...
1. 設定許可權：為`jcr:all permissions for /conf/workfront-tools/settings/cloudconfigs`設定這些許可權，`wf-workfront-users group`。
...
1. 如果在[!DNL Workfront]Workfront自訂表單欄位[!UICONTROL 中選取的]欄位是Workfront使用者預先輸入欄位，則必須指定要對應的Workfront使用者欄位。 若要這麼做，請核取從Workfront參考物件欄位取得值，然後指定要從中擷取對應值的[!UICONTROL Workfront使用者自訂表單欄位]的名稱。
