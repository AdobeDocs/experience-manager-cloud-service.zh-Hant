---
title: 管理資產集合
description: 瞭解AEM Assets中的收集概念。 瞭解如何與其他使用者一起收集、管理、編輯和編輯系列。
contentOwner: AG
mini-toc-levels: 1
...
## 導覽系列主控台 {#navigate-the-collections-console}
...
1. 點選／按一 **[!UICONTROL 下「儲存]** 」以建立智慧型系列，然後關閉訊息方塊以完成程式。 新的智慧型系列也會新增至「已儲存的 **[!UICONTROL 搜尋」清單]** 。
...
1. 在「系 **[!UICONTROL 列中繼資料]** 」頁面中，從「基本」和「進階」標籤檢視系 **[!UICONTROL 列中繼資]** 料 **** 。
...
儲存的搜尋會在您建立智慧型系列時建立。 智慧型系列會自動新增至「已儲 **[!UICONTROL 存的搜尋]** 」清單。 系列的「已保存的搜索」查詢將保存在CRX `dam:query` 中相對位置的屬性中 `/content/dam/collections/`。
...
1. 導覽至「資產」使用者介面，然後點選／按一下工 **[!UICONTROL 具列上的]** 「搜尋」。
...
1. 點選／按一下GlobalNav圖示，然後從清 **單中選擇** 「時間軸」。
1. 在時間軸中，按一下或點選底部的脫字元號圖示，然後點選／按一下「開始工 **作流程」**。
1. 在「開 **始工作流** 」部分，從清單中選擇工作流模型。 例如，選取「 **DAM更新資產」模型** 。
1. 輸入工作流的標題，然後點選／按一下「開 **始**」。
1. 在對話方塊中，點選／按一下「 **繼續**」。 工作流程會在系列中的所有資產上執行。
