...
集合是Adobe Experience Manager Assets中的一組資產。 使用收藏集在使用者之間共用資產。集合可以是靜態集合或基於搜尋結果的動態集合。
...
若要開啟 **[!UICONTROL 集合]**，選取Experience Manager標誌。 從導覽頁面，前往 **[!UICONTROL 資產]** > **[!UICONTROL 集合]**.
...
1. 選取 **[!UICONTROL 建立]**，然後選取 **[!UICONTROL 確定]** 以關閉對話方塊。 具有指定標題和屬性的系列會在「系列」主控台中開啟。
   ...
   若要將資產新增至收藏集，請導覽至「資產」使用者介面。 如需詳細資訊，請參閱 [將資產新增至收藏集](#add-assets-to-a-collection).

### 使用dropzone建立集合 {#create-collections-using-dropzone}

您可以將資產從「資產」UI拖曳至收藏集。 您也可以建立收藏集的副本，並將資產拖曳至該處。

1. 在「資產」使用者介面中，選取您要新增至集合的資產。
1. 將資產拖曳至 **[!UICONTROL 放入集合]** 區域。 或者，選取 **[!UICONTROL 至集合]** 圖示。
1. 在 **[!UICONTROL 新增至集合]** 頁面，選取 **[!UICONTROL 建立集合]** 圖示。 如果您想要將資產新增至現有的系列，請從頁面中選取資產，然後選取「 」 **[!UICONTROL 新增]**. 依預設，會選取最近更新的系列。
...
1. 導覽至「資產」UI，然後選取 **[!UICONTROL 搜尋]** 圖示。
1. 在「全搜尋」方塊中輸入搜尋關鍵字，然後選取 `Enter`. 選取GlobalNav圖示以顯示「篩選器」面板，並從「搜尋」面板套用搜尋篩選器。
...
1. 指定集合的名稱。 選取 **[!UICONTROL 公共]** 將具有檢視器角色的DAM Users群組新增至智慧型集合。
   ...
   >[!NOTE]
   >
   >如果您選取 **[!UICONTROL 公共]**，在您建立智慧型集合後，便能供所有具有擁有者角色的人使用。 如果您取消 **[!UICONTROL 公共]** 選項，DAM使用者群組不再與智慧型集合建立關聯。

1. 選取 **[!UICONTROL 儲存]** 以建立智慧型集合，然後關閉訊息方塊以完成程式。 新的智慧型系列也會新增至 **[!UICONTROL 已儲存的搜尋]** 清單。
「建立智慧選 **[!UICONTROL 擇」按鈕的標籤將更改為]** 「編 **[!UICONTROL 輯智慧選擇」]**。要編輯智慧系列的設定，請從「檔案和文 **[!UICONTROL 件夾]** 」列 **[!UICONTROL 表中選擇「檔案]** 」。然後，選取 **[!UICONTROL 編輯智慧型選取範圍]** 按鈕。
...
1. 選取資產並選取 **[!UICONTROL 至集合]** 圖示。 或者，您可以將資產拖曳至 **[!UICONTROL 放入集合]** 區域。 當拖放區域啟動且其標籤變更為時，請放開滑鼠按鈕 **[!UICONTROL 放置以新增]**.
...
1. 在「資產」使用者介面中，選取 **[!UICONTROL 搜尋]** 圖示。
1. 將游標置於Omnisearch方塊中，選取 `Enter` 機碼。
...
1. 選取&#x200B;**[!UICONTROL 儲存]**。此 **[!UICONTROL 編輯智慧型集合]** 對話方塊隨即顯示。
1. 選取 **[!UICONTROL 覆寫]** 以已編輯的集合取代原始的智慧型集合。 或者，選取 **[!UICONTROL 另存為]** 以分別儲存已編輯的集合。
...
1. 在集合控制檯中，選取一個集合併選取 **[!UICONTROL 屬性]** 圖示。
1. 在「系列 **[!UICONTROL 中繼資料]** 」頁面中，從「基本」和「進階」標籤檢視系 **[!UICONTROL 列中繼資]** 料 **** 。
1. 視需要修改中繼資料，然後選取「 」 **[!UICONTROL 儲存並關閉]** 以儲存變更。
...
1. 選取 **[!UICONTROL 儲存並關閉]** ，然後關閉確認對話方塊以完成程式。
...
「系列」中的「搜尋」功能同時支援 [搜尋集合](#search-collections) 和 [在集合中搜尋資產](#search-within-collections).

### 搜尋集合 {#search-collections}

您可以從「集合」控制檯搜尋集合。 當您在Omnisearch方塊中使用關鍵字進行搜尋時， [!DNL Experience Manager Assets] 搜尋系列名稱、中繼資料和新增至系列的標籤。

如果您從最上層搜尋集合，搜尋結果中只會傳回個別集合。 會排除收藏集中的資產或資料夾。 在所有其他情況下（例如，在個別集合或資料夾階層中），所有相關的資產、資料夾和集合都會傳回。
...
1. 選取集合，然後選取 **[!UICONTROL 設定]** 圖示加以儲存。 或者，使用 **[!UICONTROL 設定]** 集合縮圖中的快速動作。
...
1. 在對話方塊中選取 **[!UICONTROL 刪除]** 以確認刪除動作。
...
1. 從工具列中選取下載圖示。
1. 在 **[!UICONTROL 下載]** 對話方塊，選取 **[!UICONTROL 下載]**. 如果您想要下載收藏集中資產的轉譯，請選取「 」 **[!UICONTROL 轉譯]**. <!-- Select the **[!UICONTROL Email]** option to send an email notification to the owner of the collection. -->
...
Adobe Enterprise Manager Assets可讓您大量編輯許多集合的中繼資料。 使用 [!UICONTROL 屬性] 頁面以針對多個集合執行中繼資料變更，例如，將中繼資料屬性變更為通用值，或新增或修改標籤。
...
1. 在集合控制檯中，選取您要編輯的集合。
1. 從工具列中選取 **[!UICONTROL 屬性]** 以開啟 [!UICONTROL 屬性] 頁面。
1. 修改各種標籤下所選集合的中繼資料屬性。

   >[!NOTE]
   >
   >您為所選集合新增的中繼資料會覆寫這些集合先前的中繼資料，但標籤除外。 您在中新增的任何標籤 **[!UICONTROL 標籤]** 欄位附加至中繼資料中的現有標籤清單。
...
1. 從集合控制檯中，選取所需的集合或集合群組，然後選取 **[!UICONTROL 至集合]** （在工具列中）。
...
1. 選取 **[!UICONTROL 新增]**. 訊息會確認此集合已新增至中的目標集合 **[!UICONTROL 選取目的地]** 頁面。 關閉訊息以完成程式。
...
1. 將游標放在Omnisearch欄位中，選取 `Enter` 機碼。
1. 選取「全域導覽」圖示以顯示「篩選器」面板。
1. 從 **[!UICONTROL 已儲存的搜尋]** 清單，選取 **[!UICONTROL 刪除]** ，位於您要刪除的智慧型集合旁。
1. 在對話方塊中選取 **[!UICONTROL 刪除]** 以刪除儲存的搜尋。
