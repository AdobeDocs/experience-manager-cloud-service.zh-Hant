...
**自動調音**:利用人工智慧來分析影像的內容，並基於影像的獨特屬性智慧地進行光和顏色校正。**自動直立**:利用人工智慧分析影像內容，糾正影像中的傾斜透視。例如，要建立層視線。
**Lightroom預設集**:使用自訂預設集對影像套用使用者定義的外觀，以達到一致的外觀。**影像挖剪**:利用人工智慧在顯著對象周圍建立選擇，並使用單一命令去除背景。**影像遮色片**:利用人工智慧，通過單一命令在突出物體周圍建立掩模。**Photoshop動作**:套用一系列工作(在Photoshop中)至檔案或一批檔案。**智慧型物件取代**:通過允許您交換影像，同時保留PSD檔案中應用的所有效果和調整，按比例執行個性化。
...
1. 導覽至「工具>資產>處理設定檔」。
1. 選擇「建立」並指定名稱。
1. 選取「創作」標籤。 指定輸出資料夾，選擇「[!UICONTROL 新增]」以新增創意配置。 提供轉譯名稱（或輸出名稱）、副檔名（或檔案類型）、選擇質量（或輸出參數）、選擇包括和排除MIME類型清單（或輸入資產篩選器），以及選擇所需的創作操作。
1. 對於某些操作，需要其他參數（資產）。 如有要求，請提供這些額外參數的值。

1. 將更多創意操作新增為相同處理設定檔的一部分，或儲存設定檔。

1. 將處理設定檔套用至資料夾。 選取資料夾屬性、資產處理，然後選取建立的處理設定檔。

將處理設定檔套用至DAM資料夾後，除了標準處理外，此資料夾（或子資料夾中，除非被覆寫，否則）中上傳或更新的所有資產都會執行定義的操作。

若要手動處理現有資產，請選取資產並選取&#x200B;**[!UICONTROL 重新處理]**&#x200B;選項，然後選取所需的處理設定檔。
