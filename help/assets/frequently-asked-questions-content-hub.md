...
## 什麼是AEM Assets Content Hub？ {#what-is-content-hub}

AEM Assets Content Hub是Adobe Experience Manager Assets as a Cloud Service的一項功能。
...
## 我在生產程式/環境中啟用了AEM Assets Content Hub，可以停用它嗎？ {#can-i-disable-content-hub}

在生產計畫上啟用AEM Assets Content Hub會將其部署為生產基礎結構的一部分。 AEM Cloud Manager 不允許移除或停用生產基礎結構，藉此盡量降低人為錯誤對生產使用帶來的風險。
...
## 如何在組織中評估AEM Assets Content Hub？ {#how-can-i-evaluate-content-hub}

AEM Assets Content Hub是Adobe提供和維護的功能，沒有任何需要透過dev/stage/production進行典型驗證的自訂程式碼。 此外，使用者對該功能的存取權完全由管理員控制，因此您可以對其進行評估，而無需將其向所有使用者開放。
...
## 為何登入AEM Assets Content Hub後沒看到任何資產？ {#no-assets-in-content-hub}
...
## 為何我無法看到使用AEM Assets Content Hub直接上傳，或使用Content Hub從Dropbox或OneDrive帳戶匯入的資產？ {#no-assets-uploaded-from-content-hub}

是否顯示使用AEM Assets Content Hub上傳的資產，取決於您已啟用「設定」使用者介面上的[自動核准](/help/assets/configure-content-hub-ui-options.md#configure-import-options-content-hub)切換功能：
...
## 如何在AEM as a Cloud Service環境中快速找到使用AEM Assets Content Hub上傳的資產？ {#find-uploaded-assets-on-aem-cloud}

您可以在AEM as a Cloud Service環境中快速找到使用AEM Assets Content Hub上傳的資產，方法是：
...
## 為什麼我的資產卡上沒有顯示使用Adobe Express編輯選項，以便能夠使用AEM Assets Content Hub重新混合資產以建立新的變數？ {#edit-using-express-not-available}

若要在AEM Assets Content Hub中檢視資產卡上的&#x200B;**使用Adobe Express編輯**&#x200B;選項，使用者必須擁有Adobe Express Enterprise或Teams權利（請參閱[計畫](https://www.adobe.com/tw/express/pricing)），以及[Content Hub使用者有權將資產重新混合成新變數](#onboard-content-hub-users-add-assets)的許可權。
...
## 我可以設定AEM Assets Content Hub，讓我的組織品牌指導方針在首頁上顯示為連結嗎？ {#content-hub-setup-brand-guidelines}

除了AEM Assets Content Hub首頁上的標準「所有Assets」、「集合」和「前瞻分析」標籤之外，您還可以新增自訂連結作為單獨的標籤。 如需有關如何進行設定的資訊，請參閱[自訂連結](/help/assets/configure-content-hub-ui-options.md#configure-custom-links-content-hub)。

## 是否計畫將現有Brand Portal客戶移轉至AEM Assets Content Hub？ {#migration-brand-portal}

Adobe提供從Brand Portal到AEM Assets Content Hub的移轉支援，您可透過建立Adobe支援票證來使用這些支援。

## 為什麼在AEM Assets Content Hub中看不到「產品設定/組態」選項？ {#ui-configuration-option-missing}

若要存取AEM Assets Content Hub中的[設定使用者介面](/help/assets/configure-content-hub-ui-options.md)，您必須是[Content Hub管理員](/help/assets/deploy-content-hub.md##onboard-content-hub-administrator)。 如果您獲指派至 Adobe Admin Console 中生產作者執行個體上的 AEM 管理員產品設定檔，但仍看不到設定選項，請確定 AEM 管理員產品設定檔未被重新命名。如需更多詳細資訊，請參閱 [AEM as a Cloud Service 團隊和產品設定檔](/help/onboarding/aem-cs-team-product-profiles.md)。

## AEM Assets Content Hub如何解決Brand Portal的限制？ {#content-hub-brand-portal-comparison}


下表概述AEM Assets Content Hub與Brand Portal之間的主要差異：
...
## 如何選取存放庫以僅檢視AEM Assets Content Hub中選定環境的資產？ {#select-repository-multiple-environments}

當您為相同計畫配置了用於生產和其他較低環境的AEM Assets Content Hub，您可以選取存放庫並顯示所選環境的資產。 執行以下步驟：
...
## AEM Assets Content Hub如何顯示.ZIP檔案型別的縮圖預覽？ {#thumbnail-preview-zip-file}

若要在AEM Assets Content Hub中提供檔案型別（例如.ZIP）的縮圖預覽，您可以將名稱為`cq5dam.preview.jpg`或`cq5dam.preview.png`的轉譯新增至AEM as a Cloud Service編寫環境中可用.ZIP的路徑根目錄。
