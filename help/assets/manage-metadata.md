...
以上是基本中繼資料屬性， [!DNL Experience Manager] 可管理資產，讓使用者可以查看所有資產。 例如，在嘗試探索最近新增或修改的資產時，依上次修改日期排序資產很實用。
...
資產的上次修改日期反映上次修改資產原始檔案的時間。 因此，修改日期和使用者只會在下列情況下變更：
...
Exif定義的中繼資料欄位通常屬於技術性質，在描述性中繼資料管理中的使用有限。 因此， [!DNL Experience Manager Assets] 選件將Exif屬性對應至 [公用元資料結構](metadata-schemas.md) 和XMP。
...
Enterprise Manager Assets可讓您同時編輯多個資產的中繼資料，以便快速將通用中繼資料變更大量傳播至資產。 使用 [!UICONTROL 屬性] 頁面，將中繼資料屬性變更為通用值，或新增或修改標籤。 要自定義元資料屬性頁，包括添加、修改、刪除元資料屬性，請使用結構編輯器。

>[!NOTE]
>
>大量編輯方法適用於資料夾或集合中的可用資產。 對於可跨資料夾使用或符合通用條件的資產，您可以 [搜尋後大量更新中繼資料](/help/assets/search-assets.md#metadata-updates).
...
1. 從工具列，點選/按一下 **[!UICONTROL 屬性]** 開啟 [!UICONTROL 屬性] 頁面。
   ...
   >[!NOTE]
   >
   >選取多個資產時，系統會為資產選取最低的共同父表單。 換句話說， [!UICONTROL 屬性] 頁面只會顯示在所有頁面上共有的中繼資料欄位 [!UICONTROL 屬性] 所有個別資產的頁面。
...
1. 若要檢視特定資產的中繼資料編輯器，請取消在清單中選取剩餘資產。 中繼資料編輯器欄位會填入特定資產的中繼資料。
   ...
   >[!NOTE]
   >
   >* 在 [!UICONTROL 屬性] 頁面，您可以取消選取項目，從資產清單中移除資產。 資產清單預設會選取所有資產。 您從清單中移除的資產中繼資料不會更新。
   >* 在資產清單頂端，選取附近的核取方塊 **[!UICONTROL 標題]** 切換選取資產和清除清單。


1. 若要為資產選取不同的中繼資料結構，請點選/按一下 **[!UICONTROL 設定]** 從工具欄中，選擇所需的架構。 儲存變更。
...
資產as a [!DNL Cloud Service] 可使用雲端原生服務產生資產的自訂中繼資料。 設定處理設定檔以產生自訂中繼資料。 請參閱 [如何使用處理設定檔](/help/assets/asset-microservices-configure-and-use.md#use-profiles).
...
* DEX - [Optima SC說明瀏覽器](https://www.optimasc.com/products/dex/index.html) 是Windows作業系統元資料和檔案管理工具的集合。
