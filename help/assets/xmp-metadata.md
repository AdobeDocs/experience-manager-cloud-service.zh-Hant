...
除了提供可內嵌至所有檔案格式的通用中繼資料編碼之外，XMP還提供豐富的[內容模型](#xmp-core-concepts)，並受到Adobe[和其他公司的](#advantages-of-xmp)支援，因此XMP與[!DNL Assets]結合的使用者擁有可建置的強大平台。
...
例如，假設您將[!UICONTROL 標題為]之資產的`Classic Leather`Title`Nylon`屬性修改為案例。

![中繼資料](assets/metadata.png)

在此案例中，[!DNL Assets]會針對儲存在資產階層中的資產中繼資料，將變更儲存在&#x200B;**[!UICONTROL 引數中的]** Title`dc:title`屬性。
