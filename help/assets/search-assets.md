...
| [從其他解決方案和應用程式搜尋](#beyondomnisearch):資 <br />產連結 [桌](#aal) 面應用程式 <br />[](#desktopapp)<br />     [Adobe Stock影像](#adobestock)<br />     動 [態媒體資產](#dynamicmedia) |  |  |
...
![](assets/aem_search_results.png) 瞭解資產搜尋結果介面的部分&#x200B;**&#x200B;圖：瞭解Assets搜尋結果介面的部分

**** 答：將搜尋儲存為智慧型系列。 **** B.篩選（謂語）以縮小搜尋結果。 **C.在搜索結果中** ，顯示檔案、資料夾或兩者。 **** D.按一下「篩選」以開啟或關閉左側導軌。 **** E.搜尋位置為DAM。 ************ F.Omnisearch欄位，包含使用者提供的搜尋關鍵字 **G。複選框，以選擇所有搜索結果** H。顯示的搜尋結果總數 **I。關閉搜**&#x200B;索J。在卡片檢視和清單檢視之間切換
...
當您開始輸入關鍵字時，AEM會建議可能的搜尋關鍵字或片語。 建議是以AEM中的資產為基礎。 AEM為所有中繼資料欄位建立索引，以協助搜尋。 為提供搜尋建議，系統會使用下列幾個中繼資料欄位的值。 若要提供搜尋建議，請考慮將適當的關鍵字填入下列欄位：
...
在清單檢視中，您可以像排序任何資料夾中的資產一樣，對搜尋結果進行排序。 排序功能適用於這些欄——標題、狀態、維度、大小、評分、使用、（日期）修改、（日期）發佈、工作流程和檢出。

請參 [閱「名稱」列的配置排序](#sortbyname)。 有關排序功能的限制，請參見 [限制](#limitations)。
