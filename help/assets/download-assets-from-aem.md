...
1. 導覽至您要下載的資產。 選取資料夾，或選取資料夾中一或多個資產。 在工具列上，按一下&#x200B;**[!UICONTROL 下載]**。
   ...
   從![下載資產時有[!DNL Experience Manager Assets]](/help/assets/assets/asset-download1.png)可用選項
...
1. 在設定資料夾中，建立名為`nt:file`的`com.day.cq.dam.core.impl.servlet.AssetDownloadServlet.config`型別的檔案。
...
如果您不需要下載功能，請停用servlet以防止任何類似DoS的風險。 可透過更新Dispatcher設定以封鎖任何資產下載請求，在`Asset Download Servlet`作者和發佈執行個體上停用[!DNL Experience Manager]。 此servlet也可以直接透過OSGi主控台手動停用。
