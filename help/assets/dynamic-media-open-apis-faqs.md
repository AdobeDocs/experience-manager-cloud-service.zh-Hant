...
## 如何確保資產傳遞和支援 OpenAPI 之 Dynamic Media 的搜尋？ {#secure-asset-delivery}
...
| [豐富的影像修飾元組支援，例如寬度、高度、旋轉、翻轉、品質和格式](/help/assets/deliver-assets-apis.md) | 豐富的可用影像修飾元 |
| [根據使用者、角色、日期和時間來限制資產傳遞](/help/assets/restrict-assets-delivery.md) | 所有使用者都可以存取發佈到 Dynamic Media 的資產 |
| 大多數的開發人員都熟悉 OpenAPI 規格。 使用[內容建議程式](/help/assets/integrate-adobe-non-adobe-applications.md)，AEM Assets擴充性變得非常簡單。 | SOAP 型的 API 會成為開發整合自訂時的阻礙。 |
...
| 不支援影片智慧裁切功能。 | 支援影片智慧裁切功能。 |
| Dynamic Video 編碼，可確保根據輸入的影片提供最佳編碼。 原生影片傳遞不需要任何設定。 | 標準 3 編碼與輸入影片無關 (可能會影響影片傳遞效能)。 您需要針對不同的影片位元率來手動設定不同的編碼。 |
| 使用資產 UID 啟用安全且模糊化的 URL，而不會影響搜尋引擎最佳化。 | URL 模糊化僅適用於 URL 查詢參數。 URL 中的資產 ID (資產名稱) 是可識別的。 |
...
| Content Advisor和具有OpenAPI功能的Dynamic Media都可擴充，以允許自訂整合。 | 已連線資產 API 無法進行擴展，因此無法允許自訂整合。 |
...
## 如何測試實驗性修飾元？ {#modifiers-not-generally-available}

您可以測試任何修飾元，修飾元通常無法透過實驗性 API 取得。 例如，&lt;/adobe/experimental/advancememodifiers-expires-YYYYMMDD/assets>
