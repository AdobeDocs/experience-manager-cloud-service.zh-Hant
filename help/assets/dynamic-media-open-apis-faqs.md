...
## Experience Manager Assets as a Cloud Service存放庫中的所有資產是否都可供搭配OpenAPI功能使用Dynamic Media進行搜尋和遞送？ {#assets-available-for-search}

否，只有[已核准的最新版本資產](/help/assets/approve-assets.md)才可使用具有 OpenAPI 功能的 Dynamic Media 進行搜尋和傳遞，以確保所有管道和應用程式的品牌一致性。


## 管理員如何將新增到資料夾的新資產和現有資產標示為已核准？ {#add-assets-to-folder-as-approved}
...
## 如何在Experience Manager管理檢視中獲取Adobe IMS (Adobe Identity Management Services)使用者或群組ID以用於設定資產上的角色，以保障傳送和搜尋體驗？ {#set-roles-secure-delivery-search}
...
## 您可以在一個資料夾中同時核准多個資產嗎？ {#approve-multiple-assets-in-folder}
...
## 如何確保資產傳送的安全性，並搜尋Dynamic Media OpenAPI？ {#secure-asset-delivery}
...
## 您如何取得編輯資產核准狀態的許可權？ {#permissions-edit-approval-status}
...
## 支援的視訊檔案大小多大？ {#supported-file-formats-videos}

具有 OpenAPI 功能的 Dynamic Media 支援長格式影片。影片可支援高達 50 GB 和 2 小時。


## 具OpenAPI功能的Dynamic Media與Dynamic Media解決方案有何不同？ {#dynamic-media-and-dynamic-media-with-openapi-differences}
...
## 具OpenAPI功能的Dynamic Media如何解決連線Assets功能的限制？ {#dynamic-media-openapi-addresses-connected-assets-limitations}
