...
* **[!UICONTROL 所有集合]：**&#x200B;選取此選項可檢視及編輯所有集合，包括私人或與您共用的集合。
...
### 在AEM Assets Content Hub中建立集合時，可以使用哪些型別的存取控制？

AEM Assets Content Hub中有三種存取控制型別： **私人** — 只有建立者和管理員可以編輯和存取，**公用** — 僅檢視 — 所有人都可以檢視，但只有建立者和管理員可以編輯，以及&#x200B;**公用** — 檢視和編輯 — 所有人都可以存取和編輯集合，不受限制。

### 誰可以對AEM Assets Content Hub中的集合執行動作？

AEM Assets Content Hub使用者可以執行建立、檢視、共用、下載、移除、刪除、釘選收藏集以及將它們標示為我的最愛等動作。 管理員具有其他許可權，例如檢視所有存取選項和刪除集合。

### 如何將資產新增至AEM Assets Content Hub中的現有集合？

若要將資產新增至AEM Assets Content Hub中的現有集合，請選取您要新增的資產、按一下「**新增至集合**」，然後從清單中選擇集合。 您也可以使用搜尋列來搜尋集合。 按一下&#x200B;**新增至集合**&#x200B;以確認動作。
...
在AEM Assets Content Hub中啟用公開連結共用功能，即可共用Assets。 選取資產卡片上的共用圖示以複製共用連結，然後可傳送給收件者。 請注意，私人集合無法共用。

### 誰可以從AEM Assets Content Hub的集合中移除資產？

集合擁有者、管理員或具有編輯許可權的非管理員使用者可以從AEM Assets Content Hub的集合中移除單一或多個資產。 若要移除，請選取資產，然後按一下&#x200B;**從集合**&#x200B;移除，然後確認移除。
