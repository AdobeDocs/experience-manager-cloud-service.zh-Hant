...
>[!NOTE]
>
>* 共用類型的「資產」資料夾 `sling:OrderedFolder`，不支援共用至Experience Cloud。 如果要共用資料夾，請不選擇 [!UICONTROL 已訂購] 建立資料夾時。
...
如果DAM使用者上傳存放庫中已存在的一或多個資產， [!DNL Experience Manager] 會偵測重複項目並通知使用者。 重複偵測預設為停用，因為它可能會根據存放庫大小和上傳的資產數量而影響效能。

若要啟用功能：

1. 導覽至 **[!UICONTROL 工具>資產>資產設定]**.

1. 按一下 **[!UICONTROL 資產複製偵測器]**.

1. 在 [!UICONTROL 「資產重複資料偵測器」頁面]，按一下 **[!UICONTROL 已啟用]**.

   `dam:sha1` 「偵測中繼資料」欄位的值可確保即使檔案名稱不同，也能偵測到重複資產。
...
>[!NOTE]
>
>如果您已配置重複檢測器，則使用 `/apps/example/config.author/com.adobe.cq.assetcompute.impl.assetprocessor.AssetDuplicationDetector.cfg.json` 設定檔案（OSGi設定），您可以繼續使用，但Adobe建議使用新方法。
