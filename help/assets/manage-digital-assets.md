...
1. 在確認對話框中，按一下：
   ...
   * **[!UICONTROL 取消]** 停止操作
   * **[!UICONTROL 刪除]** 要確認操作，請執行以下操作：

      * 如果資產沒有參考，則資產將被刪除。
      * 如果資產有引用，則會出現一條錯誤消息通知您 **[!UICONTROL 引用一個或多個資產]**。 可以選擇 **[!UICONTROL 強制刪除]** 或 **[!UICONTROL 取消]**。
...
版本控制在特定時間點建立數字資產的快照。 版本控制可幫助將資產恢復到以後的狀態。 例如，如果要撤消對資產所做的更改，請恢復資產的未編輯版本。
