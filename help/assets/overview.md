...
**AI 驅動的資產大量重新命名**：[您可以透過資產視圖，使用人工智慧一次重新命名多項資產](/help/assets/bulk-rename-assets-view.md)。 您可以一次選取多個檔案，然後全部一起重新命名。 某些對話式重新命名提示範例包含&#x200B;*將所有檔案變更為&#39;my-file&#39;，並附加遞增數字*&#x200B;和&#x200B;*以001、002等為檔案加上前置詞，然後翻譯成英文*。
...
* **內容建議程式**：「內容建議程式」提供與Experience Manager Assets存放庫整合的使用者介面，讓您能夠瀏覽或搜尋存放庫中可用的數位資產。 然後您可以在應用程式製作體驗中使用這些內容。
您可以將「內容建議程式」與Adobe或非Adobe應用程式整合。
...
<td>
   ...
   <div>
      <a href="/help/assets/restrict-assets-delivery.md">
      <strong>限制存取Experience Manager中的資產</strong>
      </a>
   </div>
   <p>
...
<td>
   ...
   <img alt="內容顧問" src="./assets/integration-asset-selector.jpeg" />
   </a>
   <div>
      <a href="/help/assets/integrate-adobe-non-adobe-applications.md">
      <strong>內容顧問</strong>
   ...
   <p>
      <em>瞭解如何將內容警告器與Adobe或非Adobe應用程式整合。</em>
   </p>
</td>
</table>
...
* **整合AEM Assets與Edge Delivery Services的檔案式製作流程**：當[!DNL AEM Assets]與檔案式製作工具（例如[!DNL Microsoft Word]或[!DNL Google Docs]）整合時，它會在您的製作工具中提供「內容顧問」。 使用「內容建議程式」存取[!DNL AEM Assets]，並將核准的資產插入您的內容。
...
AEM as a Cloud Service提供智慧型&#x200B;**代理程式**，以加強內容管理、最佳化和治理。 這些代理程式可讓使用者快速探索內容、最佳化行銷活動，並確保所有數位資產的合規性。

**探索代理程式**

Discovery Agent透過自然的對話提示隨選提供AEM內容，提供精簡的免費點閱體驗。 它會在&#x200B;**Assets、內容片段及最適化Forms**&#x200B;間聰明地搜尋，以傳遞相關內容，例如影像、影片、PDF、文章及表單範本。 使用自然語言時，您不需要在AEM Assets介面中建置複雜查詢或套用篩選器，即可進行搜尋。 根據您的提示，代理程式會傳回已組織的結果以及資產中繼資料和傳送URL，並準備嵌入其他應用程式。

Discovery Agent的部分主要優點包括：

* **整合式內容探索：**&#x200B;從單一對話式介面存取所有型別的AEM內容，例如影像、影片、PDF檔案、文章和表單。
* **更快的行銷活動規劃：**&#x200B;快速收集跨電子郵件、網頁和社交管道行銷活動的視覺效果和表單。
* **提高生產力：**&#x200B;透過自動化、意圖式搜尋，減少瀏覽儲存庫或篩選中繼資料所花費的時間。
* **一致的內容使用率：**&#x200B;確保重複使用核准的資產和片段，維持跨管道的品牌一致性。

**技能：**&#x200B;自然語言內容探索、標籤式資產探索、資料夾式內容探索、格式與方向式資產探索\
**角色：**&#x200B;行銷活動經理、管道行銷人員、DAM程式庫、代理商和合作夥伴\
**存取：**&#x200B;透過AEM中的AI助理

**常見使用案例/範例提示：**

* 在資料夾WKND中顯示標籤為「office」的影像。
* 列出WKND飲料的所有已發佈內容片段。
* 尋找要申請工作的表單。
* 以橫向顯示個人資產。
...
**Content Optimization Agent**&#x200B;會使用自然語言提示來協助調整並調整資產。 它可以自動產生新的轉譯、調整視覺效果、變更背景並建立通道可用的變化。 與Discovery Agent和&#x200B;**Dynamic Media （含OpenAPI**）搭配使用，以順暢最佳化。

**主要優點：**

* **輕鬆轉換資產：**&#x200B;調整大小、銳利化、重新上色或映象影像。
* **頻道最佳化的輸出：**&#x200B;產生Instagram、網頁橫幅和其他行銷頻道的轉譯。
* **大規模的Creative增強功能：**&#x200B;套用背景變更或覆蓋大量工作流程。
...
治理代理程式有助於確保跨AEM內容的合規性、品牌一致性和原則執行。 它會識別不符合中繼資料、協助工具或公司指引的內容。

Governance Agent的部分主要優點包括：

* **相容性監視：**&#x200B;偵測內容中的原則違規。
* **中繼資料強制執行：**&#x200B;確保資產擁有治理所需的中繼資料。
* **品牌一致性：**&#x200B;標示不符合公司標準的內容。

**技能：**&#x200B;原則相容性檢查、中繼資料驗證、協助工具稽核、違規的自動警示\
**角色：** DAM管理員、合規專員、品牌管理員\
**存取：**&#x200B;透過AEM AI Assistant
...
* **多重音訊和字幕**：在主要影片中新增多重字幕和多重音軌。 擁有此功能代表全球觀眾都可以存取您的影片。 您可以著手自訂一部已發佈的主要影片，以多種語言提供給全球觀眾，並遵守不同地理區域的無障礙指南。 此外，作者還可以在使用者介面的單一標籤管理字幕和音軌。
...
* **緊密整合**：具有 OpenAPI 功能的 Dynamic Media 提供一套全面的搜尋和傳遞 API。 讓您的開發人員能夠輕鬆地[將資產傳遞與其應用程式進行整合](/help/assets/integrate-dynamic-media-open-apis.md)。 這些應用程式包括 Adobe 以及第三方應用程式。 它提供[內容顧問使用者介面](/help/assets/integrate-adobe-non-adobe-applications.md)，以搜尋及選取核准的資產。 選擇器可以輕鬆地與任何採用 JavaScript 框架 (例如 React JS、Angular JS，和 Vanilla JS) 的應用程式整合。
