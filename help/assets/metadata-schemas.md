...
1. （可選）訪問`https://[server]:[port]/system/console/components/`。 配置並啟用預設禁用的`com.day.cq.dam.core.impl.MissingMetadataNotificationJob`元件。 設定Experience Manager檢查資產上中繼資料有效性的頻率。

   此設定會將屬性`hasValidMetadata`新增至資產的`jcr:content`。 使用此屬性，Experience Manager可以篩選搜尋結果。
