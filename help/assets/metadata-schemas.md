...
1. 若要檢視資產的屬性頁面，請在「卡片檢視」中，按一下或點選資產圖格上的「快速動作」 **[!UICONTROL 的「檢視屬性]** 」圖示。 或者，在UI中選取資產，然後按一下或點選工具列 **[!UICONTROL 中的]** 「屬性」圖示。
...
1. 若要檢視表單/範本清單，請按一下 AEM 標誌，然後導覽至&#x200B;**[!UICONTROL 「工具」]**>**[!UICONTROL 「資產」]**>**[!UICONTROL 「中繼資料結構描述」]**。AEM提供現成可用的下列範本：
   ...
   * **預設**: 資產的基本中繼資料結構表單。
   ...
   以下子表單繼承預設表單的屬性：
i. **影像**: MIME類型為&quot;image&quot;的資產的架構表單， `image/jpeg`例如 `image/png`、等。
「影像」表單包含下列子表單範本：
a. **jpeg**: 子類型資產的結構表單 `jpeg`。
