...
在Adobe Experience Manager(AEM)資產中，元資料模式定義屬性頁面的佈局以及為使用特定模式的資產顯示的元資料屬性。 中繼資料屬性包括標題、說明、MIME類型、標籤等。

您可以使用中繼資料結構Forms編輯器來修改現有結構或新增自訂的中繼資料結構。

1. 若要檢視資產的屬性頁面，請在「卡片檢視」中按一下或點選資產圖格上的「快速動作」中的「檢視屬性&#x200B;**[!UICONTROL 」圖示。]**&#x200B;或者，在UI中選取資產，然後從工具列按一下或點選「屬性&#x200B;****」圖示。
1. 在各種標籤下編輯各種中繼資料屬性。 但是，您無法修改屬性頁面中的資產類型。
若要修改資產的MIME類型，請使用自訂中繼資料結構表單或修改現有表單。 如需詳細資訊，請參閱[編輯中繼資料結構Forms](#edit-metadata-schema-forms)。 如果您修改特定MIME類型的中繼資料架構，則會修改目前MIME類型資產和所有資產子類型的屬性頁面配置。 例如，在`default/image`下修改jpeg架構只會修改MIME類型`image/jpeg`的資產的中繼資料配置（資產屬性）。 不過，如果您編輯預設結構，您的變更會修改所有類型資產的中繼資料配置。

1. 若要檢視表單/範本清單，請按一下 AEM 標誌，然後導覽至&#x200B;**[!UICONTROL 「工具」]**>**[!UICONTROL 「資產」]**>**[!UICONTROL 「中繼資料結構描述」]**。提AEM供現成可用的下列範本：

   * **預設**:資產的基本中繼資料結構表單。

   以下子表單繼承預設表單的屬性：
i.**image**:MIME類型為&quot;image&quot;的資產的架構表單，例如`image/jpeg`、`image/png`等。
「影像」表單包含下列子表單範本：
a.**jpeg**:子類型`jpeg`的資產的架構表單。
b.**tiff**:子類型`tiff`的資產的架構表單。

   ii. **應用程式**:具有MIME類型的資產的 `application`架構表單， `application/pdf` `application/zip`例如，等等。a.**pdf**:子類型`pdf`的資產的架構表單。

   iii. **視訊**:具有MIME類型的資產的 `video`架構表單， `video/avi`如 `video/mp4`、等等。

   * **系列**:系列的結構表單。
   * **contentfragment：內** 容片段的架構表單。
...
1. 在對話框中，輸入模式表單的標題，然後按一下&#x200B;**[!UICONTROL 建立]**&#x200B;以完成表單建立過程。
...
1. 要配置元件，請選擇該元件並在&#x200B;**Settings**&#x200B;頁籤中修改其屬性。
...
要編輯表單上元資料元件的屬性，請按一下該元件，然後在&#x200B;**[!UICONTROL Settings]**&#x200B;頁籤中編輯以下全部或子集的屬性。

**欄位標籤**:顯示在資產屬性頁面上的中繼資料屬性名稱。

**對應至屬性**:此屬性指定資產節點的相對路徑／名稱，該資產節點保存在CRX儲存庫中。它以`./`開頭，因為這表示路徑位於資產的節點下。
...
#### 在架構表單{#add-delete-a-tab-in-the-schema-form}中添加和刪除頁籤

架構編輯器可讓您新增或刪除標籤。預設模式表單包 **[!UICONTROL 括Basic]**、 **[!UICONTROL Advanced]** 、 **[!UICONTROL IPTC]**&#x200B;和 **** IPTC Extension頁籤，預設情況下。

按一下`+`在模式表單上添加新頁籤。 預設情況下，新頁籤的名稱為`Unnamed-1`。 您可以從&#x200B;**[!UICONTROL Settings]**&#x200B;標籤中修改名稱。 按一下`X`刪除頁籤。
...
>[!NOTE]
>
>您無法刪除AEM Assets的開箱即用中繼資料結構表單。

## MIME類型{#schema-forms-for-mime-types}的架構表單

AEM Assets為各種現成可用的MIME類型提供預設表單。 不過，您可以為各種MIME類型的資產新增自訂表格。
...
在這種情況下，請在CRX儲存庫的`/etc/dam/metadataeditor/mimetypemappings`處建立新節點。 指定節點的名稱並定義以下屬性：

| **名稱** | **說明** | **類型** | **值** |
|---|---|---|---|
| `exposedmimetype` | 要映射的現有表單的名稱 | 字串 | `image/jpeg` |
| `mimetypes` | 使用`exposedmimetype`屬性中定義的表單的MIME類型清單 | 字串 | `image/png` |

AEM Assets映射以下MIME類型和模式表單：
...
## 授予對元資料結構{#granting-access-to-metadata-schemas}的訪問權

「中繼資料結構」功能僅供管理員使用。 不過，管理員可修改某些權限，以提供非管理員的存取權。 非管理員應具有對`/conf`資料夾的建立、修改和刪除權限。

## 應用特定於資料夾的元資料{#applying-folder-specific-metadata}

AEM Assets可讓您定義中繼資料結構的變體，並將它套用至特定資料夾。
...
1. 選取表單前的核取方塊，例如預設中繼資料表單，然後按一下或點選復製圖示，並將它儲存為自訂表單。 指定表單的自訂名稱，例如`my_default`。 或者，您也可以建立自訂表格。

1. 在&#x200B;**[!UICONTROL 中繼資料結構Forms]**&#x200B;頁面中，選擇`my_default`表單，然後按一下&#x200B;**[!UICONTROL 編輯]**&#x200B;圖示。
