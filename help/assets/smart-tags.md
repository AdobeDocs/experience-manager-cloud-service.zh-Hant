...
# 培訓智慧型內容服務並自動標籤您的影像 {#train-service-tag-assets}
...
>[!IMPORTANT]
>
>新部署 [!DNL Experience Manager Assets] 預設會與 [!DNL Adobe Developer Console] 整合。 它可協助您更快速地設定智慧標籤功能。 在舊版部署中，管理員可以手動設 [定智慧標籤整合](/help/assets/smart-tags-configuration.md#aio-integration)。
...
1. 選取資產，然後從工具列 ![選取「管理標籤](assets/do-not-localize/manage-tags-icon.png) 」圖示。

1. 從「管 **[!UICONTROL 理標籤]** 」頁面檢查標籤。 如果您不希望根據特定標籤搜尋資產，請選取標籤並從工具列選取「刪 ![除」圖示](assets/do-not-localize/delete-icon.png) 。 或者，選 `X` 擇標籤旁的符號。

1. 若要指派較高的排名給標籤，請選取標籤，然後從工具列 ![選取「提升](assets/do-not-localize/promote-icon.png) 」圖示。 您促銷的標籤會移至「標 **[!UICONTROL 記]** 」區段。

1. 選擇「 **[!UICONTROL 儲存]** 」，然後選 **[!UICONTROL 擇「確定]** 」以關閉「 [!UICONTROL 成功] 」對話方塊。

1. 導覽至資 [!UICONTROL 產的] 「屬性」頁面。 請注意，您促銷的標籤已指派高關聯性，因此在搜尋結果中會顯示高度。
