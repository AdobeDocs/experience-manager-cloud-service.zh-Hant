...
瞭解如何在中管理和編輯視訊資產 [!DNL Adobe Experience Manager Assets]. 視訊編碼和轉碼（例如FFmpeg轉碼）可以使用「處理設定檔」和 [!DNL Dynamic Media] 整合。 不含 [!DNL Dynamic Media] 授權， [!DNL Experience Manager] 提供視訊的基本支援，例如使用FFmpeg轉碼、擷取支援的檔案格式的預覽縮圖，以及直接在瀏覽器中播放支援格式的使用者介面中的預覽。
...
1. 在 **[!UICONTROL Google Cloud]** 頁面，右上角附近，選取 **[!UICONTROL 主控台]**.

   如有需要， **[!UICONTROL 登入]** 使用您的Google帳戶認證以檢視 **[!UICONTROL 主控台]** 選項。

1. 在 **[!UICONTROL 儀表板]** 頁面，右側 **[!UICONTROL Google Cloud平台]**，選取 **[!UICONTROL 專案]** 下拉式清單以開啟 **[!UICONTROL 選取專案]** 對話方塊。
...
1. 靠近上中間 **[!UICONTROL API與服務]** 頁面，選取 **[!UICONTROL 啟用API和服務]**.<!-- NEXT STEP BELOW IS STEP 10 -->
1. 在 **[!UICONTROL API程式庫]** 頁面，左側，下 **[!UICONTROL 類別]**，選取 **[!UICONTROL YouTube]**. 在頁面右側，選取 **[!UICONTROL YouTube]**.
...
1. 若要使用API，您需要憑證。 如有必要，請前往 **[!UICONTROL API與服務]** 頁面，選取 **[!UICONTROL 認證]**.
1. 在 **[!UICONTROL 認證]** 頁面，在頂端附近，選取 **[!UICONTROL 建立認證]**，然後選取 **[!UICONTROL OAuth使用者端ID]**.
1. 在 **[!UICONTROL 建立OAuth使用者端ID]** 頁面，在 **[!UICONTROL 應用程式型別]** 下拉式清單，選取 **[!UICONTROL 網頁應用程式]**.

   ![6_5_googleaccount-apis-applicationtype](/help/assets/dynamic-media/assets/6_5_googleaccount-apis-applicationtype.png)

1. 執行下列任一項作業：

   * 在 **[!UICONTROL 名稱]** 欄位，為您的OAuth 2.0使用者端輸入唯一名稱。
...
1. 在 **[!UICONTROL URI]** 文字欄位，輸入以下路徑，在路徑中替換您自己的網域和連線埠號碼，然後按 **[!UICONTROL 輸入]** 若要將路徑新增至清單：
...
1. 在 **[!UICONTROL 授權的重新導向URI]** 標題，選取新增URI。
1. 在 **[!UICONTROL URI]** 文字欄位，輸入以下路徑，在路徑中替換您自己的網域和連線埠號碼，然後按 **[!UICONTROL 輸入]** 若要將路徑新增至清單：
...
1. 在 **[!UICONTROL OAuth使用者端已建立]** 對話方塊中，執行下列動作：
...
1. 在 **[!UICONTROL OAuth使用者端已建立]** 對話方塊，選取 **[!UICONTROL 確定]**.
1. 登出您的Google帳戶。 現在建立YouTube管道。

### 建立YouTube管道 {#creating-a-youtube-channel}

若要將影片發佈至YouTube，您必須具備一或多個管道。 如果您已建立YouTube頻道，則可略過此任務並前往 [新增標籤以供發佈](/help/assets/dynamic-media/video.md#adding-tags-for-publishing).

>[!CAUTION]
>
>請確認您已在YouTube中設定一或多個管道 *早於* 您在Experience Manager的「YouTube設定」下新增管道(請參閱 [在Experience Manager中設定YouTube](#setting-up-youtube-in-aem) 下)。 如果您無法設定頻道，則不會警告您沒有現有頻道。 不過，當您新增頻道時，Google驗證仍會發生，但無法選擇傳送視訊的頻道。
...
>[!NOTE]
>
>設定Cloud Service後，此時啟用YouTube發佈復寫代理程式不需要其他設定。 原因是因為它是在儲存Cloud Service設定時啟用的。
...
1. 在Experience Manager的左上角，選取Experience Manager標誌，然後在左側邊欄中導覽至 **[!UICONTROL 工具]**（槌子圖示） > **[!UICONTROL Cloud Service]** > **[!UICONTROL YouTube發佈設定]**.
...
1. 在「中繼資料描述檔」頁面右上角附近，選取「 」 **[!UICONTROL 建立]**.
...
>
>**[!UICONTROL 立即發佈]** 這表示上傳的資產（在與IPS同步後）會自動發佈到傳送系統。 Dynamic Media雖然如此，YouTube則否。 若要發佈至YouTube，您必須透過Experience Manager Author發佈。
...
1. 在「基本」標籤的「中繼資料」標題下，選取 **[!UICONTROL 開啟選取範圍對話方塊]** 標籤欄位右側。
...
八小時後，如果您仍然看到顯示如下內容的狀態訊息 **[!UICONTROL 已上傳（正在處理，請稍候）]**，請嘗試從您的網站移除視訊並重新上傳。
...
1. 在您的資產資料夾中檢視視訊編碼進度：
   ...
   * 在清單檢視中，視訊編碼進度會顯示在 **[!UICONTROL 處理狀態]** 欄。 如果出現錯誤，則該欄會顯示此訊息。
   ...
   預設不會顯示此欄。若要啟用欄，請選取 **[!UICONTROL 檢視設定]** 從「檢視」下拉式功能表中，然後新增 **[!UICONTROL 處理狀態]** 欄並選取 **[!UICONTROL 更新]**.

   ![chlimage_1-431](/help/assets/dynamic-media/assets/chlimage_1-431.png)

1. 在資產詳細資料中檢視進度。 選取資產時，請開啟下拉式功能表並選取 **[!UICONTROL 時間表]**. 若要將其縮小至工作流程活動，例如編碼或YouTube發佈，請選取 **[!UICONTROL 工作流程]**.
...
[!DNL Experience Manager] as a [!DNL Cloud Service] 可讓您使用「處理設定檔」對MP4視訊檔案進行基本的轉碼。 功能不僅可讓您上傳，也可讓您預覽和縮放MP4視訊檔案。

![在中建立視訊轉碼的處理設定檔 [!DNL Experience Manager]](assets/video-processing-profile-for-mp4.png)

*圖：視訊轉碼的處理設定檔 [!DNL Experience Manager].*
