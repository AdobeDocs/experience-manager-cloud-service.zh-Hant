...
Content Hub可讓您使用Adobe Express建立新內容。 您可以運用易於使用的工具來編輯現有內容、使用範本和品牌元素製作符合品牌形象的變化版本，並使用 Adobe Firefly 的最新生成式 AI 功能建立新內容。
...
## 常見問題 {#faqs-edit-images-content-hub}

### 誰可以在AEM Assets Content Hub中編輯影像？

有權存取Adobe Express和AEM Assets Content Hub的使用者，以及有權將資產重新混合成新變體的使用者，可以使用AEM Assets Content Hub編輯影像。

### 如何在AEM Assets Content Hub中使用Adobe Express編輯影像？

若要在AEM Assets Content Hub中編輯影像，請按一下影像資產卡上的&#x200B;**在Adobe Express中開啟**，或開啟影像的詳細資料，然後按一下Adobe Express標誌。 這會在AEM Assets Content Hub中載入內嵌Adobe Express編輯器，讓您在不離開平台的情況下進行編輯。

### Adobe Express在AEM Assets Content Hub中提供哪些編輯功能？

Adobe Express在AEM Assets Content Hub中提供多種影像編輯功能，包括調整影像大小、移除或變更背景顏色、裁切影像，以及將它們與AI產生的影像或文字結合等功能。

### 在AEM Assets Content Hub中，可以將編輯的影像儲存為哪些檔案格式？

您可以在AEM Assets Content Hub中以PNG （高品質）、JPG （適用於小型檔案）或PDF （適用於檔案）格式儲存編輯的影像。

### 在AEM Assets Content Hub中儲存編輯的影像時，應該填寫哪些欄位？

在AEM Assets Content Hub中儲存已編輯的影像時，您應在&#x200B;**另存新檔**&#x200B;欄位中指定資產的名稱，並在&#x200B;**行銷活動名稱**&#x200B;欄位中指定行銷活動名稱。 Adobe也建議在其他欄位中指定值，例如關鍵字、管道、時間範圍和區域，以增強可搜尋性和組織性。

### 我是否可以在AEM Assets Content Hub中將編輯內容儲存為新資產？

可以，編輯之後，您可以按一下&#x200B;**另存為新資產**，將變更儲存為AEM Assets Content Hub中的新資產。

### 管理員可以在將資產上傳到AEM Assets Content Hub時自訂欄位嗎？

可以，管理員可以在將資產新增至AEM Assets Content Hub時，設定哪些欄位（例如行銷活動名稱、關鍵字和管道）為必填或選用欄位，以符合組織的需求。 使用設定UI上的&#x200B;**匯入**&#x200B;索引標籤來設定欄位。
