---
title: 使用內容警告器存取Adobe和非AEM應用程式中的Adobe內容
description: Content Advisor可跨Adobe和非Adobe應用程式提供統一的內容探索體驗，並將智慧型內容感知探索直接帶入製作體驗。
...
# 使用內容警告器來存取Adobe和非Adobe應用程式中的AEM內容{#content-advisor-aem-assets-adobe-non-Adobe-applications}

Content Advisor可跨Adobe和非Adobe應用程式提供統一的內容探索體驗。 Content Advisor原生整合至Adobe Workfront、AJO B2C （即將推出）、AEM Sites和非Adobe應用程式等應用程式，將內容（資產和內容片段）整合在單一智慧型介面中。 它可讓您在工作流程中輕鬆地探索、瀏覽及重複使用最相關的內容，因此您可以在不中斷內容的情況下更快速地移動。
...
這可讓您直接在外部應用程式中探索、評估及使用AEM Assets的已核准資產，同時與Adobe Express和其他Adobe應用程式中提供的體驗保持一致。

如需整合、屬性和自訂的詳細資訊，請參閱下列文章：

* [內容建議程式整合範例](https://github.com/adobe/aem-assets-selectors-mfe-examples/tree/consolidate-docs-to-experience-league/examples)

* [內容警告器屬性](/help/assets/content-advisor-properties.md)

* [內容建議程式自訂](/help/assets/content-advisor-customization.md)
