...
否則，會根據包含的mime類型檢查mime類型，如果它與清單匹配，則會生成格式副本（「白名單」）。
...
* 後處理工作流程(依路徑`postProcWorkflowsByPath`):可以根據不同的儲存庫路徑列出多個工作流模型。 路徑和模型應以冒號分隔。 支援簡單儲存庫路徑，並應映射到路徑中的工作流 `/var` 模型。 例如： `/content/dam/my-brand:/var/workflow/models/my-workflow`。
* 後處理工作流程(依運算式`postProcWorkflowsByExpression`):可以根據不同的規則運算式列出多個工作流程模型。 運算式和模型應以冒號分隔。 規則運算式應直接指向「資產」節點，而非其中一個轉譯或檔案。 例如： `/content/dam(/.*/)(marketing/seasonal)(/.*):/var/workflow/models/my-workflow`。
