...
第一步是將HTTP POST請求提交到資產應建立或更新的資料夾；包含選擇 `.initiateUpload.json` 器以指出請求是開始二進位上傳。 例如，資產應建立所在資料夾的路徑 `/assets/folder`。 POST請求為 `POST https://[aem_server]:[port]/content/dam/assets/folder.initiateUpload.json`。
...
| `createVersion` | 布林值 (Boolean) | 可選 | 如果 `True` 資產已具有指定名稱，Experience Manager會建立新版本的資產。 |
...
在Experience Manager中，資產處理是以使用資產微服務 **[!UICONTROL 的「處理設定檔]** 」設 [定為基礎](asset-microservices-configure-and-use.md#get-started-using-asset-microservices)。 處理程式不需要開發人員擴充功能。

若是後處理工作流程設定，請使用具有自訂步驟擴充功能的標準工作流程。

## 後處理工作流程中的工作流程步驟支援 {#post-processing-workflows-steps}

從舊版Experience Manager升級至Experience Manager的雲端服務客戶，可使用資產微服務處理資產。 雲端原生資產微服務的設定和使用更簡單。 不支援舊版 [!UICONTROL DAM更新資產工作流程中使用的幾個工作流程步驟] 。
