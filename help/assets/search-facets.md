...
Adobe Experience Manager Assets的整個企業部署能儲存許多資產。 有時候，如果您只使用Experience Manager的一般搜尋功能，尋找合適的資產可能既困難又耗時。
...
若為全文搜尋，請新增 `Fulltext` 述詞至表單。 使用屬性述詞來搜尋符合您指定之單一屬性的資產。 使用「選項」述詞來搜尋符合特定屬性之一或多個值的資產。 新增日期範圍述詞以搜尋在指定日期範圍內建立的資產。

1. 按一下Experience Manager標誌，然後前往 **[!UICONTROL 工具]** > **[!UICONTROL 一般]** > **[!UICONTROL 搜尋Forms]**.
1. 從「搜尋Forms」頁面中，選取 **[!UICONTROL Assets管理搜尋邊欄]**，然後點選  **編輯** ![aemassets_edit](assets/aemassets_edit.png).

   ![找出並選取「資產管理搜尋邊欄」](assets/assets_admin_searchrail.png)

1. 在「編輯搜尋Forms 」頁面中，從 **[!UICONTROL 選擇謂詞]** 頁簽。 例如，拖曳 **[!UICONTROL 屬性述詞]**.
...
1. 按一下 **[!UICONTROL 預覽]** ![預覽](assets/preview.png) 若要產生「篩選器」面板的預覽，如新增述詞後所顯示。
1. 在「預覽」模式中檢閱述詞的配置。
...
1. 若要關閉預覽，請按一下 **[!UICONTROL 關閉]** ![close](assets/do-not-localize/close_icon.png) 在預覽的右上角。
...
1. 執行下列任一項作業：
   ...
   * 點選 ![資產新增圖示](assets/do-not-localize/aem_assets_add_icon.png) 「選項」欄位旁邊，指定要在「篩選器」面板中提供的選項的顯示文本和值。 若要新增其他選項，請點選/按一下 ![資產新增圖示](assets/do-not-localize/aem_assets_add_icon.png) 並重複步驟。
...
此 `Multi Value Property` 述詞可讓您搜尋資產以尋找多個值。 假設您的 [!DNL Assets] 而每個影像的中繼資料包含與產品相關聯的SKU編號。 您可以使用此述詞來根據多個SKU編號來搜尋產品影像。

1. 按一下Experience Manager標誌，然後前往 **[!UICONTROL 工具]** > **[!UICONTROL 一般]** > **[!UICONTROL 搜尋Forms]**.
1. 在搜尋Forms頁面上，選取 **[!UICONTROL Assets管理搜尋邊欄]**，點選 **編輯** ![aemassets_edit](assets/aemassets_edit.png).
...
1. 在 **說明** 欄位，輸入可選說明，然後點選 **[!UICONTROL 完成]**.
...
此 `Tags` 述詞可讓您執行資產的標籤式搜尋。 依預設， [!DNL Assets] 根據您指定的標籤，搜尋一或多個標籤相符的資產。 換言之，搜尋查詢會使用指定的標籤執行OR操作。 不過，您可以使用「比對所有標籤」選項，來搜尋包含您所指定之所有標籤的資產。
...
1. 在 **[!UICONTROL 說明]** 欄位，輸入可選說明，然後按一下/點選 **[!UICONTROL 完成]**.
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>全文</p> </td>
   <td>搜尋述詞，以在整個資產節點上執行全文搜尋。 已與 <code>jcr</code>:<code>contains</code> 運算元。 如果要對資產節點的特定部分執行全文搜索，可以指定相對路徑。</td>
   <td>
    <ul>
     <li>標籤</li>
     <li>預留位置</li>
...
依預設，「鎖定」圖示會出現在 **[!UICONTROL Assets管理搜尋邊欄]** 在 **[!UICONTROL 搜尋Forms]** 頁面。 如果向表單中添加搜索Facet以指示已修改預設表單，則「鎖定」表徵圖會消失。
...
1. 點選 **[!UICONTROL 刪除]** ![刪除圖示](assets/do-not-localize/deleteoutline.png) 的下一頁。
