...
中的多站點管理員(MSM)功能 [!DNL Adobe Experience Manager] 可讓使用者重複使用製作一次並在多個網站位置重複使用的內容。 對於以下名稱的數位資產，可以使用相同的功能： MSM for [!DNL Assets]. 將MSM用於 [!DNL Assets]，您可以：
...
>[!CAUTION]
>
>內容片段的MSM僅在透過以下方式使用內容片段時可用： **[!UICONTROL 資產]** 主控台。
>
>MSM功能為 *not* 使用時可用 **[!UICONTROL 內容片段]** 主控台。

## 瞭解MSM的好處和概念 {#concepts}

### 運作方式與優點 {#how-it-works-and-the-benefits}

若要瞭解在多個網站位置重複使用相同內容（文字和資產）的使用案例，請參閱 [可能的MSM案例](/help/sites-cloud/administering/msm/overview.md). [!DNL Experience Manager] 維護原始資產與其連結副本之間的連結，稱為即時副本(LC)。 維持的連結可將集中變更推送至許多即時副本。 這樣可以在消除管理重複副本的限制的同時實現更快的更新。 變更的傳播不會發生錯誤，而且是集中化的。 功能提供空間，讓更新僅限於選取的即時副本。 使用者可以分離連結（亦即中斷繼承），並進行本機編輯，這些編輯在下次更新主要副本並轉出變更時不會覆寫。 分離功能適用於幾個選取的中繼資料欄位或整個資產。 它允許靈活地在本機更新最初從主要副本繼承的資產。
...
**繼承：** 即時副本資產/資料夾與其來源之間的連結/引用，系統使用它來記住將更新傳送到何處。 中繼資料欄位的繼承存在於精細層級，還有內容片段變數和欄位。 可以移除所選專案的繼承，同時保留來源與其即時副本之間的即時關係。
...
您可以一次建立一個資產或資料夾的即時副本。 您可以建立衍生自即時副本本身的資產或資料夾的即時副本。
...
|---|---|---|
| [!UICONTROL 引用] 邊欄 | 當您已選取來源時，請快速同步處理。 | 另請參閱 [來源參考邊欄中的快速動作](#ref-rail-source) |
...
這將表示該專案不再與來源元件同步。 如有需要，您可在稍後啟用繼承。

### 取消繼承 {#cancel-inheritance}

若要取消繼承，請執行下列動作：

1. 選取 **取消繼承** 圖示，位於必要專案旁：
...
1. 若要啟用專案的繼承，請選取 **重新啟用繼承** 必要專案旁的圖示：
   ...
   >[!NOTE]
   >
   >當您重新啟用繼承時，專案不會自動與來源同步。 如果需要，您可以手動要求同步。
...
>[!NOTE]
>
>請記住，內容片段的MSM (透過 **[!UICONTROL 資產]** console)使用Assets功能；這是因為它們儲存為Assets （雖然視為Sites功能）。

## MSM的限制和已知問題 [!DNL Assets] {#limitations}

以下為的MSM限制 [!DNL Assets].
