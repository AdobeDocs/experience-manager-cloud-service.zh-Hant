...
使用者可以將多個AEM Sites執行個體（對最大數量沒有任何限制）連線到遠端DAM部署，這是相對於 [連線的Assets](use-assets-across-connected-assets-instances.md) 功能。
...
Dynamic Media搭配OpenAPI功能提供數種其他好處，例如存取和使用內容片段中的遠端資產、擷取遠端資產的中繼資料等等。 瞭解更多關於其他的 [Dynamic Media與OpenAPI功能相較於連線Assets的優點](/help/assets/dynamic-media-open-apis-faqs.md).

## 開始之前 {#pre-requisits-sites-integration}

* 設定下列專案 [環境變數](/help/implementing/cloud-manager/environment-variables.md#add-variables) 若為AEM as a Cloud Service：
...
1. 前往 **[!UICONTROL Assets]** > **[!UICONTROL 檔案]**.
1. 選取出現內容片段的資產資料夾。
...
您也可以存取Edge Delivery Services中的遠端資產。 如需詳細資訊，請參閱 [透過OpenAPI功能利用Dynamic Media提供的Assets as a Cloud Service資產](https://www.aem.live/docs/aem-assets-sidekick-plugin#utilizing-assets-from-assets-cloud-services-delivered-via-dynamic-media-with-openapi).
