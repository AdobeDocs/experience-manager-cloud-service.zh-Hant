...
透過Adobe Experience Manager (AEM)as a Cloud Service，內容片段可讓您設計、建立、組織和 [發佈獨立於頁面的內容](/help/sites-cloud/authoring/fragments/content-fragments.md). 它們可讓您準備內容以用於多個位置/多個管道，非常適合Headless傳送。 它們也可以搭配 [多網站管理可讓您重複使用內容](#reusing-content-fragments-with-msm-assets).
...
溝通管道的數量每年都在增加。 管道通常指的是傳遞機制，可以是：

* 物理通道;例如，桌面、行動裝置。
* 物理通道中的傳遞形式;例如，桌面的「產品詳情頁面」、「產品類別 頁面」或行動裝置的「行動網站」和「行動應用程式」。

但是，您（可能）不想對所有管道使用相同的內容 - 您必須根據特定通道優化內容。
...
* 為一系列頻道構建內容池。
* 針對特定渠道設計內容變體。
* 通過插入資產（混合媒體片段）將圖像添加到文本中。
* 建立巢狀內容，讓您可以反映資料的複雜性。

然後可以組合這些內容片段，以通過各種管道提供體驗。

>[!NOTE]
>
>**內容片段和**[&#x200B;體驗片段&#x200B;**](/help/sites-cloud/authoring/fragments/content-fragments.md)**&#x200B;是 AEM 中的不同功能：
>* **内容片段** 是編輯內容，具有定義和結構，但沒有額外的視覺設計和/或佈局。 它們可用于訪問結構化資料，包括文本、數位和日期等。
>* **體驗片段** 完整佈局內容;網页頁面的片段。
>
...
* 用於 [依內容片段元件的頁面編輯器](/help/sites-cloud/authoring/fragments/content-fragments.md) （參照元件）：
...
* 頁面編輯器的「 **資產** 」瀏覽器可用的資產
  ...
  另請參閱 [資產瀏覽器](/help/sites-cloud/authoring/page-editor/editor-side-panel.md#assets-browser) 以取得詳細資訊。
...
* **片段元素**
   ...
   * 元素與保存內容的數據欄位相關。
   * 您可以使用內容模型創建內容片段。 模型中指定的元素（欄位）定義片段的結構。 這些元素（欄位）可以是各種數據類型。
...
* **Assets 插入片段 （混合媒體片段）**
   ...
   * Assets（圖像）插入到實際片段中，用作片段的內部內容。
   * 嵌入在片段的段落系統中。
   * 可以在下列情況下格式化： [在頁面上使用/參考片段](/help/sites-cloud/authoring/fragments/content-fragments.md).
...
* **變數**
   ...
   * 特定於編輯目的的片段文本的再現;可以與通道相關但不是強制性的，也可以用於廣告臨時的局部修改。
   * 會建立為 Master **的副本**，但隨後可視需要進行編輯。變數本身之間內容重迭。
   * 可在片段編寫期間定義。
   * 儲存在片段中，以避免內容副本散落。
   * 若已更新主內容，則變數可以 [與 Master 同步](/help/assets/content-fragments/content-fragments-variations.md#synchronizing-with-master) 。
   * 可以摘要](/help/assets/content-fragments/content-fragments-variations.md#summarizing-text)以[快速將文本截斷為預定義的長度。
   * 可在片段編輯者的變体](/help/assets/content-fragments/content-fragments-variations.md)標籤下[使用。

### 頁面使用內容片段創作時的中間內容 {#in-between-content-when-page-authoring-with-content-fragments}

中間內容：

* 可用於處理內容片段時的頁面編輯器。
* [在片段流程中新增的其他內容](/help/sites-cloud/authoring/fragments/content-fragments.md#adding-in-between-content) 在頁面上使用或參照之後。
* 可用於以下專案： [使用內容片段時的頁面編輯器](/help/sites-cloud/authoring/fragments/content-fragments.md).
...
* **內容模型**
   ...
   * 可使用 [配置瀏覽器](/help/assets/content-fragments/content-fragments-configuration-browser.md)啟用。
   * 使用 [工具](/help/assets/content-fragments/content-fragments-models.md)創建。
   * 需要建立 [片段](/help/assets/content-fragments/content-fragments-managing.md#creating-content-fragments)。
   * 定義片段的結構（標題、內容元素標記定義）。
   * 內容模型定義需要一個標題和一個數據元素;其他一切都是可選的。
   * 模型可以定義預設內容 - 如果適用。
...
## 透過MSM重複使用內容片段（適用於資產） {#reusing-content-fragments-with-msm-assets}
...
* [使用MSM重複使用資產功能](/help/assets/reuse-assets-using-msm.md).

這些會啟用 [繼承](/help/assets/content-fragments/content-fragments-variations.md#inheritance) 變數和片段的個別欄位。
