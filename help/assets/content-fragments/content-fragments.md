...
* 在頁面上參考時，用作[中間內容](#in-between-content-when-page-authoring-with-content-fragments)。
* 獨立於傳遞機制（即頁面、頻道）。
...
* **變數**
   ...
   * 特定編輯目的的片段文字轉譯；可以與頻道相關，但這不是強制性的；也可用於臨時本機修改。
...
* **內容片段模型**
   ...
   * 使用組態瀏覽器[啟用](/help/assets/content-fragments/content-fragments-configuration-browser.md)。
   * 是[使用工具](/help/assets/content-fragments/content-fragments-models.md)建立。
   * 需要[建立片段](/help/assets/content-fragments/content-fragments-managing.md#creating-content-fragments)。
   * 定義片段的結構（標題、內容元素、標籤定義）。
   * 模型定義需要一個標題和一個資料元素；其他內容都是選用的。
   * 模型可定義預設內容（如適用）。
