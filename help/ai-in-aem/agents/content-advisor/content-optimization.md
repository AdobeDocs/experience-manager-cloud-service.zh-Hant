...
作為[AEM的Content Advisor Agent的一部分，](/help/ai-in-aem/agents/content-advisor/overview.md)內容最佳化代理程式透過套用自然語言指令來建立適合管道的變數，以轉換使用者調整與調整資產的方式。 無論是為特定數位頻道產生新轉譯、調整視覺屬性、變更背景或準備資產，代理程式都會自動解譯使用者意圖並執行複雜的編輯工作。 它可與[內容探索代理程式](/help/ai-in-aem/agents/content-advisor/discovery.md)緊密合作，使用核心[Dynamic Media （具有OpenAPI功能](/help/assets/dynamic-media-open-apis-overview.md)）來取得它找到的資產，並產生最佳化的變化，以符合品牌、頻道和行銷活動需求，無需手動設計。
...
* **[與內容探索代理程式緊密合作](/help/ai-in-aem/agents/content-advisor/discovery.md)**：以內容探索代理程式所識別的資產為基礎，透過自然對話啟用端對端資產擷取和最佳化。
...
* **透過自然語言瞭解意圖**
  ...
  內容最佳化代理程式會從自然語言提示中解譯使用者意圖、說明頻道、行銷活動和對象內容，以決定最相關的最佳化動作。
...
* **最佳化影像內容**
  ...
  內容最佳化代理程式會套用格式轉換、解析度調整、裁切和銳利化等增強功能來改善影像品質。

* **多變體資產最佳化**

  內容最佳化代理程式可使用單一自然語言提示，從內容探索代理程式傳回的資產中產生多個最佳化影像變化，讓使用者能夠快速並有效率地產生適用於頻道的轉譯。

## 角色 {#personas-content-optimization-agent}

頻道行銷人員（內容最佳化代理程式的關鍵角色）可以選取正確的高解析度來源內容，並請求根據其頻道和受眾區段量身打造的最佳化格式。

地區行銷人員和代理商也可以使用內容最佳化代理程式，快速產生適用於管道的影像變化，以支援更快、更一致的內容製作。

## 如何存取 {#access-content-optimization-agent}

您可以透過AI助理存取AEM中的內容最佳化代理程式。 登入[`experience.adobe.com`](https://experience.adobe.com)，您可以使用`Ask AI Assistant anything`欄位以自然語言指定提示，開始與AI助理互動：

![存取內容最佳化代理程式](/help/ai-in-aem/agents/content-advisor/assets/access-discovery-agent.png)

## 常見使用案例和範例提示 {#use-cases-prompts}

透過[內容探索代理程式搜尋正確的資產，以使用內容最佳化代理程式。](/help/ai-in-aem/agents/content-advisor/discovery.md)呈現相關影像後，使用者可以直接從搜尋結果為一或多個資產產生最佳化或頻道特定變體。 或者，使用者可在提示中指定資產UUID或資產路徑，而無需執行先前的搜尋，即可產生變體。 此工作流程可確保高品質的輸入資料，並持續提供更佳的最佳化結果。 [如需詳細資訊，請參閱完整的可用最佳化清單](https://developer.adobe.com/experience-cloud/experience-manager-apis/api/stable/assets/delivery/?lang=zh-Hant)。
...
* **高解析度轉譯建立**
  ...
  使用[內容探索代理程式](/help/ai-in-aem/agents/content-advisor/discovery.md)搜尋正確的資產，然後在有多個搜尋結果時使用下列提示：
