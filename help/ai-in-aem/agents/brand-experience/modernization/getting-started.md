...
瞭解開始使用Experience Modernization Agent和Experience Modernization Console的第一步。

>[!NOTE]
>
>如果您有興趣使用Experience Modernization Console，可以透過您的客戶經理要求存取權，以確保順利上線體驗。

## 準備Edge Delivery GitHub存放庫 {#prepare-repo}

>[!NOTE]
>
>使用AEM Sites和Universal Editor專案？ 請依照[AEM Sites/Universal Editor快速入門](/help/ai-in-aem/agents/brand-experience/modernization/getting-started-aem-authoring.md)安裝步驟操作。
...
1. 確定存放庫中已安裝[AEM程式碼聯結器](https://github.com/apps/aem-code-connector)。
   * 這可讓主控台檢查您的程式碼。
...
1. 按一下&#x200B;**連線儲存機制**。
1. 這會在新的瀏覽器標籤上開啟AEM程式碼聯結器應用程式。 按一下&#x200B;**授權AEM程式碼聯結器**。
1. 返回主控台，指定網站的預覽URL。 預覽網站中的任何檔案，或是從分支、site-name和org建構檔案，都可以取得預覽URL。 系統會自動擷取關聯的Github專案，在某些情況下，系統可能也會要求您提供Github座標。
   ![正在連線到GitHub專案](assets/connect-site-and-github.png)
...
>[!TIP]
>
>[檢視提示指南](/help/ai-in-aem/agents/brand-experience/modernization/prompting-guide.md)，以取得如何提示代理程式及其技能的更多想法。

## 上傳內容 {#upload-content}

>[!TIP]
>
>如果您處理AEM Sites和Universal Editor專案，將內容上傳到AEM的運作方式略有不同。 如需具體上傳指示，請參閱[AEM Sites/Universal Editor專案的Experience Modernization Agent快速入門](/help/ai-in-aem/agents/brand-experience/modernization/getting-started-aem-authoring.md#upload-content)。
...
如果您的網站位於防火牆或存取限制之後，您可以允許列出下列IP位址，以便後端服務可以清除您的網站：
...
* [體驗現代化主控台](/help/ai-in-aem/agents/brand-experience/modernization/console.md) — 主控台、其檢視、選項和功能的詳細資料
* [Experience現代化代理程式的提示指南](/help/ai-in-aem/agents/brand-experience/modernization/prompting-guide.md) — 如何提示代理程式及其技能的想法
