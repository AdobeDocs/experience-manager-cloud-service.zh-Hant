...
表單建立工作是[Brand Experience Agent](/help/ai-in-aem/agents/brand-experience/overview.md)的一項功能，其設計是要使用自然語言提示來開發表單。 此工作會自動產生適當的表單結構和欄位型別。 此工作透過AI助理顯示。

表單建立工作的一些主要優點包括：

* **加速表單開發**：使用簡單的自然語言命令快速建立表單，無需學習傳統產品介面。
* **一致的品牌體驗**：使用核准的範本和樣式，建立符合您組織品牌、範本和樣式指導方針的表單。
* **較低的技術障礙**：讓商務使用者輕鬆建立表格，而不需要進階的技術或深厚的產品專業知識。

## 功能 {#capabilities}

* **建立具有純文字提示的新表單**：您可以透過以純文字提交您的要求來建立表單。 此技能會根據您的自然語言描述和指定的範本，自動產生適當的表單結構、欄位型別和品牌上體驗。 此功能可加速表單建立，同時確保維持品牌和合規性標準。
...
* *匯入`https://[aem-author-url]/path/to/pdf/file`*&#x200B;可用的PDF檔案，以建立信用卡申請表
* *在`https://github.com/wkndforms/wesecure`*&#x200B;使用樣版建立意見反應表單
...
若要為貴組織啟用表單建立工作，必須透過Adobe啟動啟用。 透過以下方式聯絡以開始此程式：
