...
>[!NOTE]
>
> 通訊建立工作目前使用Alpha版。 如果您想要參與，請將您的正式電子郵件地址傳送至[aem-forms-ea@adobe.com。](mailto:aem-forms-ea@adobe.com)
...
* **從範本建立**：您可以使用核准的組織範本，以確保品牌一致性和法規遵循標準。 此工作會運用您現有的範本和風格准則，建立符合法規要求的品牌對應。

* **將現有的影像與檔案匯入並轉換至互動式通訊**：您可以將現有的檔案匯入並轉換至互動式通訊。 這項工作會分析上傳的內容，以偵測欄位、保留版面，並建立與動態內容功能之間的資料導向對應。 支援的格式包括PDF、影像(JPG、PNG)和手繪範本。
...
* **新增欄位和內容**：使用自然語言提示新增欄位、文字區塊、影像、圖表、表格和其他元件至您的通訊檔案。 此工作會解譯您的指示，並以適當的結構和格式插入適當的元素。

* **編輯欄位和內容**：透過對話式命令修改通訊檔案中的現有欄位和內容。 更新欄位屬性、變更文字內容、調整資料繫結，以及調整元件設定。

* **移除欄位和內容**：使用自然語言指示，從通訊檔案中刪除不要的欄位、元件或區段。 作業會移除指定的元素，同時維持檔案結構和版面配置完整性。
