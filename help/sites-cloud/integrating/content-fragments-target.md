...
您可以匯出 [內容片段](/help/sites-cloud/authoring/fundamentals/content-fragments.md)，在Adobe Experience Manager as a Cloud Service(AEM)中建立，到Adobe Target(Target)。 然後可在Target活動中作為選件，以大規模測試並個人化體驗。

有可供將內容片段匯出至Adobe Target的選項：
...
* 還有 [刪除已匯出至Adobe Target的內容片段](#deleting-a-content-fragment-already-exported-to-adobe-target)

內容片段可匯出至Adobe Target中的預設工作區，或匯出至Adobe Target的使用者定義工作區。
...
1. 開啟 **頁面屬性** ，以取得適當的資料夾或片段。
   ...
   >[!NOTE]
   >
   >如果您將雲端設定新增至內容片段上層資料夾，則設定會由所有子項繼承。
   >
   >如果您將雲端設定新增至內容片段本身，設定會由所有變數繼承。
...
1. 在 **Cloud Service配置**，選取 **Adobe Target** 從下拉式清單中。
   ...
   >[!NOTE]
   >
   >內容片段選件的JSON格式可自訂。 若要這麼做，請定義客戶內容片段元件，然後註解如何在元件Sling模型中匯出其屬性。
   >
...
1. 在 **Adobe Target** 選擇：
   ...
   * 如有必要 — externalizer域
   ...
   >外部化程式域是可選的。
   >
   > 當您希望匯出的內容指向特定 *發佈* 網域。 如需更多詳細資訊，請參閱 [設定AEM Link Externalizer](/help/implementing/developing/extending/content-fragments-customizing.md#configuring-the-aem-link-externalizer).
   >
   > 另請注意，「外部化程式網域」僅與傳送至Target的內容片段內容相關，而與「檢視選件內容」等中繼資料無關。
...
>
>因此，內容片段及所有相關資產必須在匯出至Target之前先發佈。

若要將內容片段從AEM匯出至Target（指定雲端設定後）:
...
1. 點選/按一下 **確定** 在確認對話方塊中。
   ...
   >[!NOTE]
   >
   >[各種詳細資訊](/help/sites-cloud/authoring/fundamentals/content-fragments.md#details-of-your-content-fragment) 的 **清單檢視** 控制台和 **屬性**.

   >[!NOTE]
   >
   >在Adobe Target中檢視內容片段時， *上次修改* 顯示的日期是片段上次在AEM中修改的日期，而非片段上次匯出至Adobe Target的日期。
...
執行先前的工作後，內容片段會顯示在Target的「選件」頁面上。 請看 [特定Target檔案](https://experienceleague.adobe.com/docs/target/using/integrate/aem/fragments/content-fragments-aem.html) 了解你能在那裡實現什麼。
...
和匯出一樣，從Adobe Target刪除內容片段也可以從 **資產** 選取片段後主控台：

![在 Adobe Target 刪除](assets/cfm-export-target-02.png)

刪除已匯出至Target的內容片段，如果該片段已用於Target中的選件，則可能會造成問題。 刪除片段會導致選件無法使用，因為AEM正在傳送片段內容。

<!-- if the information about deleting-if-used correct, or is it not allowed at all? -->

要避免這種情況：

* 如果活動中目前未使用內容片段，AEM可讓使用者刪除片段，而不含警告訊息。
* 如果Target中的活動目前正在使用內容片段，則會出現錯誤訊息，警告AEM使用者刪除片段對活動可能造成的後果。

   AEM中的錯誤訊息並未禁止使用者（強制 — ）刪除內容片段。 如果內容片段被刪除：

   * 含有AEM內容片段的Target選件可能會顯示不適當的行為

      * 內容片段推送至Target時，選件可能仍會呈現
      * 如果參考的資產也在AEM中刪除，內容片段中的任何參考可能也無法正常運作。
   * 當然，由於內容片段已不存在，因此無法對內容片段進行任何進一步修改。
...
* [Adobe Target — 在Target活動中使用AEM內容片段來協助最佳化或個人化](https://experienceleague.adobe.com/docs/target/using/integrate/aem/fragments/content-fragments-aem.html)
