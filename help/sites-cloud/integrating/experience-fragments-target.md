...
1. 體驗片段會從AEM編寫執行個體匯出，因此您需要 [設定AEM連結外部化器](/help/implementing/developing/extending/experience-fragments.md#configuring-the-aem-link-externalizer) 編寫執行個體上，以確保體驗片段中的任何參考資料都外部化，以便進行Web傳送。

   >[!NOTE]
   >
   >對於預設未涵蓋的連結重寫，請 [體驗片段連結重寫程式提供者](/help/implementing/developing/extending/experience-fragments.md#the-experience-fragment-link-rewriter-provider-html) 可用。 透過此專案，可以為您的執行個體開發自訂規則。
...
1. 下 **Adobe Target** 選取：
   ...
   * 必要時 — 外部化器網域
   ...
   >外部化器網域是選用的。
   >
   > 當您想要匯出的內容指向特定時，會設定AEM外部化程式 *發佈* 網域。 如需詳細資訊，請參閱 [設定AEM連結外部化器](/help/implementing/developing/extending/experience-fragments.md#configuring-the-aem-link-externalizer).
   >
...
1. 點選/按一下 **確定** 在確認對話方塊中。
   ...
   >[!NOTE]
   >
   >[各種細節](/help/sites-cloud/authoring/fundamentals/experience-fragments.md#details-of-your-experience-fragment) 匯出的ID顯示在 **清單檢視** 控制檯和 **屬性**.
