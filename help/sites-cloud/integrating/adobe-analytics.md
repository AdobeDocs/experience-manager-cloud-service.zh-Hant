...
* [連線至Adobe Analytics和建立架構](https://docs.adobe.com/content/help/en/experience-manager-65/administering/integration/adobeanalytics-connect.html) -請注意，「Analytics架構」是AEM的舊版，而且其建立作業在AEM中無法以雲端服務的形式運作，因為它需要Classic UI。Launch by Adobe應改為用於變數對應和將JS程式庫部署至頁面。
...
* [擴充Adobe Analytics整合功](https://docs.adobe.com/content/help/en/experience-manager-65/developing/extending-aem/extending-analytics/extending-analytics.html) 能，以取得有關開發收集使用者資料的元件和自訂Adobe Analytics架構的資訊。請注意，「Analytics架構」是AEM的舊版，而且其建立在AEM中無法當做雲端服務運作，因為它需要Classic UI。 Launch by Adobe應改為用於變數對應和將JS程式庫部署至頁面。
