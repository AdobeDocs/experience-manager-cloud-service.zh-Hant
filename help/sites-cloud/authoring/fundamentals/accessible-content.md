...
如果使用顏色作為提示來提供資訊，則應提供其他視覺提示，如更改樣式（如粗體、斜體字）或字型。 這有助於視力不足或彩色視力不足的人識別資訊。 不過，這無法完全依賴，因為這對根本看不到頁面的人沒有幫助。 因此，提供隱藏文字或使用程式化解決方案(例如 [無障礙的Rich Internet Applications(ARIA)Web標準套件](https://www.w3.org/WAI/standards-guidelines/aria/)，向無視的使用者傳達此資訊。
...
>* [字型大小轉換：pixel-point-em-rem-percent](https://websemantics.uk/tools/)
