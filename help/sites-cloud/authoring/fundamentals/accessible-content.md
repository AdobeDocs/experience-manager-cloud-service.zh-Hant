...
此 [網頁內容可及性指引(WCAG)2.1](https://www.w3.org/TR/WCAG/) 繪製者 [萬維網聯盟的一個工作組](https://www.w3.org/群組/#Accessibility_Guidelines_Working_Group). 它包含一套與技術無關的指引和成功標準，以協助殘疾人士存取及使用網路內容。
...
網頁上的資訊可以以許多不同的非文本格式提供，如圖片、視頻、動畫、圖表和圖形。 盲人或視力嚴重受損者無法看到非文字內容。 但是，他們可以通過螢幕閱讀器讀取文本內容或通過盲文顯示設備以觸覺形式呈現來訪問文本內容。 因此，通過以圖形格式提供內容的替代文本，看不到該內容的人可以訪問內容提供的資訊的同等版本。

另一個有用的好處是，文本替代項使得非文本內容能夠根據搜索引擎技術編製索引。

#### 如何達成 — 非文字內容(1.1.1) {#how-to-meet-non-text-content}

對於靜態圖形，基本要求是為圖形提供等效文本替代。 此方法可在 **替代文字** 欄位。 例如，請參閱核心元件 **[影像](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/wcm-components/image.html)**.
...
當您為AEM例項實作這些版本時，您的開發團隊必須設定這些元件以支援 `alt` 屬性。 這麼做可確保作者可將其新增至內容(請參閱 [新增對其他HTML元素和屬性的支援](/help/implementing/developing/extending/rte-accessible-content.md#adding-support-for-additional-html-elements-and-attributes))。
...
* 如果非文本內容是指向其他頁面或文檔的連結，並且沒有其他文本形成部分屬於同一連結，則影像的替代文本必須指示連結的目標。 它不能描述影像。
* 如果非文字內容包含在按鈕元素中，並且沒有相同按鈕的文本形成部分，則影像的替代文本必須指示按鈕的功能。 它不能描述影像。
...
* 說明性照片：這些是人、物體或地方的影像。 請務必思考像片在頁面中的角色，並建議說明影像內容，因為輔助技術會宣佈元素類型(例如， `graphic` 或 `image`)。 可提高使用的清晰度 `screenshot` 或 `illustration` 在替代文字說明中，但這取決於內容。 一致性是重要因素，應為整個製作團隊做出決策，並套用至整個使用者體驗。
...
* 驗證碼：驗證碼是 *完全自動化的公共圖靈測試*. 這是一種用於網頁上的安全檢查，用於區分人類和惡意軟體，但可能造成無障礙障礙。 這些影像需要使用者描述他們看到的內容，才能通過安全性測試。 無法為影像提供替代文字，因此您必須考慮替代的非圖形解決方案。 W3C提供數個建議。 這些方法各有其優點和缺點。
...
* 僅限音訊和僅限視訊（預錄）:對於預錄的純音頻和預錄的純視頻媒體，以下是真的，但音頻或視頻是文本的替代媒體且明確標籤為這樣的媒體除外：
   * 僅預錄音頻：基於時間的媒體的替代方法，它為預錄的僅音頻內容提供等同的資訊。
   * 僅預錄視頻：提供時間型媒體的替代方案，或提供預先錄制僅限視訊內容之同等資訊的音訊軌道。
...
* 如果內容預錄的音訊沒有視訊（例如播客）:
   * 在內容之前或之後立即提供一個連結到音頻內容的文本記錄。 文字記錄應是HTML頁面，其文字等同於所有口語和非口語內容，還應包含說話者的指示、設定的描述、聲音表達，以及任何其他重要音頻的描述。
* 如果內容是動畫或預錄的無音頻視頻：
...
在AEM網頁中插入多媒體與插入影像類似。 然而，由於多媒體內容遠不止是靜止影像，所以在控制多媒體的播放方式方面存在不同的設定和選項。
...
對於隱藏式字幕，您必須以適當的格式建立並提供同步的字幕檔案(例如 [SMIL](https://www.w3.org/AudioVideo/))和影片檔案(有關如何執行此作業的詳細資訊，不在本指南的討論範圍內，但下方提供一些教學課程的連結 [更多資訊 — 字幕（預錄）(1.2.2)](#more-information-captions-prerecorded). 請務必提供附註或啟用視訊播放器的註解功能，讓使用者知道視訊有可用的註解。
...
盲人或視力障礙的人，如果只以視覺方式提供視訊或動畫中的資訊，或如果音軌未提供足夠的資訊，便無法透過視覺方式了解正在發生的事情，則會體驗無障礙。
...
AEM可讓您使用適當的HTML元素，輕鬆建構具語義意義的網頁內容。 在RTE中開啟頁面內容（文字元件），然後使用 **Paraformat** 菜單（段落符號），以指定相應的結構元素（例如段落、標題等）。
...
* **表格**:必須使用HTML表元素來標識資料表：
   ...
   * 此 `<summary>` 元素可協助無視的使用者提供視力不佳的使用者看得到的概要，以更輕鬆地了解表格中顯示的資訊。 若使用複雜或非常規的表格配置，此工作流程就十分實用（此屬性不會顯示在瀏覽器中，而只會閱讀至輔助技術）。
...
* **複雜資料表**:有時，如果有具有兩個或多個標題級別的複雜表，則基本表屬性可能不足以提供所有必要的結構資訊。 對於這些類型的複雜表格，需要使用header和 **id屬性在標題及其相關儲存格之間建** 立直 **接** 關係。
...
同樣，需要區分不同聲音（例如，男性或女性口語內容）的資訊對於聽力障礙的人來說是無障礙的，如果它沒有反映在音頻內容的任何文本替代中。
...
如果使用顏色作為提示來提供資訊，則應提供額外的視覺提示，例如更改樣式（如粗體、斜體字）或字型。 這有助於視力不足或彩色視力不足的人識別資訊。 不過，這無法完全依賴，因為這對根本看不到頁面的使用者沒有幫助。 因此，提供隱藏文字或使用程式化解決方案(例如 [無障礙的Rich Internet Applications(ARIA)Web標準套件](https://www.w3.org/WAI/standards-guidelines/aria/)，向無視的使用者傳達此資訊。
...
為網頁內容選取適當字型和調整大小時，請善於判斷並避免易讀性和易用性。

>[!NOTE]
對以下片語執行Web搜索，以查找可幫助您轉換為其他單位的工具：
* Px至Em計算器 <!--  (https://www.omnicalculator.com/conversion/px-to-em) -->
...
若要檢查對比度，請使用顏色對比工具，例如 [Paciello組顏色對比分析器](https://www.tpgi.com/resources/contrast-analyser.html) 或 [WebAIM顏色對比檢查程式](https://webaim.org/resources/contrastchecker/). 這些工具可讓您檢查色彩配對，並報告任何對比問題。

或者，如果您不太在意指定頁面的外觀，可以選擇不指定背景和前景文字顏色。 由於使用者的瀏覽器決定文字和背景的顏色，因此不需要進行對比檢查。

如果無法達到建議的對比層級，您必須提供頁面替代版本的連結（沒有顏色對比問題），或允許使用者根據自己的需求調整頁面顏色配置的對比。
...
此成功標準旨在確保為身心障礙的使用者提供充足的時間，以便盡可能與網頁內容互動。 失明、低視力、靈巧性缺陷和認知限制等殘疾人可能需要更多時間閱讀內容或執行功能，如填寫線上表單。 如果Web函式與時間相關，則某些用戶在時間限制發生之前就很難執行所需的操作。 這可能會使他們無法存取服務。 設計不依賴時間的功能有助於殘疾人成功完成這些功能。 提供禁用時間限制、自定義時間限制長度或在時間限制發生前請求更長時間的選項，可幫助那些需要比預期更長時間才能成功完成任務的用戶。 這些選項會依對使用者最有幫助的順序列出。 禁用時間限制比自定義時間限制長度更好，這比在時間限制發生之前請求更多時間要好。
...
* 暫停、停止、隱藏：對於移動、閃爍、滾動或自動更新資訊，以下是true:
   ...
   * 自動更新：對於(a)自動啟動且(b)與其他內容並行顯示的任何自動更新資訊，有一種機制可讓使用者暫停、停止或隱藏更新，或控制更新的頻率，除非自動更新是活動的一部分，而此活動至關重要。
...
此成功標準旨在確保當使用者依序導覽內容時，會遇到與內容含義一致且可由鍵盤操作的資訊。 這可讓使用者建立內容的一致心理模型，以減少混淆。 內容中可能會有不同的順序反映邏輯關係。 例如，以由多個欄位和/或步驟組成的線上形式在元件之間移動會反映內容中的邏輯關係。
...
對所有使用者而言，無論是否受到損害，透過適當連結文字清楚指出連結的方向至關重要。 這可協助使用者決定是否確實要追蹤連結。 對有視力的使用者而言，有意義的連結文字在頁面上有數個連結時（尤其是頁面文字繁多時）很實用，因為有意義的連結文字可提供目標頁面功能的更清楚指示。 某些輔助技術的使用者可產生單一頁面上所有連結的清單，如果該連結文字既獨特又能提供資訊，則可更輕鬆地了解內容外的連結文字。 然而，如果一個連結沒有提供足夠的資訊來準確描述連結的走向，則弱視的認知障礙個體可能會變得困惑。
...
撰寫時，使用標題屬性會有一些問題，以確保頁面上呈現的類似連結提供關於目的地的唯一資訊（例如，「了解詳情」通常是指一系列不同的目的地）:

* 標題屬性中包含的文字僅作為工具提示彈出式視窗可供滑鼠使用者使用，且無法使用鍵盤或行動使用者一致地存取。
...
雖然建議您提供可識別連結用途的連結文字，而不需要額外內容，但您仍可確認這並非總能發生。 無內容連結可用於下列情況，其HTML範例可在 [如何符合成功標準2.4.4](https://www.w3.org/WAI/WCAG21/quickref/#link-purpose-in-context).
...
焦點可以通過鍵盤（例如，對控制項進行Tab鍵操作）或滑鼠（例如，選擇文本欄位）移到控制項。 將滑鼠移到控制項上不會移動焦點，除非指令碼實施此行為。 對於某些類型的控制項，選擇控制項也可以激活控制項（例如按鈕），這反過來又可以啟動上下文中的更改。
...
如果網頁上有兩個元件，且兩者的功能與一組網頁中其他頁面上的元件相同，則這三個元件必須一致。 因此，同一頁上的兩者是一致的。
...
此成功標準的目的在於確保使用者知道發生錯誤，並可判斷錯誤。 錯誤訊息應盡可能具體。 如果表單提交失敗，重新顯示表單並指出錯誤中的欄位，不足以讓部分使用者察覺發生錯誤。 例如，螢幕助讀程式的使用者在遇到其中一個指標前，才知道有錯誤。 他們可能會在遇到錯誤指標前完全放棄表單，認為頁面無法運作。 根據WCAG中的定義， [輸入錯誤](https://www.w3.org/TR/WCAG/#dfn-input-error) 是使用者提供的未接受的資訊。 這包括：

網頁所需但用戶所省略的資訊，或用戶所提供但不符合所需資料格式或允許值的資訊。
例如：

* 用戶未在「州」、「省」、「地區」等欄位中輸入正確的縮寫；
* 用戶輸入非有效狀態的狀態縮寫；
* 用戶輸入的郵遞區號不存在；
* 用戶在未來2年內輸入出生日期；
...
在AEM WKND示範專案中，新增表單元件(例如 **文字欄位**，前往頁面。 此預設標題取決於元件類型。 您可以在 **標題和文字** 頁簽。 務必確保標籤可協助使用者了解與每個表單元件相關聯的資料。

此 **標題** 欄位必須用於欄位元素，因為它提供可用於輔助技術的標籤。 僅在欄位旁的文字中寫標籤是不夠的。

對於某些表單元件，您也可以使用 **隱藏標題** 核取方塊。 以此方式隱藏的標籤仍適用於輔助技術，但無法顯示在螢幕上。雖然這在某些情況下是個好方法，但最好盡可能加入視覺標籤，因為有些使用者可能會看到畫面的一小段（一次看一個欄位），並需要標籤才能正確識別欄位。
...
成功標準3.3.1提供錯誤通知。 然而，認知有限的人可能很難理解如何糾正錯誤。 視覺殘疾的人可能無法確切了解如何更正錯誤。 如果表單提交失敗，使用者可能會因為不確定如何更正錯誤而放棄表單，即使他們知道錯誤已發生。
...
* 錯誤預防（法律、財務、資料）:對於導致用戶發生法律承諾或財務交易、修改或刪除資料儲存系統中用戶可控資料或提交用戶測試響應的網頁，至少以下情況之一為真：
   ...
   * 已確認在提交定稿之前，可用於審核、確認和更正資訊的機制。

#### 用途 — 錯誤預防（法律、財務、資料）(3.3.4) {#purpose-error-prevention-legal-financial-data}

本成功標準旨在協助身心障礙的使用者避免執行無法還原的動作時發生錯誤而導致的嚴重後果。 例如，購買不可退還的機票或提交訂購單以在經紀帳戶中購買股票，是具有嚴重後果的金融交易。 如果用戶在航空旅行日期犯了錯誤，他們最終可能會拿到一張無法交換的錯誤日期的票。 如果用戶在要購買的股票數量上犯了錯誤，那麼他們最終可能購買的股票會超出預期。 這兩種錯誤都涉及立即發生的交易，且以後無法更改，而且代價高昂。 同樣，如果用戶無意修改或刪除儲存在資料庫中的資料（如他們在旅行服務網站中的整個旅行配置檔案），則可能是一個不可恢復的錯誤。 當提及修改或刪除「用戶可控」資料時，其目的是防止大量丟失資料，如刪除檔案或記錄。 不是為了每次保存命令或文檔、記錄或其他資料的簡單建立或編輯而需要確認。

殘疾使用者更可能犯錯。 閱讀障礙者可以轉換數字和字母，運動障礙者可以錯擊鍵。 能夠反轉動作可讓使用者更正可能導致嚴重後果的錯誤。 審查和更正資訊的能力使用戶能夠在採取具有嚴重後果的動作之前檢測錯誤。

用戶可控資料是用戶可查看的資料，用戶可以通過故意操作來更改和/或刪除這些資料。 控制此類資料的用戶的示例將更新用戶帳戶的電話號碼和地址，或從網站刪除過去發票的記錄。 它不會參照使用者無法直接檢視或與之互動的網際網路記錄和搜尋引擎監控資料等內容。
...
由於修復技術在不同的使用者代理之間有所不同，因此作者無法假設內容被精確剖析為資料結構，或內容被專業的使用者代理（包括輔助技術）正確呈現，除非內容是根據該技術的正式語法中定義的規則建立。 在標籤語言中，元素和屬性語法中的錯誤以及未提供正確巢狀的開始/結束標籤會導致錯誤，使用者代理無法可靠地剖析內容。 因此，成功標準要求只能使用正規語法的規則來剖析內容。
...
用戶介面控制項的一個重要狀態是是否具有焦點。 控制項的焦點狀態可以以程式設計方式確定，並且關於焦點改變的通知會傳送給使用者代理和輔助技術。 用戶介面控制狀態的其他示例包括是否已選中複選框或單選按鈕，或者是否展開或折疊可折疊的樹或清單節點。
