...
的 [Web內容輔助功能指南(WCAG)2.1](https://www.w3.org/TR/WCAG/) 由 [萬維網聯盟的一個工作組](https://www.w3.org/組/#Accessibility_Guidelines_Working_Group)。 它包括一套與技術無關的准則和成功標準，幫助殘疾人獲得和使用網路內容。
...
網頁上的資訊可以以多種不同的非文本格式提供，如圖片、視頻、動畫、圖表和圖形。 盲人或視力嚴重受損者無法看到非文本內容。 但是，他們可以通過讓螢幕閱讀器讀取文本內容或通過盲文顯示設備以觸覺形式呈現來訪問文本內容。 因此，通過以圖形格式提供內容的文本替代方案，無法看到內容的人可以訪問內容所提供的資訊的等效版本。

一個有用的附加好處是，文本替代方案使非文本內容能夠通過搜索引擎技術進行索引。

#### 如何見面 — 非文本內容(1.1.1) {#how-to-meet-non-text-content}

對於靜態圖形，基本要求是為圖形提供等效的文本替換。 此方法可在 **備選文本** 的子菜單。 例如，請參閱核心元件 **[影像](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/wcm-components/image.html)**。
...
為實例實施這些版本時，AEM開發團隊必須配置這些元件以支援 `alt` 屬性。 這樣做可確保作者可以將其添加到內容(請參見 [添加對附加HTML元素和屬性的支援](/help/implementing/developing/extending/rte-accessible-content.md#adding-support-for-additional-html-elements-and-attributes))。

AEM要求 **備選文本** 預設情況下要填充的欄位。 如果影像純粹是裝飾性的，並且不需要替代文字， **影像是裝飾性的** 選項。

#### 建立好文本選項 {#creating-good-text-alternatives}

非文本內容有多種形式，因此文本替換的價值取決於圖形在網頁中所扮演的角色。 您可能會發現一些有用的一般規則包括：
...
* 如果非文本內容是指向另一頁或文檔的連結，且同一連結中沒有其他文本形成部分，則影像的替代文本必須指示連結的目標。 它不能描述影像。
* 如果非文本內容包含在按鈕元素中，且同一按鈕沒有文本形成部分，則影像的替代文本必須指示按鈕的功能。 它不能描述影像。
...
* 說明性照片：這些是人、物體或地方的影像。 考慮照片在頁面中的作用以及建議對影像內容的描述非常重要，因為輔助技術會發佈元素類型(例如， `graphic` 或 `image`)。 它可提高使用的清晰度 `screenshot` 或 `illustration` 在替代文字說明中，但這取決於上下文。 一致性是一個重要因素，應為整個創作團隊做出決策，並應用於整個用戶體驗。
...
* 驗證碼：驗證碼是 *完全自動化的公共圖靈test將電腦和人區分開來*。 它是用於網頁上的安全檢查，用於區分人和惡意軟體，但會造成訪問障礙。 這些映像需要用戶描述他們看到的內容，以便通過安全test。 不可能為影像提供文本替代，因此您必須考慮其他非圖形解決方案。 W3C提供了幾條建議。 這些方法各有優缺點。
...
* [W3C對驗證碼的解釋和替代](https://www.w3.org/TR/turingtest/)
...
* 僅音頻和僅視頻（預錄）:對於預錄的純音頻和純視頻媒體，除非音頻或視頻是文本的替代介質，並且明確標籤為：
   * 僅預錄音頻：一種用於基於時間的媒體的替代方法，其提供用於預錄的僅音頻內容的等效資訊。
   * 僅預錄視頻：基於時間的媒體的替代選擇或提供用於預錄制的僅視頻內容的等效資訊的音頻軌道。
...
將多媒體插AEM入網頁與插入影像類似。 然而，由於多媒體內容遠不止於靜止影像，因此有各種不同的設定和選項來控制多媒體的播放方式。
...
聾人或聽力障礙者無法或難以訪問音頻內容。 字幕是在視頻期間的適當時間在螢幕上顯示的口語和非口語音頻的文本等效項。 它們讓聽不到音頻的人理解正在發生的事情。
...
對於隱藏字幕，必須以適當的格式建立並提供同步的字幕檔案(如 [SMIL](https://www.w3.org/AudioVideo/))旁邊(有關如何執行此操作的詳細資訊超出本指南的範圍，但提供了指向以下部分教程的連結： [詳細資訊 — 字幕（預錄）(1.2.2)](#more-information-captions-prerecorded)。 確保在視頻播放器中提供注釋或啟用字幕功能，以便讓用戶知道字幕可用於視頻。
...
如果視頻或動畫中的資訊僅以視覺方式提供，或者如果背景音樂沒有提供足夠的資訊，以便能夠從視覺上理解正在發生的情況，則盲人或視力受損的人會遇到無障礙環境。
...
* **表**:必須使用HTML表元素標識資料表：
   ...
   * 的 `<summary>` 元素通過提供有視力的用戶能夠看到的內容的概要，幫助無視的用戶更輕鬆地理解表中顯示的資訊。 在使用複雜或非常規表佈局時，此工作流非常有用（此屬性不顯示在瀏覽器中，它只讀於輔助技術）。
...
* **複雜資料表**:有時，如果存在具有兩個或更多級別標題的複雜表，則基本表屬性可能不足以提供所有必要的結構資訊。 對於這些類型的複雜表格，需要使用header和 **id屬性在標題及其相關儲存格之間建** 立直 **接** 關係。
...
同樣，需要區分不同聲音（例如，男性或女性口語內容）的資訊對聽力受損者構成無障礙，如果它沒有反映在音頻內容的任何文本替代中。
...
例如，有紅綠色視覺缺陷的人，無法區分綠色和紅色。 他們可能將這兩種顏色都視為第三種顏色（例如，棕色），在這種情況下，他們無法區分紅色、綠色和棕色。
...
如果將顏色用作提供資訊的提示，則應提供額外的視覺提示，如更改樣式（例如，粗體、斜體）或字型。 這有助於視力低或視力不足的人識別資訊。 但是，它不能完全依靠，因為它根本無助於看不到頁面的人。 因此，提供隱藏文本或使用寫程式解決方案(如 [可訪問的富Internet應用程式(ARIA)Web標準套件](https://www.w3.org/WAI/standards-guidelines/aria/)，將此資訊傳達給無視的用戶。
...
在為Web內容選擇適當的字型和調整大小時，在可讀性和可用性方面使用良好的判斷和錯誤。

>[!NOTE]
對以下短語執行Web搜索，以查找可幫助您轉換為其他單位的工具：
* Px到Em計算器 <!--  (https://www.omnicalculator.com/conversion/px-to-em) -->
* 字型大小轉換：像素點 — em-rem百分比 <!-- CAUSES 404 ERROR DESPITE URL BEING CORRECT https://www.websemantics.uk/tools/ -->
* 像素到EM轉換器 <!-- (https://www.w3schools.com/tags/ref_pxtoemconversion.asp) -->


要檢查對比度，請使用顏色對比度工具，如 [Paciello組顏色對比度分析器](https://www.tpgi.com/resources/contrast-analyser.html) 或 [WebAIM顏色對比度檢查器](https://webaim.org/resources/contrastchecker/)。 這些工具允許您檢查顏色對並報告任何對比度問題。

或者，如果您不太關心指定頁面的外觀，則可以選擇不指定背景和前景文本顏色。 無需進行對比度檢查，因為用戶的瀏覽器會確定文本和背景的顏色。

如果無法滿足建議的對比度級別，則必須提供指向頁面的替代等效版本（沒有顏色對比度問題）的連結，或允許用戶根據自己的要求調整頁面顏色方案的對比度。
...
除了遵循 [如何滿足成功標1.4.4](https://www.w3.org/WAI/WCAG21/quickref/#resize-text)，您可以鼓勵內容作者在其頁面設計和字型大小（例如，響應性Web設計）中使用流暢、靈活的寬度和高度，使讀者能夠調整文本大小。
...
如果必須使用文本影像，則使用CSS將文本影像替換為HTML中的等效文本，以便文本能夠以可自定義的方式可用。 有關如何實現此目標的示例，請參見 [C30:使用CSS將文本替換為文本影像並提供用戶介面控制項以切換](https://www.w3.org/TR/2008/NOTE-WCAG20-TECHS-20081211/C30)。
...
本成功標準旨在確保為殘疾用戶提供足夠的時間，盡可能與Web內容進行交互。 失明、低視力、靈巧性缺陷和認知限制等殘疾人可能需要更多時間閱讀內容或執行填寫線上表格等功能。 如果Web函式與時間相關，則某些用戶在出現時間限制之前很難執行所需的操作。 這可能使服務無法訪問。 設計不依賴於時間的功能有助於殘疾人成功完成這些功能。 提供選項以禁用時間限制、自定義時間限制長度或請求在時間限制發生之前的更長時間，可幫助那些需要比預期時間更長的用戶成功完成任務。 這些選項按對用戶最有幫助的順序列出。 禁用時間限制比自定義時間限制的長度要好，這比在時間限制出現之前請求更多時間要好。
...
* 暫停、停止、隱藏：對於移動、閃爍、滾動或自動更新資訊，以下是真的：
   ...
   * 自動更新：對於(a)自動啟動和(b)與其他內容並行顯示的任何自動更新資訊，用戶有暫停、停止或隱藏更新或控制更新頻率的機制，除非自動更新是活動中必不可少的一部分。
...
此成功標準的目的是確保當用戶按順序瀏覽內容時，他們會按與內容含義一致的順序遇到資訊，並且可以從鍵盤操作。 這通過讓用戶構建內容的一致心理模型來減少混淆。 可能有不同的順序反映內容中的邏輯關係。 例如，在由多個欄位和/或步驟組成的聯機表單中移動元件反映內容中的邏輯關係。
...
對所有用戶來說，無論有何損害，通過適當的連結文本明確指示連結的方向至關重要。 這有助於用戶確定他們是否真的想要遵循連結。 對於有目光的用戶，當頁面上有多個連結時（尤其是當頁面文本較重時），有意義的連結文本非常有用，因為有意義的連結文本可以更清楚地顯示目標頁面的功能。 一些輔助技術的用戶可以在一個頁面上生成所有連結的清單，如果連結文本既獨特又資訊豐富，則可以更容易地從上下文中理解連結文本。 然而，如果一個連結無法提供足夠的資訊來準確描述連結將他們帶到何處，那麼有視力的認知障礙個體可能會變得困惑。
...
雖然建議您提供可標識連結目的的連結文本，而不需要其他上下文，但是可以認識到這並不總是可能。 上下文自由連結可用於以下情況，其HTML示例可在 [如何滿足成功標2.4.4](https://www.w3.org/WAI/WCAG21/quickref/#link-purpose-in-context)。
...
* 成功標準2.4.7
* AA級
* 焦點可見：任何可鍵盤操作的用戶介面具有操作模式，其中鍵盤焦點指示器是可見的。

#### 目的 — 焦點可見(2.4.7) {#purpose-focus-visible}

此成功標準的目的是幫助人們知道哪個元素具有鍵盤焦點。

人必須能夠知道多個元素中的哪個元素具有鍵盤焦點。 如果螢幕上只有一個鍵盤可操作的控制項，則會滿足成功標準，因為可視設計只顯示一個鍵盤可操作的項。
...
焦點可通過鍵盤（例如，與控制項對應）或滑鼠（例如，選擇文本欄位）移動到控制項。 將滑鼠移到控制項上不會移動焦點，除非指令碼實現此行為。 對於某些類型的控制項，選擇控制項也可激活控制項（例如，按鈕），這反過來可以啟動上下文中的更改。
...
如果網頁上有兩個元件，它們的功能與一組網頁中另一個頁面上的元件相同，則所有這三個元件必須一致。 因此，同一頁上的兩者是一致的。
...
此成功標準的目的是確保用戶知道發生了錯誤，並能夠確定錯誤。 錯誤消息應盡可能具體。 如果提交表單失敗，則重新顯示表單並指示出錯的欄位對於某些用戶來說是不夠的，他們無法察覺到發生了錯誤。 例如，螢幕閱讀器用戶在遇到一個指示器之前不知道是否出現錯誤。 他們可能會在遇到錯誤指示器之前完全放棄表格，認為頁面根本不起作用。 根據WCAG中的定義， [輸入錯誤](https://www.w3.org/TR/WCAG/#dfn-input-error) 是用戶提供的未接受的資訊。 這包括：

網頁需要但用戶省略的資訊，或用戶提供但不屬於所需資料格式或允許值的資訊。
例如：

* 用戶未能在state、省、地區等欄位中輸入相應的縮寫；
* 用戶輸入的州/省/自治區縮寫無效；
* 用戶輸入不存在的郵遞區號；
* 用戶將來輸入出生日期2年；
...
在AEMWKND演示項目中，添加表單元件時會添加預設標籤， **文本欄位**&#x200B;的雙曲餘切值。 此預設標題取決於元件類型。 您可以在 **標題和文本** 對話框。 必須確保標籤有助於用戶瞭解與每個表單元件關聯的資料。
...
成功標準3.3.1提供錯誤通知。 但是，具有認知限制的人可能很難理解如何糾正錯誤。 視覺殘疾者可能無法確切地找出如何糾正錯誤。 如果提交表單失敗，則用戶可能會放棄該表單，因為他們可能不知道如何更正錯誤，即使他們知道錯誤已經發生。
...
由於修復技術在用戶代理中不同，因此作者不能假設內容被準確地解析為資料結構或由包括輔助技術在內的專門用戶代理正確呈現，除非內容是根據該技術的正式語法中定義的規則建立的。 在標籤語言中，元素和屬性語法中的錯誤以及未能提供正確嵌套的開始/結束標籤會導致錯誤，從而阻止用戶代理可靠地分析內容。 因此，成功標準要求只能使用形式語法的規則來分析內容。
...
用戶介面控制項的一個重要狀態是它是否具有焦點。 控制的焦點狀態可以寫程式地確定，並且關於焦點改變的通知被發送到用戶代理和輔助技術。 用戶介面控制狀態的其它示例包括是否選中了複選框或單選按鈕，或是展開或折疊了可折疊的樹或清單節點。
