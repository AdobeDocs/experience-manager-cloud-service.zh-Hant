...
* **標題與標籤**
   ...
   * **標題**  — 頁面標題會顯示在各種位置中。 例如， **網站** 標籤清單和 **網站** 卡片/清單檢視。
      * 這是必要欄位。
...
* **更多標題和說明**
   ...
   * **頁面標題**  — 要在頁面上使用的標題。 通常用於標題元件。 如果空白 **標題** 中指定的規則。
   * **導覽標題**  — 您可以指定個別標題以用於導覽（例如，如果您想要更精簡的標題）。 如果空白，則 **標題** 中指定的規則。
   * **副標題**  — 用於頁面的副標題。
   * **說明**  — 您對頁面的說明、其用途，或您要新增的任何其他詳細資訊。

* **開啟/關閉時間**

   >[!NOTE]
   >
   > 請參閱 [開啟和關閉時間 — 觸發配置](/help/operations/replication.md#on-and-off-times-trigger-configuration) 以了解如何配置相關自動複製的詳細資訊。

   >[!NOTE]
   >若 **準時** 或 **關閉時間** 過去，且已設定自動復寫，則會立即觸發相關動作。

   * **準時**  — 發佈環境中顯示（呈現）已發佈頁面的日期和時間。 必須手動或預先設定的自動復寫來發佈頁面。

      * 如果已 [已發佈（手動）](/help/sites-cloud/authoring/fundamentals/publishing-pages.md) 此頁面將保持休眠（隱藏），直到在指定時間呈現。
      * 如果未發佈並已針對自動復寫進行設定，則頁面將在指定時間自動發佈並呈現。
      * 如果未發佈且未針對自動復寫進行設定，則不會自動發佈頁面，因此在嘗試存取頁面時，會看到404。
   * **關閉時間**  — 類似並經常搭配使用 **準時**，這會定義發佈環境中隱藏已發佈頁面的時間。

   * 保留這些欄位(**準時** 和 **關閉時間**)空白，以取得您要立即發佈的頁面，並可在發佈環境中使用，直到停用為止（一般情況）。


* **虛名 URL**

   * 可讓您輸入此頁面的虛名URL，以便您擁有較短和/或更具表達力的URL。
   * 例如，如果虛名URL設為 `welcome` 到路徑所識別的頁面 `/v1.0/startpage` 網站 `http://example.com`，然後 `http://example.com/welcome` 會是 `http://example.com/content/v1.0/startpage`
...
* **設定**
   ...
   * **語言**  — 頁面語言
   * **語言根**  — 如果頁面是語言副本的根，則必須勾選此選項
   * **重新導向**  — 指出此頁面應自動以HTML重新導向的頁面 `302 Found` 狀態。
      * **永久重新導向**  — 若勾選此選項，頁面會重新導向至隨HTML提供的目標路徑 `301 Moved Permanently` 狀態。
...
* **範本設定**
   ...
   * **允許的範本** - [定義將可用的範本清單](/help/sites-cloud/authoring/features/templates.md#enabling-and-allowing-a-template-template-author) 在該子分支內
...
* **驗證需求**
   ...
   * **啟用**  — 啟用驗證以存取頁面
      ...
      >[!NOTE]
      >
      >頁面的封閉使用者群組定義於 **[權限](#permissions)** 標籤。
...
* **目前的即時副本**  — 列出以此Blueprint頁面為基礎（即為的即時副本）的頁面
...
* **設定**
   ...
   * **即時副本繼承**  — 如果勾選此選項，Live Copy設定對所有子項都有效
   * **從父項繼承轉出配置**  — 若勾選，轉出設定會繼承自頁面的上層
   * **選擇轉出設定**  — 定義修改將從Blueprint傳播的情況，且僅可在 **從父項繼承轉出配置** 未選取
