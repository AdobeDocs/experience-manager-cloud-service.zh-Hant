...
* **設定**
   ...
   * **別名**  — 指定要與此頁一起使用的別名
      ...
      * 有關詳細資訊，請參閱 [SEO和URL管理最佳實踐下的本地化頁名](/help/overview/seo-and-url-management.md#localized-page-names)。

* **設定**

   * **繼承自 &lt;path>**  — 啟用/禁用繼承；切換可用性 **雲配置** 選擇
...
* **SEO**
   ...
   * **規範URL**  — 可用於覆蓋頁面的規範Url;如果將頁面的URL留空，則該URL將是其規範URL

   * **機器人標籤**  — 選擇robots標籤以控制搜索引擎爬網程式的行為。

      >[!NOTE]
      >
      >有些選擇相互衝突。 在發生衝突時，優先選擇更寬容的選項。

   * **生成站點地圖**  — 選中後，將為此頁面及其子體生成sitemap.xml

### 影像 {#images}

* **精選影像**

   選擇並配置要顯示的影像。 這用於引用該頁面的元件；例如，預告器、頁面清單等。

   * **影像**

      你可以 **選擇** 或瀏覽要上載的檔案，然後 **編輯**&#x200B;或 **清除**。

   * **備選文本**  — 用於表示影像含義和/或功能的文本；例如，供螢幕閱讀器使用。

   * **繼承 — 從DAM資產獲取的值**  — 選中後，將用 `dc:description`DAM中的元資料
...
* **ContextHub 組態**
   ...
   * **繼承自 &lt;path>**  — 啟用/禁用繼承；切換可用性 **上下文集線器路徑** 和 **段路徑** 選擇
...
此頁籤僅對用作藍圖的頁面可見。 藍圖是即時拷貝的基礎，是 [多站點管理。](/help/sites-cloud/administering/msm/overview.md)
...
此頁籤僅對配置為即時副本的頁面可見。 與藍圖一樣，即時拷貝是 [多站點管理。](/help/sites-cloud/administering/msm/overview.md)。
...
>[!NOTE]
>
>有關詳細資訊，請參閱 [啟用漸進式Web App功能](/help/sites-cloud/authoring/features/enable-pwa.md)。

* **設定可安裝體驗**

   * **啟用PWA**  — 啟用/禁用功能；允許用戶將站點作為PWA安裝
   * **啟動URL**  — 首選啟動URL
   * **顯示模式**  — 瀏覽器應如何隱藏或以其他方式呈現給本地設備上的用戶
   * **螢幕方向** -PWA如何處理設備方向
   * **主題顏色**  — 影響本地用戶作業系統顯示本機UI工具欄和導航控制項的應用的顏色
   * **背景顏色**  — 應用的背景顏色，顯示為應用載入
   * **表徵圖**  — 表示用戶設備上的應用的表徵圖

* **快取管理 (進階)**

   * **快取策略和內容刷新頻率**  — 定義您的PWA的快取模型
   * **要快取以供離線使用的檔案**
      * **檔案預快取（技術預覽）**  — 在安裝服務工AEM作人員並使用服務工作人員之前，將將托管在上的檔案保存到本地瀏覽器快取中
      * **客戶端庫**  — 要快取的客戶端庫，以獲得離線體驗
      * **路徑包含**  — 根據配置的快取策略和內容刷新頻率截取定義路徑的網路請求並返回快取內容
      * **路徑排除**  — 無論檔案預快取和路徑包含項下的設定如何，這些檔案將永遠不會被快取
