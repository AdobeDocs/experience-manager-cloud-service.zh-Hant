...
* **激活/停用**
   ...
   * 這些術語在以前版本中使AEM用。
* **複製/複製**
   * 這些技術術語描述了在發佈頁面時資料從一個環境到另一個環境的移動（例如，頁面內容、檔案、代碼、用戶注釋）。
   * 這些術語主要供開發商使用。
...
取消發佈頁面將從發佈中刪除，或 [預覽](/help/sites-cloud/authoring/fundamentals/previewing-content.md)，這樣您的讀者就無法再使用它。

在 [與出版類似](#publishing-pages)，可從所需目標取消發佈一個或多個頁面：
...
1. 「管 **理出版物** 」嚮導將啟動。在第一個步驟中， **選項**，選擇「取消發佈」(Unpublish **)，而非「發佈」(Publish)的預設** 選項 ****。
   ...
   >[!NOTE]
   >如果你有 [預覽](/help/sites-cloud/authoring/fundamentals/previewing-content.md) 環境 **目標** 管理發布期間。
