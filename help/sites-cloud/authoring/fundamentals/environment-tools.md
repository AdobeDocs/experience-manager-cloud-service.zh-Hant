...
* 此 [**頁面資訊**](/help/sites-cloud/authoring/fundamentals/page-properties.md#page-properties) 選擇器，其中會顯示簡介幻燈片（在您第一次存取編輯器時顯示）
* 此 [設定](/help/sites-cloud/authoring/fundamentals/editing-content.md#component-toolbar) 特定元件的對話方塊(使用？ 圖示)，其中顯示相關內容說明
...
如果您的頁面包含內容片段，那麼您也可以存取 [相關內容的瀏覽器](/help/sites-cloud/authoring/fundamentals/content-fragments.md#using-associated-content).
...
* [即時副本](/help/sites-cloud/administering/msm/overview.md) 顯示基於所選資源的所有即時副本的路徑。
...
適當的資源(例如，頁面來自 **網站** 控制檯或資產 **資產** console) [時間軸可用來顯示任何選取專案的最近活動](/help/sites-cloud/authoring/getting-started/basic-handling.md#timeline).

開啟所需的主控台，然後導覽至所需的資源並開啟 **時間表**，使用：

![時間軸選項](/help/sites-cloud/authoring/assets/timeline.png)

[選取您需要的資源](/help/sites-cloud/authoring/getting-started/basic-handling.md#viewing-and-selecting-resources)，然後執行以下任一項作業 **全部顯示** 或 **活動** 若要列出所選資源上任何最近的動作：
...
* [提升啟動](/help/sites-cloud/authoring/launches/promoting.md) （僅當頁面為啟動項時）
...
* [版面](/help/sites-cloud/authoring/features/responsive-layout.md)  — 可讓您根據裝置建立及編輯回應式配置（如果頁面根據配置容器）
...
按一下 **開啟選取範圍對話方塊** 路徑欄位中的按鈕會開啟 [路徑瀏覽器](#path-browser) 對話方塊，以取得更詳細的選取選項。
...
路徑瀏覽器的組織方式如下 [欄檢視](/help/sites-cloud/authoring/getting-started/basic-handling.md#column-view) ，以取得更詳細的資源選擇。
