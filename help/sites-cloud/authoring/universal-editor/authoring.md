...
依您的後端而定，預設可用的選項可能有所不同。 RTE可設定為根據作者需求隱藏選項或顯示其他選項。 如需詳細資訊，請參閱檔案[為通用編輯器設定RTE](/help/implementing/universal-editor/configure-rte.md)。
...
1. 元件選取器對話方塊隨即開啟。
   ...
   * 按一下選擇器外部以取消元件插入。
   ...
   ![元件選擇器](assets/component-picker.png)
...
1. 以滑鼠右鍵按一下元件，或按一下所選元件的徽章，以開啟[內容功能表。](#context-menu)
1. 選取所需的移動選項。
   * 移至頂端
   * 上移
   * 下移
   * 移至底部
     ![在內容功能表中移動選項](assets/move-options-in-conext-menu.png)

元件會在編輯器和內容樹中移動。

>[!TIP]
>
>使用快速鍵`Command-U`或`Shift-Command-U`分別向上或向上移動。
>使用快速鍵`Command-J`或`Shift-Command-J`分別向下移動或移至底部。
