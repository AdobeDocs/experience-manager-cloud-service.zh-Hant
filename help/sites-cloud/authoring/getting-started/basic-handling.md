...
本檔案旨在概述使用AEM製作環境時的基本處理方式。 它會使用 **網站** 控制台作為基礎。

>[!NOTE]
>
>* 並非所有主控台都提供某些功能，某些主控台可能提供其他功能。 有關個別主控台及其相關功能的特定資訊，將在其他頁面上詳細說明。
>* AEM提供鍵盤快速鍵。 尤其是 [使用主控台](/help/sites-cloud/authoring/getting-started/keyboard-shortcuts.md) 和 [編輯頁面](/help/sites-cloud/authoring/fundamentals/keyboard-shortcuts.md).
...
為了充分了解AEM的基本使用，本檔案以 **網站** 控制台。 按一下或點選 **網站** 開始使用。
...
* **[導覽](#navigation-panel)**  — 在您登入AEM時，以羅盤和預設面板表示
...
| 主控台 | 用途 |
|---|---|
| 專案 | 專案主控台可讓您直接存取您的專案。 [專案是虛擬控制面板](/help/sites-cloud/authoring/projects/overview.md) 可用來組建團隊。 然後，您可以讓該團隊存取資源、工作流程和任務，讓人員能夠朝著共同目標努力。 |
| Sites | 網站主控台可讓您 [建立、檢視和管理網站](/help/sites-cloud/authoring/fundamentals/organizing-pages.md) 在AEM執行個體上執行。 您可以透過此主控台建立、編輯、複製、移動和刪除頁面、啟動工作流程及發佈頁面。 |
| 體驗片段 | 安 [體驗片段](/help/sites-cloud/authoring/fundamentals/experience-fragments.md) 是獨立體驗，可跨管道重複使用，且有變數，可避免重複複製和貼上體驗或體驗的部分所帶來的麻煩。 |
| Assets | Assets主控台可讓您匯入和管理 [影像、視訊、檔案和音訊檔案等數位資產](/help/assets/overview.md). 然後，同一AEM執行個體上執行的任何網站都可以使用這些資產。 您也可以建立和管理 [內容片段](/help/assets/content-fragments/content-fragments.md) 從「資產」主控台。 |
| 個人化 | 此主控台提供的工具架構 [製作目標式內容並呈現個人化體驗。](/help/sites-cloud/authoring/personalization/overview.md) |
| 內容片段 | [內容片段](/help/sites-cloud/administering/content-fragments/content-fragments.md) 可讓您設計、建立、組織和發佈頁面無關的內容。 它們可讓您準備結構化內容，以便在多個位置/多個頻道中使用，非常適合於頁面編寫和無頭傳送。 |
...
![導覽標題](/help/sites-cloud/authoring/assets/navigation-bar.png)
...
* [搜尋](/help/sites-cloud/authoring/getting-started/search.md)
   ...
   您也可以使用 [快捷鍵](/help/sites-cloud/authoring/getting-started/keyboard-shortcuts.md) `/` （正斜線），從任何主控台叫用搜尋。
...
* [邊欄選取器](#rail-selector)
   ...
   ![邊欄選取器按鈕](/help/sites-cloud/authoring/assets/rail-selector.png)

   顯示的選項取決於您目前的主控台。 例如，在 **網站** 您只能選取內容（預設）、時間軸、參照或篩選端面板。

   ![邊欄選取器範例](/help/sites-cloud/authoring/assets/rail-selector-example.png)

* 階層連結

   ![導覽列中的階層連結](/help/sites-cloud/authoring/assets/breadcrumbs-navigation.png)

   瀏覽路徑標示位於邊欄的中間，一律會顯示目前所選項目的說明，可讓您在特定主控台中導覽。 在 **網站** 控制台，您可以瀏覽您網站的各個層級。
...
* 鍵盤導覽
   ...
   您只能使用鍵盤來導覽網站。 這會使用 **標籤** 鍵(或 **選擇+標籤**)，可在頁面上可聚焦的元素之間移動。

   在 **網站** 主控台有新增的選項 **跳到主要內容**. 當您在標題選項中切換索引標籤時，就會顯示這個選項，並可讓您略過（產品）工具列中的標準元素，直接前往主要內容，加快導覽速度。
...
* **導覽**
   ...
   第一次瀏覽系統時， [一系列幻燈片介紹AEM導航](#product-navigation).
...
* **頁面編輯器**
   ...
   如您所需導覽此概觀 [產品導覽概觀](#product-navigation) 首次存取任何主控台時。

   從 [**頁面資訊** 您可以選擇的功能表 **說明**](/help/sites-cloud/authoring/fundamentals/environment-tools.md#accessing-help) 以隨時再次顯示。
...
您 [選擇資源](#viewing-and-selecting-resources) 取決於視圖。
...
在 [卡片檢視](#card-view) 某些動作可作為快速動作圖示，也可在工具列上使用。 一次只有一個項目可使用快速操作表徵圖，無需預選。
...
| 檢視 | 選取觸控 | 選擇案頭 | 取消選取觸控 | 取消選擇案頭 |
|---|---|---|---|---|
| 欄 | 點選縮圖 | 按一下縮圖 | 點選縮圖 | 按一下縮圖 |
| 卡片 | 點選並按住卡片 | 將滑鼠移過，然後使用複選標籤快速操作 | 點選卡片 | 按一下資訊卡 |
...
您可以透過下列方式取消選取所有項目並退出選取模式：

* 按一下或點選 **X** 在計數旁邊
* 使用 **逃逸** key

![取消選取全部](/help/sites-cloud/authoring/assets/deselect-all.png)
...
1. 例如，在卡片檢視中：
   ...
   ![卡片視圖選擇](/help/sites-cloud/authoring/assets/card-view-select.png)
...
* 卡片視圖顯示當前級別上每個項目的資訊卡。 這些功能提供下列資訊：
   ...
   * 如果適用，需在工作流程中採取動作時
      * 表示所需操作的標籤可能與 [收件匣](/help/sites-cloud/authoring/getting-started/inbox.md).

* [快速動作](#quick-actions) 也可在此檢視中使用，例如選取項目和常用動作，例如編輯。

   ![快速動作](/help/sites-cloud/authoring/assets/quick-actions.png)

* 您可以點選/按一下資訊卡（小心以避免快速動作），或使用 [標題中的階層連結](#the-header).
...
* 清單視圖列出當前級別上每個資源的資訊。
* 您可以點選/按一下資源名稱，然後使用 [標題中的階層連結](#the-header).
* 若要輕鬆選取清單中的所有項目，請使用清單左上角的核取方塊。

   ![清單視圖選擇全部](/help/sites-cloud/authoring/assets/list-view-select-all.png)
...
![欄位選取器已擴充](/help/sites-cloud/authoring/assets/rail-selector-expanded.png)

例如，在 **網站** Console您只能選取內容（預設值）、內容樹狀結構、時間軸、參考、網站詳細資料，或篩選端面板。

如果僅選取內容，則只會顯示邊欄圖示。 選取任何其他選項時，選項名稱會出現在邊欄圖示旁。

>[!NOTE]
>
>[鍵盤快速鍵](/help/sites-cloud/authoring/getting-started/keyboard-shortcuts.md) 可快速切換邊欄顯示選項。
...
>
>請參閱 [鍵盤快捷鍵](/help/sites-cloud/authoring/getting-started/keyboard-shortcuts.md) 以取得更多資訊。
...
* 檢視與選取項目相關的各種事件。
   ...
   * 可從下拉式清單中選取事件類型：
      ...
      * [工作流程](/help/sites-cloud/authoring/workflows/overview.md)
         * 除了暫時性工作流程，因為不會儲存這些工作流程的歷史記錄資訊 <!--With the exception of [transient workflows](/help/sites-developing/workflows.md#transient-workflows) as no history information is saved for these-->
      * 顯示全部
...
**參考** 顯示與所選資源的任何連接。 例如，在 **網站** 主控台 [參照](/help/sites-cloud/authoring/fundamentals/environment-tools.md#references) 頁面顯示：
...
* 內容參考：
   ...
   * 從其他頁面連結至所選頁面
   * 由參考元件從選取的頁面借入和/或借出的內容

![參考範例](/help/sites-cloud/authoring/assets/references-example.png)

### 網站 {#site}

**網站** 顯示網站的詳細資訊 [使用網站範本建立。](/help/sites-cloud/administering/site-creation/create-site.md)

![網站邊欄](../assets/site-rail.png)

請參閱檔案 [使用網站邊欄管理網站主題](/help/sites-cloud/administering/site-creation/site-rail.md) 如需如何使用邊欄來管理的詳細資訊 [主題。](/help/sites-cloud/administering/site-creation/site-themes.md)

>[!TIP]
>
>從範本建立新網站及自訂其主題的程式的端對端說明，位於 [快速網站建立歷程。](/help/journey-sites/quick-site/overview.md)

### 篩選 {#filter}

這會開啟類似以下的面板： [搜尋](/help/sites-cloud/authoring/getting-started/search.md) 已設定適當的位置篩選器，可讓您進一步篩選您要檢視的內容。

![篩選範例](/help/sites-cloud/authoring/assets/filter.png)
