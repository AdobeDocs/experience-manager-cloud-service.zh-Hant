...
本檔案旨在概述使用AEM製作環境時的基本處理方式。 它以&#x200B;**Sites**&#x200B;控制台為基礎。

>[!NOTE]
>
>* 並非所有主控台都提供某些功能，某些主控台可能提供其他功能。 有關個別主控台及其相關功能的特定資訊，將在其他頁面上詳細說明。
>* AEM提供鍵盤快速鍵。 尤其是當[使用主控台](/help/sites-cloud/authoring/getting-started/keyboard-shortcuts.md)和[編輯頁面](/help/sites-cloud/authoring/fundamentals/keyboard-shortcuts.md)時。
...
為了充分了解AEM的基本使用，本檔案以&#x200B;**Sites**&#x200B;主控台為基礎。 按一下或點選&#x200B;**Sites**&#x200B;以開始使用。
...
* [邊欄選取器](#rail-selector)
   ...
   顯示的選項取決於您目前的主控台。 例如，在&#x200B;**Sites**&#x200B;中，您只能選取內容（預設值）、時間軸、參照或篩選器端面板。
...
* 階層連結
   ...
   瀏覽路徑標示位於邊欄的中間，一律會顯示目前所選項目的說明，可讓您在特定主控台中導覽。 在&#x200B;**Sites**&#x200B;主控台中，您可以瀏覽網站的層級。
...
* 鍵盤導覽
   ...
   您只能使用鍵盤來導覽網站。 這會使用&#x200B;**TAB**&#x200B;索引鍵（或&#x200B;**OPT+TAB**）的標準瀏覽器功能，在可聚焦的頁面上的元素之間移動您。

   在&#x200B;**Sites**&#x200B;主控台中，新增了&#x200B;**跳至主要內容**&#x200B;的選項。 當您在標題選項中切換索引標籤時，就會顯示這個選項，並可讓您略過（產品）工具列中的標準元素，直接前往主要內容，加快導覽速度。
...
* **導覽**
   ...
   第一次導航系統時， [一系列幻燈片將介紹AEM導航](#product-navigation)。
...
* **頁面編輯器**
   ...
   如同首次存取任何主控台時[產品導覽概述](#product-navigation)，請導覽此概述。

   從&#x200B;[**頁面資訊**&#x200B;功能表中，您可以隨時選擇&#x200B;**Help**](/help/sites-cloud/authoring/fundamentals/environment-tools.md#accessing-help)&#x200B;以再次顯示此資訊。
...
如何選擇資源](#viewing-and-selecting-resources)取決於該視圖。[
...
在[卡片檢視](#card-view)中，某些動作既可作為快速動作圖示，也可作為工具列使用。 一次只有一個項目可使用快速操作表徵圖，無需預選。
...
1. 在您選擇了資源後，[actions工具欄](#actions-toolbar)將覆蓋頂部標題，該工具欄提供對當前適用於所選資源的操作的訪問。
...
* 卡片視圖顯示當前級別上每個項目的資訊卡。 這些功能提供下列資訊：
   ...
   * 如果適用，需要您在工作流程中採取動作時
      * 指示所需操作的標籤可能與[收件箱](/help/sites-cloud/authoring/getting-started/inbox.md)中的條目相關。

* [此檢](#quick-actions) 視中也提供快速動作，例如選取和編輯等常見動作。

   ![快速動作](/help/sites-cloud/authoring/assets/quick-actions.png)

* 您可以點選/按一下資訊卡（注意以避免快速動作），或使用標題](#the-header)中的[導覽路徑標示，以向下導覽樹狀結構。
...
* 清單視圖列出當前級別上每個資源的資訊。
* 您可以點選/按一下資源名稱，然後使用標題](#the-header)中的[階層連結來向下導覽樹狀結構。
...
例如，在&#x200B;**Sites**&#x200B;中，您只能選取內容（預設值）、內容樹、時間軸、參照或篩選端面板。

如果僅選取內容，則只會顯示邊欄圖示。 選取任何其他選項時，選項名稱會出現在邊欄圖示旁。

>[!NOTE]
[鍵盤](/help/sites-cloud/authoring/getting-started/keyboard-shortcuts.md) 快速鍵可快速切換邊欄顯示選項。
...
* 檢視與選取項目相關的各種事件。
   ...
   * 可從下拉式清單中選取事件類型：
      ...
      * [工作流程](/help/sites-cloud/authoring/workflows/overview.md)
         * 除了暫時性工作流，因為沒有為這些<!--With the exception of [transient workflows](/help/sites-developing/workflows.md#transient-workflows) as no history information is saved for these-->保存歷史資訊
      * 顯示全部
...
**** 參考會顯示與所選資源的任何連線。例如，在&#x200B;**Sites**&#x200B;控制台[參考](/help/sites-cloud/authoring/fundamentals/environment-tools.md#references)的頁面中，會顯示：
...
這會開啟類似[search](/help/sites-cloud/authoring/getting-started/search.md)的面板，其中已設定適當的位置篩選器，讓您能夠進一步篩選您要檢視的內容。
