...
本檔案旨在提供使用AEM作者環境時基本處理的概觀。 它以&#x200B;**Sites**&#x200B;控制台為基礎。

>[!NOTE]
>
>* 有些功能並非在所有控制台中都可用，有些控制台中可能還提供其他功能。 有關各個控制台及其相關功能的具體資訊將在其他頁面上詳細介紹。
>* AEM提供鍵盤快速鍵。 尤其當[使用控制台](/help/sites-cloud/authoring/getting-started/keyboard-shortcuts.md)和[編輯頁面](/help/sites-cloud/authoring/fundamentals/keyboard-shortcuts.md)時。


## 啟用觸控的UI {#a-touch-enabled-ui}

AEM的使用者介面已啟用觸控功能。 觸控式介面可讓您使用觸控功能，透過點選、觸控和滑動等手勢與軟體互動。 由於AEM UI已啟用觸控功能，因此您可以在您的觸控裝置（例如行動電話或平板電腦）上使用觸控手勢。 不過，傳統桌上型裝置上也提供滑鼠動作，讓您有彈性選擇如何製作內容。
...
為了充份瞭解AEM的基本使用，本檔案以&#x200B;**Sites**&#x200B;主控台為基礎。 按一下或點選&#x200B;**Sites**&#x200B;以開始使用。
...
除非您檢視所有投影片或勾選「不要再顯示此項目」選項&#x200B;**，否則下次您存取主控台時，概述會重新開始。**
...
| 主控台 | 目的 |
|---|---|
| 專案 | 專案主控台可讓您直接存取專案。 [項目是虛擬](/help/sites-cloud/authoring/projects/overview.md) 儀表板，可用於構建團隊。然後，您就可讓該團隊存取資源、工作流程和工作，讓人們朝共同目標努力。 |
| 網站 | Sites控制台可讓您[建立、檢視和管理在AEM例項上執行的網站](/help/sites-cloud/authoring/fundamentals/organizing-pages.md)。 透過此主控台，您可以建立、編輯、複製、移動和刪除頁面、啟動工作流程和發佈頁面。 |
| 體驗片段 | [體驗片段](/help/sites-cloud/authoring/fundamentals/experience-fragments.md)是可跨通道重複使用的獨立體驗，並有各種變化，省去了重複複製和貼上體驗或體驗部分的麻煩。 |
| 資產 | 「資產」主控台可讓您匯入及管理數位資產，例如影像、視訊、檔案和音訊檔案。 然後，在相同AEM例項上執行的任何網站都可以使用這些資產。<!--add some kind of assets link--> |
| 個性化 | 此主控台提供工具架構，以製作目標內容並呈現個人化體驗。[](/help/sites-cloud/authoring/personalization/overview.md) |
...
* [軌道選擇器](#rail-selector)
   ...
   ![軌道選擇器範例](/help/sites-cloud/authoring/assets/rail-selector-example.png)

* 階層連結

   ![導覽列中的階層連結](/help/sites-cloud/authoring/assets/breadcrumbs-navigation.png)

   導覽路徑標示位於導覽軌的中間，並一律顯示目前選取項目的說明，可讓您在特定主控台中導覽。 在&#x200B;**Sites**&#x200B;主控台中，您可以瀏覽網站的各個層級。

   只要按一下階層連結文字，就會顯示下拉式清單，列出目前選取項目的階層層級。 按一下項目可跳至該位置。

   ![擴充的階層連結範例](/help/sites-cloud/authoring/assets/breadcrumbs-example.png)
...
* [檢視](#viewing-and-selecting-resources)
   ...
   您可以在欄檢視、卡片檢視和清單檢視之間切換。 在清單檢視中，也會顯示檢視設定。
...
* 鍵盤導覽
   ...
   您只能使用鍵盤來導覽網站。 這會使用&#x200B;**TAB**&#x200B;鍵（或&#x200B;**OPT+TAB**）的標準瀏覽器功能，在可聚焦頁面的元素之間移動。

   在&#x200B;**Sites**&#x200B;控制台中，「跳至主內容」(a3/>)中添加了選項。 ****&#x200B;當您在頁首選項中切換時，這個選項就會顯示，並可讓您略過（產品）工具列中的標準元素，直接將您帶往主要內容，以加速導覽。
...
* **導覽**
   ...
   第一次導覽系統時，[系列投影片會介紹AEM navigation](#product-navigation)。
...
* **頁面編輯器**
   ...
   首次存取任何控制台時，請像瀏覽[產品導覽概述](#product-navigation)一樣導覽此概述。

   從&#x200B;[**頁面資訊**&#x200B;功能表，您可以選擇&#x200B;**說明**](/help/sites-cloud/authoring/fundamentals/environment-tools.md#accessing-help)，隨時再次顯示。
...
* 當前控制台
* 目前的內容
* 是否處於[選擇模式](#viewing-and-selecting-resources)

工具列中可用的動作會變更，以反映您可對選取的特定項目所執行的動作。

[如何選擇資源](#viewing-and-selecting-resources)取決於視圖。
...
在[卡片檢視](#card-view)中，某些動作可當成快速動作圖示，也可在工具列上使用。 快速動作圖示可同時用於單一項目，讓您不需要預先選取。
...
| 檢視 | 選取觸控 | 選擇案頭 | 取消選取「觸控」 | 取消選擇案頭 |
|---|---|---|---|---|
| 欄 | 點選縮圖 | 按一下縮圖 | 點選縮圖 | 按一下縮圖 |
| 卡片 | 點選並按住卡片 | 將滑鼠移到上方，然後使用核取標籤快速動作 | 點選資訊卡 | 按一下資訊卡 |
...
在所有檢視中，如果您使用案頭裝置，則可點選鍵盤上的逸出，以取消選取所有項目。
...
1. 選擇資源後，[操作工具欄](#actions-toolbar)將覆蓋頂部標題，該工具欄提供對當前適用於所選資源的操作的訪問。
...
* 卡片檢視會顯示目前層級上每個項目的資訊卡。 這些提供的資訊包括：
   ...
   * 如果適用，當您需要在工作流程中採取動作時
      * 表示必要操作的標籤可能與[收件箱](/help/sites-cloud/authoring/getting-started/inbox.md)中的條目相關。

* [此檢](#quick-actions) 視中也提供快速動作，例如選擇和常見動作，例如編輯。

   ![快速動作](/help/sites-cloud/authoring/assets/quick-actions.png)

* 您可以點選／按一下資訊卡（小心以避免快速動作），或使用標題](#the-header)中的[階層連結，再次瀏覽樹狀結構。
...
* 清單視圖列出了當前級別上每個資源的資訊。
* 您可以點選／按一下資源名稱，然後使用標題](#the-header)中的[階層連結，在樹狀結構中向下導覽。
* 若要輕鬆選取清單中的所有項目，請使用清單左上角的核取方塊。

   ![清單檢視選擇所有](/help/sites-cloud/authoring/assets/list-view-select-all.png)
...
![導軌選擇器已擴充](/help/sites-cloud/authoring/assets/rail-selector-expanded.png)

例如，在&#x200B;**Sites**&#x200B;中，您只能選取內容（預設值）、內容樹狀結構、時間軸、參照或篩選側面板。

如果僅選取內容，則只會顯示邊欄圖示。 當選取任何其他選項時，選項名稱會出現在邊欄圖示旁。

>[!NOTE]
>
>[有鍵盤](/help/sites-cloud/authoring/getting-started/keyboard-shortcuts.md) 快速鍵可快速切換滑軌顯示選項。
...
時間軸可用於查看和／或啟動在所選資源上發生的事件。 若要開啟時間軸欄，請使用邊欄選擇器：
...
* 檢視與選取項目相關的各種事件。
   ...
   * 可從下拉式清單中選取事件類型：
      ...
      * [工作流程](/help/sites-cloud/authoring/workflows/overview.md)
         * 除了暫時性工作流程外，這些<!--With the exception of [transient workflows](/help/sites-developing/workflows.md#transient-workflows) as no history information is saved for these-->不會儲存任何歷史資訊
      * 顯示全部
...
**參** 考顯示與選定資源的任何連接。例如，在&#x200B;**Sites**&#x200B;控制台[頁面參考](/help/sites-cloud/authoring/fundamentals/environment-tools.md#references)中，頁面顯示：

* [啟動](/help/sites-cloud/authoring/launches/overview.md#launches-in-references-sites-console)
* 即時副本<!--[Live copies](/help/sites-administering/msm-livecopy-overview.md#openingthelivecopyoverviewfromreferences)-->
* 語言副本<!--[Language copies](/help/sites-administering/tc-prep.md#seeing-the-status-of-language-roots)-->
* 內容參考：
...
如此會開啟類似[search](/help/sites-cloud/authoring/getting-started/search.md)的面板，其中已設定適當的位置篩選條件，讓您進一步篩選您想要檢視的內容。
