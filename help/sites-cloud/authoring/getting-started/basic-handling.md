...
| 網站 | 網站主控台可讓您 [建立、檢視和管理網站](/help/sites-cloud/authoring/fundamentals/organizing-pages.md) 在AEM執行個體上執行。 您可以透過此主控台建立、編輯、複製、移動和刪除頁面、啟動工作流程及發佈頁面。 |
| 體驗片段 | 安 [體驗片段](/help/sites-cloud/authoring/fundamentals/experience-fragments.md) 是獨立體驗，可跨管道重複使用，且有變數，可避免重複複製和貼上體驗或體驗的部分所帶來的麻煩。 |
| 資產 | Assets主控台可讓您匯入和管理 [影像、視訊、檔案和音訊檔案等數位資產](/help/assets/overview.md). 然後，同一AEM執行個體上執行的任何網站都可以使用這些資產。 您也可以建立和管理 [內容片段](/help/assets/content-fragments/content-fragments.md) 從「資產」主控台。 |
| 個性化 | 此主控台提供的工具架構 [製作目標式內容並呈現個人化體驗。](/help/sites-cloud/authoring/personalization/overview.md) |
...
* 階層連結
   ...
   瀏覽路徑標示位於邊欄的中間，一律會顯示目前所選項目的說明，可讓您在特定主控台中導覽。 在 **網站** 控制台，您可以瀏覽網站的各個層級。
...
點選或按一下 **下載主題來源** 自訂 [主題。](/help/sites-cloud/administering/site-creation/site-themes.md)
