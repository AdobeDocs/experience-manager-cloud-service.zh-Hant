...
>[!NOTE]
>
>進階功能是使用目標內容的多站台支援。 若要使用此功能，您應該熟悉 [多站點管理員](/help/sites-cloud/administering/msm/overview.md) 和 [Adobe Target整合](/help/sites-cloud/integrating/integrating-adobe-target.md) 使用AEM。
...
1. [建立新區域](#creating-new-areas) 或 [建立區域作為即時副本](#creating-new-areas). 區域包含可用於的所有活動 *區域* ，也就是元件在頁面上的目標位置。 建立新區域會建立空白區域，而建立區域作為即時副本可讓您跨網站結構繼承內容。
...
您可在以下位置管理此專案： **區域**. 區域可區隔不同網站中使用的目標內容（活動、體驗和選件），並提供以MSM為基礎的機制，以建立和管理目標內容的繼承以及網站繼承。 這可讓您無須在繼承的網站中重新建立目標內容。
...
行銷人員想要將這些變更推廣到歐洲地區，並 [轉出即時副本](/help/sites-cloud/administering/msm/creating-live-copies.md) 點選或按一下 **轉出頁面**. 重新整理標籤後，Great Britain網站會有新影像，因為Europe區域繼承自主區域（轉出後）。
...
>[!NOTE]
>
>當您選取品牌名稱時，預設會收合名為「主版區域」的預設區域 **直到** 您會建立另一個區域。 然後，當您在「活動」或「選件」控制台中選 **取品牌** 時，您會看到「 **區域****** 」控制台。
...
1. 在 **轉出設定** 在下拉式功能表中，選取適當的設定。
   ...
   另請參閱 [已安裝的轉出設定](/help/sites-cloud/administering/msm/live-copy-sync-config.md#installed-and-custom-rollout-configurations) 每個選項的說明。

   另請參閱 [建立和同步化即時副本](/help/sites-cloud/administering/msm/creating-live-copies.md) 以取得即時副本的詳細資訊。
...
1. 導覽至您要分離或暫停繼承的頁面，然後選取 **目標定位** 在「模式」下拉式功能表中。
1. 如果您的頁面連結至即時副本區域，您會看到繼承狀態。 選取 **開始定位**.
...
1. 導覽至您要從主版分離即時副本的頁面，然後選取 **目標定位** 在「模式」下拉式功能表中。
1. 如果您的頁面連結至即時副本區域，您會看到繼承狀態。 選取 **開始定位**.
1. 從工 **具列的下拉式選單中選取「分離即時副本** 」。AEM會確認您要分離即時副本。
1. 選取 **分離** 將即時副本與活動中斷連結。 分離後，有關繼承的下拉式功能表不再顯示。 活動現在是本機活動。
...
1. 導覽至您要還原繼承的頁面，然後選取 **目標定位** 在「模式」下拉式功能表中。
1. 選取 **開始定位**.
