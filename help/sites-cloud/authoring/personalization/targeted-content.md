...
>[!NOTE]
>
>目標模式在頁面編輯器和體驗片段編輯器中都可用。
>
>以下文檔適用於兩者（因為它們都以相同的基礎操作），但是它們是為頁面編輯器編寫的。

>[!CAUTION]
>
>在頁面編輯器中進行目標時，只有「體驗片段」元件可以作為目標。
>
>其他元件類型可以使用 **轉換為體驗片段變體** 表徵圖。
...
您可以使用AEM或Adobe Target作為目標引擎(您必須具有有效的Adobe Target帳戶才能使用Adobe Target)。 如果使用Adobe Target，則必須先配置整合。 查看 [與Adobe Target整合的指示](/help/sites-cloud/integrating/integrating-adobe-target.md)。
