...
>
>其他元件型別可使用來轉換為體驗片段 **轉換為體驗片段變數** 圖示來切換元件。
...
1. [Target](#target-configuring-the-audiences)：指定每個體驗鎖定的對象。 您可以鎖定特定對象，如果使用A/B測試決定流向哪個體驗的流量百分比。
...
1. 若要顯示引導您完成鎖定目標程式的控制項，請選取 **開始定位**.
...
在您之後 [開始目標定位流程](#the-targeting-process-create-target-and-goals-settings)，選取體驗以檢視為該體驗提供的選件。 當您選取體驗時，頁面上的目標元件會變更，顯示該體驗的選件。
...
符合以下條件的優惠 [在頁面上撰寫](#adding-a-custom-offer) 用於單一體驗的稱為自訂選件。 下列影像重疊在自訂選件的內容上：

![自訂優惠圖示](../assets/targeted-custom-offer-icon.png)

符合以下條件的優惠 [從優惠資料庫新增](#adding-an-offer-from-an-offer-library) 與下列影像重疊：
...
使用的建立步驟 [目標定位程式](#the-targeting-process-create-target-and-goals-settings)，您可以新增和移除體驗。 此外，您可以複製體驗並重新命名。

#### 使用定位模式新增體驗 {#adding-experiences-using-targeting-mode}

若要新增體驗：

1. 若要新增體驗，請選取「 」 **+** **新增體驗鎖定目標** 會顯示在中現有體驗下方 **受眾** 窗格。
...
* [新增自訂優惠方案到優惠方案庫](#adding-a-custom-offer-to-a-library).
...
之後執行下列程式 [開始目標定位程式](#the-targeting-process-create-target-and-goals-settings).
...
1. 選取Target圖示。
   ...
   元件內容是預設體驗的選件。 鎖定元件為目標時，會為每個體驗復寫其預設節點。 在體驗特定撰寫期間，需要此動作來編輯正確的內容節點。 對於這些非預設體驗，可以 [新增自訂選件](#adding-a-custom-offer) 或 [新增資料庫選件](#adding-an-offer-from-an-offer-library).
...
>[!NOTE]
>
>您使用建立的選件 [選件主控台](/help/sites-cloud/authoring/personalization/offers.md) 也可以包含數個元件。 這些選件屬於一個選件程式庫，且可用於多個體驗。
...
1. 建立選件：
   ...
   * 對於非預設體驗，可以 [新增自訂選件](#adding-a-custom-offer) 或 [新增資料庫選件](#adding-an-offer-from-an-offer-library).
...
從新增選件 [優惠資料庫](/help/sites-cloud/authoring/personalization/offers.md) 至一個體驗。 您可以從目前鎖定目標的品牌庫新增任何選件。
...
1. 從資料庫中選取選件，然後選取核取記號圖示。
   ...
   * 選取 **瀏覽** 以導覽至其他資料夾。 導覽窗格隨即開啟，按一下箭頭可向下展開資料夾。 選取 **瀏覽** 以關閉導覽窗格。

   ![瀏覽內容](../assets/targeted-select-content-browse.png)

   * 選取 **篩選** 依關鍵字或標籤篩選選件。 您可以輸入關鍵字，然後從下拉式選單中選取標籤。 選取 **篩選** 以關閉篩選窗格。
   ...
   * 按一下或點選選旁的箭頭，變更優惠排序方式 **最新到最舊**. 選件可依最新到最舊或最舊到最新的順序排序。
   ...
   選取旁的圖示 **檢視方式** 以圖磚或清單形式檢視優惠方案。

   ![檢視方式按鈕](../assets/targeted-view-as-button.png)

#### 新增自訂選件至程式庫 {#adding-a-custom-offer-to-a-library}

新增自訂選件至 [優惠資料庫](/help/sites-cloud/authoring/personalization/offers.md) 當您想要重複使用它做為多個體驗的選件時。 您可以將選件新增至您鎖定目標的目前品牌資料庫。
...
的目標步驟： [目標定位程式](#the-targeting-process-create-target-and-goals-settings) 包括將對象與您在「建立」步驟中使用的體驗對應。 Target頁面會顯示每個體驗鎖定目標的對象。 您可以指定或變更每個體驗的對象。 如果您使用Adobe Target，也可以建立A/B測試，好讓您鎖定特定體驗的對象流量百分比。
...
如需鎖定目標引擎的詳細資訊，請參閱 [定位引擎](/help/sites-cloud/authoring/personalization/overview.md#targeting-engine).
...
的目標與設定步驟 [目標定位程式](#the-targeting-process-create-target-and-goals-settings) 涉及設定品牌活動的行為。 指定活動何時開始和結束，以及活動優先順序。 此外，您也可以追蹤目標。 具體來說，您可以決定要與活動一起測量哪些專案。
...
1. 如果您想要進一步控制成功量度的計數方式，請選取「 」 **進階設定** 以存取這些。
1. 按一下「**儲存**」。
...
* 此 [活動在](/help/sites-cloud/authoring/personalization/activities.md).
...
![Target做為引擎](../assets/targeted-target-as-enging.png)
...
>
>您選取 **Adobe Campaign** 作為引擎(如果您整合AEM與Adobe Campaign)。 如需詳細資訊，請參閱整合AEM與Adobe Campaign 。
>
>選取 **ContextHub** 做為引擎（如果您使用ContextHub進行目標定位）。 如需詳細資訊，請參閱設定ContextHub 。
