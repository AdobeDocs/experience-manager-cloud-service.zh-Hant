...
區段是建立促銷活動時的主要考量。 如需細分運作方式和關鍵詞語的詳細資訊，請參閱[瞭解細分](segmentation.md)。

根據您已收集的網站訪客相關資訊以及您要達到的目標，您需要定義目標內容所需的區段和策略。

然後，這些區段會用來為訪客提供特定的目標內容。 [此處](activities.md) 定義的活動可包含在任何頁面上，並定義專業內容適用的訪客區段。

AEM可讓您輕鬆個人化使用者的體驗。 此外，還可讓您驗證區段定義的結果。

## 存取區段{#accessing-segments}

[Audiences](audiences.md)主控台可用來管理ContextHub的區段以及Adobe Target帳戶的觀眾。 本檔案涵蓋管理ContextHub的區段。
...
當整個陳述式評估為true時，區段即已解決。 若有多個區段可適用，則也會使用&#x200B;**Boost**&#x200B;因子。 如需提升系數的詳細資訊，請參閱[建立新區段](#creating-a-new-segment)。
...
下列容器是現成可用的，可讓您將比較和參考分組，以便進行布林值評估。 它們可從元件瀏覽器拖曳至編輯器。 如需詳細資訊，請參閱以下「使用AND和OR容器」一節。[](#using-and-and-or-containers)
...
>
>因此，當[建立段](#creating-a-new-segment)時，只要已知比較值的類型，您應選擇&#x200B;**資料類型**。 例如：
>
...
|---|---|
| 區段引用 | 評估參考的區段 |
| 指令碼引用 | 評估參考的指令碼。 如需詳細資訊，請參閱以下章節[使用指令碼參考](#using-script-references)。 |
...
1. 點選或按一下「建立&#x200B;****」按鈕，然後選取「建立ContextHub區段&#x200B;**」。**
...
1. 連按兩下或點選新參考或區段的設定選項，以編輯特定參數。 在此範例中，我們正在為巴塞爾的人員進行測試。
   ...
   請盡量設定&#x200B;**資料類型**，以確保正確評估比較。 如需詳細資訊，請參閱[比較](#comparisons)。

1. 按一下&#x200B;**Done**&#x200B;保存定義：
1. 視需要新增更多元件。 您可以使用容器元件來建立布林運算式，以用於AND和OR比較（請參閱下方的[使用AND和Or容器](#using-and-and-or-containers)）。 使用區段編輯器，您可以刪除不再需要的元件，或將元件拖曳至陳述式中的新位置。
...
1. 開啟&#x200B;**指令碼引用**&#x200B;元件的編輯對話框。 如果[已正確設定](#defining-a-script-to-reference)，則指令碼應可在&#x200B;**指令碼名稱**&#x200B;下拉式清單中使用。
...
一旦定義了區段，就可在&#x200B;**[ContextHub](contexthub.md)的協助下測試潛在結果。**
...
如果您已設定活動和體驗，則可以使用活動輕鬆測試區段。 如需設定活動的詳細資訊，請參閱製作目標內容的相關[檔案](targeted-content.md)。
...
區段可用來控制特定目標對象所檢視的實際內容。 如需觀眾和區段的詳細資訊，請參閱[管理觀眾](audiences.md)，以及[編寫目標內容](targeted-content.md)有關使用觀眾和區段來定位內容的詳細資訊。
