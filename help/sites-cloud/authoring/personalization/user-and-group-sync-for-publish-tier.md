...
您可以撰寫自訂註冊代碼，至少需要使用者的使用者名稱和密碼，並在AEM中建立使用者記錄，之後可在登入期間用來進行身分驗證。 以下步驟通常用於建構此註冊機制：
...
在某些情況下，註冊或使用者建立先前是在AEM之外的基礎架構中進行。 在這種情況下，使用者記錄會在登入期間在AEM中建立。
...
* 並考慮[向AEM社群專家座談會](https://bit.ly/ATACEFeb15)詢問登入的相關資訊。
...
客戶可透過其偏好的SAML IdP使用SAML型驗證。 搭配AEM使用IdP時，IdP會負責驗證使用者的認證並代理使用者與AEM的驗證、視需要在AEM中建立使用者記錄，以及在AEM中管理使用者的群組成員資格，如SAML判斷提示所述。
...
請參閱[單一登入(SSO)檔案](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/single-sign-on.html?lang=zh-Hant)，以取得使用AEM的SSO驗證處理常式服務的相關資訊。
...
您可以即時存取協力廠商服務以擷取設定檔屬性，但請務必確保這不會對AEM中的請求處理造成重大影響。
...
發佈層存取原則(也稱為「封閉式使用者群組(CUG)」)是在AEM作者中定義的，請參閱[建立封閉式使用者群組](https://experienceleague.adobe.com/docs/experience-manager-65/administering/security/cug.html?lang=zh-Hant#applying-your-closed-user-group-to-content-pages)。 若要限制某些使用者存取網站的特定區段或頁面，請視需要使用AEM作者套用CUG （如此處所述），並將它們復寫至發佈階層。
...
### 自訂程式碼和移轉需求 {#custom-code-and-migration-requirements}

下列要求僅適用於使用自訂程式碼建立本機使用者或本機群組的情況。 啟用資料同步時，必須更新這類自訂程式碼，以建立具有動態群組成員資格的外部使用者和外部群組。

**必要的步驟：**

* **修改自訂程式碼**：負責建立使用者或群組的任何自訂邏輯都必須更新為：

   * 設定`rep:externalId`屬性以建立外部使用者
   * 設定`rep:externalId`屬性以建立外部群組
   * 使用`rep:externalPrincipalNames`屬性實作動態群組成員資格，而不是使用直接的使用者與群組關係

* **移轉預先存在的資料**：在生產環境中啟用資料同步之前，必須將所有現有的本機使用者和群組移轉至外部身分模型。

如需更新自訂實作以及移轉現有使用者和群組的詳細技術指引，請參閱[移轉至外部身分識別與動態群組成員資格](/help/security/migrating-to-external-identity.md)。
