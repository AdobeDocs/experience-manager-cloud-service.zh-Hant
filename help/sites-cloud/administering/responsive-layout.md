...
[回應式版面](/help/sites-cloud/authoring/features/responsive-layout.md) 是一種用於實現 [回應式網頁設計。](https://en.wikipedia.org/wiki/Responsive_web_design) 這可讓內容作者建立根據使用者使用之裝置而具有版面和維度的網頁。

AEM使用一組機製為頁面實現回應式佈局：

* **[配置容器](/help/sites-cloud/authoring/features/responsive-layout.md#adding-a-layout-container-and-its-content-edit-mode)**  — 此元件提供格線段落系統，可讓您在回應式格線內新增及放置元件。
...
* **[版面模式](/help/sites-cloud/authoring/fundamentals/environment-tools.md)**  — 將版面容器放置到頁面上後，您就可以使用 **版面** 在回應式格線內放置內容的模式。
* **[模擬器](/help/sites-cloud/authoring/features/responsive-layout.md#selecting-a-device-to-emulate)**  — 這可讓您建立並編輯回應式網站，這些網站會透過以互動方式調整元件大小，根據裝置/視窗大小重新排列版面。 之後，使用者可以使用模擬器檢視內容的呈現方式。
...
* **透過作者環境** - [編輯您的頁面範本](/help/sites-cloud/authoring/features/templates.md) 以啟用頁面的「配置容器」。
* **元件定義**  — 使用 `allowedComponent` 或定義元件時的靜態包含。

### 設定配置容器的格線 {#configure-the-grid-of-the-layout-container}

您可以設定可用於配置容器的每個特定例項的欄數 [編輯您的頁面範本。](/help/sites-cloud/authoring/features/templates.md)
