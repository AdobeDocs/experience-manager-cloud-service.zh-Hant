...
建立將AEM連接至翻譯服務提供者的雲端設定。 AEM包含 [連接到Microsoft Translator](connect-ms-translator.md) 依預設。
...
安裝連接器封裝後，您可以為連接器建立雲端設定。 通常，您需要提供認證以驗證翻譯服務。 如需為Microsoft Translator連接器新增雲端設定的詳細資訊，請參閱 [與Microsoft翻譯人員整合](connect-ms-translator.md).
...
配置翻譯整合框架後，您可以 [將其與頁面建立關聯](preparation.md) 用它。

>[!TIP]
>
>如需AEM中內容翻譯功能的概觀，請參閱 [轉譯多語言網站的內容](overview.md).

架構的單一設定可控制頁面內容和資產的翻譯方式。 要建立新的翻譯配置：

1. 在 [全局導航菜單，](/help/sites-cloud/authoring/getting-started/basic-handling.md#global-navigation) 按一下或點選 **工具 — >Cloud Services — 和翻譯Cloud Services**.
...
1. 在 **編輯配置** 窗口，在 **網站** 和 **資產** 標籤，然後按一下或點選 **儲存並關閉**.

### 站點配置屬性 {#sites-configuration-properties}

此 **網站** 標籤控制頁面內容翻譯的執行方式。

![網站的翻譯設定](../assets/translation-configuration.png)

| 屬性 | 說明 |
|---|---|
| 翻譯方法 | 此屬性定義框架對網站內容執行的翻譯方法：<br> — 機器翻譯：翻譯提供者使用機器翻譯即時執行翻譯。<br> — 人類翻譯：內容被發送到翻譯提供者，由翻譯者翻譯。<br> — 不翻譯：內容不會傳送以供翻譯。 這是為了略過某些內容分支，這些分支不會翻譯，但可以以最新內容更新。 |
| 翻譯提供者 | 此屬性定義要執行翻譯的翻譯提供程式。 當安裝了相應的連接器時，提供程式將出現在清單中。 |
| 內容類別 | （僅限機器翻譯）此屬性是描述要翻譯的內容的類別。 翻譯內容時，類別可能會影響術語和措辭的選擇。 |
| 翻譯標記 | 此選項可轉譯與頁面相關聯的標籤。 |
| 翻譯頁面資產 | 此屬性定義如何轉換從檔案系統新增至元件或從資產參照的資產：<br> — 不翻譯：頁面資產不會翻譯。<br> — 使用網站翻譯工作流程：系統會根據 **網站** 標籤。<br> — 使用資產翻譯工作流程：系統會根據 **資產** 標籤。 |
| 自動執行翻譯 | 啟用此屬性可在建立翻譯專案後自動執行翻譯工作。 選擇此選項時，您沒有機會複查和調整翻譯工作的範圍。 |
| 禁用僅更新翻譯 | 核取此選項後，更新翻譯專案將提交所有可翻譯的欄位以供翻譯，而不只是上次翻譯後變更的欄位。 |

### Assets設定屬性 {#assets-configuration-properties}

資產屬性可控制如何設定資產。 如需轉譯資產的詳細資訊，請參閱 [建立資產的語言復本](/help/assets/translate-assets.md).

![網站的翻譯設定](../assets/translation-configuration-assets.png)

| 屬性 | 說明 |
|---|---|
| 翻譯方法 | 此屬性會選取架構對資產執行的轉譯類型：<br> — 機器翻譯：翻譯提供者使用機器翻譯立即執行翻譯。<br> — 人類翻譯：內容會自動傳送至翻譯提供者，以供手動翻譯。<br> — 不翻譯：不會傳送資產以進行翻譯。 |
| 翻譯提供者 | 此屬性定義要執行翻譯的翻譯提供程式。 當安裝了相應的連接器時，提供程式將出現在清單中。 |
...
| 禁用僅更新翻譯 | 核取此選項後，更新翻譯專案將提交所有可翻譯的欄位以供翻譯，而不只是上次翻譯後變更的欄位。 |
| 啟用翻譯的內容模型欄位 | 啟用此選項將使用 **可翻譯** 欄位 [內容片段模型](/help/sites-cloud/administering/content-fragments/content-fragments-models.md#properties) 以確定欄位是否已翻譯，並自動建立 [翻譯規則](rules.md) 因此。 此選項將取代您可能建立的任何翻譯規則。 |
...
將頁面與雲配置關聯時，頁面的子體將繼承關聯。 例如，如果您將 `/content/wknd/language-masters/en/magazine` 頁面，此頁面會 `magazine` 其下的頁面和子頁面會根據架構進行翻譯。
