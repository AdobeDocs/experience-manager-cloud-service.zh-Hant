...
| 停用僅更新翻譯 | 核取此選項時，更新翻譯專案會提交所有可翻譯的欄位以供翻譯，而不只是自上次翻譯以來變更的欄位。 |
| 啟用內容模型欄位以進行翻譯 | 啟用此選項會使用 **可翻譯** 欄位於 [內容片段模型](/help/sites-cloud/administering/content-fragments/content-fragment-models.md#properties) 以判斷欄位是否已轉譯並自動建立 [翻譯規則](rules.md) 視情況而定。 此選項會取代您可能已建立的任何翻譯規則。 |
