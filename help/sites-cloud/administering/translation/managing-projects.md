...
1. 在強制回應視窗中，選取您要新增之分支的最上層專案，然後按一下或點選核取記號圖示。 此視窗中已啟用多重選取。
...
在您的翻譯專案中，按一下或點選「翻譯摘要」方塊底部的省略符號。 大部分專案特定屬性位於 **進階** 標籤。
...
1. 在 **基本** 索引標籤中，使用的日期選擇器 **到期日期** 屬性以選取到期日期。
...
1. 在專案主控台中，開啟翻譯專案。
1. 在翻譯工作圖磚上，按一下或點選命令功能表，然後按一下或點選 **取消**.
...
1. 在專案主控台中，開啟翻譯專案。
1. 在翻譯工作圖磚上，按一下或點選命令功能表，然後按一下或點選 **完成**.
1. 工作現在具有狀態 **完成**.
...
1. 在專案主控台中，開啟翻譯專案。
1. 在翻譯工作圖磚上，按一下或點選命令功能表，然後按一下或點選 **封存**.
...
![工作已可供檢閱](../assets/job-ready-for-review.png)
...
1. 開啟 [引用邊欄](/help/sites-cloud/authoring/getting-started/basic-handling.md#references).
1. 在 **份數** 標題選取 **語言副本。**
...
如果需要已翻譯的來源頁面 [已重新命名或已移動](/help/sites-cloud/authoring/fundamentals/organizing-pages.md#moving-or-renaming-a-page)，在移動後再次翻譯頁面時，會根據新的頁面名稱/位置建立語言副本。 根據先前名稱/位置的舊語言副本仍會存在。
