...
>[!NOTE]
>
>一系列 [此主控台提供鍵盤快速鍵](/help/sites-cloud/administering/content-fragments/content-fragments-console-keyboard-shortcuts.md).
   ...
   * 檢視樹狀結構所選分支中的所有內容片段清單：
      ...
      * 所選資料夾的內容片段和所有子資料夾都會顯示：
         * [各種資訊欄位](#selectuse-available-columns) 關於內容片段提供連結；根據欄位，這些連結可以：
            ...
            * 顯示片段語言版本的相關資訊
            * 依選取的標籤快速篩選
      * 在欄標題上使用滑鼠游標時，畫面會顯示下拉式動作選擇器和寬度滑桿。 這些功能可讓您進行下列工作：
...
* 可直接執行各種動作 [可從主控台取得](#available-actions)
...
選取特定片段將開啟工具列，其焦點為該片段可用的動作。 您也可以選取多個片段 — 動作的選取會據此調整。
...
* **模型**
   * 僅供參考。
   * 可用於 [快速篩選](#fast-filtering#fast-filtering)
* **資料夾**
   * 提供可在主控台中開啟資料夾的連結。
...
* **語言**
   ...
   * 表示內容片段的地區設定，以及與內容片段關聯的地區設定/語言副本總數。
