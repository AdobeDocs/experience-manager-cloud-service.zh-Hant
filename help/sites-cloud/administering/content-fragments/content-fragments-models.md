...
* **可翻譯**
   ...
   * 對於GraphQL:設定 `<translatable>` 「內容片段」欄位上的屬性 `yes`，允許僅具有可翻譯內容的JSON輸出的GraphQL查詢篩選器。
...
* **[片段引用](#fragment-reference-nested-fragments)** （嵌套片段）
   ...
   * 允許您包括/檢索結構化資料。
      ...
      >[!NOTE]
      >
      >該方法與 [使用GraphQL的內容片段進行無頭內容傳遞](/help/sites-cloud/administering/content-fragments/content-fragments-graphql.md)。
...
>[!NOTE]
>
>這與 [使用GraphQL的內容片段進行無頭內容傳遞](/help/sites-cloud/administering/content-fragments/content-fragments-graphql.md)。
