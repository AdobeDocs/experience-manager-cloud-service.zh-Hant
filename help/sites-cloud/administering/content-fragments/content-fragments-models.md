---
title: 內容片段模型
description: 瞭解內容片段模型如何作為您在AEM中內容片段的基礎。 這些功能可讓您建立結構化內容，以用於Headless傳送或頁面製作。
...
AEM中的內容片段模型會定義 [內容片段](/help/sites-cloud/administering/content-fragments/content-fragments.md). 這些可用於頁面製作，或用作Headless內容的基礎。
...
1. 導覽至適合您的檔案夾 [組態或子組態](/help/sites-cloud/administering/content-fragments/content-fragments-configuration-browser.md).
1. 使用 **建立** 以開啟精靈。
...
在模型編輯器中顯示為分隔線，用於分隔內容資料型別清單的各個區段。 每個例項代表新索引標籤的開始。
...
從目前模型建立的所有內容片段內容（適用於特定欄位）必須是唯一的。
  ...
  >[!CAUTION]
  >
  >如果您想使用MSM （這會建立內容片段的復本），則任何 **獨特** 應從在各自內容片段模式中使用的任何資料型別中移除限制。 內容片段的MSM只能從 **資產** 主控台。
...
* **[片段引用](#fragment-reference-nested-fragments)** （巢狀片段）
   ...
   * 可讓您包含/擷取結構化資料。
     ...
     >[!NOTE]
     >
     >此方法與下列專案特別相關： [搭配GraphQL使用內容片段的Headless內容傳送](/help/sites-cloud/administering/content-fragments/content-fragments-graphql.md).
   * 可以為一個或多個參考（在產生的片段中）設定。
...
>[!NOTE]
>
>這與以下內容特別相關： [搭配GraphQL使用內容片段的Headless內容傳送](/help/sites-cloud/administering/content-fragments/content-fragments-graphql.md).
...
主控台會指出發佈狀態。
   ...
   >[!NOTE]
   >
   >如果您發佈的內容片段尚未發佈模型，選擇清單會指出這一點，而模型會與片段一起發佈。
