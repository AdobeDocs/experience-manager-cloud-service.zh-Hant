...
這是使用[Day CQ WCM轉出管理員](/help/implementing/deploying/configuring-osgi.md)的&#x200B;**OSGi組態**&#x200B;完成的。 如果轉出管理員應處理在Live Copy中建立的頁面與Blueprint中已存在的名稱發生的衝突，則將值&#x200B;**處理與手動建立的頁面**&#x200B;的衝突(`rolloutmgr.conflicthandling.enabled`)設定為true。
...
>[!CAUTION]
>
>停用衝突處理時，AEM不會指出系統已忽略衝突。 由於在這種情況下必須明確設定此行為，因此假設這是所需的行為。
