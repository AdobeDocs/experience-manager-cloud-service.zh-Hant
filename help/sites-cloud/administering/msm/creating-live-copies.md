...
description: 瞭解如何建立並同步即時副本，以便在您的網站上重複使用您的內容。
...
您可以從頁面或Blueprint設定建立即時副本，以便在您的網站上重複使用該內容。 管理繼承和同步，您可以控制內容的變更的傳播方式。

## 管理Blueprint設定 {#managing-blueprint-configurations}

Blueprint設定會識別您想要用作一或多個Live Copy頁面來源的現有網站。
...
* 每個語言分支的根都有一個或多個子頁面。 建立即時副本時，會顯示子頁面，讓您可將其納入即時副本。
...
* **說明**. （可選） Blueprint設定的說明，顯示在建立網站時可從中進行選擇的Blueprint設定清單中

使用您的Blueprint設定時，您可以將其與轉出設定相關聯，以決定來源/Blueprint的即時副本同步的方式。 另請參閱 [指定要使用的轉出設定](live-copy-sync-config.md#specifying-the-rollout-configurations-to-use).
...
* 如果您未指定任何轉出設定，MSM會決定要使用哪個轉出設定。 另請參閱 [指定要使用的轉出設定](live-copy-sync-config.md#specifying-the-rollout-configurations-to-use).
...
* 如果您從來源頁面建立即時副本，且 **是** 在Blueprint設定中參照，則轉出命令將可在來源/Blueprint頁面上使用。
* 如果您從來源頁面建立即時副本，且 **不是** 在Blueprint設定中參照，則轉出命令將無法在來源/Blueprint頁面上使用。
...
1. （可選）若要指定一個或多個轉出設定以用於即時副本，請使用 **轉出設定** 下拉式清單加以選取。 選取的設定將顯示在下拉式選擇器的下方。
...
使用Blueprint設定建立即時副本，以根據Blueprint （來源）內容建立網站。 當您從Blueprint設定建立即時副本時，請選取要複製的Blueprint來源的一或多個語言分支，然後從語言分支選取要複製的章節。 另請參閱 [建立Blueprint設定](#creating-a-blueprint-configuration).

如果您從即時副本中省略一些語言分支，您可以稍後新增它們。 另請參閱 [在即時副本中建立即時副本（Blueprint設定）](#creating-a-live-copy-inside-a-live-copy-blueprint-configuration) 以取得詳細資訊。
...
* **網站擁有者**：（選用）負責即時副本之當事方的相關資訊
* **即時副本**：選取此選項可與來源網站建立即時關係。 如果您未選取此選項，則會建立Blueprint的復本，但之後不會與來源同步。
* **轉出設定**：（選用）選取一或多個轉出設定以用於同步即時副本。 依預設，轉出設定是從Blueprint繼承。 另請參閱 [指定要使用的轉出設定](live-copy-sync-config.md#specifying-the-rollout-configurations-to-use) 以取得更多詳細資料。
...
1. 選取要用作即時副本來源的Blueprint設定，並繼續 **下一個**：
   ...
   ![從藍圖建立網站](../assets/create-site-from-blueprint.png)
...
1. 使用 **初始章節** 下拉式清單，以選取要納入即時副本中的Blueprint區段。 預設會包含所有可用的章節，但可將其移除。
...
* **來源**：即時副本頁面的來源頁面
* **狀態**：即時副本的同步狀態，包括即時副本是否與來源保持同步、上次同步發生的時間以及同步的執行者
* **設定**:
...
Blueprint頁面（在Blueprint設定中參照）會為您提供使用目前(Blueprint)頁面作為來源的Live Copy頁面清單。 使用此清單來追蹤即時副本。 清單會顯示在 **Blueprint** 的標籤 [頁面屬性](/help/sites-cloud/authoring/fundamentals/page-properties.md).
...
1. 在 **網站** 控制檯中，選取即時副本中的頁面並開啟 **[引用](/help/sites-cloud/authoring/getting-started/basic-handling.md#references)** 面板（從工具列）。
...
>[!NOTE]
>
>同步會執行使用下列專案的轉出設定： [轉出時](live-copy-sync-config.md#rollout-triggers) 觸發器。
...
此 [即時副本概觀中也提供同步動作](live-copy-overview.md#using-the-live-copy-overview)，在選取即時副本頁面時。
...
>[!TIP]
>
>如果您在即時副本中手動建立新頁面，則新頁面是即時副本的本機頁面，這表示它沒有附加到的對應來源頁面。
>
>若要建立屬於關聯一部分的本機頁面，最佳做法是在來源中建立本機頁面，並執行深層轉出。 這會將頁面在本機建立為即時副本。
...
>[!TIP]
>
>在本機對標示為容器的元件進行的變更將不會被轉出時的Blueprint內容覆寫。 另請參閱 [MSM最佳作法](best-practices.md#components-and-container-synchronization) 以取得詳細資訊。

### 暫停頁面的繼承 {#suspending-inheritance-for-a-page}

當您建立即時副本時，即時副本設定會儲存在已複製頁面的根頁面上。 根頁面的所有子頁面都會繼承即時副本設定。 即時副本頁面上的元件也會繼承即時副本設定。
...
1. 使用「 」開啟「即時副本」頁面的屬性。 **檢視屬性** 命令 **網站** 主控台或使用 **頁面資訊** （在頁面工具列上）。
1. 按一下或點選 **即時副本** 標籤。
...
此 [即時副本概觀也提供暫停動作](live-copy-overview.md#using-the-live-copy-overview)，在選取即時副本頁面時。
...
1. 開啟 [即時副本概觀](live-copy-overview.md#using-the-live-copy-overview) 並選取已暫停的即時副本頁面。 頁面將顯示為 **已取消繼承**.
1. 選取 **繼續** （從工具列）。
...
* 切換到淺層即時副本：
   ...
   * 從即時副本明確分離子頁面。 如果還原，則無法保留對子項所做的進一步修改。
...
* 切換至淺層即時副本，然後返回深層：
   ...
   * 將（先前的）淺層即時副本的所有子項視為手動建立，因此使用將其移開 `[oldname]_msm_moved name`.
...
* 轉出時，元件順序將從Blueprint恢復。 如果在轉出前已將新元件新增到即時副本，則這些新元件將與其上方新增的元件一起重新排序。
...
* **在即時副本的子頁面上分離**
   ...
   * 即時副本分支中的（子）頁面會被視為已手動建立。
   ...
   但是，子頁面仍然受父分支的即時關係約束，因此藍圖頁面的進一步轉出將：

   1. 重新命名分離的頁面：

      * 這是因為MSM將它們視為手動建立的頁面，這些頁面因與其嘗試建立的即時副本頁面具有相同名稱而造成衝突。
   1. 以原始名稱建立包含轉出變更的新即時副本頁面。

   >[!NOTE]
   >
   >另請參閱 [MSM轉出衝突](rollout-conflicts.md) 以取得這類情況的詳細資訊。
