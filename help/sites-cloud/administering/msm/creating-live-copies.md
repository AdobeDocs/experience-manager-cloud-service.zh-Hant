...
1. 選取所需的Blueprint設定 — 適當的動作即可在工具列中取得：
...
* 如果您從來源頁面建立即時副本，且 **是** 在Blueprint設定中參照，則轉出命令可在來源/Blueprint頁面上使用。
...
1. （可選）若要指定一個或多個轉出設定以用於即時副本，請使用 **轉出設定** 下拉式清單加以選取。 選取的設定會顯示在下拉式選擇器的下方。
...
>
>為了建立屬於關聯一部分的本機頁面，最佳做法是在來源中建立本機頁面，並執行深層轉出。 這會將頁面在本機建立為即時副本。
...
* 轉出時，元件順序會從Blueprint還原。 如果在轉出前已將新元件新增到即時副本，則它們會與其上方新增的元件一起重新排序。
