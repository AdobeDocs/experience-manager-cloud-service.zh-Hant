...
# 重複使用內容：多站點管理器和即時副本{#multi-site-manager-and-live-copy}

Multi Site Manager(MSM)可讓您在多個位置中使用相同的網站內容。 MSM會使用其Live Copy功能來達成此目標。

* 透過MSM，您可以：
   * 先建立內容一次
   * 在相同或其他網站的其他區域（透過[Live Copy](#live-copies)）中重複使用此內容。
...
* **跨國公司 — 全球到本地公司**
   ...
   例如，會為美國客戶建立[ WKND教學課程範例](/help/implementing/developing/introduction/develop-wknd-tutorial.md)的英文區段。 此網站中的大部分內容也可用於其他WKND網站，以迎合不同國家和文化的英語客戶。 所有網站的核心內容都保持不變，而且可進行地區調整。

   下列結構可用於美國和加拿大的網站。 請注意`language-masters`節點如何維護主副本，該副本不僅包含英語，還包含其他語言內容。 此內容可作為英文以外其他地區語言內容的基礎。
   ...
   >
   >
   >如需此範例，請參閱[轉譯多語言網站的內容](/help/sites-cloud/administering/translation/overview.md)。
...
* **建立網站** (**網站**)
   ...
   * MSM可協助您管理多個共用相同內容的網站。 例如，網站通常為國際受眾提供，因此大部分內容在所有國家/地區都是共有的，而其中的一部分內容是個別國家/地區特有的。 MSM可讓您[建立即時副本，以根據您的來源網站](creating-live-copies.md#creating-a-live-copy-of-a-site-from-a-blueprint-configuration)自動更新一或多個網站。 這也可協助您強制建立通用的基礎結構、跨多個網站使用通用內容、維持通用的外觀和風格，並著重於管理實際上不同網站的內容。 以此方式建立網站：
...
* **建立即時副本** (**網站**)
   ...
   * MSM可讓您[建立個別頁面或網站子分支的臨機（一次性）即時副本。](creating-live-copies.md#creating-a-live-copy-of-a-page) 例如，複製子分支以提供關於產品的新版本/更新版本的資訊。以下列方式建立即時副本：
...
* **檢視屬性** (**網站**)
   ...
   * 如果適用，此選項可協助您[透過提供相關&#x200B;**Live Copy**&#x200B;或&#x200B;**Blueprint**&#x200B;的資訊來監視Live Copy](creating-live-copies.md#monitoring-your-live-copy)。

* **參考** (**網站**)

   * [參考](/help/sites-cloud/authoring/getting-started/basic-handling.md#references)邊欄提供關於&#x200B;**即時副本**&#x200B;的資訊，以及對適當動作的存取。

* **Live Copy概述** (**Sites**)

   * 此控制台允許您[查看和管理您的Blueprint及其Live Copy。](live-copy-overview.md)
...
| 同步 | 用於同步源和Live Copy之間內容的通用術語（由&#x200B;**Rollout**&#x200B;和&#x200B;**Synchronize**&#x200B;選項共同使用） |  |
...
在上一個示例中， `/content/wknd/language-masters/en`是全局主網站，用英語表示。 若要重複使用此網站的內容，會建立MSM Live Copy:

* `/content/wknd/language-masters/en`下方的內容為來源。
* `/content/wknd/language-masters/en`下的內容會複製到`/content/wknd/us/en/`和`/content/wknd/ca/en`節點下。 這些是即時副本。
* 作者對`/content/wknd/language-masters/en`下方的頁面進行變更。
...
可以根據需求[已同步](creating-live-copies.md#synchronizing-your-live-copy)更改。

![即時副本組成概觀](../assets/live-copy-composition.png)

#### 具有非即時副本頁面的即時副本{#live-copy-with-non-live-copy-pages}
...
* MSM可能會以特殊情況處理的情況。 例如，當您（或程式）在來源/Blueprint和Live Copy分支中建立位置和名稱相同的頁面時。 如需詳細資訊，請參閱[MSM轉出衝突](rollout-conflicts.md) 。

![包含非Live Copy頁面的Live Copy](../assets/live-copy-with-non-live-copy-pages.png)

#### 巢狀即時副本{#nested-live-copies}

當您（或程式）在現有即時副本中建立[新頁面時，此新頁面也可設為不同藍圖的即時副本。 ](#live-copy-with-non-live-copy-pages)這稱為巢狀即時副本。 在巢狀即時副本中，第二個或內部即時副本的行為會以下列方式受到第一個或外部即時副本的影響：
...
當即時副本建立為淺層即時副本的子項時，即稱為堆疊即時副本。 其行為方式與[巢狀即時副本](#nested-live-copies)相同。
...
* 允許作者在Blueprint上使用&#x200B;**轉出**&#x200B;選項。 也就是說，將修改明確推送至繼承自此Blueprint的Live Copy。
...
* [建立Blueprint設定](creating-live-copies.md#creating-a-blueprint-configuration)  — 此設定會預先定義要用來建立Live Copy的頁面。
* [建立頁面的即時副本](creating-live-copies.md#creating-a-live-copy-of-a-page)  — 用來建立即時副本的頁面（來源頁面）為Blueprint頁面。Blueprint設定可能參照或未參照來源頁面。

### 轉出並同步{#rollout-and-synchronize}

轉出是與即時副本與其來源同步的中央MSM動作。 您可以手動執行轉出，也可以自動執行。

* 可定義[轉出設定](#rollout-configurations)，使得特定[事件](live-copy-sync-config.md#rollout-triggers)可導致自動轉出。
* 編寫Blueprint頁面時，您可以使用&#x200B;**[Rollout](creating-live-copies.md#rolling-out-a-blueprint)**&#x200B;命令將變更推送至Live Copy。
   * **Rollout**&#x200B;命令可在Blueprint配置引用的Blueprint頁面上使用。

   ![轉出](../assets/live-copy-rollout.png)

* 編寫Live Copy頁時，可以使用&#x200B;**[Synchronize](creating-live-copies.md#synchronizing-a-live-copy)**&#x200B;命令將更改從源提取到Live Copy。
   * 無論源/Blueprint頁是否被Blueprint配置包圍，Live Copy頁上始終都可以使用&#x200B;**Synchronize**&#x200B;命令。
...
轉出設定可重複使用，因此多個Live Copy可使用相同的轉出設定。 標準安裝中包含數個[轉出設定](live-copy-sync-config.md#installed-rollout-configurations)。

### 轉出衝突{#rollout-conflicts}

轉出可能會變得複雜，尤其是當作者同時編輯來源和即時副本中的內容時。 因此，請務必注意AEM如何處理轉出期間可能發生的任何[衝突。](rollout-conflicts.md)
...
您可以&#x200B;**暫停**&#x200B;即時副本繼承的即時副本頁面，以便您可以變更頁面屬性和元件。 暫停繼承時，頁面屬性和元件不再與源同步。

編輯個別頁面時，作者可以&#x200B;**取消元件的繼承**。 取消繼承時，即時關係會暫停，且該元件不會同步。 當需要自訂內容的子區段時，取消繼承和同步很實用。

### 分離即時副本{#detaching-a-live-copy}

您也可以從Blueprint中[分離Live Copy](creating-live-copies.md#detaching-a-live-copy)以移除所有連線。
...
>[!TIP]
>
>如需完整詳細資訊，包括對子頁面和上層頁面的相關影響，請參閱[分離即時副本](creating-live-copies.md#detaching-a-live-copy) 。
...
1. 決定要使用的轉出設定。
   ...
   1. MSM [安裝數個可滿足許多使用案例的轉出設定](live-copy-sync-config.md#installed-rollout-configurations)。
   1. 您可以視需要選擇建立轉出設定](live-copy-sync-config.md#creating-a-rollout-configuration)。[

1. 決定您需要在哪個位置[指定要使用](live-copy-sync-config.md#specifying-the-rollout-configurations-to-use)的轉出設定，並視需要進行設定。
1. 如有需要，請[建立Blueprint設定](creating-live-copies.md#creating-a-blueprint-configuration)以識別即時副本的來源內容。
1. [建立即時副本。](creating-live-copies.md#creating-a-live-copy)
1. 視需要變更來源內容。 您應採用貴組織已建立的一般內容審核和核准程式。
1. [滾](creating-live-copies.md#rolling-out-a-blueprint) 出Blueprint，或將 [Live Copy](creating-live-copies.md#synchronizing-a-live-copy) 與變更同步。

## 自訂MSM {#customizing-msm}

MSM提供工具，讓您的實作能因應共用內容時可能存在的異常複雜情況。

* **自訂轉出設定**  — 當 [安裝的轉](live-copy-sync-config.md#creating-a-rollout-configuration) 出設定不符合您的需求時，建立轉出設定。您可以使用任何可用的轉出觸發器和同步動作。
...
[MSM最佳實務](best-practices.md)頁面包含與您的實作相關的重要資訊。
