...
* 檢查 [MSM常見問題集](#faq) 因為您的問題或疑問可能已在那裡解決。
* 檢查 [MSM最佳實務文章](best-practices.md) 因為其中提供幾個秘訣，並澄清一些誤解。
...
* `cq:LiveSync` mixin型別
   ...
   * 這些頁面具有 `cq:LiveSyncConfig` 型別的子節點 `cq:LiveCopy` 透過下列屬性包含即時副本的基本和必要資訊：
...
沒有 **轉出** 可為Adobe Experience Manager主體（使用者或群組）設定或移除的許可權。
...
但在後一種情況下，如果頁面沒有 `cq:LiveRelationship` 屬性以相同名稱存在，此頁面會重新命名，以便在建立即時副本頁面之前。
...
為了避免此情況，請使用即時副本 **暫停** 功能而非 **分離**. 更多詳細資訊，請參閱 **分離** 動作可在以下位置找到： [本文章。](creating-live-copies.md)
