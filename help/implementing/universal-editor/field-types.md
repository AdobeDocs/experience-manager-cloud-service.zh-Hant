...
只要在`model`檔案的`components`陣列中的元件物件中包含`component-definition.json`屬性即可。
...
| `multi` | `boolean` | 欄位是否為多欄位<br/>請注意，屬性面板中的多欄位不允許容器巢狀化 | 否 |
...
>[!TIP]
>
>請參閱檔案[管理分類資料](https://www.aem.live/docs/authoring-taxonomy)，瞭解如何使用試算表來管理Edge Delivery Services專案的分類資料的詳細資訊。
...
容器元件型別可將包含多欄位支援的元件分組。 它提供額外設定。 請注意，屬性面板中的多個欄位不允許容器巢狀
