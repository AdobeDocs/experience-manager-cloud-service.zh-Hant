...
### aue&amp;amp；冒號；導覽 {#navigate}

在將導覽轉送至Universal Editor Shell之前，Universal Editor CORS程式庫(`@aem-sites/universal-editor-cors`)會傳送可取消的`aue:navigate`事件。
...
* **變更**&#x200B;以修改`.html`導覽URL （例如，附加`event.detail.href`副檔名）。
* **完全透過**&#x200B;取消`event.preventDefault()`導覽。
