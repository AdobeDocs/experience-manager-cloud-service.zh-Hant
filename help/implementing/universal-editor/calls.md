...
此外掛程式也包含`content`物件，其中包含每個外掛程式](/help/implementing/universal-editor/architecture.md)要儲存之內容的特定於端點的詳細資料[的其他物件。 例如，如果您的應用程式是以AEM和Magento的內容為基礎，裝載會包含每個系統的資料物件。
