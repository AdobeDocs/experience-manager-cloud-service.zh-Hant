...
## 定義應開啟通用編輯器的內容路徑或`sling:resourceType`及其方式。 (選用) {#content-paths}

若您有現有的 AEM 專案使用[頁面編輯器](/help/sites-cloud/authoring/page-editor/introduction.md)，則當內容作者編輯頁面時，頁面會自動使用頁面編輯器開啟。您可以根據內容路徑或`sling:resourceType`定義應開啟哪個編輯器AEM以及應如何開啟，讓您的作者獲得流暢的體驗，無論所選內容需要哪個編輯器。

1. 開啟 Configuration Manager。

   `http://<host>:<port>/system/console/configMgr`

1. 在清單中找到&#x200B;**`Universal Editor URL Service`**&#x200B;並按一下&#x200B;**編輯組態值**。

1. 定義通用編輯器應針對哪些內容路徑或 `sling:resourceType` 而開啟。

   * 在&#x200B;**`mappings`**&#x200B;欄位中，提供已開啟通用編輯器的路徑。
   * 在&#x200B;**`resourcetypes`**&#x200B;欄位中，提供由通用編輯器直接開啟的資源清單。

1. 定義應如何開啟編輯器。

   * 核取&#x200B;**`aemdomain`**&#x200B;選項以在AEM網域（例如`https://author-pXX-eYYY.adobeaemcloud.com...`）下開啟編輯器，否則將會在`https://experience.adobe.com...`下開啟。
   * 核取&#x200B;**`editorreleasepreview`**&#x200B;選項以在[預覽環境中自動開啟通用編輯器，](/help/release-notes/universal-editor/preview.md)否則將在目前的發行環境中開啟。
