---
title: 使用Dispatcher Tools（舊版）驗證和調試
description: 使用Dispatcher Tools（舊版）驗證和調試
feature: Dispatcher
hidefromtoc: true
...
# 使用Dispatcher Tools（舊版）驗證和調試  {#Dispatcher-tools-legacy}

## 簡介 {#apache-and-dispatcher-configuration-and-testing}

>[!NOTE]
>有關雲中Dispatcher的詳細資訊以及如何下載Dispatcher Tools，請參見 [雲中的調度程式](/help/implementing/dispatcher/disp-overview.md) 的子菜單。

以下各節介紹舊模式檔案結構、本地驗證、調試以及如何從舊模式遷移到 [柔性模式](/help/implementing/dispatcher/validation-debug.md)。

本文假定項目的調度程式配置不包括檔案opt-in/USE_SOURCES_DIRECTLY。 因此，它在檔案數和大小方面存在限制，例如：

* 必須使用的單個重寫檔案，而不是特定於站點的檔案。
* 可自定義檔案的內容總和必須小於1MB。

自Cloud Manager 2021.7.0版開始，新的Cloud Manager程式將生成原型為28及更高的AEM大型項目結構，其中包括上述檔案。

是 **建議** 從舊模式遷移到靈活模式（如遷移部分中所述） [從舊模式遷移到靈活模式](#migrating-flexible)。 使用靈活模式還會使SDK和運行時以改進的方式驗證和部署配置。

## 檔案結構 {#legacy-mode-file-structure}

項目的Dispatcher子資料夾（在舊模式下）的結構如下：
...
>[!NOTE]
>
>當前，必須使用單個重寫檔案，而不是特定於站點的檔案。 通常，可自定義檔案的內容總和必須小於1MB。
...
## 本地驗證 {#local-validation-legacy-mode}
...
3. 檢查Dispatcher SDK配置檔案的子集（如中所述）是否不可變 [「檔案結構」部分](##legacy-mode-file-structure)，尚未修改。 這是SDK版本2021.1.4738中引入的新檢查，AEM該版本還包括Dispatcher Tools 2.0.36版。在此更新之前，客戶可能錯誤地認為這些不可變檔案的任何本地SDK修改也會應用到雲環境。
...
或者，您可以 **預設** 檔案的版本，其名稱前加單詞 `default_`，例如， `../filters/default_filters.any`。
...
>[!TIP]
>
>要能夠以更少的限制方式包括更多檔案，您可能需要切換到靈活的調度程式配置模式。 請參閱文檔 [使用Dispatcher工具驗證和調試](/help/implementing/dispatcher/validation-debug.md) 的子菜單。
...
此階段也可獨立運行 `validator full -d out src/dispatcher`，生成下一個命令所需的out目錄 `bin/docker_run.sh out host.docker.internal:4503 8080`。
...
此階段也可獨立運行 `validator full -d out src/dispatcher`，生成下一個命令所需的out目錄 `bin/docker_immutability_check.sh out`。

## 調試Apache和Dispatcher配置 {#debugging-apache-and-dispatcher-configuration}

請注意，您可以使用 `./bin/docker_run.sh out docker.for.mac.localhost:4503 8080`。
...
在本地運行Dispatcher時，日誌會直接打印到終端輸出。 大多數時候，您希望這些日誌處於DEBUG中，這可以通過在運行Docker時將調試級別作為參數傳遞來完成。 例如：`DISP_LOG_LEVEL=Debug ./bin/docker_run.sh out docker.for.mac.localhost:4503 8080`。
...
## 從舊模式遷移到靈活模式 {#migrating-flexible}

隨著Cloud Manager 2021.7.0版的推出，新的Cloud Manager程式生成了原型為28或更高AEM的大型項目結構，包括檔案 **opt-in/USE_SOURCES_DIRECTLY**。 這消除了舊模式在檔案數量和大小方面的先前限制，還導致SDK和運行時以改進的方式驗證和部署配置。 如果您的調度程式配置沒有此檔案，強烈建議您遷移。 使用中介紹的方法 [柔性模式](/help/implementing/dispatcher/validation-debug.md#migrating) 的子菜單。
