...
1. URL放入瀏覽器
2. 對DNS中對應至該網域的CDN提出請求
3. 如果內容已完全快取到CDN,CDN會將內容提供給瀏覽器
4. 如果內容未完全快取，CDN會呼叫（反向proxy）給分派程式
5. 如果內容已完全快取在Dispatcher上，則Dispatcher會將內容提供給CDN
6. 如果內容未完全快取，則分派程式會呼叫（反向proxy）至AEM發佈
7. 由瀏覽器呈現的內容
...
1. 受管理的CDN - AEM現成可用的CDN。
2. 未管理的CDN —— 客戶自帶CDN，並負責管理它。

強烈建議使用第一個選項，而Adobe不會因使用第二個選項時任何設定錯誤而負責。

#### 受管理的CDN {#managed-cdn}

在Adobe提供CDN後，您應建立CNAME，將應用程式的網域對應至托管於受管理CDN網域的Adobe控制網域。

CDN是第7層Web應用程式防火牆，需要SSL終止，因此需要客戶簽署的SSL憑證。 在發行前階段，您必須透過手動程式將憑證提供給Adobe。

在正式推出時，您應將憑證上傳至Cloud Manager,Cloud Manager會將憑證上傳至CDN。 當SSL憑證即將到期時，您會收到通知，以便在Cloud manager中重新整理憑證。

#### 未管理的CDN {#unmanaged-cdn}

您可以管理自己的CDN，但需：

1. 您有現有的CDN。
2. 你會搞定的。
3. 您的應用程式不會對AEM架構中尚未容納的CDN進行大量API呼叫。
4. AEM作為雲端服務，可以建立端對端系統正常運作。
5. 您必須提供Adobe要允許的CDN URL白名單。

Adobe將提供AEM cloudURL，以用作您的CDN來源。
...
而應使用複製API方法。 [API文檔](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/day/cq/replication/Replicator.html) ，如需刷新快取的示例，請參閱 [API示例頁](https://helpx.adobe.com/experience-manager/using/aem64_replication_api.html) ，特別是向所有可用代理髮出類型為ACTIVATE的複製操作的CustomStep示例。
...
此發佈觸發的失效與快速啟動相同：當發佈實例從作者（通過複製和管線隊列）收到新版本的頁面或資產時，它使用刷新代理使其調度程式上的適當路徑失效。 更新的路徑會連同其父代一起從調度器快取中刪除，最高可以使用statfiles級別進行配 [置的級別](https://docs.adobe.com/content/help/en/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#invalidating-files-by-folder-level)。
