...
description: 瞭解Dispatcher工具、支援的Apache模組以及舊版和彈性模式。
feature: Dispatcher
...
本頁面介紹 Dispatcher 工具，並說明如何下載和擷取此工具、受支援的 Apache 模組，並提供對傳統模式和靈活模式的概略介紹。此外，還有關於驗證和偵錯以及將 Dispatcher 設定從 AMS 移轉到 AEM as a Cloud Service 的進一步參考。<!-- ERROR: NOT FOUND (HTTP ERROR 404) Also, see [this video](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/cloud-5/cloud5-aem-dispatcher-cloud.html) for additional details about deploying dispatcher files in a cloud service environment. -->
...
對於發佈層級，用於提供 Blob 的預設值是作為附件。使用 Dispatcher 中的標準[內容處置標頭](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Disposition)覆寫此設定。
...
專案的 Apache 和 Dispatcher 資料夾結構根據專案使用的模式略有不同，如上文[使用 Dispatcher 工具進行驗證和偵錯](#validation-debug)章節所述。
