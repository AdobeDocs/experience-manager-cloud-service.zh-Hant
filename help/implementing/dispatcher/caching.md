...
* 如需其他詳細資料，請參閱[使用者端資料庫和版本一致性](#content-consistency)一節，包括[當快取的HTML仍參考無法再使用的長快取URL時，遞補至短快取clientlib URL](#clientlib-shortcache-fallback)。
...
AEM as a Cloud Service中預設會啟用嚴格clientlib版本設定。
...
### 當長快取clientlibs無法使用時，退回短快取URL {#clientlib-shortcache-fallback}

發佈新內容後，中介快取（例如Adobe CDN或瀏覽器快取）可以繼續提供&#x200B;**較舊的HTML**&#x200B;回應。 該快取HTML仍可參考發佈層上已不存在的長快取clientlib URL （`lc-`選擇器模式），因為部署已取代這些成品。

在此情況下，AEM as a Cloud Service會發出&#x200B;**HTTP重新導向**&#x200B;至&#x200B;**short-cache** clientlib URL。 短快取URL使用`sc-`選擇器模式和以時間為基礎的區段，因此請求可以解析為目前可用的&#x200B;**最新**&#x200B;版本使用者端資料庫。
...
**重新導向**&#x200B;至短快取URL與&#x200B;**短快取clientlib回應**&#x200B;本身都是短期：回應使用&#x200B;**最大快取存留期60秒** (`max-age=60`)。 **下游快取** （客戶管理的CDN、代理或類似專案）應該&#x200B;**不**&#x200B;快取這些回應的時間超過此時間。 避免增加TTL的自訂快取規則，或將短快取clientlib URL視為不可變的長期資產；這麼做可能會將使用者串連到過時的JS或CSS上。

此行為表示&#x200B;**使用者端程式庫必須保持回溯相容於HTML**，只要快取HTML，仍可從快取提供服務。 在實踐中，當HTML快取使用一般TTL （包括上述[以上](#html-text)所述的預設值）時，很少出現此問題。 如果您設定&#x200B;**很長** HTML快取存留期，請考量使用者可能會載入較舊的標籤，以及透過短快取遞補傳遞的較新JS和CSS。
