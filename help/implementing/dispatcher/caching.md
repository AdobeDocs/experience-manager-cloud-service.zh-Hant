...
在`Cache-Control`中包含`private`、`no-cache`或`no-store`的原始回應不會由AEM as a Cloud Service的CDN快取（請參閱[&#128279;](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-learn/cloud-service/caching/how-to/disable-caching)如何停用CDN快取）
...
* 可在`global.vars`中定義`DISABLE_DEFAULT_CACHING`變數，以停用預設的HTML/文字快取設定：
...
可以使用[CDN設定](https://experienceleague.adobe.com/zh-hant/docs/experience-manager-cloud-service/content/implementing/content-delivery/cdn-configuring-traffic#request-transformations)中的`requestTransformations`旗標來開啟或關閉此功能。
...
1. 儲存變更。 不必將此配置保存在原始程式碼管理中，因為作為Cloud Service AEM會自動在開發、階段和生產環境中啟用此配置。
1. 每當更改用戶端資料庫的內容時，都會生成新的雜湊密鑰並更新 HTML 引用。
