...
流量通過CDN傳遞到Apache Web伺服器層，該層支援包括Dispatcher的模組。 為了提高效能，Dispatcher主要用作快取以限制對發佈節點的處理。
規則可應用到Dispatcher配置以修改任何預設的快取過期設定，從而在CDN處進行快取。 請注意，如果 `enableTTL` 在Dispatcher配置中啟用，這意味著即使在重新發佈的內容之外，它也會刷新特定內容。

本頁還介紹如何使Dispatcher快取無效，以及在客戶端庫方面如何在瀏覽器級別進行快取。
...
* 可以在更細的粒度級別上覆蓋，包括使用以下Apache獨立控制CDN和瀏覽器快取 `mod_headers` 指令：
   ...
   在設定全局快取控制標頭或與寬規則運算式匹配的標頭時，請小心謹慎，以便這些標頭不會應用於需要保持私有的內容。 考慮使用多個指令來確保以細粒度方式應用規則。 如此，如AEM果as a Cloud Service檢測到快取頭已應用到Dispatcher檢測到的不可執行的內容，則它將刪除快取頭，如Dispatcher文檔中所述。 為了強制始終AEM應用快取頭，您可以添加 **總是** 選項：
...
* 雖然設定為私有的HTML內容將不會在CDN中快取，但是，如果 [權限敏感快取](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/permissions-cache.html?lang=zh-Hant) 已配置，確保只能向授權用戶提供內容。
   ...
   >[!NOTE]
   >請注意， Dispatcher仍可能根據其自己的內容快取內容 [快取規則](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17497.html)。 要使內容真正私有，您應確保Dispatcher不快取該內容。

### 客戶端庫(js,css) {#client-side-libraries}

* 使用AEM客戶端庫框架時，JavaScript和CSS代碼的生成方式是瀏覽器可以無限期地快取它，因為任何更改都以具有唯一路徑的新檔案的形式顯示。  換句話說，將根據需要生成引用客戶端庫的HTML，以便客戶在發佈新內容時能夠體驗新內容。 對於不尊重「不可變」值的較舊瀏覽器，快取控制設定為「不可變」或30天。
...
在這兩種情況下，可以使用Apache在Apache/Dispatcher層的更細粒度級別上覆蓋快取標頭 `mod_headers` 指令，例如：
...
在Dispatcher層修改快取標頭時，請小心不要快取太廣，請參閱HTML/文本部分中的討論 [上](#html-text)。 另外，確保本應保持私有（而非快取）的資產不屬於 `LocationMatch` 指令篩選器。
...
目前，無法在調度程式中使用 [權限敏感快取](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/permissions-cache.html?lang=zh-Hant)。 該影像始終從源請求，AEM如果用戶被授權，則提供該影像。
...
* 避免使用 `User-Agent` 作為 `Vary` 標題。 預設Dispatcher安裝程式的舊版本（早於原型版本28）包含此內容，我們建議您使用以下步驟將其刪除。
...
* 下面是各種內容類型的一些示例，在設定自己的快取規則時，這些內容可作為指南。 請仔細考慮並test您的特定設定和要求：
   ...
   * 快取HTML頁面5分鐘，後台刷新1h（在瀏覽器上）,CDN上刷新12h。 始終會添加Cache-Control標頭，因此必須確保/content/*下的匹配html頁面是公開的。 如果不是，請考慮使用更具體的規則運算式。
...
當在HEADCDN上接收到Adobe請求時， **不** 快取後，該請求被調度程式和/或實例轉AEM換並作為GET請求接收。 如果響應是可快取的，則將從CDN提供後續HEAD請求。 如果響應不可快取，則後續HEAD請求將在一段時間內傳遞給Dispatcher和/AEM或實例，該時間段取決於 `Cache-Control` TTL。

### 市場營銷活動參數 {#marketing-parameters}

網站URL通常包括用於跟蹤市場活動成功的市場營銷活動參數。 為了有效地使用調度程式快取，建議您配置調度程式配置 `ignoreUrlParams` 屬性 [此處](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html?lang=en#ignoring-url-parameters)。

的 `ignoreUrlParams` 必須取消對部分的注釋，並應引用檔案 `conf.dispatcher.d/cache/marketing_query_parameters.any`。 可通過取消注釋與您的市場營銷渠道相關的參數對應的行來修改檔案。 也可以添加其他參數。
...
通常，不必使Dispatcher快取無效。 相反，當內容重新發佈時，您應依靠Dispatcher刷新其快取，而CDN則遵守快取過期標頭。

### 激活/停用期間Dispatcher快取無效 {#cache-activation-deactivation}

與以前版本的頁AEM面一樣，發佈或取消發佈頁面會從Dispatcher快取中清除內容。 如果懷疑存在快取問題，您應重新發佈有關的頁面，並確保虛擬主機與 `ServerAlias` localhost ，這是Dispatcher快取失效所必需的。

>[!NOTE]
>要獲得正確的分派程式失效，請確保來自&quot;127.0.0.1&quot;、&quot;localhost&quot;、&quot;。local&quot;、&quot;。adobeamcloud.com&quot;和&quot;。adobeamcloud.net&quot;的請求都由vhost配置匹配和處理，以便能夠提供這些請求。 可以通過以下任一方法完成：在參照中的模式後，在捕獲全部vhost配置中全局匹配「*」 [AEM原型](https://github.com/adobe/aem-project-archetype/blob/develop/src/main/archetype/dispatcher.cloud/src/conf.d/available_vhosts/default.vhost) 或者確保上述清單被某個主機捕獲。

當發佈實例從作者處收到頁面或資產的新版本時，它使用刷新代理使其Dispatcher上的適當路徑無效。 更新的路徑將從Dispatcher快取及其父快取中刪除到一個級別(您可以使用 [statfilesel（狀態檔案級）](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#invalidating-files-by-folder-level))。

## Dispatcher快取的顯式失效 {#explicit-invalidation}

Adobe建議依靠標準快取頭來控制內容交付生命週期。 但是，如果需要，可以直接使Dispatcher中的內容無效。
...
* 當必須保證每個事件（例如與需要準確知識的外部系統同步）時，需要SCD API。 如果在無效調用時存在發佈層升級事件，則當每個新發佈處理無效時引發附加事件。

* 使用複製API不是常見的使用情形，但應用於使快取失效的觸發器來自發佈層而不是作者層的情況。 如果配置了調度程式TTL，則此功能可能非常有用。

總之，如果您希望使Dispatcher快取失效，建議的選項是使用作者提供的SCD API Invalidate操作。 此外，您還可以偵聽事件，這樣您就可以觸發更多下游操作。
...
>[!NOTE]
>
>AdobeCDN在Dispatcher失效時不刷新。 Adobe管理的CDN考慮TTL，因此不需要刷新它。

### 複製API {#replication-api}

下面是使用複製API停用操作時的實現模式：

1. 在發佈層上，調用複製API以觸發發佈Dispatcher刷新複製代理。

刷新代理終結點不可配置，但是預配置為指向Dispatcher，與與刷新代理一起運行的發佈服務匹配。
...
1. 儲存變更。無需在原始碼管理中保存此配置，AEM因為as a Cloud Service會在開發、階段和生產環境中自動啟用此配置。
