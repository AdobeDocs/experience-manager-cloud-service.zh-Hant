...
>[!TIP]
>
>有關預先定義角色的詳細資訊，請參閱檔案 [AEMas a Cloud Service團隊和產品設定檔。](/help/onboarding/aem-cs-team-product-profiles.md)
...
>[!NOTE]
>
>您必須在Adobe Experience Manager as a Cloud Service的Admin Console中擁有產品管理員許可權，才能為Cloud Manager建立新設定檔和管理許可權。
...
1. 在Admin Console中，選取 [您剛建立的新產品設定檔。](#create)
...
1. 選取加號(`+`)圖示來將許可權專案新增至欄 **包含的許可權專案**.

   * 選取 `i` 圖示加以存取，以瞭解更多相關資訊。

1. 選取 **全部新增** 按鈕上方的 **可用許可權** 欄以新增所有許可權。
...
1. 在Admin Console中，選取 [您剛指派自訂許可權的新產品設定檔。](#assign-permissions)
...
| 術語 | 說明 |
|---|---|
| 預先定義的權限 | 預先定義的角色，例如 **企業所有者**， **部署管理員**&#x200B;等等，以控管Cloud Manager的各種功能。 有關預先定義角色的詳細資訊，請參閱檔案 [AEMas a Cloud Service團隊和產品設定檔。](/help/onboarding/aem-cs-team-product-profiles.md) |
| 自訂權限 | Cloud Manager 功能，允許使用者建立權限設定檔來定義角色以管理 Cloud Manager 支援的功能 |
| 產品設定檔 | 在 Admin Console 中建立，用於管理可設定的權限，該權限將適用於屬於權限設定檔的使用者 |
...
* 自訂許可權設定檔在設定許可權時，也將列出AMS程式、環境和管道。
* 在Cloud Manager中建立的計畫、環境、管道等資源可能需要兩分鐘才能在Admin Console中顯示許可權設定。
