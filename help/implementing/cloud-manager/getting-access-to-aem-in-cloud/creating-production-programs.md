...
1. 在&#x200B;**安全性**&#x200B;標籤上，選取您要使用的安全性選項。 請參閱[安全性](#security)。

   在設定生產精靈中的![安全性索引標籤](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/assets/create-production-program-security.png)
...
<!--   ![Select add-ons](assets/setup-prod-commerce.png) -->
    ...
    >[！NOTE]
    >
    >如果您的程式使用Edge Delivery Services來傳遞，則可能不需要發佈階層。 使用彈性的發佈層級功能(Beta)，您可以設定是否在解決方案和附加元件索引標籤上布建發佈層級。 請參閱[彈性發佈階層(Beta)](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/creating-production-programs.md#flexible-publish-tier)。
    
    ![選取解決方案](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/assets/create-production-program-solutions.png)

1. 按一下「**繼續**」。

1. 在&#x200B;**傳遞型別**&#x200B;索引標籤中，請注意，它是根據上一步驟中選擇的解決方案和附加元件預先填入。 如果您選取&#x200B;**AEM Publish**，稍後可依需求布建。

   ![傳遞型別索引標籤](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/assets/create-production-program-delivery-type.png)
...
1. 如果您有必要的權益，**SLA**&#x200B;標籤會顯示為&#x200B;**`Set up for production`**&#x200B;對話方塊中的第二個或第三個標籤。 請參閱[SLA](#sla)。

   ![SLA選項](assets/create-production-program-sla.png)

   Sites和Forms提供標準的99.9% service level agreement (SLA)。

1. 按一下「**繼續**」。

1. 在&#x200B;**上線日期**&#x200B;索引標籤上，輸入您計畫讓生產計畫上線的日期。

   ![缺少規劃的上線日期](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/assets/create-production-program-go-live-date.png)
...
1. 按一下「**建立**」。Cloud Manager會建立您的程式，並將其顯示在登陸頁面上以供選取。
   ...
   ![Cloud Manager 概觀](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/assets/create-production-program-my-programs.png)
...
![安全性選項](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/assets/create-production-program-security.png)
...
* **HIPAA** — 此選項可啟用Adobe的HIPAA就緒解決方案實作。
   * 深入瞭解[Adobe Experience Manager as a Cloud Service的HIPAA整備程度](/help/compliance/hipaa/hipaa-readiness.md)以及[Adobe的HIPAA整備解決方案實作](https://www.adobe.com/trust/compliance/hipaa-ready.html)。
   * HIPAA無法在方案建立後啟用或停用。
...
### 彈性發佈階層(Beta) {#flexible-publish-tier}

>[!NOTE]
>
>此處所述的彈性發佈層級位於Beta中。 若要加入Beta，請使用您的Adobe組織ID和計畫ID透過電子郵件傳送[grp-beta_xwalk-publish_config@adobe.com](mailto:grp-beta_xwalk-publish_config@adobe.com)。

如果您的組織已啟用彈性的發佈層級功能，您可以設定方案的環境是否需要發佈層級。 此選項會顯示在&#x200B;**為生產設定**&#x200B;對話方塊的&#x200B;**傳遞型別**&#x200B;索引標籤中（在[程式建立](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/creating-production-programs.md)期間）。

![生產設定精靈中的傳遞型別標籤](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/assets/create-production-program-delivery-type.png)

它也出現在&#x200B;**編輯程式**&#x200B;對話方塊中（當您[編輯程式](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/editing-programs.md)時）。

![編輯程式對話方塊，其中顯示傳送型別選項](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/assets/edit-program-delivery-type.png)

並非所有架構都需要發佈階層。 下表顯示哪些架構需要發佈層級，哪些不需要發佈層級：
...
* 程式中的所有新環境預設僅以&#x200B;**作者階層**&#x200B;布建。 使用者介面中顯示的資訊性訊息會確認此行為。
* 如果使用者在建立方案期間選取&#x200B;**AEM發佈**，發佈層級就會啟動並布建為&#x200B;*新環境*。
* 您稍後也可以編輯程式來啟用發佈階層。 請參閱[編輯程式](/help/implementing/cloud-manager/getting-access-to-aem-in-cloud/editing-programs.md)。

>[!NOTE]
>
>如果您的程式使用Edge Delivery Services進行內容傳送，並使用AEM Author建立內容，則不需要發佈階層。 內容是透過Edge Delivery傳遞，不會透過AEM發佈層級。 請參閱關於Edge Delivery Services與AEM製作(Beta) 。
