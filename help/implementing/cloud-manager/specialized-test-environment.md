...
專業化測試環境是您可以建立的新型Cloud Manager環境。 其設計可支援進階使用案例，例如使用者驗收測試(UAT)和效能驗證。 和傳統的開發、快速開發或測試環境不同，專業測試環境會在生產部署管道之外操作。 因此，它們可為您提供更大的彈性，同時維持嚴格的隔離以防止干擾生產工作流程。

專門測試環境的建置可反映典型中繼環境的大小、擴充性和設定。 此方法可確保在Specialized Testing Environment中執行的測試可產生實際見解，瞭解程式碼和內容在類似生產的條件中如何執行。 此環境也支援從「生產」或「預備」直接複製內容。 此外，在部署工作流程、存取控制和網路設定方面，也能維持與開發環境的同位性。

## 專業測試環境的主要功能與設定 {#key-features}
...
1. 在「**新增環境**」對話框中，執行下列操作：
   ...
   * （選用）提供環境的&#x200B;**描述**。
   * 從下拉式清單中選取&#x200B;**主要區域**。 建立後，專用測試環境的主要區域(例如&#x200B;*英國（南部）*)就會鎖定，無法變更。
