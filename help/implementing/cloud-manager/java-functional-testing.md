---
title: Java功能測試
description: 了解如何編寫AEMas a Cloud Service的Java功能測試
...
了解如何編寫AEMas a Cloud Service的Java功能測試
...
1. Cloud Manager 中的測試將使用技術管理員使用者來執行。

>[!NOTE]
>
>若要從本機電腦執行功能測試，請建立一個具備類似管理員權限的使用者來達到相同的行為。

1. 功能測試範圍內的集裝箱化基礎架構受以下邊界限制：


| 類型 | 值 | 說明 |
|----------------------|-------|--------------------------------------------------------------------|
| CPU | 0.5 | 每次測試執行保留的CPU時間量 |
| 記憶體 | 0.5Gi | 分配測試的記憶體量（以GB為單位） |
| 逾時 | 30m | 測試將在其後終止的持續時間。 |
| 建議的持續時間 | 15m | 建議您編寫測試時間不要超過此次。 |

>[!NOTE]
>
> 若您需要更多資源，請建立客戶服務案例並說明您的使用案例；我們的團隊將審核您的請求並提供適當協助。
...
| 屬性 | 說明 | 範例 |
|-------------------------------------|------------------------------------------------------------------|-------------------------|
| `sling.it.instances` | 為了符合雲端服務，應將例項數設為 `2` | `2` |
| `sling.it.instance.url.1` | 應設為作者URL | `http://localhost:4502` |
| `sling.it.instance.runmode.1` | 第一個例項的執行模式應設為 `author` | `author` |
| `sling.it.instance.adminUser.1` | 應設為作者管理員使用者。 | `admin` |
| `sling.it.instance.adminPassword.1` | 應設為作者管理員密碼。 |  |
| `sling.it.instance.url.2` | 應設為發佈URL | `http://localhost:4503` |
| `sling.it.instance.runmode.2` | 第二個執行個體的執行模式應設為 `publish` | `publish` |
| `sling.it.instance.adminUser.2` | 應設為「發佈管理員」使用者。 | `admin` |
| `sling.it.instance.adminPassword.2` | 應設為「發佈管理員密碼」 。 |  |
