...
一鍵式建立您的第一個 Edge Delivery Site 之功能，目的為協助您在 Cloud Manager 中自動完成 Edge Delivery Sites 上線和部署的工作。 只需按一下按鈕即可，大幅簡化了流程。 按一下即可布建所需的基礎架構、與GitHub整合以進行版本控制，以及在Google Drive中設定您的檔案和資產儲存空間。

這種自動化有助於減少設定初始網站所需的手動工作。 這可確保無縫的工作流程和可擴縮性，並在管理 Edge 內容方面提高團隊的效能。

>[!IMPORTANT]
>
>一鍵式Edge Delivery網站布建針對概念證明網站和快速原型進行最佳化。 針對生產工作負載，Adobe建議您將網站移轉至專用的Edge Delivery Services組織。
...
1. 按一下&#x200B;**立即建立網站**。
   ...
   當Cloud Manager完成網站布建與驗證時，**網站名稱** （您先前輸入的專案名稱）會顯示在Edge Delivery頁面的&#x200B;**Edge Delivery網站**&#x200B;清單方塊中。 **已驗證**&#x200B;狀態列的左側也會出現綠色圓點。
...
1. 在 Edge Delivery 頁面上，執行下列任一項操作：
   ...
   | 發佈網站 | <ul><li> 在 **Edge Delivery 網站**&#x200B;清單方塊中，按一下網站名稱最右端的「![更多](https://spectrum.adobe.com/static/icons/workflow_18/Smock_More_18_N.svg)」圖示開啟下拉式選單。</li><li>在下拉式功能表中按一下![發佈檢查圖示](https://spectrum.adobe.com/static/icons/workflow_18/Smock_PublishCheck_18_N.svg) **發佈網站**。<br>會顯示一則快顯通知訊息，通知您已成功開始發佈網站。</li></ul> |
...
* **來自Cloud Manager** -
   ...
   1. 在&#x200B;**總覽**&#x200B;頁面的&#x200B;**發佈傳遞**&#x200B;標籤上，在&#x200B;**環境**&#x200B;卡片中，按一下醒目提示的![資訊或資訊圖示](https://spectrum.adobe.com/static/icons/ui_18/InfoMedium.svg)。
