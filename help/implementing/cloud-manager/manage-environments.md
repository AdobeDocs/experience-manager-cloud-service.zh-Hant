...
1. 在 **[我的計畫](/help/implementing/cloud-manager/navigation.md#my-programs)** 主控台，點選或按一下要新增環境的計畫。

1. 在 **[我的計畫](/help/implementing/cloud-manager/navigation.md#my-programs)** 主控台，按一下 **新增環境** 於 **環境** 卡片以新增環境。
...
現在&#x200B;**概觀**&#x200B;畫面會在&#x200B;**環境**&#x200B;卡中顯示您的新環境。現在您可以設定新環境的管道。
...
從 **概觀** 頁面中，您可以透過兩種方式存取環境的詳細資訊。

1. 從 **概觀** 頁面，按一下 **環境** 標籤進行瀏覽。
...
1. 請耐心等候。 Cloud Manager會不斷嘗試復原區域，並且隨時可能提供使用。
1. 如果問題在數小時後仍持續存在，您可以移除其他發佈區域，並重新新增該區域（相同區域或其他區域），以觸發完整部署。

在採取其他動作之前等待系統自行恢復的時間，取決於該區域的故障對您系統的影響。

無論如何， [流量一律會路由至另一個最接近的線上區域。](/help/operations/additional-publish-regions.md) 如果您持續看到問題，請聯絡Adobe客戶服務。
...
>[!NOTE]
> 截至2024年，開發執行個體和某些沙箱計畫已自動更新，因此無需手動管理其更新。 由於此轉變，可能無法使用為開發執行個體手動更新環境的選項 _部分_ 您的程式。

此 **更新** 選項可從以下網址取得： **環境** 卡片適用於沙箱計畫中的某些開發環境和環境，方法是按一下環境的省略符號按鈕。
...
按一下個別環境的省略符號選單時，此選項也可從&#x200B;**概觀**&#x200B;視窗的&#x200B;**環境**&#x200B;索引標籤中使用。
...
此外，您可以從&#x200B;**概觀**&#x200B;頁面的&#x200B;**環境**&#x200B;索引標籤本機登入。

![從環境索引標籤本機登入](assets/environ-login-locally-2.png)

## 管理客戶網域名稱 {#manage-cdn}

Cloud Manager for Sites計畫支援發佈和預覽服務的自訂網域名稱。
...
要管理 IP 允許清單，請瀏覽到計畫&#x200B;**概觀**&#x200B;頁面的&#x200B;**環境**&#x200B;索引標籤。按一下單個環境，以便您可以管理其詳細資訊。

### 套用 IP 允許清單 {#apply-ip-allow-list}

套用IP允許清單會將允許清單定義中包含的所有IP範圍與環境中的作者或發佈服務相關聯。
