...
1. 在出現的&#x200B;**新增環境**&#x200B;對話框中：
   ...
   * 提供環境&#x200B;**名稱**。
   * 提供環境&#x200B;**說明**。
   * 如果要新增 **生產 + 中繼** 環境，則需要為生產環境和中繼環境提供環境名稱和說明。
   * 從下拉清單中選取一個&#x200B;**主要區域**。
      * 請注意，這在建立後無法改變。
...
具有&#x200B;**業務負責人**&#x200B;角色的使用者可設定生產和中繼環境，除了主要區域外，最多可包含最多三個額外的發佈區域。額外的發佈區域可提高可用性。如需詳細資訊，請參閱[「其他發佈區域」文件](/help/operations/additional-publish-regions.md)。
...
新增環境時，除了主要區域外，您可以選擇設定其他區域。

1. 選取&#x200B;**主要區域**。
   * 請注意，環境建立後，這可能即無法變更。
1. 選取「**新增其他發佈區域**」選項，一個新的「**其他發佈區域**」下拉式清單隨即出現。
1. 在「**其他發佈區域**」下拉式清單中，選取一個額外區域。
1. 選取區域會新增到下拉式清單下方以指出其選取範圍。
   * 點選或按一下選取區域旁邊的 X，即可將其取消選取。
1. 從「**其他發佈區域**」下拉式清單中選取另一個區域，以新增另一個區域。
...
如果您未指定任何其他區域，[您可以稍後在建立環境後再執行此步驟。](#edit-regions)

如果您想要針對計畫佈建[進階網路](/help/security/configuring-advanced-networking.md)，建議在使用 Cloud Manager API 將其他發佈區域新增至該環境之前執行此步驟。否則，其他發佈區域的流量會通過主要區域的代理程式。

### 編輯多個發佈區域 {#edit-regions}

如果您最初沒有指定任何其他區域，並且您擁有必要的權益，則可以在建立環境後再執行此步驟。

您也可以移除其他發佈區域。但是，在一次異動中，您只能新增或移除區域。如果您需要新增一個區域並移除一個區域，請先新增、儲存您的變更，然後再進行移除 (反之亦然)。

1. 從計畫的「計畫概觀」主控台中，按一下生產環境的省略號按鈕並從選單中選取「**編輯**」。

   ![編輯環境](assets/select-edit-environment.png)

1. 在「**編輯生產環境**」對話框中，對其他發佈區域進行必要的變更。
   * 使用「**其他發佈區域**」下拉式清單，以選取額外區域。
   * 按一下選取的其他發佈區域旁邊的 X，即可將其取消選取。
...
如果您想要針對計畫佈建[進階網路](/help/security/configuring-advanced-networking.md)，建議在將其他發佈區域新增至該環境前執行此步驟。否則，其他發佈區域的流量會通過主要區域的代理程式。
...
>[!TIP]
>
>檢視檔案 [AEMas a Cloud Service團隊和產品設定檔](/help/onboarding/aem-cs-team-product-profiles.md) 以瞭解AEMas a Cloud Service團隊和產品設定檔如何能夠授與和限制您的授權Adobe解決方案的存取權。
