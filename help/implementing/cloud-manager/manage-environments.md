...
如果您要為程式布建[進階網路](/help/security/configuring-advanced-networking.md)，建議先完成此布建，再使用Cloud Manager API將其他發佈區域新增至環境。 否則，額外發佈區域的流量會通過主要區域的代理程式。
...
如果您要為程式布建[進階網路](/help/security/configuring-advanced-networking.md)，建議先完成此布建，然後再將其他發佈區域新增到環境。 否則，額外發佈區域的流量會通過主要區域的代理程式。
...
已在瀏覽器中開啟新索引標籤，其登入頁面為&#x200B;**Developer Console**。
