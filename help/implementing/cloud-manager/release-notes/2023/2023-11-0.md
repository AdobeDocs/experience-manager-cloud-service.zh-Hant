...
如果您使用 GitHub 來管理您的存放庫，[現在您可以透過 Cloud Manager 直接在 GitHub 存放庫中驗證程式碼](/help/implementing/cloud-manager/managing-code/private-repositories.md)。此整合免除了與Adobe存放庫一致同步程式碼的必要，並可讓您在合併提取請求至主要分支之前先驗證提取請求。
