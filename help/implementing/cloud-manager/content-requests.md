...
對於使用現成CDN的客戶，Cloud Service內容請求是透過伺服器端資料收集來測量。 系統會透過CDN記錄分析啟用此集合。 透過自動分析源自AEMas a Cloud ServiceCDN的記錄檔，可在Adobe Experience Manager as a Cloud Service邊緣的伺服器端自動收集內容請求。 這是透過隔離傳回HTML的請求來完成的 `(text/html)` 或JSON `(application/json)` 來自CDN的內容，且以下文詳述的幾個包含和排除規則為基礎。 內容請求會與從CDN快取提供的傳回內容或回到CDN原始來源(AEM Dispatchers)的內容分開進行。

對於使用自己CDN的客戶，使用者端集合可提供互動的更精確反映，確保透過以下方式可靠地測量網站參與度： [實際使用監控](/help/sites-cloud/administering/real-use-monitoring-for-aem-as-a-cloud-service.md) 服務。 這可提供客戶對其頁面流量和效能的進階深入分析。 雖然此功能對所有客戶都有好處，但可提供使用者互動的有代表性反映，從使用者端擷取頁面檢視次數，確保可靠衡量網站參與度。

對於在AEMas a Cloud Service之上自有CDN的客戶，伺服器端報表會產生無法與授權內容請求相比較的數字。 使用 [實際使用監控](/help/sites-cloud/administering/real-use-monitoring-for-aem-as-a-cloud-service.md)，Adobe可反映可靠衡量網站參與度的方式。


### Cloud Service內容請求的差異 {#content-requests-variances}

內容請求在組織的Analytics報告工具中可能有差異，如下表所述。 一般而言， *不要* 使用透過使用者端儀器收集資料的分析工具，報告指定網站的內容請求數量，原因很簡單，這些請求通常取決於使用者是否同意觸發，因此會遺漏相當一部分的流量。 收集記錄檔中資料伺服器端的Analytics工具，或為在AEMas a Cloud Service上新增自己CDN的客戶提供CDN報告，可提供較佳的計數。 若要報告頁面檢視及其相關效能， [AdobeRUM資料服務](/help/sites-cloud/administering/real-use-monitoring-for-aem-as-a-cloud-service.md) 是Adobe建議的選項。
...
| AEMas a Cloud ServiceNew Relic監控 | 已排除 | AEMas a Cloud Service全域監視。 |
| 供客戶監控其Cloud Service計畫的URL | 已排除 | 建議用於外部監視可用性的URL。<br><br>`/system/probes/health` |
| AEMas a Cloud ServicePod熱身服務 | 已排除 |
