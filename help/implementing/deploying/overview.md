...
由於部署模式而導致的應用程式變更是由交換器啟用的，除了服務使用者、其ACL、節點型別和索引定義變更之外，它們不能依賴可變儲存庫中的變更。
...
* 可透過 jackrabbit 保存庫定義的所有其他內容。例如：
   ...
   * 指令碼 (可以在套件安裝流程的各個階段觸發安裝 Hook。請參閱 [Jackrabbit Filevault 文件](https://jackrabbit.apache.org/filevault/installhooks.html)，瞭解如何安裝 Hook。請注意，AEM CS 目前使用 Filevault 版本 3.4.0，它將安裝 Hook 限制為管理員使用者、系統使用者和管理員群組的成員))。
...
如同 AEM 更新，客戶版本使用滾動部署策略進行部署，以便在適當的情況下消除作者叢集停機時間。事件的一般順序如下所述，其中具有舊版和新版客戶代碼的節點正在執行相同版本的AEM代碼。

* 具有舊版本的節點處於作用中狀態，而且會建立新版本的發行候選版本，並可供使用。
* 如果有任何新的或更新的索引定義，則處理對應的索引。請注意，舊版本的節點將一律使用舊索引，而新版本的節點將一律使用新索引。
* 具有新版本的節點會在舊版本仍提供流量時啟動。
* 具有舊版本的節點正在執行並持續服務，而具有新版本的節點則透過健康狀態檢查來檢查就緒狀態。
* 具有新版本的節點準備好後會接受流量並使用已停止使用的舊版本取代節點。
* 隨著時間推移，具有舊版本的節點會由具有新版本的節點取代，直到僅保留具有新版本的節點為止，從而完成部署。
* 然後會部署任何新的或修改過的可變內容。

## 索引 {#indexes}

新索引或修改的索引在新版本可以接收流量之前，將導致額外的索引或重新索引步驟。 有關 AEM as a Cloud Service 中索引管理的詳細資訊，請參閱[這篇文章](/help/operations/indexing.md)。您可以在 Cloud Manager 建置頁面上檢查索引作業的狀態，並在新版本準備就緒可接收流量時收到通知。

>[!NOTE]
>
>滾動部署所需的時間會因索引大小而異，因為新版本在產生新索引之前無法接受流量。
...
變更存取內容或程式碼的服務使用者和 ACL，可能會導致 AEM 舊版本出現錯誤，從而導致使用過時的服務使用者存取該內容或程式碼。為解決此行為，建議將變更分散到至少兩個版本，在後續版本中清理之前，第一個版本會充當橋接器。

### 索引變更 {#index-changes}

如果對索引進行了變更，新版本必須繼續使用其索引，直到終止為止，而舊版本則使用其自己修改的索引集。 開發人員應遵循[本篇文章](/help/operations/indexing.md)中描述的索引管理技術。

### 針對回復作業的保守編碼 {#conservative-coding-for-rollbacks}

如果在部署後報告或偵測到失敗，則可能需要復原到舊版本。 建議確保新程式碼與該新版本建立的任何新結構相容，因為不會復原新結構（任何可變的內容內容）。 如果舊程式碼不相容，則需要在後續的客戶版本中套用修復。
...
* OSGI 設定執行模式必須參照快速開發環境、開發、預備、生產環境，或作者、發佈服務。支援 `<service>.<environment_type>` 的組合，但必須按此特定順序 (例如 `author.dev` 或 `publish.prod`) 使用。應直接從程式碼中參考 OSGI 權杖，而不是使用 `getRunModes` 方法，後者在執行階段將不再包含 `environment_type`。如需詳細資訊，請參閱[為 AEM as a Cloud Service 設定 OSGi](/help/implementing/deploying/configuring-osgi.md)。
* OSGI 套裝執行模式限制在服務 (作者、發佈)。預執行模式 OSGI 套裝應安裝在 `install.author` 或 `install.publish` 下的內容套件中。

AEM as a Cloud Service 不允許使用執行模式為特定環境或服務安裝內容。如果開發環境需要使用暫存或生產環境中不存在的資料或 HTML 進行植入，則可以使用套件管理器。
