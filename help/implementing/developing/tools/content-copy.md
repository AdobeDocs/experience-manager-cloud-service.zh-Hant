---
title: 內容複製工具
description: 內容複製工具可讓使用者依需將可變內容從其AEMas a Cloud Service的生產環境複製到較低的環境，以用於進行測試。
...
內容複製工具可讓使用者依需將可變內容從其AEMas a Cloud Service的生產環境複製到較低的環境，以用於進行測試。

## 簡介 {#introduction}

目前的真實資料對於測試、驗證和用戶接受度很有價值。內容複製工具可讓您將內容從生產AEM環境複製到測試、開發或 [快速開發環境(RDE)](/help/implementing/developing/introduction/rapid-development-environments.md) 測試環境。

要複製的內容由內容集定義。內容集由JCR路徑清單組成，該清單包含要從來源製作服務環境複製到相同Cloud Manager程式內目標製作服務環境的可變內容。 內容集中允許使用以下路徑。
...
必須先定義內容集，才能複製任何內容。 定義後，內容集就可以重複使用來複製內容。請依照下列步驟，建立新的內容集。
...
1. 在 **詳細資料** 頁簽，為內容集提供名稱和說明，然後點選或按一下 **繼續**.
...
請注意，在編輯內容集時，您可能需要展開已設定的路徑，以顯示已排除的子路徑。
...
1. 在&#x200B;**複製內容**&#x200B;對話框中，指定內容副本作業的來源和目的地。
   ...
   * 內容只能從較高的環境複製到較低的環境，或在環境階層如下的開發/RDE環境之間複製（從最高到最低）:
      * 生產
      * 預備
      * 開發/RDE

1. 如有需要，您也可以選取 **包括訪問控制清單** 在復製程式中。
...
如果啟動內容複製操作後需要中止該操作，則可以選擇取消該操作。

若要這麼做，請在 **複製內容活動** 頁面，選擇 **取消** 從您先前啟動的復製程式的省略號選單中執行動作。

![取消內容副本](assets/content-copy-cancel.png)

>[!NOTE]
>
>取消內容復製作業時，可能會導致目標環境中內容的部分複製。 這會使目標環境處於無法使用的狀態。
>
>如果您的環境因取消而處於此狀態，請洽詢Adobe客戶服務以取得協助。
...
* 內容複製工具沒有版本控制能力，不能自動檢測自上次內容複製作業以來在內容集來源環境上修改的內容或新建立的內容。
   * 如果您只想更新目標環境，但內容僅自上次內容復製作業後有所變更，則需要建立內容集，並指定自上次內容復製作業後在來源執行個體上進行變更的路徑。
* 版本資訊不包含在內容副本中。
