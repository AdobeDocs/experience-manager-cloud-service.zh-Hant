...
>[!NOTE]
>
>本檔案假設您已熟悉建立和編輯範本。 請參閱製作檔案[建立頁面範本](/help/sites-cloud/authoring/features/templates.md) ，其中詳細說明了模板作者所公開的可編輯模板的功能。

>[!TIP]
>
>[WKND教學](/help/implementing/developing/introduction/develop-wknd-tutorial.md) 課程透過實作範例深入探討如何使用頁面範本，對於了解如何在新專案中設定範本非常有用

## 建立新模板{#creating-a-new-template}

建立頁面範本主要是由範本作者使用[範本主控台和範本編輯器](/help/sites-cloud/authoring/features/templates.md)完成。 本節提供此程式的概觀，並隨後說明在技術層級發生的情況。
...
1. 選擇[模板類型](#template-type)。 這是複製以建立[範本定義](#template-definitions)。
   ...
   >[!NOTE]
   >
   >提供一系列範本類型的現成可用功能。 您也可以視需要[建立您自己的網站特定範本類型](#creating-template-types)。
...
1. 配置新模板的結構、內容策略、初始內容和佈局。
   ...
   * 元件可以再次解除鎖定和鎖定，以便定義初始內容。
   ...
   有關結構的技術詳細資訊，請參閱本文檔中的[Structure](#structure)。
   ...
   * 然後頁面作者就可以編輯初始內容。
   ...
   有關初始內容的技術詳細資訊，請參閱本文檔中的[初始內容](#initial-content)。
   ...
   * 範本的回應式版面運作方式與頁面編寫相同。
   ...
   有關模板佈局的技術詳細資訊，請參閱本文檔中的[佈局](#layout)。
...
1. 啟用範本，然後允許它建立特定內容樹。
   ...
   * 範本可供某些頁面分支使用或無法使用。
   ...
   有關啟用模板的技術詳細資訊，請參閱本文檔中的[啟用和允許用於Us](#enabling-and-allowing-a-template-for-use)e的模板
...
1. 使用它建立內容頁面。
   ...
   * 對於頁面作者而言，程式是透明的。
   ...
   如需頁面作者如何使用範本來建立頁面的詳細資訊，請參閱[建立及組織頁面](/help/sites-cloud/authoring/fundamentals/organizing-pages.md#templates)。

   有關使用可編輯模板建立頁面的技術詳細資訊，請參閱本文檔中的[結果內容頁面](#resultant-content-pages)。
...
>[!NOTE]
>
>最佳作法是建立新資料夾以存放自訂範本，而不使用`global`資料夾。
...
1. 除了標準製作權限(例如`content-authors`)您現在需要指派群組並定義必要的存取權限(ACL)，供作者在新資料夾中建立範本。
   ...
   `template-authors`群組是需要指派的預設群組。 有關詳細資訊，請參閱[ACL和組](#acls-and-groups)部分。

   <!--See [Access Right Management](/help/sites-administering/user-group-ac-admin.md#access-right-management) for full details on managing and assigning access rights.-->

### 使用配置瀏覽器{#using-the-configuration-browser}

1. 轉至&#x200B;**全局導航** -> **工具** > [**配置瀏覽器**。](/help/implementing/developing/introduction/configurations.md#using-configuration-browser)

   左側列出了現有資料夾，包括`global`資料夾。
...
>[!NOTE]
>
>在[配置瀏覽器中，如果要在此資料夾中建立模板，可以編輯全局資料夾並激活&#x200B;**可編輯模板**&#x200B;選項，但這不是建議的最佳做法。](/help/implementing/developing/introduction/configurations.md#using-configuration-browser)

### ACL和組{#acls-and-groups}

建立範本資料夾後（透過CRXDE或使用設定瀏覽器），必須為範本資料夾的適當群組定義ACL，以確保適當的安全性。

[WKND教程](/help/implementing/developing/introduction/develop-wknd-tutorial.md)的模板資料夾可以用作示例。
...
>[!CAUTION]
>
>您不得變更`/libs`路徑中的任何項目。 這是因為`/libs`的內容可以隨時由AEM的更新覆寫。
...
1. 如此處](/help/sites-cloud/authoring/features/templates.md#creating-a-new-template-template-author)所述，建立範本，作為範本類型的基礎。[
1. 使用CRXDE Lite，將新建立的模板從`templates`節點複製到[模板資料夾](#template-folders)下的`template-types`節點。
1. 從[範本資料夾](#template-folders)下的`templates`節點刪除範本。
...
可編輯模板的定義儲存在[用戶定義的資料夾](#template-folders)（建議）中，或儲存在`global`中。 例如：
...
1. **啟用範本**
   ...
   * [從範本](/help/sites-cloud/authoring/features/templates.md) 控制台啟用 **** 範本。
...
1. **允許的範本**
   ...
   * [在子分支的適當頁面或根 **頁**](/help/sites-cloud/authoring/features/templates.md#allowing-a-template-author) 面的「頁面屬性」上定義「允許的範本路徑」。
   * 設定屬性：
      `cq:allowedTemplates`
在 
`jcr:content` 所需分支的節點。
...
* Model - OSGi套件組合 — [ OSGI套件組合](/help/implementing/deploying/configuring-osgi.md)實作功能。
...
* 如果上述兩個屬性為空或不存在，則拒絕`T`，除非它屬於與`P`相同的應用程式。 `T` 屬於同一應用程 `P` 式，只有當路徑的第二級名稱與路 `T` 徑的第二級名稱相同時才 `P`會。例如，範本`/apps/wknd/templates/foo`與頁面`/content/wknd`屬於相同的應用程式。
...
要限制哪些模板可用於在指定頁面下建立子頁面，請使用頁面`jcr:content`節點的`cq:allowedTemplates`屬性指定允許作為子頁面的模板清單。 清單中的每個值必須是允許的子頁面（例如`/apps/wknd/templates/page-content`）的模板的絕對路徑。
