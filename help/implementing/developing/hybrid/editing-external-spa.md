...
決定時 [哪一個層級的整合](/help/implementing/developing/headful-headless.md) 您想要在外部SPA和AEM之間使用，請考量您通常必須能夠在AEM中編輯和檢視SPA。
...
此 `@adobe/aem-spa-page-model-manager` 套件提供用於初始化模型管理員和從AEM執行個體擷取模型的API。 然後，此模型可用於使用來自的API轉譯AEM元件 `@adobe/aem-react-editable-components` 和 `@adobe/aem-spa-component-mapping`.

#### 安裝 {#installation}

執行以下命令 `npm` 命令來安裝所需的套裝程式。
...
* `path`  — 初始化時，會擷取定義路徑上的模型並儲存在 `ModelStore`. 此路徑可用來擷取 `rootModel` 初始化時（如有需要）。
...
1. 從以下位置執行以下Maven命令： `aem-guides-wknd-spa` 目錄，讓您可以建置專案並部署至AEM。
...
1. 建立檔案(例如， `Page.js`)作為可編寫的頁面元件使用。 使用中提供的頁面元件 `@adobe/cq-react-editable-components`.
...
1. 如同先前一樣，套用 `MapTo` 至頁面中所有子元件的AEM資源型別。
   ...
   >[!NOTE]
   >
   >在此範例中，會使用未包裝的React文字元件，而非包裝的 `AEMText` 先前建立的。 原因是當元件屬於頁面/容器且不是獨立元件時，容器會負責遞回對應元件。 此外，每個子項也不需要啟用編寫功能和其他包裝函式。

1. 若要在SPA中新增可編寫頁面，請遵循區段中的相同步驟 [將可編寫的元件新增至頁面](#add-authorable-component-to-page). 在此，您可以略過 `itemPath` 屬性。
...
在此範例中，您會重複使用 `AEMText` 先前建立的元件。 您想要在WKND首頁上的現有文字元件下方新增文字。 新增的元件與一般葉元件相同。 然而， `itemPath` 可更新為必須新增新元件的路徑。

因為新元件必須新增到現有文字的下方 `root/responsivegrid/text`，新路徑為 `root/responsivegrid/{itemName}`.
...
* 可在任何層級建立元件。
   * 如果您提供 `itemPath='text_20'` 在上一個範例中，新節點會直接建立於頁面下方，也就是 `/content/wknd-spa-react/us/en/home/jcr:content/text_20`
...
依預設，您可以假設React應用程式是在 `div` 元素ID的 `spa-root`. 如有需要，可自訂此語法。

例如，假設您有一個SPA，應用程式在其中呈現 `div` 元素ID的 `root`. 此語法必須反映在三個檔案中。
...
1. 在AEM應用程式的頁面元件本文中，透過兩個步驟：
   ...
   1. 在新中新增根元素 `body.html` 檔案。

   ![將根元素新增至body.html](assets/external-spa-add-root.png)

### 使用路由編輯React SPA {#editing-react-spa-with-routing}

如果外部React SPA應用程式有多個頁面， [它可使用路由來決定要呈現的頁面/元件](/help/implementing/developing/hybrid/routing.md). 基本的使用案例是將目前作用中的URL與為路由提供的路徑進行比對。 若要在啟用路由的應用程式上啟用編輯，必須轉換要比對的路徑以容納AEM的特定資訊。

在下列範例中，您有一個包含兩個頁面的簡單React應用程式。 要呈現的頁面是根據提供給路由器的路徑與作用中URL相符來決定。 例如，如果您在 `mydomain.com/test`， `TestPage` 已呈現。
