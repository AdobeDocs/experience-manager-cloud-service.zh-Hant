...
前端開發人員必須自行負責建立用於建置應用程式的SPA元件程式庫。 前端開發人員可完全控制元件的內部結構。 [但是，AEM一律擁有網站](editor-overview.md)的結構。
...
任何指向特定內容的明確呈現都會視為靜態呈現，雖然受到支援，但與AEM的內容製作功能相容。 這也不符合[可攜性](#portability)的原則。

### 動態路由 {#dynamic-routing}

如同轉譯一樣，所有路由也應是動態的。 在AEM中，[SPA應一律擁有路由](routing.md)，而AEM會監聽路由並根據路由擷取內容。

任何靜態路由都違反可攜性[原則](#portability)，而且會因為與AEM的內容製作功能不相容而限製作者。 例如，使用靜態路由，如果內容作者想要變更路由或變更頁面，他們必須要求前端開發人員執行此操作。
...
如果遵循AEM](#spa-development-principles-for-aem)中開發SPA的[原則，則您的SPA可運作所有支援的AEM內容製作功能。
...
>[!NOTE]
>
>雖然AEM支援所有模型，但只有實作第三個(並遵循建議的[SPA開發原則](#spa-development-principles-for-aem))後，內容作者才能在AEM中與SPA內容互動及編輯。

## 將現有SPA移轉至AEM {#migrating-existing-spas-to-aem}

一般而言，如果您的SPA遵循AEM](#spa-development-principles-for-aem)的[SPA開發原則，則您的SPA可在AEM中運作，並可使用AEM SPA編輯器進行編輯。
...
* **Dispatcher / CDN**
   ...
   * JavaScript、CSS、JSON和HTML會快取，最佳化頁面以實現快速傳送。
...
* [使用React在AEM中開始使用SPA](getting-started-react.md)顯示如何使用React在AEM中建立基本SPA以使用SPA編輯器。
