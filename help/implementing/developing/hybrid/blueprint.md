...
如需有關AEM適用的Javascript SPA SDK中如何進行動態模型與元件對應的詳細資訊，請參閱文章[SPA適用的動態模型與元件對應](model-to-component-mapping.md)。
...
要執行此操作，模型提供程式必須註冊到[`PageModelManager`](#pagemodelmanager)。 然後，當發生變更時，會接收並將更新的資料傳遞至委派元件。 根據慣例，將承載模型片段的委派元件可用的屬性命名為`cqModel`。 實作可免費提供此屬性給元件，但應考慮到與架構架構整合、可探索性和易用性等方面。
...
容器會從[`ComponentMapping`](#componentmapping)程式庫的存放區動態取得子元件。 然後，容器會使用「模型提供者」功能擴充子元件，最後將其實例化。

### 頁面 {#page}

`Page`元件擴展`Container`元件。 容器是用來包含和呈現子元件（包括子頁）的元件。 為此，容器會反覆其模型的`:itemsOrder`、`:items`和`:children`屬性。 `Page`元件會從[`ComponentMapping`](#componentmapping)程式庫的儲存區動態取得子元件。 `Page`負責實例化子元件。
...
#### 回應式格線{#placeholder-of-the-responsive-grid}的預留位置
...
可以封裝和擴展底層的[`Component Mapping`](#componentmapping)庫及其`MapTo`函式，以提供與當前元件類附帶的編輯配置相關的功能。
...
在上述實作中，在[元件對應](#componentmapping)存放區中實際註冊之前，會以空白功能擴充專案元件。 要完成此操作，請封裝並擴展[`ComponentMapping`](#componentmapping)庫，以引入`EditConfig`配置對象的支援：
...
* 與可編輯資源對應的元素會包含`data-cq-data-path`屬性。 請參閱本檔案的[與頁面編輯器](#contract-with-the-page-editor)合約一節。
...
基礎的[`PageModelManager`](#pagemodelmanager)庫及其[`ModelRouter`](routing.md)模組（預設為啟用）負責預取並提供對與給定資源路徑關聯的模型的訪問。

這兩個實體與路由概念相關，但[`ModelRouter`](routing.md)只負責載入[`PageModelManager`](#pagemodelmanager)，該具有與當前應用程式狀態同步的資料模型。

如需詳細資訊，請參閱[SPA模型路由](routing.md)一文。

## SPA在動作中{#spa-in-action}

請繼續參閱下列檔案，了解簡單的SPA如何運作並親自試驗SPA:

* [AEM中SPA使用React快速入門](getting-started-react.md)。
* [開始使用AEM中的SPA，使用Angular](getting-started-angular.md)。

## 進一步閱讀{#further-reading}

如需AEM中SPA的詳細資訊，請參閱下列檔案：

* [SPA編](editor-overview.md) 輯器概觀以取得AEM中SPA和通訊模型的概觀
