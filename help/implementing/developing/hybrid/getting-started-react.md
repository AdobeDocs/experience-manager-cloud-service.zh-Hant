...
>[!NOTE]
>
>本文章以React框架為基礎。 如需Angular架構的對應檔案，請參閱 [AEM中的SPA快速入門 — Angular](getting-started-angular.md).
...
`App.js` 主要用於包住構成應用程式的根元件。 任何應用程式的進入點是頁面。
...
此 `Page` 擷取頁面模型的JSON表示法，並處理內容以包裝/裝飾頁面的每個元素。 進一步詳細資訊，請參閱 `Page` 可以在檔案中找到 [SPA Blueprint。](blueprint.md)
