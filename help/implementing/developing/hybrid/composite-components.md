...
* React應用程式已使用RemotePage元件[載入AEM編輯器](remote-page.md)中。
...
1. 在SPA中為此建立對應的元件。 確保子元件對應至SPA專案中其對應的AEM資源型別。 在此範例中，我們使用與先前案例`AEMText`中詳細`AEMImage`相同的[和](#component-does-not-exist)元件。
...
1. 將已建立的`wknd-spa/components/imagecard`元件新增至頁面範本[中容器元件](/help/sites-cloud/authoring/page-editor/templates.md)的允許元件。
...
`AEMCard`元件與先前使用案例[中定義的](#content-does-not-exist)相同。 此處，上述位置在AEM專案中定義的內容包含在SPA中。
