...
當建置 SPA 以利用 AEM SPA 編輯器時，內容作者在編輯和建立內容時不會注意到任何差異。可以使用常用的 AEM 功能，作者的工作流程無需變更。
...
1. 在新索引標籤中載入 `home.model.json`。
   ...
   AEM SPA 編輯器利用 [AEM 內容服務](/help/sites-cloud/administering/content-fragments/content-fragments.md)將頁面的全部內容以 JSON 模型傳遞。
...
可以透過 AEM 內靈活的整合層級啟用 SPA，包括在 AEM 外部開發和維護的 SPA。此外，可以在 AEM 中利用 SPA，同時也可以使用 AEM 將內容以無周邊方式傳遞到其他端點。
