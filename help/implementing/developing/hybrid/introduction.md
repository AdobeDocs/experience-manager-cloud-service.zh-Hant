...
逐步說明以標準AEM功能和範例WKND SPA專案應用程式為基礎。 若要繼續操作，請在此處[從GitHub下載並安裝範例WKND SPA專案應用程式。](https://github.com/adobe/aem-guides-wknd-spa)

>[!CAUTION]
>
>本檔案僅將[WKND SPA專案應用程式](https://github.com/adobe/aem-guides-wknd-spa)用於示範用途。 它不應用於任何項目工作。
...
1. 選取文字元件，並注意工具列的顯示方式與任何其他元件相同。 選擇&#x200B;**Edit**。
...
支援其他製作工具，例如在頁面上拖放其他元件、重新排列元件以及修改版面，如同任何非SPA AEM應用程式。

>[!NOTE]
SPA編輯器不會修改應用程式的DOM。 SPA本身負責DOM。
若要查看其運作方式，請繼續閱讀本文的下一節[SPA應用程式和AEM SPA編輯器](#spa-apps-and-the-aem-spa-editor)。
...
1. 在發佈伺服器上或使用頁面編輯器中&#x200B;**頁面資訊**&#x200B;功能表的&#x200B;**檢視為已發佈**&#x200B;選項，載入WKND SPA專案應用程式。
...
1. 如果尚未載入，請在發佈伺服器上或使用頁面編輯器中&#x200B;**頁面資訊**&#x200B;功能表的&#x200B;**檢視為已發佈**&#x200B;選項，載入WKND SPA專案應用程式。
...
1. 使用瀏覽器的內建工具檢視頁面來源。
1. 請注意，來源的內容有限。
   * 頁面內文中沒有任何內容。 它主要由樣式表和對各種指令碼（如`clientlib-react.min.js`）的調用組成。
...
1. 模型也可以將頁面分組，以便同步載入，減少所需的頁面重新載入次數。
   ...
   在WKND SPA專案應用程式的範例中，會同步載入`home`、`page-1`、`page-2`和`page-3`頁面，因為訪客通常會瀏覽這些頁面。

   此行為非強制性，且可完全定義。

   ![WKND SPA專案項目項目分組](assets/wknd-pages.png)

1. 要查看行為中的這種差異，請重新載入`home`頁並清除檢查器的網路活動。 導覽至頁面功能表中的`page-1`，並查看唯一的網路活動是對`page-1`影像的要求。 `page-1` 本身不需要載入。
...
1. 使用瀏覽器的內建開發人員工具來檢查頁面內容。 使用選取工具，在頁面上選取可編輯的元件並檢視元素詳細資料。
   ...
   >[!NOTE]
   此行為與AEM中的伺服器端轉譯頁面不同，該頁面會為每個可編輯元件插入`cq`元素。
...
* [使用Angular開始使用AEM中的](getting-started-angular.md) SPA ，說明如何建置基本SPA以搭配AEM中的SPA編輯器使用Angular
* [SPA編輯](editor-overview.md) 器概述深入探討AEM與SPA之間的通訊模型。
* [開發SPA for ](developing.md) AEM說明如何與前端開發人員互動，以開發SPA for AEM以及SPA如何與AEM架構互動。
