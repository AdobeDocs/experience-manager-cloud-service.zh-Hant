...
aem.segmentation儲存區候選項已延伸 [`ContextHub.Store.PersistedJSONPStore`](contexthub-api.md#contexthub-store-persistedjsonpstore)。

### 設定 {#configuration-segmentation}

當您建立商 `aem.segmentation` 店時，不需要提供詳細的設定。 預設配置指定ContextHub段定義的位置。
...
範例 `contexthub.geolocation` 商店申請人使用Google地圖來取得並儲存有關用戶端位置的資訊。
...
儲存 `contexthub.geolocation` 候選擴展 [`ContextHub.Store.PersistedJSONPStore`](contexthub-api.md#contexthub-store-persistedjsonpstore)。
...
>
>如需 [Chrome變更的詳細資訊](https://developers.google.com/web/updates/2016/04/geolocation-on-secure-contexts-only) ，請參閱此Google部落格文章。
...
儲存 `contexthub.surferinfo` 候選擴展 [`ContextHub.Store.PersistedStore`](contexthub-api.md#contexthub-store-persistedstore)。
...
樣本 `granite.emulators` 儲存候選儲存關於客戶設備的資訊。
...
儲存 `granite.emulators` 候選擴展 [`ContextHub.Store.PersistedStore`](contexthub-api.md#contexthub-store-persistedstore)。
...
儲存 `granite.profile` 候選擴展 [`ContextHub.Store.PersistedJSONPStore`](contexthub-api.md#contexthub-store-persistedjsonpstore)。
