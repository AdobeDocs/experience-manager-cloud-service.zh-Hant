...
目的 `p.guessTotal` 參數是返回通過組合最小可行性可顯示的適當數目的結果 `p.offset` 和 `p.limit` 值。 在大結果集的情況下，使用該參數的優點是提高了效能。 這避免了計算全部總數（例如調用） `result.getSize()`)和讀取整個結果集，一直優化到OAK引擎和索引。 當結果數以萬計時，這可能是一個顯著的區別，無論是在執行時間還是記憶體使用方面。
...
預設情況下，QueryBuilder JSON Servlet將為搜索結果中的每個節點（例如，路徑、名稱、標題等）返回一組預設屬性。 為了獲得對返回屬性的控制，可以執行以下操作之一：
...
1. 啟用QueryBuilder的DEBUG日誌以獲取基礎、可解釋的XPath查詢
   * 導覽至 `https://<host>:<port>/system/console/slinglog`。為建立新記錄器 `com.day.cq.search.impl.builder.QueryImpl` 在 **調試**。
