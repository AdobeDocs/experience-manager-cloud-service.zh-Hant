...
>[!NOTE]
>
>此功能位於Beta中。 如果您有興趣使用這項新功能並分享您的意見回饋，請傳送電子郵件至[aemcs-rde-support@adobe.com](mailto:aemcs-rde-support@adobe.com)，說明您的使用案例。

RDE支援對程式碼和內容的目前狀態製作快照，以供稍後還原。 在同步可能需要還原的程式碼時，或在開發不同功能之間切換時，快照會很有用。 也可以只從快照還原可變內容，作為測試的已知起點。

請注意，特定RDE的快照無法還原到其他RDE上。

每個RDE環境最多有七個快照。 標示為要刪除但仍在7天保留期內的快照會繼續計入該限制，直到它們完全移除為止。 如果您達到限制並需要立即使用新快照的容量，請依照[刪除快照](#delete-snapshot)中的說明使用強制刪除，而非標準刪除。

支援的命令說明如下。 如需旗標和選項的完整清單，請使用`aio aem rde snapshot --help`，或如需特定子命令的說明，請使用`aio aem rde snapshot <subcommand> --help`。

### 列出快照 {#list-snapshots}

您可以透過執行以下動作列出組織中的所有快照：

`aio aem rde snapshot`

這會傳回可用快照的表格，可以使用`-s`旗標進行排序：

`aio aem rde snapshot -s <column-header>`

在欄標題前面加上減號以反向排序。 也支援`--json`全域旗標。

### 建立快照 {#create-snapshot}

若要建立RDE目前狀態（包括內容和部署）的快照，請執行：

`aio aem rde snapshot create <name>`

其中`<name>`是環境中快照的唯一名稱。 可選擇加入帶有`-d`旗標的簡短說明：
...
若要僅從快照還原可變內容（不還原部署），請使用`--only-mutable-content`旗標：

`aio aem rde snapshot restore <name> --only-mutable-content`

### 刪除快照 {#delete-snapshot}

將快照標籤為刪除並不會立即將其移除。 快照會在7天後刪除，讓您有時間視需要取消刪除。

若要將快照標示為刪除，請執行：

`aio aem rde snapshot delete <name>`

若要將所有快照一次標示為已刪除，請使用`-a`旗標：

`aio aem rde snapshot delete -a`

若要立即刪除快照（略過保留期間，因此不再計入環境快照限制），請新增`-f`旗標（或`--force`）：

`aio aem rde snapshot delete <name> -f`

`undelete`無法復原強制刪除。 合併旗標時，請針對完整的選項集使用`aio aem rde snapshot delete --help` （例如，強制刪除所有快照）。

### 取消刪除快照 {#undelete-snapshot}
