...
* 第一個目標是保存內容的資源（JCR節點）
* 其次，表示或指令碼與請求的某些部分（例如，選擇器和/或擴展）組合從資源屬性中定位
...
* First Sling檢查節點是否位於請求中指定的位置；比如說， `../content/corporate/jobs/developer.html`
* 如果找不到節點，則刪除擴展並重複搜索；比如說， `../content/corporate/jobs/developer`
...
* 解除安裝
* 查看當前狀態
* 訪問有關特定捆綁包的更多詳細資訊（例如，符號名稱、版本、位置等）
