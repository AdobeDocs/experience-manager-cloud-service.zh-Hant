...
Adobe建議使用提供的[Apache HttpComponents Client 4.x庫](https://hc.apache.org/httpcomponents-client-ga/)進行HTTP連接。

已知可行但可能需要自行提供依賴性的替代方案：

* [java.net.](https://docs.oracle.com/javase/7/docs/api/java/net/URL.html) URLand/ [or java.net.URLConnection](https://docs.oracle.com/javase/7/docs/api/java/net/URLConnection.html) （由AEM提供）
...
## 無串流二進位檔透過AEM做為雲端服務{#no-streaming-binaries}
...
>[!NOTE]
>
>若要執行下列的設定變更，您必須在本機開發環境上建立這些變更，然後將它們推送至AEM做為雲端服務例項。 如需如何進行此作業的詳細資訊，請參閱[部署至AEM as a Cloud Service](/help/implementing/deploying/overview.md)。
...
AEM會在執行時期自動將`smtp.starttls`屬性設為Cloud Service。 因此，如果`smtp.tls`設為true，則會忽略`smtp.startls`。 如果`smtp.ssl`設為false，則`smtp.starttls`設為true。 無論在OSGI配置中設定的`smtp.starttls`值如何。