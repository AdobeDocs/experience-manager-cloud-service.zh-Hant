...
例如，當解析度低於1024畫素（如同在行動裝置上）時，畫面會據此調整：
...
* 標誌與您目前使用的特定產品/解決方案。 對於AEM，此元素也會形成全域導覽的連結
* 搜尋
* 用於存取說明資源的圖示
* 存取其他解決方案的圖示
* 任何等待您的警報或收件匣專案的指示器 — 以及對其的存取權
* 使用者圖示，以及設定檔管理的連結
...
包含三個預設標籤。 此 **資產** 和 **元件** 索引標籤可讓您選取這類元素，並將其從面板拖曳至頁面上。 此 **內容樹狀結構** 索引標籤可讓您檢查頁面上內容的階層。

側面板預設為隱藏。 選取時，它會顯示於左側，或當視窗寬度小於1024畫素時，它會滑過以覆蓋整個視窗，例如在行動裝置上。
...
每個元件例項在按一下時都會顯示其工具列（按一下或緩慢按兩下）。 工具列包含頁面上元件執行個體可用的特定動作（例如，複製、貼上、開啟編輯器）。
