...
>[!TIP]
>
>熟悉基本 [項AEM目原型使用](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html?lang=zh-Hant)的 [FileVault內容管理插件](/help/implementing/developing/tools/maven-plugin.md) 因為本文是以這些學習和概念為基礎的。
...
AEM需要 **內容** 和 **代碼**，即單個內容包 **不能** 部署 **兩者** `/apps` 和運行時可寫區域(例如， `/content`。 `/conf`。 `/home`，或者 `/apps`)。 而應用程式必須將程式碼和內容分隔為獨立套件，以便部署至AEM。
