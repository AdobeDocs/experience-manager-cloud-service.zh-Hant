...
![回應式設計的範例](assets/responsive-example.png)
...
模擬器可讓您預覽不同檢視區大小的頁面，以便測試回應式設計的行為。 在Sites Console中編輯頁面時，您可以點選或按一下 **模擬器** 圖示以顯示模擬器。

![工具列中的模擬器圖示](assets/emulator-icon.png)

在模擬器工具列中，您可以點選或按一下 **裝置** 圖示以顯示下拉式功能表，讓您在其中選取裝置。 選取裝置時，頁面會隨著檢視區大小而改變。

![模擬器工具列](assets/emulator.png)

### 指定裝置群組 {#specifying-device-groups}

若要指定出現在「 」中的 **裝置** 清單，新增 `cq:deviceGroups` 屬性至 `jcr:content` 網站範本頁面的節點。 屬性的值是裝置群組節點的路徑陣列。

例如，WKND網站的範本頁面為 `/conf/wknd/settings/wcm/template-types/empty-page/structure`. 以及 `jcr:content` 其下方的節點包含下列屬性：
...
[核心元件影像元件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/wcm-components/image.html) 如自我調整影像選取等功能。

* 依預設，影像元件會使用 [自我調整影像Servlet](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/adaptive-image-servlet.html) 以傳遞適當的轉譯。
* [網頁最佳化的影像交付](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/web-optimized-image-delivery.html) 也可透過其原則中的簡單核取方塊取得，該核取方塊會以WebP格式從DAM傳送影像資產，且平均可將影像的下載大小減少約25%。

## 配置容器 {#layout-container}

AEM配置容器可讓您有效率且有效地實作回應式配置，以根據使用者端檢視區調整頁面尺寸。

請參閱檔案 [設定配置容器和配置模式](/help/sites-cloud/administering/responsive-layout.md) 有關版面容器如何運作以及如何為您的內容啟用回應式版面的詳細資訊。
