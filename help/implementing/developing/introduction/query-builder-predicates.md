...
* **`self`**  — 搜索子樹，但包括作為路徑指定的基節點（無通配符）。
   * *重要說明*:已確定問題 `self` 在querybuilder的當前實現中以及在查詢中使用該屬性可能無法生成正確的搜索結果。 更改當前的實施 `self` 屬性也不可行，因為它可能會破壞依賴它的現有應用程式。 因此， `self` 屬性已被棄用，建議避免使用它。
...
可以定義下界、上界或兩者。 也可以為下界和上界分別指定操作（例如，小於或等於）。
