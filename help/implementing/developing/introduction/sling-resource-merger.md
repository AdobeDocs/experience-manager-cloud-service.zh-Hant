...
* **[覆蓋](/help/implementing/developing/introduction/overlays.md)** 資源的 [搜尋路徑](/help/implementing/developing/introduction/overlays.md#search-paths).
...
>[!CAUTION]
>
>Sling Resource Merger和相關方法只能搭配觸控式UI (這是AEMas a Cloud Service唯一可用的UI)使用。
...
>
>這是因為 `/libs` 升級套用至您的執行個體時，可能會遭到覆寫。
>
...
* **隱藏節點的子系（同時保留節點的屬性）**
  ...
  節點、其屬性及其子系定義於 `/libs`. 節點及其屬性需要在 `/apps` 覆蓋/覆寫，但某些或所有子節點不需要在 `/apps` 覆蓋/覆寫。
