...
# 為伺服器端API生成訪問令牌（舊版） {#generating-access-tokens-for-server-side-apis-legacy}
...
具有IMS組織管理員角色，並且還是AEM作者上的「用戶」或「管理員AEM產品配置文AEM件」的成員的用戶可以生成AEMas a Cloud Service憑據。 該憑據稍後可由具有AEMas a Cloud Service環境管理員角色的用戶檢索，並應安裝在伺服器上，需要謹慎地作為密鑰處理。 此JSON格式檔案包含與as a Cloud ServiceAPI整合所需的所AEM有資料。 該資料用於建立與IMS交換的簽名JWT令牌，以用於IMS訪問令牌。 然後，此訪問令牌可用作承載驗證令牌，以向AEMas a Cloud Service請求。 預設情況下，憑據將在一年後過期，但在需要時可以刷新，如所述 [這裡](#refresh-credentials)。

伺服器到伺服器流包括以下步驟：

* 從開發人員控制台AEM獲取as a Cloud Service的憑據
* 在非伺服器AEM上安裝as a Cloud Service的憑AEM據以調用AEM
...
對as a Cloud Service開發者控制AEM台具有訪問權限的用戶可以查看給定環境的開發者控制台中的整合頁籤和兩個按鈕。 具有「AEMas a Cloud Service環境」管理員角色的用戶可以 **生成服務憑據** 按鈕以生成和顯示服務憑據json。 json包含非伺服器所需的所有資訊AEM，包括客戶端ID、客戶端密碼、私鑰、證書以及環境的作者和發佈層的配置，而不管選擇什麼儲存盒。
...
>[!IMPORTANT]
>
>IMS組織管理員（通常是通過雲管理器設定環境的用戶）應該是「AEM作者」上的「用戶」或「管理員產品配置檔案」的成AEM員AEM，可以訪問開發人員控制台。 然後，他們必須按一下 **生成服務憑據** 按鈕，以便具有as a Cloud Service環境管理權限的用戶生成並稍後檢索AEM憑據。 如果IMS組織管理員尚未完成此任務，則會收到一條消息通知他們他們需要IMS組織管理員角色。

### 在非服AEM務器上安裝服務憑AEM據 {#install-the-aem-service-credentials-on-a-non-aem-server}

調用的AEM非應用AEM程式應能訪問as a Cloud Service的憑AEM據，將其視為機密。

### 生成JWT令牌，並將其交換為訪問令牌 {#generate-a-jwt-token-and-exchange-it-for-an-access-token}

在呼叫Adobe的IMS服務時，使用憑據建立JWT令牌，以檢索有效期為24小時的訪問令牌。
...
訪問令牌定義何時過期，通常為24小時。 Git儲存庫中有示例代碼，用於管理訪問令牌並在其過期之前刷新它。
...
在中建立技術帳戶用戶後AEM（在具有相應訪問令牌的第一個請求後發生），必須正確地對技術帳戶用戶進行許可 **在** AEM。

預設情況下，在AEM Author服務上，技術帳戶用戶將添加到提供讀取訪問權限的「參與者」用戶AEM組。

中的此技術帳戶用AEM戶可以使用通常的方法進一步設定權限。

## 顯影劑流 {#developer-flow}

開發人員應test使用其非應用程式(AEM在其筆記型電腦上運行或托管)的開發實例，該實例向開發as a Cloud Service的開AEM發環境發出請求。 但是，由於開發人員不一定具有IMS管理員角色權限，因此Adobe不能假定他們能夠生成常規伺服器到伺服器流中描述的JWT承載。 因此，Adobe為開發者提供了一種機制，用於直接生成訪問令牌，該訪問令牌可用於對他們有權訪問的AEMas a Cloud Service環境的請求中。
...
開發人員可以使用此令牌從其非test應AEM用程式向AEMas a Cloud Service環境進行呼叫。 通常，開發人員在自己的筆記型電腦上將此令牌與AEM非應用程式一起使用。 此外，AEM作為雲通常是非生產環境。
...
要生成訪問令牌，請在「開發人員控制台」中按一下 **獲取本地開發令牌**。
...
預設情況下，as a Cloud Service上AEM的憑據將在一年後過期。 為確保服務連續性，開發人員可以選擇刷新憑據，將其可用性延長一年。 使用 **刷新服務憑據** 從 **整合** 頁籤，如下所示。
...
如果必須撤消憑據，則必須使用以下步驟向客戶支援提交請求：

1. 在用戶介面中禁用Adobe Admin Console的技術帳戶用戶：
   * 在雲管理器中，按 **...** 按鈕。 此操作將開啟產品配置檔案頁
