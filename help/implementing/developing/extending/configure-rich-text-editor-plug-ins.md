---
title: 在 [!DNL Adobe Experience Manager]中配置RTF編輯器插件。
...
預設情況下， `format`、`link`、`list`、`justify`和`control`外掛程式及其所有功能都會在RTE中啟用。
...
|---|---|---|---|
| 名稱 | 功能 | 功能 | 功能 |
| 類型 | 字串 | `String` (多字串；將「類型」設 `String` 為，然後按 `Multi` 一下「CRXDE Lite」) | 字串 |
| 值 | `*` （星號） | 設定為一個或多個特徵值。 | - |
...
* 將剪貼簿內容貼為純文字。 在[!DNL Experience Manager]元件中插入之前，它會先從複製的內容中移除樣式和格式的所有元素。 使用下方的`plaintext`進行配置。
...
1. 在`edit`節點中，使用以下詳細資訊建立屬性：
   ...
   * **名稱** `defaultPasteMode`
   * **類型** `String`
   * **** 值是來自、或模式的必 `browser`要貼 `plaintext`上模 `wordhtml` 式之一。

### 配置貼上內容時允許的格式 {#pasteformats}

可以進一步配置貼上形式 — Microsoft-Word(`paste-wordhtml`)模式，以便當從其他程式（如[!DNL Microsoft Word]）貼上到[!DNL Experience Manager]時，可以顯式允許一些樣式。

例如，如果在[!DNL Experience Manager]中貼上時只允許使用粗體格式和清單，則可以過濾掉其他格式。 這稱為可設定的貼上篩選，可針對下列兩項執行：
...
要配置從其他程式將文本貼入[!DNL Experience Manager]時允許的格式：
...
| `fallbackBlockTag` | `String` | 定義用於任何塊的塊標籤，這些塊標籤具有未包含在`allowBlockTags`中的塊標籤。 通常，`p`足夠。 |
| `table` | `nt:unstructured` | 定義貼上表格時的行為。 此節點必須具有屬性允許（類型布林值），才能定義是否允許貼上表。 如果allow設定為false，則必須指定屬性ignoreMode（類型字串）以定義如何處理貼上的表內容。 ignoreMode的有效值為`remove`以移除表格內容，以及`paragraph`以將表格儲存格轉換為段落。 |
| `list` | `nt:unstructured` | 定義貼上清單時的行為。 必須具有屬性`allow`（類型布林值），才能定義是否允許貼上清單。 如果`allow`設為`false`，請指定屬性`ignoreMode`（類型`String`）以定義如何處理貼上的任何清單內容。 ignoreMode的有效值為`remove`，用於刪除清單內容，`paragraph`用於將清單項轉換為段落。 |

有效`htmlPasteRules`結構的範例如下：
...
>[!NOTE]
>
>也可為[表或表單元格](configure-rich-text-editor-plug-ins.md#tablestyles)定義樣式。 這些設定需要個別的程式。
...
RTF編輯器使用ID為`CQrte`的容器DOM元素，提供不同樣式供檢視和編輯：
...
使用[!DNL Experience Manager]製作日文內容的作者可將樣式套用至字元，以避免不需要分行符號的分行符號。 這可讓作者讓句子在想要的位置中斷。 此功能的樣式以CSS樣式表中預先定義的CSS類別為基礎。
...
>[!NOTE]
>
>如果無法將區塊標籤（例如`<hr>`標籤）指派給段落，則對`paraformat`外掛程式來說，它不是有效的使用案例。

首次啟用「段落格式」插件時，沒有預設的段落格式可用。 快顯清單為空。 要向作者提供段落格式，請執行以下操作：

* 啟用[!UICONTROL Format]彈出選擇器清單。
...
>[!CAUTION]
>
>配置RTE的段落格式時，請勿刪除段落標籤&lt;p>作為格式選項。 如果移除`<p>`標籤，則內容作者即使設定了其他格式，也無法選取[!UICONTROL Paragraph formats]選項。
...
1. 在`paraformat`節點下建立節點，以保存格式清單：

   * **名稱** `formats`
   * **類型** `cq:WidgetCollection`

1. 在`formats`節點下建立節點，這保留單個格式的詳細資訊：
...
在標準[!DNL Experience Manager]安裝中，當`misctools`插件為特殊字元(`specialchars`)啟用時，預設選項立即可用；例如，版權符號和商標符號。
...
1. 在`table`節點下，根據需要建立以下節點：
   ...
   * 要定義整個表的樣式（可在&#x200B;**[!UICONTROL 表屬性]**&#x200B;下使用）:
      ...
      * **名稱** `tableStyles`
      * **類型** `cq:WidgetCollection`
   * 要定義單個單元格的樣式（可在&#x200B;**[!UICONTROL 單元格屬性]**&#x200B;下使用）,

      * **名稱** `cellStyles`
      * **類型** `cq:WidgetCollection`


1. 建立節點（在`tableStyles`或`cellStyles`節點下，視情況）以表示個別樣式，
...
>[!NOTE]
標準字典位於`/libs/cq/spellchecker/dictionaries`，以及相應的讀我檔案。 請勿修改檔案。
...
1. 在`../items/text`節點中，在元件的對話框定義中，建立屬性：
...
在[!DNL Experience Manager]中新增連結時，您可以定義要使用的CSS樣式和要自動接受的通訊協定。 要配置如何從其他程式在[!DNL Experience Manager]中添加連結，請定義HTML規則。

1. 使用CRXDE Lite，找出專案的文字元件。
1. 在與`<rtePlugins-node>`相同的級別建立節點，即在`<rtePlugins-node>`的父節點下建立節點：
   ...
   * **值** `richtext`
   ...
   `../items/text`節點的位置可能會因對話方塊的結構而異。 `/apps/myProject>/components/text/dialog/items/text`和`/apps/<myProject>/components/text/dialog/items/panel/items/text`是兩個範例。
