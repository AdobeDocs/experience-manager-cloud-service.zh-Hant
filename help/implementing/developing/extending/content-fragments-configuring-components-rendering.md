...
有幾個與內容片段呈現相關的[進階服務](#definition-of-advanced-services-that-need-configuration)。 若要使用這些服務，此類元件的資源類型必須讓內容片段架構知道。
...
>[!CAUTION]
>
>* **如果您不需要下列 [的進](#definition-of-advanced-services-that-need-configuration)** 階服務，可忽略此設定。
   >
   >
...
對於某些功能，您的元件必須遵循預先定義的慣例。 下表詳細說明了需要由元件為每個段落(即`jcr:paragraph`（每個元件例項），讓服務能夠正確偵測和處理。
