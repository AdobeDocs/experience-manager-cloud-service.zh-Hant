...
1. 為預先載入的 **AEM 示範資產**&#x200B;端點的清單查詢複製以下程式碼片段。清單查詢將傳回使用特定內容片段模式的所有內容清單。庫存和類別頁面通常使用這種查詢格式。
...
1. 選取「**修飾**」按鈕將您的程式碼自動格式化，使更容易閱讀。

1. 完成後，在編輯器左上方選取「**播放**」按鈕以進行查詢。編輯器自動完成 `items`，簡要地以黃色醒目標示，且查詢執行。
...
1. 貼上複製的程式碼，以取代查詢編輯器中的現有內容。
   ...
   >[!NOTE]
   >
   >如果您沒有使用前面單元中提到的相同欄位描述，請在此查詢中更新該欄位名稱。
   >
...
1. 為了使持續性查詢公開可用，必須將其發佈，就像您的內容片段需要發佈一樣。在查詢編輯器右上方按一下「**發佈**」以發佈查詢。
