...
在此單元中，您使用該內容片段主控台，根據該模式建立新的內容片段。 將內容片段主控台視為您的 Headless 內容資料庫。使用該控制台來建立新的內容片段並管理現有片段。
...
>[!TIP]
>
>根據您的瀏覽器設定，新的瀏覽器索引標籤可能會被快顯視窗封鎖程式禁止。如果您的新片段在按一下後未開啟 **建立並開啟**，檢查您的瀏覽器設定。

## 新增內容至您的內容片段 {#add-content}

儲存並開啟新內容片段後，內容片段編輯器會在新標籤上開啟。 您可以在此處新增新片段的內容。
