...
1. 「建立模型」精靈會啟動，然後會引導您建立模型。提供必填資訊。
   ...
   * **模型標題**  — 這是模型的一個簡短標籤，通常表示模型的目的。 我們將新模型稱為 `Adventure`。
...
我們現在有個模型叫 **冒險**，但是它沒有詳細資訊，如持續時間、目標、活動等。 在使用模式之前，您需要定義其資料結構。

內容片段模式編輯器就是您設定定義模式內容的資料類型和屬性的地方。

>[!TIP]
>
>按照以下說明中的命名架構非常重要，因為我們將在以後的模組中引用這些特定名稱。
...
您已經有一個用於冒險標題的欄位。現在，您需要添加欄位來捕獲該冒險的描述、價格和代表性影像。

>[!TIP]
>
>**冒險**&#x200B;模型是以 AEM 的 WKND 範例網站為基礎。你可以 [訪問此處](https://wknd.site/us/en/adventures/yosemite-backpacking.html) 查看使用 **冒險** 模型。
