...
第一步是選擇要使用的翻譯服務。 有多種人工和機器翻譯服務可供 AEM 使用。大部分提供者都提供要安裝的翻譯套件。請參閱[其他資源](#additional-resources)章節，了解一系列可選用的翻譯服務。
...
1. 在全域導覽功能表中，選取 **工具** > **Cloud Service** > **翻譯Cloud Service**.
...
1. 選取 **建立** 和 **編輯設定** 視窗會出現，您可以在其中設定組態屬性。
...
1. 在資產控制檯中，選取要設定的語言根並選取 **屬性**.
1. 選取 **Cloud Service** 標籤。
1. 在 **Cloud Service設定** 在 **新增設定** 下拉式清單，選取您的聯結器。 當您將其套件安裝為 [前述內容。](#connect-translation-provider)
1. 在 **Cloud Service設定** 在 **新增設定** 下拉式清單，也選取您的設定。
1. 選取 **儲存並關閉**.
