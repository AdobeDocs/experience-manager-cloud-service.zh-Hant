...
無論您選擇的API為何，透過根據通用API定義無頭系統，您都可以使用最新瀏覽器和其他網路技術，例如漸進式網頁應用程式(PWA)。 API 建立了一個易於擴展和適應的標準介面。
...
此歷程的重點是放在僅無周邊的內容傳遞模型。不過，一旦您擁有這些基本知識，您就可以進一步探索如何使用這兩種模型的力量。
...
以這些知識為基礎，接下來查看文件 [AEM Headless as a Cloud Service 快速入門](getting-started.md)以繼續您的 AEM 無周邊歷程，您將在此文件了解如何設定必要工具，並開始思考 AEM 如何處理無周邊內容傳遞及其先決條件。

## 其他資源 {#additional-resources}

雖然建議您查閱文件 [AEM Headless as a Cloud Service 快速入門](getting-started.md)來繼續無周邊開發歷程的下個部分，以下也有一些其他選擇性資源，在深入探究本文件提到的一些概念，但不是繼續無周邊歷程的必要條件。
