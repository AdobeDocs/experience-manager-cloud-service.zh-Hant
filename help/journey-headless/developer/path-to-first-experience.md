...
您想要促成一致的體驗並管理跨管道的個人化行銷活動，您可以將每個單獨的管道和表面視為其自己獨特的內容結構來傳遞。不過，要讓每個管道都有自己的內容模型，維護起來相當困難。
...
這是概述需要哪些東西才能使用 AEM 實作您的第一個無周邊應用程式以傳遞您的內容。Headless開發人員歷程的稍後部分將詳細描述如何執行這些步驟。
...
* 如果在模型中設定了根資料夾，則內聯內容片段編輯器建立新內容片段的作業將可簡化。那麼從業者不必選擇位置，只需提供名稱，就可以開始編輯新參照。
