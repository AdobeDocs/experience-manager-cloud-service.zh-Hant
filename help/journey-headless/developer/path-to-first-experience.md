...
在 AEM Headless 歷程的上一個文件「[AEM Headless as a Cloud Service 快速入門](getting-started.md)」中，您已了解 Headless CMS 的基本理論，您現在應該：
...
建立內容模式並清楚檢視必須呈現內容的不同使用者端後，您需要確保將存取各種內容模式的相關的GraphQL/API發佈給需要此內容的所有使用者端。 有不同方法可以存取特定內容。您可以要求一段特定的靜態內容，這樣可以快取內容並提高效能。您也可以要求動態產生的內容，這將需要更多處理。確保客戶使用最符合其業務需求的API。
...
* 如果在模型中設定了根資料夾，則內聯內容片段編輯器建立新內容片段的作業將可簡化。那麼從業者不必選擇位置，只需提供名稱，就可以開始編輯新的參照。
