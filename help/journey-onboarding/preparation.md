---
title: 上線準備
description: 在啟動過程開始之前，系統管理員在登入系統之前必須了解一些準備步驟。
...
>[!VIDEO](https://video.tv.adobe.com/v/3431507/?captions=chi_hant&quality=12&learn=on)
...
* 您可以存取之環境的數量和類型
* 您服務的開始日期。
* 您的Premier Support層級（如果適用）。
* 您的入門聯絡人。

如果您不了解這些詳細資訊，請聯絡您組織內簽訂合約的團隊。

合約簽署後，Adobe會布建您的環境、傳送一封歡迎電子郵件給合約中定義的入門聯絡人，以及Adobe組織入門諮詢電話。 此工作流程通常發生在合約結束後的幾天，除非另有安排未來的服務開始日期。

## 系統布建 {#provisioning}

合約簽訂後，合約涵蓋的所有AEM雲端環境，Adobe會自動布建。 這些環境也同樣連結到您在Cloud Manager中的計畫（稍後將詳細介紹此主題）。 每個訂閱都包含AEM雲端環境：至少一個生產環境、一個測試環境，以及選用的開發或測試環境。

## 上線呼叫 {#onboarding-call}
...
完成環境布建流程後，合約中定義的系統管理員會收到一封歡迎電子郵件，確認已授予他們對Adobe Experience Cloud的存取權。 如果您正在閱讀此文件歷程，那麼那個人很可能就是您！

此歡迎電子郵件會包含有關如何開始使用 Experience Cloud 服務和 Cloud Manager 自助服務入口網站的詳細資訊。此外，該電子郵件包含一些重要資訊，例如從何處取得支援資源、論壇、常見問題集等。在電子郵件中提供的資源清單中，您也會取得有關如何存取AEM雲端環境的詳細資訊。
