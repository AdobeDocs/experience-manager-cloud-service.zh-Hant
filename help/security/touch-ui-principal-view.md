...
此檢視可讓使用者有效地搜尋指定路徑和所選主體上套用的許可權。 在此檢視中，使用者可以清楚地識別針對所選路徑授予一或多個主體的許可權型別。

選取專用核取方塊，可選擇顯示群組成員資格。 啟用此選項後，許可權評估會考慮主參與者的所有群組，而不只是選取的主參與者。

此外，「稽核檢視」可提供有效ACL的深入分析。 它會顯示與所選路徑之父節點相關聯的ACL，並會考慮所選主體以及任何一般主體。

![篩選器視圖](assets/audit-view.png)

### 存放庫瀏覽器許可權和稽核檢視 {#the-repository-browser-permissions-and-audit-view}
...
1. 進入存放庫瀏覽器後，按一下「**權限**」索引標籤
   ...
   ![權限索引標籤](/help/security/assets/permissions-tab.png)

1. 稽核檢視可讓使用者有效地搜尋指定路徑和所選主體上套用的許可權。 選取專用核取方塊，可選擇顯示群組成員資格。

   ![稽核標籤](/help/security/assets/audit-tab.png)
