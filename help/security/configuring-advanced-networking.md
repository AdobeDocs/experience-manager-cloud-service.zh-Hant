...
>[!CAUTION]
>
>如果您已佈建舊版專用輸出技術並希望設定這些進階網路選項之一， [請先聯絡 Adobe Client Care。](https://experienceleague.adobe.com/?support-solution=Experience+Manager#home)
>
...
1. 在啟動的「**新增網路基礎設施**」精靈中，選取「**彈性連接埠輸出**」，以及從「**區域**」下拉式選單選取應建立該輸出的區域，然後點選或按一下「**繼續**」。
...
>[!TIP]
>
>完整的參數集和確切的語法，以及哪些參數之後不能變更等重要資訊，[可在 API 文件中引用。](https://developer.adobe.com/experience-cloud/cloud-manager/reference/api/#operation/createNetworkInfrastructure)
...
1. 在啟動的「**新增網路基礎設施**」精靈中，選取「**專用輸入 IP 位址**」，以及從「**區域**」下拉式選單選取應建立該位址的區域，然後點選或按一下「**繼續**」。
...
1. 在精靈的「**連線** 」標籤中，提供「**連線名稱**」以識別您的 VPN 連線，並點選或按一下「**新增連線**」。

   ![新增連線](assets/advanced-networking-ui-vpn-add-connection.png)

1. 在「**新增連線**」對話框中，定義您的 VPN 連線，然後點選或按一下「**儲存**」。
