...
使用進階網路時，連線數量會受到限制，以確保跨環境的穩定性，並防止較低層環境耗儘可用的連線。

連線限製為每個AEM執行個體1000個，當數量達到750個時，會向客戶傳送警報。

##### 連線限制是否只套用至非標準連線埠的傳出流量或所有傳出流量？

此限制僅適用於使用進階網路（非標準連線埠上的輸出，使用專用輸出IP或VPN）的連線。

##### 傳出連線的數目沒有明顯差異。 為什麼現在會收到通知？

如果客戶以動態方式建立連線（例如，每個請求一個或多個），流量的增加可能會導致連線激增。

##### 我們過去有沒有可能在沒有收到警報的情況下遇到過類似的情況？

只有在達到軟限制時才會傳送警報。

##### 如果達到最大限制會發生什麼事？

當達到硬性限制時，將會捨棄從AEM透過進階網路（非標準連線埠上的輸出，使用專用輸出IP或VPN）的新輸出連線，以防止DoS攻擊。

##### 可以提高限制嗎？

否，大量的連線可能會對效能造成重大影響，並造成各種Pod和環境的DoS。

##### AEM系統在特定時段後是否自動關閉連線？

是的，連線在JVM層級和網路基礎結構的不同點關閉。 但是，這對於任何生產服務來說都太晚了。 當不再需要連線或在使用連線集區時傳回集區時，應該明確關閉連線。 否則，資源消耗量將會太高，並可能導致資源耗盡。

##### 如果達到最大連線限制，是否會影響任何授權並導致額外成本？

否，此限制無相關授權或成本。 這是技術限制。

##### 我們離限制有多近？ 最大限制是多少？

當連線超過750時會觸發警報。 每個AEM執行個體的最大限製為1000個連線。

##### 此限制是否適用於VPN？

可以，此限制適用於使用進階網路的連線，包括VPN。

##### 如果我們使用專用輸出IP，此限制是否仍適用？

可以，如果使用專用輸出IP，該限制仍適用。
