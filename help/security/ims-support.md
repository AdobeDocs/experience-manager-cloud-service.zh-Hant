...
* 可將使用者和群組指派給與AEMas a Cloud Service執行個體相關聯的產品設定檔，讓他們能登入該執行個體。

>[!TIP]
>
>另請參閱 [設定管理員的AEM存取權](https://experienceleague.adobe.com/?recommended=ExperienceManager-A-1-2020.1.aem) 瞭解使用者如何使用Adobe IMS向AEMas a Cloud Service進行驗證。 也瞭解Adobe IMS使用者、使用者群組和產品設定檔如何用來控制AEM存取權及其功能。 需要 Adobe ID。
...
AEMas a Cloud Service僅對作者、管理員和開發使用者提供IMS驗證支援。 不支援客戶網站的外部使用者，例如網站訪客。

* Admin Console將客戶表示為IMS組織，將環境中的作者和發佈執行個體表示為產品內容執行個體。 此代表可讓系統和產品管理員管理執行個體的存取權。
* Admin Console中的產品設定檔可決定使用者可存取的執行個體。
...
首先，客戶必須擁有布建於Adobe IMS的組織。 Adobe企業客戶在中表示為IMS組織 [Adobe Admin Console](https://helpx.adobe.com/tw/enterprise/using/admin-console.html). 此區域是Adobe客戶用來管理其使用者和群組的產品權益的入口網站。

AEM客戶應先布建組織，而在IMS布建過程中，客戶執行個體可在Admin Console中管理使用者權益和存取許可權。

客戶以IMS組織形式存在後，必須依下列摘要內容設定其系統：
...
有三種方式可引導使用者。 每種方法都取決於客戶的大小和偏好設定。 您可以在Admin Console中手動建立使用者、上傳.csv檔案，或從客戶的企業Active Directory同步使用者。

**透過 Admin Console UI 手動新增**

客戶可在 Admin Console UI 中手動建立使用者和群組。如果您沒有太多要管理的使用者，可以使用此方法。 例如，少於50名AEM使用者，或您已使用此方法管理其他Adobe產品(例如Analytics、Target或Creative Cloud應用程式)。
...
使用者同步工具（簡稱UST）可讓Adobe企業客戶使用Active Directory建立和管理Adobe使用者。 此UST也適用於其他經過測試的OpenLDAP目錄服務。 目標使用者是IT身分管理員（企業目錄或系統管理員），他們能夠安裝和設定此工具。 開放原始碼工具可自訂，因此您修改該工具的客戶可符合您自己的特定需求。
...
若要設定「使用者同步」，組織必須建立一組憑證，其使用方式與使用 [使用者管理API](https://developer.adobe.com/umapi/).

![使用者同步工具](/help/security/assets/ims5.png)

使用者同步工具是透過AdobeGitHub存放庫提供 [在此位置](https://github.com/adobe-apiplatform/user-sync.py/releases/tag/v2.9.0rc2).

>[!NOTE]
>
>搶鮮版 **2.4RC1** 提供建立動態群組的相關支援，可從[此處](https://github.com/adobe-apiplatform/user-sync.py/releases/tag/v2.4rc1)取得。

此版本的主要功能是能夠動態對應新的LDAP群組，以取得Admin Console中的使用者成員資格，以及建立動態使用者群組。
...
如需Adobe Developer Console檔案，請前往 [此處](https://developer.adobe.com/developer-console/).
...
以下是重要的OSGI設定，必須修改以變更「使用者自動成員資格」或「群組對應」等屬性。
...
當產品管理員登入Admin Console時，他們會看到AEMas a Cloud Service產品內容的多個例項，如下所示。 例如，從「 」中選取任何產品， **概觀** 頁面：
...
在此產品設定檔中新增的任何使用者和群組都可以登入該執行個體，如以下範例所示：

![產品設定檔](/help/security/assets/ims8.png)

>[!WARNING]
>
>請勿變更 **AEM管理員** 產品設定檔名稱。 變更的名稱 **AEM管理員** 產品設定檔會移除指派給該設定檔之所有使用者的管理員許可權。

### 登入 Adobe Experience Manager as a Cloud Service {#logging-in-to-aem}

**本機管理員登入**

AEM 可繼續為管理員使用者支援本機登入。登入畫面可讓您在本機登入：
...
若是其他使用者，在執行個體上設定IMS後，就會使用IMS登入。 使用者按一下使用Adobe登入按鈕，如下所示：
...
>[!NOTE]
>
>在 IMS 中建立的任何使用者都可使用 Adobe ID 或 Federated ID 來建立。如果使用者是使用Federated ID設定的，則會使用其公司的身分提供者進行驗證以登入。

系統會將使用者重新導向至IMS登入畫面，且必須輸入其認證：
...
AEM會繼續管理ACL和許可權。 您可將從 IMS 同步的使用者群組，指派給定義 ACL 和權限的本機群組。

在以下範例中，同步的群組會新增至本機 **Dam_Users** 群組為例。
...
如下所示，該群組 **AEM-GRP_008** 繼承的許可權和 **DAM使用者**. 此繼承是管理同步群組許可權的有效方式，通常用於LDAP型驗證方法。
...
1. 將您新增到適當的設定檔後，您應該就能在Cloud Manager中透過以下方式存取個別租使用者： [Adobe Experience Cloud](https://my.cloudmanager.adobe.com) 使用使用者介面的右上角。
...
例如，在下方熒幕擷圖中，您會看到兩個可用的環境，即 *開發作者* 和 *發佈*.

![ACL3](/help/security/assets/ims19.png)

若要存取AEM執行個體，必須將使用者新增至適當Cloud Service產品的群組。

每個作者執行個體都有AEM管理員和AEM使用者設定檔，而每個發佈執行個體都有AEM使用者設定檔。 您可以視需求新增其他設定檔。
