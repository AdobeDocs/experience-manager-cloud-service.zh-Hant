...
* 要求屬性 `clientIp` 只能與以下述詞一起使用：`equals`、`doesNotEqual`、`in`、`notIn`。`clientIp` 也可以比對 IP 範圍 (在使用 `in` 和 `notIn` 述詞時)。下列範例實作條件以評估使用者端IP是否在192.168.0.0/24的IP範圍內（所以從192.168.0.0到192.168.0.255）：
...
| **標幟 ID** | **標幟名稱** | **說明** |
|---|---|---|
| 攻擊 | 攻擊 | 此旗標可識別包含該表格中所列之一或多種攻擊型別的要求 |
| ATTACK-FROM-BAD-IP | 來自錯誤IP的攻擊 | 標幟以識別來自`BAD-IP`且包含該表格中所列的一或多種攻擊型別的請求 |
...
| CVE | CVE | 可識別CVE的旗標。 一律與旗標`CVE-<CVE Number>`結合。 請聯絡Adobe以進一步瞭解Adobe將保護您免於哪些CVE。 |
...
| BAD-IP | 錯誤的IP | 標幟以識別來自IP的已識別為錯誤的請求，可能是因為存在已識別為惡意的來源(`SANS`， `TORNODE`)，或是因為在它們傳送過多惡意請求後，WAF已將其識別為錯誤 |
...
| 已壓縮 | 偵測到壓縮 | POST 要求內文被壓縮，並且無法檢查。例如，若指定`Content-Encoding: gzip`要求標頭，且POST內文不是純文字。 |
...
| OOB網域 | 頻外網域 | 頻外網域通常用於滲透測試期間，以識別允許網路存取的弱點。 |
...
此規則會封鎖發佈上包含查詢引數`foo`的請求，但允許來自IP 192.168.1.1的每個請求：
