...
| **標幟 ID** | **標幟名稱** | **說明** |
|---|---|---|
| 攻擊 | 攻擊 | 可識別包含該表中所列一或多種攻擊類型之請求的旗標 |
| ATTACK-FROM-BAD-IP | 來自惡意 IP 的攻擊 | 可識別來自 `BAD-IP` 且包含該表中所列一或多種攻擊類型之請求的旗標 |
...
| 惡意-IP | 惡意 IP | 可識別請求是否來自識別為惡意之 IP 的旗標，可能是因為其識別為惡意來源 (`SANS`、`TORNODE`) 或因為其在傳送過多惡意請求後由 WAF 識別為惡意 |
