...
* **規則語法：**&#x200B;閱讀有關如何在 `cdn.yaml` 設定檔案宣告流量篩選規則。這包括適用於所有Sites和Forms客戶的流量篩選規則，以及適用於授權該功能的客戶的WAF規則子類別。
...
`cdn.yaml` 檔案中流量篩選規則的格式如下所述。檢視部分 [其他範例](#examples) 在稍後的章節中，以及在 [費率限制規則](#rate-limit-rules).
...
| 視窗 | 整數列舉：1、10 或 60 | 10 | 計算要求速率的取樣視窗（以秒為單位）。 計數器的準確性取決於時間範圍的大小 (範圍愈大，準確度愈高)。例如，1秒視窗的準確度可望達到50%，60秒視窗的準確度可望達到90%。 |
| 懲罰 | 60 到 3600 之間的整數 | 300 (5 分鐘) | 已封鎖相符要求的期間（以秒為單位）（舍入至最接近的分鐘）。 |
| groupBy | array[Getter] | 無 | 速率限制器計數器將依一組請求屬性（例如clientIp）彙總。 |
