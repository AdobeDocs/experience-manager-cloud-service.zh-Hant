...
>[!NOTE]
>有興趣了解在 CDN 設定流量的其他選項？其中包括修改請求/回應、聲明重新導向以及代理至非 AEM 來源。[了解如何操作並嘗試使用](/help/implementing/dispatcher/cdn-configuring-traffic.md) - 加入早期採用者計畫 。
...
| **屬性** | **類型** | **說明** |
|---|---|---|
| reqProperty | `string` | 要求屬性。<br><br>之一：<br><ul><li>`path`：傳回不帶查詢參數的 URL 完整路徑。</li><li>`queryString`：傳回 URL 的查詢部分</li><li>`method`：傳回要求中所使用的 HTTP 方法。</li><li>`tier`：返回其中之一`author`、`preview`或`publish`。</li><li>`domain`：傳回小寫的網域屬性 (如 `Host` 標頭的定義)</li><li>`clientIp`：傳回用戶端 IP 位址。</li><li>`clientCountry`：傳回兩個字母的代碼 ([https://en.wikipedia.org/wiki/tw/Regional_indicator_symbol](https://en.wikipedia.org/wiki/tw/Regional_indicator_symbol) 識別客戶位於哪個國家。</li></ul> |
...
| CMDEXE-NO-BIN | 命令執行，但不在 `/bin/` | 提供與相同層級的保護 `CMDEXE` 在上停用誤判時 `/bin` 得益於AEM架構。 |
...
此規則會封鎖發佈上包含查詢引數的請求 `foo`，但允許來自IP 192.168.1.1的每個請求：
...
此規則會封鎖路徑請求 `/block-me` 發佈時，並封鎖符合 `SQLI` 或 `XSS` 模式。 此範例包含 WAF 流量篩選規則，其參考了 `SQLI` 和 `XSS`[WAF 標幟](#waf-flags-list)，因此需要單獨的授權。
...
會根據到達邊緣的流量、到達原點的流量或錯誤次數來評估速率限制。
...
| 計數 | 全部、擷取、錯誤 | 全部 | 根據邊緣流量（全部）、來源流量（擷取）或錯誤數（錯誤）進行評估。 |
...
當路徑/critical/resource上的請求超過10秒時間範圍內每秒（每個CDN POP）100個請求的平均值時，封鎖請求60秒：
...
規則可設定為在 5 分鐘內觸發 10 次時發送行動中心通知，進而可在發生特定流量模式時向您發出警報，讓您採取任何必要的措施。了解更多關於[行動中心](/help/operations/actions-center.md)，包括如何設定接收電子郵件所需的通知設定檔。
...
[Tutorials](#tutorial) 適用於如何使用儀表板工具的具體指示。
...
[完成教學課程](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/security/traffic-filter-and-waf-rules/overview.html) 以取得有關流量篩選規則（包括WAF規則）的一般實用知識和經驗。
...
[深入瞭解如何封鎖](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/security/blocking-dos-attack-using-traffic-filter-rules) 使用速率限制流量篩選規則和其他策略的阻斷服務(DoS)和分散式阻斷服務(DDoS)攻擊。

本教學課程將引導您完成：

* 瞭解保護
* 在超過速率限制時接收警示
* 使用儀表板工具來設定速率限制流量篩選規則的臨界值，以分析流量模式
