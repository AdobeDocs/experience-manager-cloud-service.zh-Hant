...
* **業務負責人**
   * 在此角色中，您有權新增計畫或編輯計畫、新增或更新環境、將計畫碼部署到AEM環境或執行計畫碼品質檢查。
   * 這類使用者會負責定義 KPI、核准生產部署並在必要時覆寫重要的 3 層級失敗。
* **部署管理員**
   * 在此角色中，您有權新增或更新環境、執行管道，和將計畫碼部署到AEM環境或執行計畫碼品質檢查。
   * 這類使用者會管理部署操作，並使用 Cloud Manager 來執行中繼/生產部署、編輯 CI/CD 管道、核准重要的 3 層級失敗 (如有必要)，並可存取 Git 存放庫。
* **開發人員**
   * 在此角色中，您有權產生個人存取權杖以存取Git。
   * 這類使用者會開發和測試自訂應用程式，並主要使用 Cloud Manager 來檢視部署狀態，且可存取程式碼認可的 Git 存放庫。
* **計畫管理員**
   * 在此角色中，您有權排程管道、覆寫3層品質門檻並提供生產核准。
