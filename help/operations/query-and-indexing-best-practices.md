...
在AEMas a Cloud Service中，有關索引的所有操作方面都是自動化的。 如此一來，開發人員就能專注於建立有效率的查詢，以及對應的索引定義。
...
此外，在設計分類法時，考慮排序是否重要也很重要。 如果不需要明確排序，並且需要大量同層級節點，則最好使用無序節點型別，例如 `sling:Folder` 或 `oak:Unstructured`. 若需要排序， `nt:unstructured` 和 `sling:OrderedFolder` 會更合適。

### 元件中的查詢 {#queries-in-components}

由於查詢可能是在AEM系統上執行的最繁重的操作之一，因此最好在元件中避免查詢。 每次轉譯頁面時執行數個查詢通常會降低系統效能。 有兩種策略可用於在呈現元件時避免執行查詢： **[周遊節點](#traversing-nodes)** 和 **[預先擷取結果。](#prefetching-results)**
...
Oak檔案提供 [查詢執行方式概觀。](https://jackrabbit.apache.org/oak/docs/query/query-engine.html#query-processing) 這是本檔案所述所有最佳化活動的基礎。

AEMas a Cloud Service提供 [查詢效能工具](#query-performance-tool)，可支援實作有效率的查詢。
...
有時需要將自訂屬性新增到現有索引，因此可以使用索引查詢其他限制。 檢視檔案 [內容搜尋與索引](/help/operations/indexing.md#changing-an-index) 以取得更多詳細資料。 此 [JCR查詢速查表](#jcr-query-cheatsheet) 本節說明索引上的屬性定義必須如何支援特定查詢型別。
...
1. 查詢引擎會執行排序程式。
   * 當查詢引擎在索引之外執行篩選，或排序屬性未使用進行註解時，可能會發生這種情況 `ordered=true` 屬性。
...
這也表示只有在擷取所有結果時，才能正確決定結果集的大小。 因此，請務必限制擷取的結果集，可透過增加查詢來實現(請參閱 [JCR查詢速查表](#jcr-query-cheatsheet) 區段以取得詳細資訊)或藉由限制結果的讀取來取得。

這樣的限制也會防止查詢引擎達到 **周遊限制** 100,000個節點中，這會導致強制停止查詢。

請參閱區段 [具有大型結果集的查詢](#queries-with-large-result-sets) 如果必須完全處理潛在的大型結果集，則為本檔案。

## 查詢效能工具 {#query-performance-tool}

查詢效能工具(位於 `/libs/granite/operations/content/diagnosistools/queryPerformance.html` 並可透過 [Cloud Manager中的開發人員控制檯](https://experienceleague.adobe.com/docs/experience-manager-learn/cloud-service/debugging/debugging-aem-as-a-cloud-service/developer-console.html#queries))提供 — 
...
這些表格可協助識別未完整編制索引的查詢(請參閱 [使用索引](#use-an-index) 或讀取太多節點(另請參閱 [存放庫周遊](#repository-traversal) 和 [索引周遊](#index-traversal))。 這類查詢將會反白顯示 — 以紅色標示適當的關注區域。

此 `Reset Statistics` 提供的選項可移除表格中收集的所有現有統計資料。 這允許執行特定查詢（透過應用程式本身或Explain Query工具）並分析執行統計資料。
...
* 如有必要，請使用提供的核取方塊選取查詢的執行方式。
   ...
   * 提供三個選項來執行查詢 — 
      ...
      * `Include Node Count`  — 執行查詢並讀取整個結果集(通常不建議這樣做 — 請參閱 [索引周遊](#index-traversal))。

#### 查詢說明快顯功能表 {#query-explanation-popup}

![查詢說明快顯功能表](./assets/query-explanation-popup.png)

選取後 `Explain`，使用者會看到一個快顯視窗，說明查詢說明的結果（和執行，如果選取）。
此快顯視窗包括 — 
* 執行查詢時使用的索引（如果查詢是使用來執行，則無索引） [存放庫周遊](#repository-traversal))。
* 執行時間(如果 `Include Execution Time` 核取方塊已勾選)和讀取的結果計數(如果 `Read first page of results` 或 `Include Node Count` 核取方塊)。
* 執行計畫，允許有關如何執行查詢的詳細分析 — 請參閱 [讀取查詢執行計畫](#reading-query-execution-plan) 以瞭解如何解譯。
* 前20個查詢結果的路徑(如果 `Read first page of results` 核取方塊已勾選)
...
計畫的這個區段指出 — 
* 索引是用來執行此查詢 — 
   * 在此案例中，Lucene索引 `/oak:index/damAssetLucene-9` 將會使用，因此其餘資訊採用Lucene查詢語法。
* 所有3個限制都由索引處理 — 
   * 節點型別限制
      * 隱含，因為 `damAssetLucene-9` 僅索引dam：Asset型別的節點。
   * 路徑限制
      * 因為 `+:ancestors:/content/dam` 會顯示在Lucene查詢中。
   * 屬性限制
      * 因為 `+jcr:content/metadata/dc:title:My Title` 會顯示在Lucene查詢中。
* 排序由索引處理
   * 因為 `ordering:[{ propertyName : jcr:created, propertyType : UNDEFINED, order : ASCENDING }]`  會顯示在Lucene查詢中。

此類查詢的執行狀況可能良好，因為從索引查詢傳回的結果不會在查詢引擎中進一步篩選（除了存取控制篩選之外）。 但是，如果沒有遵循最佳實務，這類查詢還是可以緩慢執行 — 請參閱 [索引周遊](#index-traversal) 底下。
...
* 索引只會處理2個限制（共3個） — 
   ...
   * 路徑限制
      * 因為 `+:ancestors:/content/dam` 會顯示在Lucene查詢中。
* 屬性限制 `jcr:content/metadata/myProperty = "My Property Value"` 不會在索引處執行，而是會在基礎Lucene查詢的結果上套用為查詢引擎篩選。
   * 這是因為 `+jcr:content/metadata/myProperty:My Property Value` 不會出現在Lucene查詢中，因為此屬性並未在 `damAssetLucene-9` 用於此查詢的索引。
...
它包含QueryBuilder、XPath和SQL-2的範例查詢，涵蓋了在查詢效能方面行為不同的多個案例。 此外，也提供如何建置或自訂Oak索引的建議。 本速查表內容適用於AEMas a Cloud Service和AEM 6.5。

## 索引定義最佳實務 {#index-definition-best-practices}

以下是定義或擴充索引時可考量的一些最佳實務。

* 針對具有現有索引的節點型別(例如 `dam:Asset` 或 `cq:Page`)除了新增索引以外，偏好擴充OOTB索引。
   * 新增索引 — 尤其是全文檢索索引 — 於 `dam:Asset` 強烈建議不要使用節點型別(請參閱 [此備註](/help/operations/indexing.md##index-names-index-names))。
* 新增索引時
   * 一律定義「lucene」型別的索引。
...
Automated Cloud Manager管道檢查將強制執行上述某些最佳實務。
...
有了這些資訊，您就可以使用 [正在最佳化查詢](#optimizing-queries) 小節。

### 索引周遊 {#index-traversal}

使用索引但仍讀取大量節點的查詢將記錄類似以下內容的訊息（請注意該詞語） `Index-Traversed` 而非 `Traversed`)。
