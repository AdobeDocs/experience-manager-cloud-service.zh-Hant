...
1. 完全自定義索引。 一個例子是 `/oak:index/acme.product-1-custom-2`。 為避免命名衝突，我們要求完全自定義索引具有前置詞，例如， `acme.`
...
然後，可以執行以下任一步驟來解決此問題 —  <br>
1. 將檔案降級到1.0.4版，並將以下內容添加到頂級pom:
...
1. 禁用nodetype驗證。 在檔案插件配置的jackrabbit-nodetypes部分中設定以下屬性：
...
當前僅支援類型索引的索引管理 `lucene`, `compatVersion` 設定為 `2`。 在內部，可以配置其他索引並將其用於查詢，例如Elasticsearch索引。 針對 `damAssetLucene` 在as a Cloud Service上，AEM索引實際上可能針對此索引的Elasticsearch版本執行。 這種差異對於應用程式最終用戶是不可見的，但某些工具(如 `explain` 功能將報告其他索引。 有關Lucene和Elasticsearch索引之間的差異，請參見 [Apache Jackrabbit Oak中的Elasticsearch文檔](https://jackrabbit.apache.org/oak/docs/query/elastic.html)。 客戶不能也不需要直接配置Elasticsearch索引。

僅支援內置分析器（即隨產品一起發運的分析器）。 不支援自定義分析器。

為獲得最佳運行效能，索引不應過大。 所有索引的總大小可用作指南：如果在添加自定義索引和在開發環境上調整標準索引後，此增幅超過100%，則應調整自定義索引定義。 AEMas a Cloud Service可以阻止對系統穩定性和效能產生負面影響的索引的部署。
