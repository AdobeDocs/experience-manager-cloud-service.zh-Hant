...
<table style="table-layout:auto">
 <tbody>
  ...
  <tr>
    <td>版本清除</td>
    <td>Adobe</td>
    <td>對於現有環境（在2023年11月1日之前建立的環境），清除功能會停用，且未來將不會啟用，除非客戶明確啟用，屆時他們也可以使用自訂值來設定。<br><br> <!--Alexandru: leave the two line breaks in place, otherwise spacing won't render properly-->新環境（自2023年11月1日起建立的環境）預設會啟用永久刪除，使用的值如下，客戶可設定自訂值。
     <ol>
       <li>超過30天的版本會被移除</li>
  ...
  <tr>
    <td>稽核記錄清除</td>
    <td>Adobe</td>
    <td>對於現有環境（在2023年11月1日之前建立的環境），清除功能會停用，且未來將不會啟用，除非客戶明確啟用，屆時他們也可以使用自訂值來設定。<br><br> <!-- See above for the two line breaks -->新環境（自2023年11月1日起建立的環境）將預設啟用下方的清除 <code>/content</code> 存放庫節點中的屬性(S)：
     <ol>
