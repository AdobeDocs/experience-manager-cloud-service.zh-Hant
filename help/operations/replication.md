...
>建議不要構建您自己的批量發佈自定義代碼。
>如果您確實因任何原因需要自定義，則可以使用現有的工作流API觸發此工作流/工作流步驟。
>雖然只發佈需要發佈的內容並謹慎地不要嘗試在不必要的情況下發佈大量內容始終是一種良好做法，但通過發佈內容樹工作流可以發送的內容數量沒有限制。
...
1. 按一下 **進程** 頁籤 `Publish Content Tree` 從下拉清單中，然後 **處理程式高級** 複選框
