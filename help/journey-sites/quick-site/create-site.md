---
title: 從範本建立網站
description: 解如何使用網站範本快速建立新的 AEM 網站。
...
本文基於這些基礎之上，因此您可以進行第一個設定步驟並從範本建立新網站，之後您可以使用前端工具來自訂該範本。
...
網站範本是一種將基本網站內容結合成方便且可重複使用之封裝的方法。 網站範本通常包含基本網站內容和結構以及網站樣式資訊，以便快速啟動新網站。 實際結構如下：
...
>[!NOTE]
>
>不要將網站範本與頁面範本混淆。 此處說明的網站範本定義網站的整體結構。 頁面範本會定義個別頁面的結構和初始內容。

## 取得網站範本 {#obtaining-template}

最簡單的入門方法是 [從其GitHub存放庫下載最新版的AEM標準網站範本。](https://github.com/adobe/aem-site-template-standard/releases)

下載後，您可以像上傳任何其他套件一樣將它上傳到您的AEM環境。 請參閱 [其他資源區段](#additional-resources) 如果您需要有關本主題的詳細資訊，請參閱有關如何使用套件的詳細資訊。

>[!TIP]
>
>AEM標準網站範本可以根據您的專案需求進行自訂，而且不需要進行進一步自訂。 不過，此主題不在本歷程的討論範圍內。 如需詳細資訊，請參閱標準網站範本的GitHub檔案。

>[!TIP]
>
>您也可以選擇從來源建立範本，作為專案工作流程的一部分。 不過，此主題不在本歷程的討論範圍內。 如需詳細資訊，請參閱標準網站範本的GitHub檔案。
...
網站範本功能非常強大且有彈性，您可以為專案建立任何數量，以便輕鬆建立網站變體。 根據在您使用的網站範本上已執行的自訂等級，您甚至可能不需要額外的前端自訂。

* 如果您的網站不需要額外的自訂，恭喜您！ 您的歷程到此結束！
...
1. 在編輯器中，選取 **頁面資訊** 工具列中的按鈕，然後 **以發佈的形式檢視**.
...
1. 在 **群組** 索引標籤，將Proxy使用者新增至 `contributors` 群組。
   * 輸入術語 `contributors` 觸發AEM自動完成功能以輕鬆選取群組。
...
在此基礎上繼續您的AEM快速網站建立歷程，接下來檢閱檔案 [設定您的管道，](pipeline-setup.md) 您將在此建立前端管道來管理網站主題的自訂。
