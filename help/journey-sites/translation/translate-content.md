...
1. 找出專案的語言根目錄。 例如，如果語言根是英文， `/content/<your-project>/en`.
   * 在第一次翻譯前，其他語言資料夾是空白的預留位置。 這些通常由內容架構者建立。
1. 找出專案的語言根目錄。
1. 選取邊欄選擇器並顯示 **引用** 面板。
1. 選取 **語言副本**.
1. 勾選&#x200B;**語言副本**&#x200B;核取方塊。
...
1. 在 **進階** 標籤，使用 **目標語言** 下拉式清單，以選取您的內容應翻譯成的語言。 選擇 **建立**。
...
1. 瀏覽至 **導覽** > **專案**.
1. 選取在前一個區段中建立的專案(無論是 [根據內容路徑自動建立翻譯專案](#automatically-creating) 或 [選取您的內容，手動建立翻譯專案](#manually-creating) 視您的情況而定)。
...
自動建立翻譯專案時，AEM會根據您先前定義的翻譯規則，評估所選路徑下的內容。 根據該評估，它將需要翻譯的內容擷取到新的翻譯專案。
...
1. 選取條列專案的選取核取方塊，即可檢視其他選項，例如從工作中刪除專案或在網站主控台中檢視專案的選項。
...
1. 路徑瀏覽器開啟，讓您可明確選取要新增的內容。找到您的內容並選取「 」以選取。
...
選取該圖示可在其主控台中開啟翻譯內容，以檢視翻譯內容的詳細資料。
...
我們建議您檢閱檔案，以繼續翻譯歷程的下一部分 [發佈翻譯的內容，](publish-content.md) 以下是一些其他可選資源，這些資源對本文中提到的一些概念進行了更深入的探究，但並非繼續此歷程所必需的。
