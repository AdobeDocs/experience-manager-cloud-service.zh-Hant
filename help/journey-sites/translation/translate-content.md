...
設定好聯結器和翻譯規則後，本文將引導您完成翻譯AEM Sites內容的下一個步驟。
...
翻譯專案可讓您管理AEM內容的翻譯。 翻譯專案會將要翻譯的內容收集到一個位置，以便集中檢視翻譯工作。
...
1. 導覽至 **導覽** -> **網站** 然後點選或按一下您的專案。
1. 找到專案的語言根目錄。 例如，如果語言根是英文， `/content/<your-project>/en`.
   * 請注意，在第一次翻譯之前，其他語言資料夾是空的預留位置。 這些通常由內容架構師建立。
1. 找到專案的語言根目錄。
...
1. 導覽至&#x200B;**導覽** -> **專案**。
1. 點選或按一下在上一個區段中建立的專案(可以 [根據內容路徑自動建立翻譯專案](#automatically-creating) 或 [選取您的內容以手動建立翻譯專案](#manually-creating) 視您的情況而定)。
...
若要更清楚瞭解AEM的翻譯流程，對專案設定進行一項變更會很有用。 生產翻譯不需要此步驟，但有助於瞭解此程式。

1. 於 **摘要** 卡片，點選或按一下卡片底部的省略符號按鈕。
1. 於 **進階** 標籤，取消核取選項 **促銷活動後刪除啟動**.
...
1. 點選或按一下條列專案的選取核取方塊，以檢視更多選項，例如從工作中刪除專案或在網站主控台中檢視專案的選項。
...
>[!TIP]
>
>處理翻譯工作可能需要一些時間，您可能會看到翻譯專案從以下狀態移動 **草稿** 至 **翻譯進行中** 至 **可供檢閱** 在他們到達之前 **已核准** 州別。 這是正常現象。

>[!NOTE]
>
>如果您未停用專案選項 **促銷活動後刪除啟動** 作為 [如上一節所述，](#using-translation-project) 已翻譯專案會以 **已刪除** 州別。 這是正常的，因為一旦翻譯的專案到達，AEM就會自動捨棄翻譯記錄。 已將翻譯專案匯入為語言副本，僅刪除翻譯記錄，因為不再需要這些記錄。
>
>如果不清楚，請不要擔心。 這些是AEM如何運作的深入細節，不會影響您對歷程的理解。 如果您想深入瞭解AEM處理翻譯的方式，請參閱 [其他資源](#additional-resources) 區段。

### 使用手動建立的翻譯專案 {#using-manual-project}

手動建立翻譯專案時，AEM會建立必要工作，但不會自動選取要包含在這些工作中的任何內容。 這使翻譯專案經理可以靈活地挑選要翻譯的內容。
...
點選或按一下該圖示，即可在翻譯內容的主控台中開啟翻譯內容，以檢視翻譯內容的詳細資料。

![翻譯頁面](assets/translated-page.png)

如果您有適當的許可權，可以進一步修改必要的翻譯內容，但編輯內容超出了此歷程的範圍。 請參閱 [其他資源](#additional-resources) 區段，以取得有關本主題的詳細資訊。

專案的目的在將所有與翻譯相關的資源集中在一處以方便存取和清楚概覽。不過，正如您檢視已翻譯專案的詳細資訊所看到的，翻譯本身會流回翻譯語言的網站資料夾。 在此範例中，資料夾是

```text
/content/<your-project>/es
```

如果您透過以下方式導覽至此資料夾： **導覽** -> **網站**，您會看到翻譯的內容。

![已翻譯的內容資料夾結構](assets/translated-sites-content.png)

AEM 的翻譯框架從翻譯連接器接收翻譯，然後根據語言根並使用連接器提供的翻譯，自動建立內容結構。

請務必瞭解此內容未發佈，因此無法供使用。 我們將了解此作者-發佈結構，並了解如何在翻譯歷程的下一步發佈已翻譯的內容。

## 人工翻譯 {#human-translation}

如果您的翻譯服務提供人工翻譯，則審核流程會提供更多選項。例如，翻譯返回專案時狀態為&#x200B;**草稿**，並且必須人工審核並核准或拒絕。

人工翻譯不在此歷程的討論範圍內。請參閱 [其他資源](#additional-resources) 區段，以取得有關本主題的詳細資訊。 然而，除了額外的核准選項之外，人工翻譯的工作流程與本歷程描述的機器翻譯相同。
...
在此知識的基礎上繼續您的AEM Sites翻譯歷程，接下來檢視檔案 [發佈翻譯的內容](publish-content.md) 您將瞭解如何發佈翻譯內容，以及如何在語言根內容變更時更新這些翻譯。

## 其他資源 {#additional-resources}

我們建議您檢閱檔案，繼續翻譯歷程的下一部分 [發佈翻譯的內容，](publish-content.md) 以下是一些其他可選資源，這些資源對本文檔中提到的一些概念進行了更深入的探究，但並非繼續此歷程所必需的。
