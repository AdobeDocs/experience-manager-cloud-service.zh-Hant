...
GraphQL 規格提供了一系列指南，說明如何建立健全的 API 來查詢特定執行個體上的資料。若要這麼做，使用者端必須擷取 [結構描述](#schema-generation)，其中包含查詢所需的全部型別。
...
* 產生的欄位。
   ...
   * 也必須考慮&#x200B;**呈現為**&#x200B;設定，因為使用者可以設定某些資料類型。例如，單行文字欄位可以設定為包含多個單行文字，方法是選擇 `multifield` 下拉式清單中的。
...
>[!NOTE]
>
>A **內容參考** 可同時用於DAM資產和Dynamic Media資產。 擷取適當的URL時會使用不同的引數：
>* `_dynamicUrl` ：DAM資產
...
>[!NOTE]
>
>為此，您需要擁有 [Dynamic Media雲端設定](/help/assets/dynamic-media/config-dm.md).
>
>這會新增 `dam:scene7File` 和 `dam:scene7Domain` 屬性位於資產的中繼資料上。
