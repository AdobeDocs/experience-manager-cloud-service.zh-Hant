...
除了使用者產生欄位的資料型別外，GraphQL for AEM也會產生許多欄位 *協助程式* 可協助識別內容片段或提供有關內容片段的其他資訊的欄位。
...
>[!NOTE]
>
>* 分頁需要穩定的排序順序，才能在多個查詢要求同一結果集的不同頁面時，跨多個查詢正常運作。依預設，它使用結果集中每個項目的存放庫路徑來確保順序始終相同。如果使用不同的排序順序，且如果無法在JCR查詢層級完成排序，則會對效能造成負面影響，因為必須先將整個結果集載入記憶體，才能判斷分頁。
>
