---
title: 用於內容片段傳送的AEM REST OpenAPI
description: 瞭解用於內容片段傳送的AEM REST OpenAPI
...
* 是[AEMEdge Delivery Services](/help/edge/overview.md)上的HTTP REST API，旨在以JSON格式從內容片段傳送結構化內容
* 提供現代CDN整合，讓作用中內容失效
...
AEM也隨附使用中CDN快取失效。 這代表每當內容更新或發佈時，對應的JSON OpenAPI回應都會透過對Fastly的軟清除請求自動失效。 這可讓您在到達實際CDN快取存留期(`s-maxage`)之前，檢視JSON輸出中反映的變更。